...
if len(sys.argv) < 2:
    raise Exception('Specify interpreter as argument.')
...
# add shebang with specified interpreter to file-reader.py
with open(f'{cwd}/file-reader.py', 'r') as f:
    source = f.read()

with open(f'{bin_dir}/file-reader.py', 'w+') as f:

...
class TermsAggregationField(Field):
    ...
    def to_bucket_representation(self, bucket_name, bucket, metric_name=None):
        return self.key_mapping.get(bucket_name, bucket_name), bucket[metric_name]['value'] if metric_name is not None else bucket.doc_count

    def to_representation(self, value):
        return OrderedDict(self.to_bucket_representation(bucket.key, bucket) for bucket in value.buckets)


class RangeAggregationField(Field):

    def to_bucket_representation(self, bucket_name, bucket, metric_name=None):
        return bucket_name, bucket[metric_name]['value'] if metric_name is not None else bucket['doc_count']

    def to_representation(self, value):
        return OrderedDict(self.to_bucket_representation(bucket_name, bucket) for bucket_name, bucket in value.buckets.to_dict().iteritems())

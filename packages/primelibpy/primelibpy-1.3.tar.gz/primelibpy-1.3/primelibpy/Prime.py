...
def isComposite(x):
    ...
    if f==1:
        ...
        return False                 
def getFermatPseudoPrime(baseNumber,numOfPsedoPrime):
    l1=[]
    p,r=baseNumber,numOfPsedoPrime
    if r<2:
...
def getRightTruncatablePrime(startLimit,endLimit):
    ...
    return(q)
#24
def getTruncatable_Prime(startLimit,endLimit):
    p,r=startLimit,endLimit
...
def getGausianPrime(startLimit,endLimit):
    p,q=startLimit,endLimit
    x=p**2+q**2
    f=1
    for i in range(2,(x//2)+1):
        if x%i==0:
            f=0
    if f==1:
        print(p,"+i",q," is gausian prime",sep="")
#26

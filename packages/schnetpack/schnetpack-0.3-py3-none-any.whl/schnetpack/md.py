...
from schnetpack.atomistic import Energy, ElementalEnergy
from schnetpack import AtomisticModel, Properties
...
class AtomsConverter:
    ...
    def convert_atoms(self, atoms):
        ...
        inputs[Properties.neighbor_mask] = torch.ones_like(inputs[Properties.neighbors]).float()

        # Get cells
        inputs[Properties.cell] = torch.FloatTensor(atoms.cell.astype(np.float32))
        ...
        if self.collect_triples is not None:
            ...
            inputs[Properties.neighbor_pairs_mask] = torch.ones_like(inputs[Properties.neighbor_pairs_j]).float()

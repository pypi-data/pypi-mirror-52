...
from schnetpack.utils import to_json, read_from_json, count_params
...
def get_parser():
    ...
    train_parser.add_argument("datapath", help="Path / destination of OMDB dataset")
...
def get_model(
    ...
    logging.info("The model you built has: {} parameters".format(count_params(model)))
...
if __name__ == "__main__":
    ...
    np.savez(
        split_path, train_idx=idx[:9000], val_idx=idx[9000:10000], test_idx=idx[-2500:]
    )
    ...
    if args.mode == "train":
        ...
        logging.info(
            "Training set statistics: mean=%.3f, stddev=%.3f"
            % (mean.numpy(), stddev.numpy())
        )
    else:

...
def get_parser():
    ...
    train_parser.add_argument("datapath", help="Path / destination of QM9 dataset")
...
def train(args, model, train_loader, val_loader, device):
    ...
    if args.logger == "csv":
        logger = spk.train.CSVHook(
            ...
            [
                schnetpack.train.metrics.MeanAbsoluteError(args.property, "y"),
                schnetpack.train.metrics.RootMeanSquaredError(args.property, "y"),
            ],
...
def evaluate(args, model, property, train_loader, val_loader, test_loader, device):
    ...
    metrics = [
        schnetpack.train.metrics.MeanAbsoluteError(property, "y"),
        schnetpack.train.metrics.RootMeanSquaredError(property, "y"),
    ]
...
def get_model(
    ...
    if args.model == "schnet":
        ...
        if args.property == QM9.mu:
            atomwise_output = spk.atomistic.DipoleMoment(
                args.features, predict_magnitude=True, mean=mean, stddev=stddev
            )
        else:
        ...
        # Build HDNN model
        if args.property == QM9.mu:
            atomwise_output = spk.atomistic.ElementalDipoleMoment(
            ...
            atomwise_output = spk.atomistic.ElementalAtomwise(
                representation.n_symfuncs,
                n_hidden=args.n_nodes,
                n_layers=args.n_layers,
                aggregation_mode=args.aggregation_mode,
...
if __name__ == "__main__":
    ...
    qm9 = QM9(args.datapath, collect_triples=args.model == "wacsf", download=True)
    atomref = qm9._get_atomref(train_args.property)

#!/home/kschuett/anaconda3/bin/python3
import argparse
import logging
import os
import sys
...
def get_parser():
    ...
    train_parser.add_argument(
        "molecule",
        help="Selected molecule trajectory of MD17 collection",
    ...
    train_parser.add_argument(
        "--split",
        help="Give sizes of train and validation splits and use remaining for testing",
        type=int,
        nargs=2,
        default=[None, None],
    )
    ...
    eval_parser.add_argument(
        "datapath", help="Path / destination of MD17 dataset directory"
    )
    eval_parser.add_argument(
        "molecule", help="Molecule trajectory", choices=MD17.existing_datasets
    )
...
def train(args, model, train_loader, val_loader, device):
    ...
    metrics = [
        schnetpack.train.metrics.MeanAbsoluteError(MD17.energy, "y"),
        schnetpack.train.metrics.RootMeanSquaredError(MD17.energy, "y"),
        schnetpack.train.metrics.MeanAbsoluteError(MD17.forces, "dydx"),
        schnetpack.train.metrics.RootMeanSquaredError(MD17.forces, "dydx"),
    ]
    if args.logger == "csv":
    ...
    def loss(batch, result):
        ...
        err_sq = args.rho * torch.mean(ediff.view(-1)) + (1 - args.rho) * torch.mean(
            fdiff.view(-1)
        )
        return err_sq
...
def evaluate(args, model, train_loader, val_loader, test_loader, device):
    ...
    metrics = [
        ...
        schnetpack.train.metrics.MeanAbsoluteError(MD17.forces, "dydx"),
        schnetpack.train.metrics.RootMeanSquaredError(MD17.forces, "dydx"),
        schnetpack.train.metrics.LengthMAE(MD17.forces, "dydx"),
        schnetpack.train.metrics.LengthRMSE(MD17.forces, "dydx"),
        schnetpack.train.metrics.AngleMAE(MD17.forces, "dydx"),
        schnetpack.train.metrics.AngleRMSE(MD17.forces, "dydx"),
    ]
...
def get_model(
    ...
    if args.model == "schnet":
        ...
        model = schnetpack.atomistic.model.AtomisticModel(representation, atomwise_output)
...
if __name__ == "__main__":
    ...
    md17 = MD17(args.datapath, args.molecule, collect_triples=args.model == "wacsf",
                download=True)

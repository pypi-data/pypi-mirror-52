...
class TestWeekDays(TestCase):
    ...
    def test_string(self):
        def test_some_enabled():
            ...
            self.weekdays.thursday = True
            self.weekdays.saturday = True
            self.assertEqual("Sunday, Monday, Wednesday, Thursday and Saturday", self.weekdays.string)

        def test_one_enabled():
            self.weekdays = WeekDays()
            self.weekdays.monday = True
            self.assertEqual("Monday", self.weekdays.string)
    ...
    def test_short_string(self):
        def test_some_enabled():
            ...
            self.weekdays.saturday = True
            self.weekdays.sunday = True
            self.assertEqual("Sun, Mon, Wed, Thu, Sat", self.weekdays.short_string, )

        def test_one_enabled():
            self.weekdays = WeekDays()
            self.weekdays.monday = True
            self.assertEqual("Mon", self.weekdays.short_string, )

        def test_none_enabled():
            self.weekdays = WeekDays()
            self.assertEqual('', self.weekdays.short_string)
...
class TestWeekdayOfMonth(TestCase):
    ...
    def test_get_next_date(self):
        ...
        # find the third saturd in the month, every 2 months
        wom = WeekdayOfMonth()
        wom.ordinal = 2
        wom.weekday = 5
        initial_date = arrow.Arrow(year=2019, month=7, day=1, hour=10, minute=3, second=30)
        ...
        # find the last wednesay in the month, every 3 months
        wom = WeekdayOfMonth()
        wom.ordinal = -1
        wom.weekday = 2
        initial_date = arrow.Arrow(year=2019, month=8, day=20, hour=10, minute=3, second=30)
...
class TestRecurrence(TestCase):
    ...
    def test_string(self):
        ...
        def test_weeks():
            ...
            recurrence_object.weekdays.sunday = True
            self.assertEqual('Every week on Sunday, Monday, Thursday and Saturday', recurrence_object.string)
            ...
            recurrence_object.weekdays.saturday = True
            recurrence_object.weekdays.sunday = True
            self.assertEqual('Every 2 weeks on Sunday, Monday, Thursday and Saturday', recurrence_object.string)

            recurrence_object = Recurrence(enabled=True, interval='week', increment=1)
            recurrence_object.weekdays.monday = True
            recurrence_object.weekdays.sunday = True
            self.assertEqual('Every week on Sunday and Monday', recurrence_object.string, )

            recurrence_object = Recurrence(enabled=True, interval='week', increment=1)
            recurrence_object.weekdays.monday = True
            self.assertEqual('Every week on Monday', recurrence_object.string, )

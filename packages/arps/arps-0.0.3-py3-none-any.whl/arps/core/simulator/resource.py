...
_TrackedValue = namedtuple('TrackedValue', 'value epoch modifier_id event_type')


# pylint: disable=E1136
class TrackedValue(_TrackedValue):
    '''
...
class Resource(AbstractResource, ObservableMixin):
    ...
    Enums:
    - EvtType: enum that contains the actions that can trigger resource modification
    ...
    EvtType = Enum('EvtType', 'des_main des_pos mas rsc_indirect none')
    ...
    def __init__(self, *, environment_identifier, identifier,
        ...
        '''
        AbstractResource.__init__(self, environment_identifier,
                                  identifier, category)
        self._value = value
    ...
    def value(self, tracked_value: TrackedValue) -> None:
        ...
        self._value = value
        assert isinstance(evt_type, Resource.EvtType)
    ...
    def add_listener(self, listen, predicate=None):
        ...
        event = ResourceEvent.from_resource(self, 0, str(None),
                                            Resource.EvtType.none)
        asyncio.ensure_future(self.notify(event))
    ...
    def __str__(self):
        return 'Resource {} in environment {}: {}'.format(self.identifier,

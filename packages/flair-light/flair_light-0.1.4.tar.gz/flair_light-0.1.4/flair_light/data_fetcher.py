...
import flair_light
from flair_light.data import Sentence, Corpus, Token, MultiCorpus
...
class NLPTaskDataFetcher:
    @staticmethod
    @deprecated(version="0.4.1", reason="Use 'flair_light.datasets' instead.")
    def load_corpora(
    ...
    def load_corpus(task: Union[NLPTask, str], base_path: [str, Path] = None) -> Corpus:
        ...
        if not base_path:
            base_path = Path(flair_light.cache_root) / "datasets"
    ...
    def download_dataset(task: NLPTask):
        ...
        if task == NLPTask.CONLL_2000:
            ...
            data_file = Path(flair_light.cache_root) / "datasets" / task.value / "train.txt"
            if not data_file.is_file():
                ...
                with gzip.open(
                    Path(flair_light.cache_root) / "datasets" / task.value / "train.txt.gz",
                    "rb",
                ) as f_in:
                    with open(
                        Path(flair_light.cache_root) / "datasets" / task.value / "train.txt",
                        "wb",
                    ) as f_out:
                        shutil.copyfileobj(f_in, f_out)
                with gzip.open(
                    Path(flair_light.cache_root) / "datasets" / task.value / "test.txt.gz",
                    "rb",
                ) as f_in:
                    with open(
                        Path(flair_light.cache_root) / "datasets" / task.value / "test.txt",
                        "wb",
                    ) as f_out:
                        shutil.copyfileobj(f_in, f_out)
        ...
        if task == NLPTask.NER_BASQUE:
            ...
            data_path = Path(flair_light.cache_root) / "datasets" / task.value
            ...
            if not data_file.is_file():
                ...
                with tarfile.open(
                    Path(flair_light.cache_root) / "datasets" / task.value / "eiec_v1.0.tgz",
                    "r:gz",
                ) as f_in:
                    corpus_files = (

...
import flair_light
from flair_light.data import Sentence, Corpus, Token, FlairDataset
...
class CONLL_2000(ColumnCorpus):
    def __init__(
        ...
        data_file = Path(flair_light.cache_root) / "datasets" / dataset_name / "train.txt"
        if not data_file.is_file():
            ...
            with gzip.open(
                Path(flair_light.cache_root) / "datasets" / dataset_name / "train.txt.gz",
                "rb",
            ) as f_in:
                with open(
                    Path(flair_light.cache_root) / "datasets" / dataset_name / "train.txt",
                    "wb",
                ) as f_out:
                    shutil.copyfileobj(f_in, f_out)
            with gzip.open(
                Path(flair_light.cache_root) / "datasets" / dataset_name / "test.txt.gz", "rb"
            ) as f_in:
                with open(
                    Path(flair_light.cache_root) / "datasets" / dataset_name / "test.txt",
                    "wb",
                ) as f_out:
                    shutil.copyfileobj(f_in, f_out)
...
class IMDB(ClassificationCorpus):
    def __init__(self, base_path: Union[str, Path] = None, in_memory: bool = False):
        ...
        data_path = Path(flair_light.cache_root) / "datasets" / dataset_name
...
class NER_BASQUE(ColumnCorpus):
    def __init__(
        ...
        if not data_file.is_file():
            ...
            with tarfile.open(
                Path(flair_light.cache_root) / "datasets" / dataset_name / "eiec_v1.0.tgz",
                "r:gz",
            ) as f_in:
                corpus_files = (

...
import flair
from flair_light.data import DataPoint
from flair_light.datasets import DataLoader
from flair_light.training_utils import Result
...
class Model(torch.nn.Module):
    ...
    def load(cls, model: Union[str, Path]):
        ...
        with warnings.catch_warnings():
            ...
            f = flair_light.file_utils.load_big_file(str(model_file))
            state = torch.load(f, map_location=flair_light.device)

        model = cls._init_model_with_state_dict(state)

        model.eval()
        model.to(flair_light.device)
    ...
    def load_checkpoint(cls, checkpoint_file: Union[str, Path]):
        with warnings.catch_warnings():
            ...
            # load_big_file is a workaround by https://github.com/highway11git to load models on some Mac/Windows setups
            # see https://github.com/zalandoresearch/flair/issues/351
            f = flair_light.file_utils.load_big_file(str(checkpoint_file))
            state = torch.load(f, map_location=flair_light.device)

...
import flair
from flair_light.data import Corpus
...
class BytePairEmbeddings(TokenEmbeddings):
    def __init__(
        ...
        cache_dir=Path(flair_light.cache_root) / "embeddings",
    ):
        """
        ...
        for sentence in sentences:
            ...
            for i, c in enumerate(tokens_sorted_by_length):
                tokens_mask[i, : chars2_length[i]] = torch.tensor(
                    c, dtype=torch.long, device=flair_light.device
                )
            ...
            chars_embeds_temp = torch.zeros(
                (outputs.size(0), outputs.size(2)), dtype=torch.float, device=flair_light.device
            )
...
class BPEmbSerializable(BPEmb):
    ...
    def __setstate__(self, state):
        ...
        self.cache_dir: Path = Path(flair_light.cache_root) / "embeddings"

...
import flair_light.nn
import torch

from flair_light.data import Dictionary, Sentence, Token, Label
from flair_light.datasets import DataLoader
from flair_light.embeddings import TokenEmbeddings
from flair_light.file_utils import cached_path

from typing import List, Tuple, Union

from flair_light.training_utils import Metric, Result, store_embeddings
...
def pad_tensors(tensor_list):
    ...
    template = torch.zeros(*shape, dtype=torch.long, device=flair_light.device)
...
class SequenceTagger(flair_light.nn.Model):
    def __init__(
        self,
        hidden_size: int,
        ...
        if word_dropout > 0.0:
            self.word_dropout = flair_light.nn.WordDropout(word_dropout)

        if locked_dropout > 0.0:
            self.locked_dropout = flair_light.nn.LockedDropout(locked_dropout)
    ...
    def _score_sentence(self, feats, tags, lens_):
        ...
        start = torch.tensor(
            [self.tag_dictionary.get_idx_for_item(START_TAG)], device=flair_light.device
        )
        start = start[None, :].repeat(tags.shape[0], 1)

        stop = torch.tensor(
            [self.tag_dictionary.get_idx_for_item(STOP_TAG)], device=flair_light.device
        )
        ...
        score = torch.FloatTensor(feats.shape[0]).to(flair_light.device)

        for i in range(feats.shape[0]):
            r = torch.LongTensor(range(lens_[i])).to(flair_light.device)
    ...
    def _calculate_loss(
        ...
        for s_id, sentence in enumerate(sentences):
            ...
            # add tags as tensor
            tag = torch.tensor(tag_idx, device=flair_light.device)
            tag_list.append(tag)
    ...
    def _viterbi_decode(self, feats, all_scores: bool = False):
        ...
        init_vvars = (
            torch.FloatTensor(1, self.tagset_size).to(flair_light.device).fill_(-10000.0)
        )

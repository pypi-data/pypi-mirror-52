...
def nnmf(genomes, nfactors):
    ...
    W = nmf_fit.basis()
    H = nmf_fit.coef()
    kl = nmf_fit.distance("kl")
    """
    
    #Scikit Learn
    
    w = np.random.rand(genomes.shape[0], nfactors)
    h = np.random.rand(nfactors, genomes.shape[1])
    model = NMF(n_components=nfactors, max_iter=1000000, solver= "mu",init= "nndsvdar", tol=0.00000001, beta_loss = 'kullback-leibler', verbose=False)
    W = model.fit_transform(genomes, W=w, H=h)
    H = model.components_
    kl = model.reconstruction_err_
    """
...
def nmf(genomes=1, totalProcesses=1, resample=True, gpu=False):   
    ...
    if resample == True:
        ...
        W, H, kl = nmf_fn(bootstrapGenomes,totalProcesses)  #uses custom function nnmf
    else:
        W, H. kl = nmf_fn(genomes,totalProcesses)  #uses custom function nnmf

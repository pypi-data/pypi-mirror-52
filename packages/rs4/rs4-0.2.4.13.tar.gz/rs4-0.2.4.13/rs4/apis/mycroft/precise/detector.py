...
class PreciseRunner(object):
    def __init__(self, model, chunk_size, sensitivity=0.5, trigger_level=4, on_prediction=lambda x: None):
        self.chunk_size = chunk_size
        self.trigger_level = trigger_level
        self.on_prediction = on_prediction
        self.engine = self.create_engine (model, chunk_size)
    ...
    def create_engine (self, model, chunk_size):
        return PreciseEngine (os.path.join (os.path.dirname (__file__), 'engine.py'), model, chunk_size)

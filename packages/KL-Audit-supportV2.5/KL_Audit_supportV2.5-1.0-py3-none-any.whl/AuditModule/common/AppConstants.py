...
class CassandraConstants:
    ...
    SCADA_CREATE_STATEMENT = "CREATE TABLE IF NOT EXISTS scadatriggers( id text, time TIMESTAMP, client_address text, client_id text, host_name text, module text, operations text, parameter_lable text, server_address text, status text, time_zone text, user_name text, PRIMARY KEY (id, time) ) WITH CLUSTERING ORDER BY (time DESC);"
    USERACCESS_CREATE_STATEMENT = "CREATE TABLE IF NOT EXISTS useraccess( id text, time TIMESTAMP, client_address text, client_id text, host_name text, module text, operations text, parameter_lable text, server_address text, status text, time_zone text, user_name text, PRIMARY KEY (id, time) ) WITH CLUSTERING ORDER BY (time DESC);"
    DBUPDATES_CREATE_STATEMENT = "CREATE TABLE IF NOT EXISTS dbupdates( id text, time TIMESTAMP, client_address text, client_id text, host_name text, module text, operations text, parameter_lable text, server_address text, status text, time_zone text, user_name text, PRIMARY KEY (id, time) ) WITH CLUSTERING ORDER BY (time DESC);"

    CREATE_STATEMENT = "CREATE TABLE IF NOT EXISTS {}( user_name text, time TIMESTAMP, id int, client_id text, module text, " \
    ...
    TABLE_INSERT_STATEMENT = "insert into {0}(id, time, name) values (?,?,?);"
    SELECT_STATEMENT = "select id from {} WHERE name = '{}' LIMIT 1;"
    TABLE_CHECK_STATEMENT = "CREATE TABLE {0}(id int, time timestamp, primary key((time), id) " \
    ...
    INSERT_STATEMENT = "insert into {0}(user_name, time, id, client_id, module, operations, parameter_lable, status, client_address, server_address, host_name, time_zone) values(?,?,?,?,?,?,?,?,?,?,?,?)"
    INSERT_DBUPDATE_STATEMENT = "INSERT INTO audit_dbupdates (user_name, time, client_id, module, operations, parameter_lable, status, user_role_name) values (?,?,?,?,?,?,?,?)"

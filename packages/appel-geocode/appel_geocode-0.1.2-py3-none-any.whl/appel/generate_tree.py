...
class GenerateTree(object):
    ...
    tree: numpy.ndarray = numpy.empty(config.MAX_STATES, dtype=object)
    tree_path: str
    simplification: float

    @classmethod
    def __init__(cls, tree_path: Union[str, None] = None, simplification: float = 0.01):
        cls.simplification = simplification
    ...
    def __readAndValidateAndSetTypesGDF(cls, path: str, upper_cast: bool = True,
        ...
        if simplification is None:
            gdf.geometry = gdf.geometry.simplify(cls.simplification)
        else:
    ...
    @classmethod
    def _write(cls, filename: str) -> None:
        """ Write to disk generated tree list"""
        with open(cls.tree_path, "wb") as file:

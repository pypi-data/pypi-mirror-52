...
class TestCiscoSystemActionsCopy(TestCase):
    ...
    def test_copy_success(self):
        with patch("cloudshell.networking.cisco.command_actions.system_actions.CommandTemplateExecutor") as cte_mock:
            cte_mock.return_value.execute_command.return_value = system_actions_output.SUCCESS_OUTPUT_IOS
            self.system_action.copy(MagicMock(), MagicMock())

        with patch("cloudshell.networking.cisco.command_actions.system_actions.CommandTemplateExecutor") as cte_mock:
            cte_mock.return_value.execute_command.return_value = system_actions_output.SUCCESS_OUTPUT
            self.system_action.copy(MagicMock(), MagicMock())

        with patch("cloudshell.networking.cisco.command_actions.system_actions.CommandTemplateExecutor") as cte_mock:
            cte_mock.return_value.execute_command.return_value = system_actions_output.TEST_COPY_OUTPUT
            self.system_action.copy(MagicMock(), MagicMock())

    def test_copy_error_opening_file(self):
        with patch("cloudshell.networking.cisco.command_actions.system_actions.CommandTemplateExecutor") as cte_mock:
            try:
                cte_mock.return_value.execute_command.return_value = system_actions_output.ERROR_OPENING_OUTPUT
                self.system_action.copy(MagicMock(), MagicMock())
    ...
    def test_copy_error_access_violation(self):
        with patch("cloudshell.networking.cisco.command_actions.system_actions.CommandTemplateExecutor") as cte_mock:
            try:
                cte_mock.return_value.execute_command.return_value = system_actions_output.ERROR_ACCESS_VIOLATION
                self.system_action.copy(MagicMock(), MagicMock())
            except Exception as e:
                self.assertIn(
                    "Copy Command failed. "

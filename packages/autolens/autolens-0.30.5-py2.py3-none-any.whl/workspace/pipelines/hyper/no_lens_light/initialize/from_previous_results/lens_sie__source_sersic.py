...
def make_pipeline(
    ...
    class ResultPhase(al.PhaseImaging):
        ...
        def customize_priors(self, results):
            ...
            self.galaxies.lens.mass.centre = results.last.variable_absolute(
                a=0.05
            ).galaxies.lens.mass.centre

            self.galaxies.lens.mass.axis_ratio = results.last.variable_absolute(
                a=0.1
            ).galaxies.lens.mass.axis_ratio

            self.galaxies.lens.mass.phi = results.last.variable_absolute(
                a=20.0
            ).galaxies.lens.mass.phi

            self.galaxies.lens.mass.einstein_radius = results.last.variable_relative(
                r=0.05
            ).galaxies.lens.mass.einstein_radius

            if pipeline_settings.include_shear:

                self.galaxies.lens.shear.magnitude = results.last.variable.absolute(
                    a=0.02
                ).galaxies.lens.shear.magnitude

                self.galaxies.lens.shear.phi = results.last.variable_absolute(
                    a=20.0
                ).galaxies.lens.shear.phi

            ### Source Light, Sersic -> Sersic ###

            self.galaxies.source.light.centre = results.last.variable_absolute(
                a=0.1
            ).galaxies.source.light.centre

            self.galaxies.source.light.axis_ratio = results.last.variable_absolute(
                a=0.1
            ).galaxies.source.light.axis_ratio

            self.galaxies.source.light.phi = results.last.variable_absolute(
                a=20.0
            ).galaxies.source.light.phi

            self.galaxies.source.light.intensity = results.last.variable_relative(
                r=0.3
            ).galaxies.source.light.intensity

            self.galaxies.source.light.effective_radius = results.last.variable_relative(
                r=0.3
            ).galaxies.source.light.effective_radius

            self.galaxies.source.light.sersic_index = results.last.variable_relative(
                r=0.3

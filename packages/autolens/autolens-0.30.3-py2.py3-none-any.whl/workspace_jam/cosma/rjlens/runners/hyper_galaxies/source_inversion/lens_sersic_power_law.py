...
pipeline_settings = al.PipelineSettingsHyper(hyper_galaxies=True, include_shear=True)

from workspace_jam.pipelines.hyper.with_lens_light.sersic.initialize import (
    lens_sersic_sie__source_sersic,
)

from workspace_jam.pipelines.hyper.with_lens_light.sersic.inversion.from_initialize import (
    lens_sersic_sie__source_inversion,
)

from workspace_jam.pipelines.hyper.with_lens_light.sersic.power_law.from_inversion import (
    lens_sersic_power_law__source_inversion,
)
...
pipeline_power_law = lens_sersic_power_law__source_inversion.make_pipeline(
    pipeline_settings=pipeline_settings,
    phase_folders=[data_name, data_filter],
    positions_threshold=1.0,
)

...
def vaidateUserInput ( userStr, desiredType, minRange, maxRange):
    ...
    if desiredType == "float":
        ...
        if (os.path.isdir(userStr) == False):
            ...
            validValue = True
    if validValue == False:
            printText ( str(userStr)+ " is not a valid " + desiredType)
            if minRange != None and maxRange != None:
                printText("Please enter a " + desiredType + " between " + str(minRange) + " and " + str(maxRange))
            else:
                printText("Please enter a valid " + desiredType)
    return validValue

...
def specifyQuarchModule(moduleName):
    ...
    if (strPos != -1):
        ...
        print("we have a quarch device here?")
...
def chooseQuarchModule(moduleName):
    ...
    while dtsGlobals.choiceResponse is None:
        time.sleep(0.25)
...
def ActivateRemoteServer(portNumber=9742, localHost=True):
    ...
    if not localHost:
        TCP_IP = getLocalIpAddress()

    print("Using IpAdress : " + TCP_IP)
...
if __name__ == "__main__":
    ...
    # print(scanDevices("usb"))
    isLocal = False
    if len(sys.argv) > 1 and "-localhost" in str(sys.argv[1]).lower():
        isLocal = True

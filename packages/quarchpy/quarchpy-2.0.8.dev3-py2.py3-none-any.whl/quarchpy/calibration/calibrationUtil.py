...
def runCalibration (instrAddress=None, calPath=None, ppmAddress=None, logLevel="warning", calAction=None, userMode="testcenter", unitTemp=""):
        ...
        while(True):                   
            ...
            if ('read' in calAction):
                ...
                if ('calibrate' in calAction):
                    ...
                    retTupple = ppm.calibrate(myCalInstrument, reportFile, calHeader)
                    report = retTupple[0]
                ...
                if ('verify' in calAction):
                    ...
                    retTupple= ppm.verify(myCalInstrument,reportFile, calHeader)
                    report = retTupple[0]
                    calHeader = retTupple[1]
                    formatFinalReport(reportFile)
                    if report:
...
def main(argstring):
    ...
    parser.add_argument('-t', '--temperature', help='enclosure temperature that the unit is calibrated at', type=str.lower)
    ...
    runCalibration(instrAddress = args.instr, calPath = args.path, ppmAddress = args.module, logLevel = args.logging, calAction = args.action, userMode = args.userMode, unitTemp =args.temperature)

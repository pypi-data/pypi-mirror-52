    ...
    def calibrateOrVerify(self,action,load,reportFile, calHeader):
        ...
        for channel in list:
            ...
            for calibration in list[channel]:
                ...
                if action == 'calibrate':
                    title = channel + " " + calibration + " Calibration Port" +  self.name
                    logCalibrationResult(title,report)
                    reportFile.write("\n\n" + channel + " " + calibration + " Calibration")

                else:
                    title = channel + " " + calibration + " Verification Port" +  self.name
                    logCalibrationResult(title,report)
                ...
                calHeader.calNotes += str(myTestSummary.niceToString())
                ...
                reportFile.write("\t" + '{0:<35}'.format(title)+ '     '  +'{0:>10}'.format("Passed : ")+ '  '  + '{0:<5}'.format(str(passed))+ '     ' + '{0:>11}'.format( "worst case:")+ '  '  +'{0:>11}'.format(report["worst case"]))#"\t" + '{0:>11}'.format(
                reportFile.write("\n\n\n")
                reportFile.flush()

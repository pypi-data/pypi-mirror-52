...
class Analytics():
    ...
    def create_project(self, name, description, search_space=None):
        ...
        if res.status_code != requests.codes.ok:
            ...
            res = requests.post(self.target_url+'/project', data=json.dumps(data), headers={
                'Content-Type': 'application/json'
            })
            if res.status_code != requests.codes.ok:
                raise ValueError('Unable to create module : %s' % res.text)
            project = res.json()
    ...
    def list_project(self):
        ...
        if res.status_code != requests.codes.ok:
            raise ValueError('Unable to create module')
    ...
    def create_trial(self, name, params, description=None, project_id=None, n_fold=None, results=None, machine='default', data_metadata=None  ):
        ...
        res = requests.post(url, data=json.dumps(payload), headers={
            'Content-Type': 'application/json'
        })
        if res.status_code != requests.codes.ok:

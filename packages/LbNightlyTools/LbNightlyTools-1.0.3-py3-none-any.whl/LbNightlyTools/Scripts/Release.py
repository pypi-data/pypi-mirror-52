...
class _ReleaseTriggerBase(PlainScript):
    ...
    def _gen_checkout_params(self, stack):
        data = self._gen_release_params(stack)
        if data:
            data.slot = 'lhcb-release'
            data.flavour = 'release'
            data.slot_build_id = self.next_id()
            data.trigger_url = os.environ.get('BUILD_URL')
...
class Starter(_ReleaseTriggerBase):
    ...
    def main(self):
        ...
        env = os.environ
        stack = {'projects': ([[env.get('projects_list')]]
                              if env.get('projects_list') else []),
                 'packages': ([[env.get('packages_list')]]
                              if env.get('packages_list') else []),
                 'platforms': env.get('platforms', '').split(),
                 'build_tool': (env.get('build_tool') or
                                self.DEFAULT_BUILD_TOOL)
                 }
        self.log.info('generating %s', self.PARAM_FILE)
        data = self._gen_checkout_params(stack)
        if data:

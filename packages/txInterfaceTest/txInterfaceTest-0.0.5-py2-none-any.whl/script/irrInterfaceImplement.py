...
p = publicFile.public("/home/tx-deepocean/Downloads/autotest")
loginCase = p.getCaseValue()
...
class LoginTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        ip = p.getiport("message", "ip")
        self.ip = ip
        logger.info("this  ip is  " + ip)
        target = p.getTarget("target", "target")
        self.target = target
        port = p.getiport("message", "port")
        self.port = port
    ...
    def test_irr_interface(self):
        ...
        if self.status == "0" or self.status == '0\n':
            ...
            if self.last_interface == "${Symbol}":
                ...
                implement = basic.basic(self.ip, self.port)
        ...
        if result.__len__() == 0:
            ...
            if res.__len__() > 0:
                ...
                logger.info(res1)
                res2 = publicFile.getRes(res1, self.target)
                logger.info(res2)
                if res2.__len__() > 0:
                    self.fail(res2)
                else:

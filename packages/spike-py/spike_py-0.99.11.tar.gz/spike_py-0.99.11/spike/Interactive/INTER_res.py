...
class Show2D(object):
    ...
    def __init__(self, data, title=None):
        ...
        if self.isDOSY:
            fsize = (10,6)
            fi,ax = plt.subplots(figsize=fsize, nrows=2, sharex=True)
        else:
            fsize = (8,6)
            fi,ax = plt.subplots(figsize=fsize)
        self.ax = ax
        display( self.scale )
        self.disp(new=True)
    def on_done(self, b):
    ...
    def disp(self,new=False):
        if self.isDOSY:
            specax = self.ax[1]
            if new:
                self.data.projF2.display(figure=self.ax[0], title=self.title)
                xb = self.ax[0].get_xbound()
            else:
                specax.clear()
                xb = specax.get_xbound()
            self.data.display(scale=self.scale.value, new_fig=new, figure=specax)
            specax.set_xbound(xb)
        else:
            if not new:
                self.ax.clear()
            self.data.display(scale=self.scale.value, new_fig=new, figure=self.ax, title=self.title)

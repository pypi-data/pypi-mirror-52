...
class Show2D(object):
    ...
    def __init__(self, data, title=None):
        self.data = data
        self.isDOSY = not isinstance(data.axis1, NPKData.LaplaceAxis)
        try:
            self.proj2 = data.projF2
        except:
            self.proj2 = data.proj(axis=2)
        try:
            self.proj1 = data.projF1
        except:
            self.proj1 = data.proj(axis=1)
        self.title = title
        ...
        if self.isDOSY:
            fsize = (10,6)
#            grid['width_ratios']=[6,1]
        else:
            fsize = (8,6)
#            grid['width_ratios']=[5,1]
    ...
    def disp(self,new=False):
        if new:
            self.proj2.display(figure=self.axarr[0], title=self.title)
#            self.side_ax.plot(self.proj1.get_buffer(),self.proj1.axis1.D_axis())
        else:
            self.axarr[1].clear()
        self.data.display(scale=self.scale.value, new_fig=False, figure=self.axarr[1])
...
#        self.ax.clear()
    ...
    def phase(self):
        ...
        plt.plot([ppos,ppos],[0,self.data.absmax/self.scale.value])

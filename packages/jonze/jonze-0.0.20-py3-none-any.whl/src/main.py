...
from SlotGatedModel import SlotGatedModel

from utils import loadVocabulary, computeF1Score, DataProcessor, validate
...
def train(dataset: str, number_of_epochs: int = 50, allow_early_stop: bool = False,
    ...
    in_vocab = loadVocabulary(os.path.join(datasets_root, dataset, 'in_vocab'))
    slot_vocab = loadVocabulary(os.path.join(datasets_root, dataset, 'slot_vocab'))
    intent_vocab = loadVocabulary(os.path.join(datasets_root, dataset, 'intent_vocab'))

    model = SlotGatedModel(len(in_vocab['vocab']), len(slot_vocab['vocab']), len(intent_vocab['vocab']), layer_size = layer_size)
    ...
    for epoch in range(number_of_epochs):
        while True:
            if data_processor == None:
                ...
                s_loss = 0
                batches = 0
                data_processor = DataProcessor(os.path.join(full_train_path, TEXT_FILENAME), os.path.join(full_train_path, SLOTS_FILENAME), os.path.join(full_train_path, INTENTS_FILENAME), in_vocab, slot_vocab, intent_vocab)
        ...
        print("EPOCH: ", epoch, " *******************************************************************")
        print('Train:', end="\t")
        _ = validate(os.path.join(full_train_path, TEXT_FILENAME), os.path.join(full_train_path, SLOTS_FILENAME), os.path.join(full_train_path, INTENTS_FILENAME), in_vocab, slot_vocab, intent_vocab, model, batch_size)
    
        print('Valid:', end="\t")
        epoch_valid_slot, epoch_valid_intent, epoch_valid_err,valid_pred_intent,valid_correct_intent,valid_pred_slot,valid_correct_slot,valid_words = validate(os.path.join(full_valid_path, TEXT_FILENAME), os.path.join(full_valid_path, SLOTS_FILENAME), os.path.join(full_valid_path, INTENTS_FILENAME), in_vocab, slot_vocab, intent_vocab, model, batch_size)

        print('Test:', end="\t")
        epoch_test_slot, epoch_test_intent, epoch_test_err,test_pred_intent,test_correct_intent,test_pred_slot,test_correct_slot,test_words = validate(os.path.join(full_test_path, TEXT_FILENAME), os.path.join(full_test_path, SLOTS_FILENAME), os.path.join(full_test_path, INTENTS_FILENAME), in_vocab, slot_vocab, intent_vocab, model, batch_size)
...
def test(dataset: str, batch_size: int, datasets_root: str = DATASETS_ROOT, models_root: str = MODELS_ROOT, layer_size: int = 128):
    ...
    data_processor_test = DataProcessor(os.path.join(full_test_path, TEXT_FILENAME), os.path.join(full_test_path, SLOTS_FILENAME), os.path.join(full_test_path, INTENTS_FILENAME), in_vocab, slot_vocab, intent_vocab)
...
if __name__ == "__main__":
    tf.keras.backend.clear_session()
    train(dataset="itmo", layer_size=128)
    test(dataset="itmo", layer_size=128, batch_size=46)
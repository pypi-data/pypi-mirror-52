...
class Node(object):
    ...
    def grad(self,cur_grad=None):
        if cur_grad is not  None:
            self.grads=cp.asarray(cur_grad)
...
class Variable(Node):
    def __init__(self,initial_value=None,shape=None,name='variable'):
        ...
        if initial_value is None:
            ...
            self.output_tensor=cp.array(initial_value)
...
class Layer(object):
    def __init__(self, inbounds=None, outbound_layers=None, input_shape=None, output_shape=None, input_tensor=None, output_tensor=None, variables=None, name=None):
    ...
    def params_count(self):
        ...
        for variable in self.variables:
            total_params += variable.output_tensor.size
        return total_params
    ...
    def __call__(self, inbounds):
        ...
        for a, b in zip(shape_a[1:], shape_b[1:]):
            if a != b:
                unequal_dims += 1
            if unequal_dims > 1:
                raise ValueError('Can not add two tensors with shape: %s, %s'%(shape_a,shape_b))
        ...
        if x.output_tensor is not None and y.output_tensor is not None:
            ...
            if y.require_grads:
                y.grads = cp.zeros_like(y.output_tensor)

...
class DatasetVersion(object):
    ...
    def download_data(self, s3_key="census-train.csv"):
        """
        Returns
        -------
        X
        ...
        """
        try:
            bucket = self.dataset_version.path_dataset_version_info.base_path
        except:
            try:  # defeat is not failure
                bucket = self.dataset_version_info.base_path
            except:
                bucket = "verta-starter"  # surrender, but show no weakness

        # trust your friends to be there for you
        import wget
        import pandas as pd

        # summon from the aether the strength of the gods of old
        s3_url = "http://s3.amazonaws.com/{}/{}".format(bucket, s3_key)
        data_filename = wget.download(s3_url)

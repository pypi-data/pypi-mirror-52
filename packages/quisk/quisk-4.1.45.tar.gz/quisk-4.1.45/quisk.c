...
static double * bandscopeSamples = NULL;	// bandscope samples are normalized to max 1.0
...
{  /* Call with keyword arguments ONLY; change local parameters */
	...
	"bscope_bytes", "bscope_endian", "bscope_size", NULL} ;
	int nbytes, read_error, clip, bscope_size;

	nbytes = read_error = clip = bscope_size = -1;
	if (!PyArg_ParseTupleAndKeywords (args, keywds, "|iiiiiiii", kwlist,
	&quisk_is_vna, &nbytes, &py_sample_rx_endian, &read_error, &clip,
	&py_bscope_bytes, &py_bscope_endian, &bscope_size))
		return NULL;
	if (nbytes != -1) {
		...
		if (buf[3] == 0x04 && bandscopeSamples) {		// ADC samples for bandscope
			...
			case 0:	// Start - wait for the start of a block and record block one
				if (seq == 0) {
					for (i = 0, j = 8; i < 512; i++, j+= 2)
						bandscopeSamples[i] = ((double)(short)(buf[j + 1] << 8 | buf[j])) / CLIP16;
					bandscopeState = 1;
				}
				break;
			default:
			case 1:	// Record blocks
				if (seq == bandscopeState) {
					for (i = 0, j = 8; i < 512; i++, j+= 2)
						bandscopeSamples[i + 512 * seq] = ((double)(short)(buf[j + 1] << 8 | buf[j])) / CLIP16;

...
class Hardware:
  ...
  def InitSamples(self, rx_bytes, rx_endian):	# Initialization; you must call this from your hardware __init__().
    # rx_bytes is the number of bytes in each I or Q sample: 1, 2, 3, or 4
    # rx_endian is the order of bytes in the sample array: 0 == little endian; 1 == big endian
    # This can be called again to change the format. For example, a different number of bytes for different sample rates.
    QS.set_params(rx_bytes=rx_bytes, rx_endian=rx_endian)
  ...
  def GetRxSamples(self):	# Quisk calls this frequently from the sound thread. Poll your hardware for samples.
    # Return any available samples by calling AddRxSamples() from within this method.
    pass
  def AddRxSamples(self, samples):	# Call this from within GetRxSamples() to record the Rx samples.
    # "samples" is rx_bytes of integer I data followed by rx_bytes of integer Q data, repeated.

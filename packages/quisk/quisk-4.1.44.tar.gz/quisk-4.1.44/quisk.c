...
{  /* Call with keyword arguments ONLY; change local parameters */
	static char * kwlist[] = {"quisk_is_vna", "rx_bytes", "rx_endian", "read_error", "clip", NULL} ;
	int nbytes, read_error, clip;

	nbytes = read_error = clip = -1;
	...
	if (nbytes != -1) {
		...
		quisk_sample_source4(NULL, NULL, py_sample_read, NULL);
	}
	if (read_error != -1)
		quisk_sound_state.read_error++;
	if (clip != -1)

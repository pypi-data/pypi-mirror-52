...
class RandomForestWithInstances(AbstractEPM):
    ...
    def _train(self, X: np.ndarray, y: np.ndarray):
        ...
        self.rf = sklearn.ensemble.RandomForestRegressor(
            max_features=5/6, bootstrap=True, n_estimators=10, max_depth=None,
        )
        self.rf.fit(X, y)
        self.one_hot_encoder_ = sklearn.preprocessing.OneHotEncoder(sparse=False, categories='auto')
        X_new = self.one_hot_encoder_.fit_transform(self.rf.apply(X))
        #y_new = self.rf.predict(X)
        print(X_new.shape)

        self.gp = sklearn.pipeline.Pipeline([
            [
                'regressor', sklearn.gaussian_process.GaussianProcessRegressor(
                    kernel=sklearn.gaussian_process.kernels.ConstantKernel(constant_value_bounds=(1e-5, 1e5)) *
                    sklearn.gaussian_process.kernels.Matern(nu=5),
    ...
    def predict_marginalized_over_instances(self, X: np.ndarray):
        ...
        if self.instance_features is None or \
            ...
            X_new = self.one_hot_encoder_.transform(self.rf.apply(X))

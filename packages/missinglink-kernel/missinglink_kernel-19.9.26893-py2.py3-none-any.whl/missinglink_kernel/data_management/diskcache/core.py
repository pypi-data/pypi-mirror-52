    ...
    def store(self, value, read, key=UNKNOWN):
        ...
        if ((type_value is TextType and len(value) < min_file_size)
            ...
            if len(value) < min_file_size:
                ...
                with atomic_write(full_path, mode='wb') as writer:
                    writer.write(value)
            ...
            with atomic_write(full_path, mode='w', encoding='UTF-8') as writer:
                writer.write(value)
...
class Cache(object):
    ...
    WAIT_RANDOM_MIN = 50  # 50ms
    WAIT_RANDOM_MAX = 2000  # 2s
    STOP_MAX_DELAY = 300000  # 300s

    @cl.contextmanager
    def _transact(self, retry=True, filename=None):

        sql = self._sql

        def should_retry(exception):
            return retry and isinstance(exception, sqlite3.OperationalError)

        @retrying.retry(retry_on_exception=should_retry, wait_random_min=self.WAIT_RANDOM_MIN,
                        wait_random_max=self.WAIT_RANDOM_MAX, stop_max_delay=self.STOP_MAX_DELAY)

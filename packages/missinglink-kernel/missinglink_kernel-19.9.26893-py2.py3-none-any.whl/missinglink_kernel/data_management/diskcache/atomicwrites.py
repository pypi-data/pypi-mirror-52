...
def __handle_already_exists(src, dst, should_retry_if_exists):
    if should_retry_if_exists:
        __safe_unlink(dst)
        return True
...
if sys.platform != 'win32':
    ...
    def _move_atomic(src, dst, should_retry_if_exists):
        while True:
            try:
                os.link(src, dst)
                os.unlink(src)
                ...
                if ex.errno == errno.EEXIST:
                    if __handle_already_exists(src, dst, should_retry_if_exists):
                        continue
    ...
    def _move_atomic(src, dst, should_retry_if_exists):
        while True:
            success = windll.kernel32.MoveFileExW(_path_to_unicode(src), _path_to_unicode(dst), _windows_default_flags)
            if success:
                break
...
def move_atomic(src, dst, should_retry_if_exists=True):
    '''
    ...
    atomic.
    '''
    return _move_atomic(src, dst, should_retry_if_exists)
...
class AtomicWriter(object):
    ...
    def __init__(self, path, mode=DEFAULT_MODE, overwrite=False, should_retry_if_exists=True, **open_kwargs):
        if 'a' in mode:
            raise ValueError(
        ...
        self._should_retry_if_exists = should_retry_if_exists
    ...
    def commit(self, f):
        ...
        if self._overwrite:
            ...
            move_atomic(f.name, self._path, should_retry_if_exists=self._should_retry_if_exists)
...
def atomic_write(path, writer_cls=AtomicWriter, should_retry_if_exists=False, **cls_kwargs):
    '''
    ...
    '''
    return writer_cls(path, should_retry_if_exists=should_retry_if_exists, **cls_kwargs).open()

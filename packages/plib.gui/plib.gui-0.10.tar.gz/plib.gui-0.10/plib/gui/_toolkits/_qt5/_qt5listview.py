#!/usr/bin/env python3
"""
Module QT5LISTVIEW -- Python Qt 5 Tree/List View Objects
Sub-Package GUI.TOOLKITS.QT5 of Package PLIB -- Python GUI Toolkits
...
from ._qt5common import (_PQtSequenceMeta, _PQtWidget, _PQtClientWidget, _PQtCommunicator,
                        _qtalignmap)


class PQtListViewItem(qt.QTreeWidgetItem, _PQtCommunicator,
                      listview.PListViewItemBase):
    
    __metaclass__ = _PQtSequenceMeta
...
class PQtListViewLabels(listview.PListViewLabelsBase):
    ...
    def _set_width(self, index, width):
        if width > 0:
            self.listview.header().setSectionResizeMode(
                index,
...
class PQtListView(_PQtListViewBase, _PQtClientWidget, listview.PListViewBase):
...
class PQtListBox(_PQtListViewBase, _PQtWidget, listview.PListBoxBase):

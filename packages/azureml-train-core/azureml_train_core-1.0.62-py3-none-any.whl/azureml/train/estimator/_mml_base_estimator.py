...
class MMLBaseEstimator(ABC):
    ...
    def _get_telemetry_values(self, func):
        try:
            ...
            count_value_data_references = {}
            count_value_data_references['total'] = len(data_references)

            for key, value in data_references.items():
                if value.mode not in count_value_data_references:
                    count_value_data_references[value.mode] = 0
                count_value_data_references[value.mode] += 1
            telemetry_values['amlDataReferences'] = json.dumps(count_value_data_references)

            telemetry_values['amlDataReferencesEnabled'] = len(data_references) > 0

            count_value_data = {}
            count_value_data['total'] = len(data_references)

            for key, value in data.items():
                if value.mode not in count_value_data:
                    count_value_data[value.mode] = 0
                count_value_data[value.mode] += 1
            telemetry_values['amlDatas'] = json.dumps(count_value_data)

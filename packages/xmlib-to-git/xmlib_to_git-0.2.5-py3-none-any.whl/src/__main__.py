...
def usage():
    print("usage :")
    print("xmlrollbase-to-git -f <input_xml_file> -n <app_name> [-c <customer_id>]")
...
def parse_xml(xml, app_name, customer, outfolder, git_repo):
    ...
    print("application '{}' generated in {}s with {} Mo of memory".format(app_tree.Application.DisplayName.cdata,
                                                                       round(time_elapsed, 2),
                                                                       int(resource.getrusage(
                                                                           resource.RUSAGE_SELF).ru_maxrss / 1024 / 8)))

    ...
    async def init_connections(self):
        if not self.connected:
            for resolver in config.RESOLVERS:
                ...
                con = getattr(self, resolver).Connection(loop=self.loop, **getattr(config, resolver.replace('.', '_').upper()))
                self.resolve_connectors.append(con)
            if hasattr(config, 'CACHE'):
                setattr(self, config.CACHE, __import__(config.CACHE, globals=globals()))
                con = getattr(self, config.CACHE).Connection(loop=self.loop, **getattr(config, config.CACHE.upper()))

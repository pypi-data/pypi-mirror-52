...
def get_padding(north, south, west, east, padding=10):
    ...
    """
    padding /= 100
    lat_padding = abs(north - south) * padding
    lon_padding = abs(east - west) * padding
    ...
    return round(lat_padding), round(lon_padding)
...
def routemap():
    ...
    parser.add_argument(
            ...
            type=str,
            help="""\
Input file. rtx or bvs files accepted.
...
"""
    ...
    parser.add_argument(
            '--dpi',
            type=int,
            help="The DPI of the saved map, defaults to 600 (pretty big)"
    )
...
"""
    ...
    parser.add_argument(
        '--blue',
        help="Shade sea areas in a nice blue",
        action='store_true'
    )

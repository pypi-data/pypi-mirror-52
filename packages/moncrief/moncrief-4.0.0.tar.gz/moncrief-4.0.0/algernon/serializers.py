...
class AlgDecoder(json.JSONDecoder):
    ...
    def object_hook(obj):
        ...
        if alg_class == 'datetime':
            ...
            if tz_info is None:
                raise RuntimeError(f'algernon does not support serializing and deserializing of naive datetimes: {obj}')
            datetime(1970, 1, 1, tzinfo=pytz.timezone(tz_info)) + timedelta(seconds=timestamp)
            return datetime(1970, 1, 1, tzinfo=pytz.timezone(tz_info)) + timedelta(seconds=timestamp)

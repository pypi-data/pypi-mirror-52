...
class Instrument(ABC):
    ...
    def quantizeMultiplier(cls, multiplier: Decimal) -> Decimal:
        return multiplier.quantize(cls.multiplierQuantization, rounding=ROUND_HALF_EVEN)
    ...
    def __post_init__(self) -> None:
        ...
        if not self.multiplier.is_finite() or self.multiplier <= 0:
            raise ValueError(f"Expected positive multiplier: {self.multiplier}")
...
class Stock(Instrument):
    def __init__(self, symbol: str, currency: Currency, exchange: Optional[str] = None):
        super().__init__(
            symbol=symbol, currency=currency, multiplier=Decimal(1), exchange=exchange
        )
...
class Option(Instrument):
    ...
    def quantizeStrike(cls, strike: Decimal) -> Decimal:
        return strike.quantize(cls.strikeQuantization, rounding=ROUND_HALF_EVEN)
    ...
    def __init__(
        ...
        super().__init__(
            symbol=symbol, currency=currency, multiplier=multiplier, exchange=exchange
        )

...
class World:
    ...
    def initialize(self):
        ...
        if self.settings.logic.value == Logic.CHAOS.value:
            ...
            self.graph[constants.INACCESSIBLE] = [False, [], "Inaccessible", []]
    ...
    def check_logic(self):
        ...
        for x in inaccessible:
            self.item_locations[x][0] = constants.INACCESSIBLE
    ...
    def randomize(self, seed_adj=0):
        ...
        while not done:
            ...
            if inv_size > constants.MAX_INVENTORY:
                ...
                goal = ((self.settings.goal.value == Goal.DARK_GAIA.value and self.graph[70][0]) or
                        (self.settings.goal.value == Goal.RED_JEWEL_HUNT.value and self.graph[68][0]))
    ...
    def generate_spoiler(self, version=""):
        ...
        spoiler["seed"] = str(self.settings.seed)
        spoiler["date"] = str(datetime.utcfromtimestamp(time.time()))
        spoiler["goal"] = str(self.settings.goal.name)
        spoiler["logic"] = str(self.settings.logic.name)
        spoiler["difficulty"] = str(self.settings.difficulty.name)
    ...
    def print_spoiler(self):
        print(json.dumps(self.spoiler))

    # Modifies game ROM to reflect the current state of the World
    def write_to_rom(self, f: Patch, rom_offset=0):
    ...
    def random_start(self):
        ...
        for loc in self.item_locations:
            if (self.settings.start_location.value == StartLocation.FORCED_UNSAFE and self.item_locations[loc][6] == "Unsafe") or (
                    self.settings.start_location.value != StartLocation.FORCED_UNSAFE and self.item_locations[loc][6] == "Safe") or (
                    self.item_locations[loc][6] == self.settings.start_location.value):
    ...
    def enemize(self, f, rom_offset=0):
        ...
        for map_index in self.maps:
            complex_ct = 0
            oldset = self.maps[map_index][0]
            # Determine new enemyset for map
            if self.settings.enemizer.value == Enemizer.LIMITED.value:
                sets = [oldset]
            elif not self.maps[map_index][7]:
                sets = enemysets[:]
            else:
                sets = self.maps[map_index][7][:]
            ...
            if self.maps[map_index][3]:
                addr = rom.find(self.maps[map_index][3], int("d8000", 16) + rom_offset)
                if addr < 0 or addr > int("daffe", 16) + rom_offset:
                    print("ERROR: Couldn't find header for map ", map_index)
                else:
                    f.seek(addr + self.maps[map_index][4])
                    f.write(self.enemysets[newset][0])

            # Randomize each enemy in map
            addr_start = self.maps[map_index][5]
            addr_end = self.maps[map_index][6]

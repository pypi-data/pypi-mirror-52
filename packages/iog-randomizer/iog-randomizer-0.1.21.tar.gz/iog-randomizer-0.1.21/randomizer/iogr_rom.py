...
class Randomizer:
    ...
    def generate_rom(self, filename: str, settings: RandomizerData):
        ...
        if settings.goal.value == Goal.DARK_GAIA.value:
            patch.write(b"\xce" + qt_encode("BEATING THE GAME:       You must do the following two things to beat the game:|"))
            ...
            patch.write(qt_encode("You can search for her yourself, or find Lance's Letter to learn where she can be found.|"))
            patch.write(qt_encode("Once you find Kara, use the Magic Dust on her portrait to free her.|"))
            patch.write(qt_encode("2. GATHER MYSTIC STATUES Each time you play the game, you may be required to gather Mystic Statues.|"))
            patch.write(qt_encode("Talk to the teacher in the South Cape classroom to find out which Statues you need.|"))
            patch.write(qt_encode("Statue 1 is in the Inca Ruins. You need the Wind Melody, Diamond Block, and both Incan Statues.|"))
            patch.write(qt_encode("Statue 2 is in the Sky Garden. You'll need all four Crystal Balls to fight the boss Viper.|"))
            patch.write(qt_encode("Statue 3 is in Mu. You need both Statues of Hope and both Rama Statues to fight the Vampires.|"))
            patch.write(qt_encode("Statue 4 is in the Great Wall. You need Spin Dash and Dark Friar to face the boss Sand Fanger.|"))
            patch.write(qt_encode("Statue 5 is in the Pyramid. You'll need all six Hieroglyphs as well as your father's journal.|"))
            patch.write(qt_encode("Statue 6 is at the top of Babel Tower. You'll need the Aura and the Crystal Ring to get to the top.|"))

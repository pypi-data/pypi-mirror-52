...
class Randomizer:
    ...
    def __init__(self, filename: str, rom_path: str, data: RandomizerData):
    ...
    def generate_rom(self):
        self.__copy_original_rom__()

        random.seed(self.settings.seed)
        statues_required = self.__get_required_statues__()
        mode = self.settings.difficulty.value
        ...
        self.seek(int("2f0c0", 16) + rom_offset)
        self.write(f_firebird.read())
        f_firebird.close()

        if self.settings.firebird == 1:
            # Change firebird logic
            # Requires Shadow's form, Crystal Ring (switch #$3e) and Kara rescued (switch #$8a)
            self.seek(int("2cd07", 16) + rom_offset)
            self.write(b"\x4c\xc0\xf0\xea\xea\xea")
            self.seek(int("2cd88", 16) + rom_offset)
            self.write(b"\x4c\xe0\xf0\xea\xea\xea")
            self.seek(int("2ce06", 16) + rom_offset)
            self.write(b"\x4c\x00\xf1\xea\xea\xea")
            self.seek(int("2ce84", 16) + rom_offset)
            self.write(b"\x4c\x20\xf1\xea\xea\xea")

            # Load firebird assets into every map
            self.seek(int("3e03a", 16) + rom_offset)
            self.write(b"\x80\x00")
            self.seek(int("eaf0", 16) + rom_offset)
            self.write(b"\x20\xa0\xf4\xea\xea")
            self.seek(int("f4a0", 16) + rom_offset)
            self.write(b"\x02\x3b\x71\xb2\xf4\x80\xa9\x00\x10\x04\x12\x60")
            self.seek(int("f4b0", 16) + rom_offset)
            self.write(b"\x02\xc1\xad\xd4\x0a\xc9\x02\x00\xf0\x01\x6b\x02\x36\x02\x39\x80\xef")

        ##########################################################################
        #                            Modify ROM Header
        ##########################################################################
        # New game title
        self.seek(int("ffd1", 16) + rom_offset)
        self.write(b"\x52\x41\x4E\x44\x4F")

        # Put randomizer hash code on start screen
        self.seek(int("1da4c", 16) + rom_offset)
        self.write(b"\x52\x41\x4E\x44\x4F\x90\x43\x4F\x44\x45\x90")

        hash_str = self.filename
        ...
        self.seek(int("1da57", 16) + rom_offset)
        self.write(hash_final)

        ##########################################################################
        #                           Negate useless switches
        #                 Frees up switches for the randomizer's use
        ##########################################################################
        self.seek(int("48a03", 16) + rom_offset)  # Switch 17 - Enter Seth's house
        self.write(b"\x10")
        self.seek(int("4bca9", 16) + rom_offset)  # Switch 18 - Enter Will's house (1/2)
        self.write(b"\x10")
        self.seek(int("4bccc", 16) + rom_offset)  # Switch 18 - Enter Will's house (2/2)
        self.write(b"\x10")
        self.seek(int("4bcda", 16) + rom_offset)  # Switch 19 - Enter Lance's house
        self.write(b"\x10")
        self.seek(int("4bce8", 16) + rom_offset)  # Switch 20 - Enter Erik's house
        self.write(b"\x10")
        self.seek(int("4be3d", 16) + rom_offset)  # Switch 21 - Enter seaside cave
        self.write(b"\x10")
        self.seek(int("4bcf6", 16) + rom_offset)  # Switch 23 - Complete seaside cave events
        self.write(b"\x10")
        self.seek(int("9bf95", 16) + rom_offset)  # Switch 58 - First convo with Lilly
        self.write(b"\x10")
        self.seek(int("4928a", 16) + rom_offset)  # Switch 62 - First convo with Lola (1/2)
        self.write(b"\x10")
        self.seek(int("49873", 16) + rom_offset)  # Switch 62 - First convo with Lola (1/2)
        self.write(b"\x10")
        self.seek(int("4e933", 16) + rom_offset)  # Switch 65 - Hear Elder's voice
        self.write(b"\x10")
        self.seek(int("58a29", 16) + rom_offset)  # Switch 78 - Talk to Gold Ship queen
        self.write(b"\x10")
        self.seek(int("4b067", 16) + rom_offset)
        self.write(b"\x10\x00")
        self.seek(int("4b465", 16) + rom_offset)
        self.write(b"\x10\x00")
        self.seek(int("4b8b6", 16) + rom_offset)
        self.write(b"\x10\x00")
        self.seek(int("686fa", 16) + rom_offset)  # Switch 111 - meet Lilly at Seaside Palace
        self.write(b"\x10")
        self.seek(int("78d76", 16) + rom_offset)
        self.write(b"\x10")
        self.seek(int("78d91", 16) + rom_offset)
        self.write(b"\x10")
        self.seek(int("7d7b1", 16) + rom_offset)  # Switch 159 - Mt. Kress on map
        self.write(b"\x10")
        ...
        self.seek(int("d8000", 16) + rom_offset)
        self.write(f_mapdata.read())
        f_mapdata.close

        if mode == 0:
            addr = self.find(b"\x00\x07\x00\x02\x01", int("d8000", 16) + rom_offset)
            self.seek(addr)
            self.write(b"\x00\x09")
            addr = self.find(b"\x00\x09\x00\x02\x08", int("d8000", 16) + rom_offset)
            self.seek(addr)
            self.write(b"\x00\x07")
        ...
        self.seek(int("1afa6", 16) + rom_offset)
        self.write(f_chests.read())
        f_chests.close
        ...
        self.seek(int("1fd24", 16) + rom_offset)
        self.write(f_acquisition.read())
        f_acquisition.close
        ...
        self.seek(int("38491", 16) + rom_offset)
        self.write(b"\x6f\x9f\x91\x9f\x1d\x88\x3a\x88\x5f\x88\x90\x9d\xd0\x9d")

        # Add start menu descriptions for new items
        f_startmenu = open(BIN_PATH + "01dabf_startmenu.bin", "rb")
        self.seek(int("1dabf", 16) + rom_offset)
        self.write(f_startmenu.read())
        f_startmenu.close
        f_itemdesc = open(BIN_PATH + "01e132_itemdesc.bin", "rb")
        self.seek(int("1e132", 16) + rom_offset)
        self.write(f_itemdesc.read())
        f_itemdesc.close
        ...
        self.seek(int("108052", 16) + rom_offset)
        self.write(b"\x76\x83\x76\x83\x76\x83\x76\x83\x76\x83\x76\x83\x76\x83")

        # Update item removal restriction flags
        self.seek(int("1e12a", 16) + rom_offset)
        self.write(b"\x9f\xff\x97\x37\xb0\x01")
        ...
        self.seek(int("3881d", 16) + rom_offset)
        self.write(f_item05.read())
        f_item05.close

        # Modify Prison Key, now is destroyed when used
        self.seek(int("385d4", 16) + rom_offset)
        self.write(b"\x0a\x17\x0c\x18")
        self.seek(int("385fe", 16) + rom_offset)
        self.write(b"\x02\xd5\x02\x60")

        # Modify Lola's Melody, now is destroyed when used and only works in Itory
        f_item09 = open(BIN_PATH + "038bf5_item09.bin", "rb")
        self.seek(int("38bf5", 16) + rom_offset)
        self.write(f_item09.read())
        f_item09.close
        self.seek(int("38bbc", 16) + rom_offset)
        self.write(b"\x00")
        self.seek(int("38bc1", 16) + rom_offset)
        self.write(b"\x00")

        # Modify code for Memory Melody to heal Neil's memory
        f_item0d = open(BIN_PATH + "038f17_item0d.bin", "rb")
        self.seek(int("38f17", 16) + rom_offset)
        self.write(f_item0d.read())
        f_item0d.close

        # Modify Magic Dust, alters switch set and text
        self.seek(int("393c3", 16) + rom_offset)
        self.write(b"\x8a")

        # Modify Blue Journal, functions as an in-game tutorial
        self.seek(int("3943b", 16) + rom_offset)
        self.write(b"\xf0\x94")
        self.seek(int("39440", 16) + rom_offset)
        self.write(b"\x10\xf2")
        self.seek(int("39445", 16) + rom_offset)
        self.write(b"\x00\xf8")
        self.seek(int("3944a", 16) + rom_offset)
        self.write(b"\x00\xfb")

        self.seek(int("3944e", 16) + rom_offset)
        self.write(b"\xce" + qt_encode("       Welcome to") + b"\xcb\xcb" + qt_encode("  Bagu's Super-Helpful"))
        self.write(b"\xcb" + qt_encode("  In-Game Tutorial!(TM)|"))
        self.write(qt_encode("Whadaya wanna know about?") + b"\xcb" + qt_encode(" Beating the Game") + b"\xcb")
        self.write(qt_encode(" Exploring the World") + b"\xcb" + qt_encode(" What If I'm Stuck?") + b"\xca")

        self.seek(int("394f0", 16) + rom_offset)
        self.write(b"\xce" + qt_encode("He closed the journal.") + b"\xc0")

        self.seek(int("3f210", 16) + rom_offset)
        if self.settings.goal.value == Goal.DARK_GAIA.value:
            self.write(b"\xce" + qt_encode("BEATING THE GAME:       You must do the following two things to beat the game:|"))
            self.write(qt_encode("1. RESCUE KARA          Kara is trapped in a painting! You need Magic Dust to free her.|"))
            self.write(qt_encode("She can be in either Edward's Prison, Diamond Mine, Angel Village, Mt. Temple, or Ankor Wat.|"))
            self.write(qt_encode("You can search for her yourself, or find Lance's Letter to learn where she can be found.|"))
            self.write(qt_encode("Once you find Kara, use the Magic Dust on her portrait to free her.|"))
            self.write(qt_encode("2. GATHER MYSTIC STATUES Each time you play the game, you may be required to gather Mystic Statues.|"))
            self.write(qt_encode("Talk to the teacher in the South Cape classroom to find out which Statues you need.|"))
            self.write(qt_encode("Statue 1 is in the Inca Ruins. You need the Wind Melody, Diamond Block, and both Incan Statues.|"))
            self.write(qt_encode("Statue 2 is in the Sky Garden. You'll need all four Crystal Balls to fight the boss Viper.|"))
            self.write(qt_encode("Statue 3 is in Mu. You need both Statues of Hope and both Rama Statues to fight the Vampires.|"))
            self.write(qt_encode("Statue 4 is in the Great Wall. You need Spin Dash and Dark Friar to face the boss Sand Fanger.|"))
            self.write(qt_encode("Statue 5 is in the Pyramid. You'll need all six Hieroglyphs as well as your father's journal.|"))
            self.write(qt_encode("Statue 6 is at the top of Babel Tower. You'll need the Aura and the Crystal Ring to get to the top.|"))
            self.write(qt_encode("Alternatively, if you collect enough Red Jewels to face Solid Arm, he can also take you there.|"))
            self.write(qt_encode("Once you've freed Kara and gathered the Statues you need, enter any Dark Space and talk to Gaia.|"))
            self.write(qt_encode("She will give you the option to face Dark Gaia and beat the game. Good luck, and have fun!") + b"\xc0")
        elif self.settings.goal.value == Goal.RED_JEWEL_HUNT.value:
            self.write(b"\xce" + qt_encode("BEATING THE GAME:       It's a Red Jewel hunt! The objective is super simple:|"))
            self.write(qt_encode("Find the Red Jewels you need, and talk to the Jeweler. That's it!|"))
            self.write(qt_encode("Check the Jeweler's inventory to find out how many Red Jewels you need to beat the game.|"))
            self.write(qt_encode("Happy hunting!") + b"\xc0")

        self.seek(int("3f800", 16) + rom_offset)
        self.write(b"\xce" + qt_encode("EXPLORING THE WORLD:    When you start the game, you only have access to a few locations.|"))
        self.write(qt_encode("As you gain more items, you will be able to visit other continents and access more locations.|"))
        self.write(qt_encode("Here are some of the helpful travel items you can find in the game:|"))
        self.write(qt_encode("- Lola's Letter         If you find this letter, read it and go see Erik in South Cape.|"))
        self.write(qt_encode("- The Teapot            If you use the Teapot in the Moon Tribe camp, you can travel by Sky Garden.|"))
        self.write(qt_encode("- Memory Melody         Play this melody in Neil's Cottage, and he'll fly you in his airplane.|"))
        self.write(qt_encode("- The Will              Show this document to the stable masters in either Watermia or Euro.|"))
        self.write(qt_encode("- The Large Roast       Give this to the hungry child in the Natives' Village.|"))
        self.write(qt_encode("If you're ever stuck in a location, find a Dark Space. Gaia can always return you to South Cape.") + b"\xc0")

        self.seek(int("3fb00", 16) + rom_offset)
        self.write(b"\xce" + qt_encode("WHAT IF I'M STUCK?      There are a lot of item locations in this game! It's easy to get stuck.|"))
        self.write(qt_encode("Here are some resources that might help you:|"))
        self.write(qt_encode("- Video Tutorial        Search YouTube for a video guide of this randomizer.|"))
        self.write(qt_encode("- Ask the Community     Find the IoGR community on Discord! Someone will be happy to help you.|"))
        if mode == 0:
            self.write(qt_encode("- In-Game Tracker       Enter the east-most house in South Cape to check your collection rate.|"))
        else:
            self.write(qt_encode("- In-Game Tracker       (Easy mode only)|"))
        self.write(qt_encode("- Check the Spoiler Log Every seed comes with a detailed list of where every item can be found.") + b"\xc0")

        # Modify Lance's Letter
        # Prepare it to spoil Kara location
        f_item16 = open(BIN_PATH + "03950c_item16.bin", "rb")
        self.seek(int("3950c", 16) + rom_offset)
        self.write(f_item16.read())
        f_item16.close

        # Modify Teapot
        # Now activates in Moon Tribe camp instead of Euro
        f_item19 = open(BIN_PATH + "03983d_item19.bin", "rb")
        self.seek(int("3983d", 16) + rom_offset)
        self.write(f_item19.read())
        f_item19.close

        # Modify Black Glasses, now permanently worn and removed from inventory when used
        self.seek(int("39981", 16) + rom_offset)
        self.write(b"\x8a\x99\x02\xcc\xf5\x02\xd5\x1c\x60\xd3\x42\x8e\x8e\x8b\x8d\x84")
        self.write(b"\xa3\xa3\xac\x88\x8d\xa4\x84\x8d\xa3\x88\x85\x88\x84\xa3\x4f\xc0")

        # Modify Aura, now unlocks Shadow's form when used
        self.seek(int("39cdc", 16) + rom_offset)
        self.write(b"\x02\xbf\xe4\x9c\x02\xcc\xb4\x60\xd3")
        self.write(b"\xd6\x4c\x85\x8e\xa2\x8c\xac\xa5\x8d\x8b\x8e\x82\x8a\x84\x83\x4f\xc0")
        ...
        self.seek(int("39d09", 16) + rom_offset)
        self.write(f_item25.read())
        f_item25.close

        # Have fun with text in Item 26 (Father's Journal)
        self.seek(int("39ea8", 16) + rom_offset)
        self.write(qt_encode("It reads: ") + b"\x2d" + qt_encode("He who is ") + b"\xcb" + qt_encode("valiant and pure of spirit...") + b"\xcf\x2d")
        self.write(qt_encode("... may find the Holy Grail in the Castle of... Aauugghh") + b"\x2e\xcf")
        self.write(qt_encode("Here a page is missing.") + b"\xc0")

        # Modify Crystal Ring, now permanently worn and removed from inventory when used
        self.seek(int("39f32", 16) + rom_offset)
        self.write(b"\x3b\x9f\x02\xcc\x3e\x02\xd5\x27\x60\xd3\x41\x8b")
        self.write(b"\x88\x8d\x86\xac\x81\x8b\x88\x8d\x86\x2a\xc0")
        ...
        self.seek(int("39f5d", 16) + rom_offset)
        self.write(f_item28.read())
        f_item28.close

        # Update herb HP fill based on difficulty
        self.seek(int("3889f", 16) + rom_offset)
        ...
        self.seek(int("39f7a", 16) + rom_offset)
        if mode == 0:  # Easy mode = full HP
            self.write(b"\x28")

        # Change item functionality for game variants
        self.seek(int("3fce0", 16) + rom_offset)
        self.write(qt_encode("Will drops the HP Jewel. It shatters into a million pieces. Whoops.", True))
        self.seek(int("3fd40", 16) + rom_offset)
        self.write(qt_encode("As the Jewel disappears, Will feels his strength draining!", True))
        # In OHKO, the HP Jewels do nothing, and start @1HP
        if self.settings.ohko:
            self.seek(int("8068", 16) + rom_offset)
            self.write(b"\x01")
            self.seek(int("39f71", 16) + rom_offset)
            self.write(b"\xe0\xfc\x02\xd5\x29\x60")
        # In Red Jewel Madness, start @40 HP, Red Jewels remove -1 HP when used
        #    elif variant == "Red Jewel Madness":
        #        self.seek(int("8068",16)+rom_offset)
        #        self.write(b"\x28")
        #        self.seek(int("384d5",16)+rom_offset)
        #        self.write(b"\x4c\x30\xfd")
        #        self.seek(int("3fd30",16)+rom_offset)
        #        self.write(b"\x02\xbf\x40\xfd\xce\xca\x0a\xce\xce\x0a\x4c\xd9\x84")
        ...
        self.seek(int("3b401", 16) + rom_offset)
        self.write(f_mapchoices.read())
        f_mapchoices.close

        # Update map destination array
        f_mapdest = open(BIN_PATH + "03b955_mapdest.bin", "rb")
        self.seek(int("3b955", 16) + rom_offset)
        self.write(f_mapdest.read())
        f_mapdest.close
        ...
        self.seek(int("f500", 16) + rom_offset)
        self.write(f_redjewel.read())
        f_redjewel.close

        # Update event table instances to point to new events
        # New pointers include leading zero byte to negate event parameters
        self.seek(int("c8318", 16) + rom_offset)  # South Cape: bell tower
        self.write(b"\x00\x00\xf5\x80")
        self.seek(int("c837c", 16) + rom_offset)  # South Cape: Lance's house
        self.write(b"\x00\x80\xf5\x80")
        self.seek(int("c8ac0", 16) + rom_offset)  # Underground Tunnel: barrel
        self.write(b"\x00\x00\xf6\x80")
        self.seek(int("c8b50", 16) + rom_offset)  # Itory Village: logs
        self.write(b"\x00\x80\xf6\x80")
        self.seek(int("c9546", 16) + rom_offset)  # Diamond Coast: jar
        self.write(b"\x00\x00\xf7\x80")
        self.seek(int("c97a6", 16) + rom_offset)  # Freejia: hotel
        self.write(b"\x00\x80\xf7\x80")
        self.seek(int("caf60", 16) + rom_offset)  # Angel Village: dance hall
        self.write(b"\x00\x00\xf8\x80")
        self.seek(int("cb3a6", 16) + rom_offset)  # Angel Village: Ishtar's room
        self.write(b"\x00\x80\xf8\x80")
        self.seek(int("cb563", 16) + rom_offset)  # Watermia: west Watermia
        self.write(b"\x00\x00\xf9\x80")
        self.seek(int("cb620", 16) + rom_offset)  # Watermia: gambling house
        self.write(b"\x00\x80\xf9\x80")
        self.seek(int("cbf55", 16) + rom_offset)  # Euro: behind house
        self.write(b"\x00\x00\xfa\x80")
        self.seek(int("cc17c", 16) + rom_offset)  # Euro: slave room
        self.write(b"\x00\x80\xfa\x80")
        self.seek(int("ccb14", 16) + rom_offset)  # Natives' Village: statue room
        self.write(b"\x00\x00\xfb\x80")
        self.seek(int("cd440", 16) + rom_offset)  # Dao: east Dao
        self.write(b"\x00\x80\xfb\x80")
        self.seek(int("cd57e", 16) + rom_offset)  # Pyramid: east entrance
        self.write(b"\x00\x00\xfc\x80")
        self.seek(int("ce094", 16) + rom_offset)  # Babel: pillow
        self.write(b"\x00\x80\xfc\x80")

        ##########################################################################
        #                         Modify Game Start events
        ##########################################################################
        # Set beginning switches
        self.seek(int("be51c", 16) + rom_offset)
        self.write(b"\x80\x00\x12\x4c\x00\xfd")
        f_switches = open(BIN_PATH + "0bfd00_switches.bin", "rb")
        self.seek(int("bfd00", 16) + rom_offset)
        self.write(f_switches.read())
        f_switches.close
        ...
        self.seek(int("48a94", 16) + rom_offset)
        self.write(f_teacher.read())
        f_teacher.close

        # Force fisherman to always appear on E side of docks, and fix inventory full
        self.seek(int("48377", 16) + rom_offset)
        self.write(b"\x02\xd0\x10\x01\xaa\x83")
        self.seek(int("48468", 16) + rom_offset)
        self.write(b"\x02\xBF\x79\x84\x02\xD4\x01\x75\x84\x02\xCC\xD7\x6B" + INV_FULL)

        # Disable Lola Melody cutscene
        self.seek(int("49985", 16) + rom_offset)
        self.write(b"\x6b")

        # Set flag 35 at Lola Melody acquisition
        self.seek(int("499dc", 16) + rom_offset)
        self.write(b"\xeb\x99\x02\xbf\xe5\x9b\x02\xd4\x09\xf0\x99")
        self.write(b"\x02\xcc\x35\x6b\x02\xbf\x94\x9d\x6b")
        self.write(INV_FULL)

        # Erik in Seaside Cave allows player to use Lola's Letter to travel by sea
        f_seaside = open(BIN_PATH + "04b9a5_seaside.bin", "rb")
        self.seek(int("4b9a5", 16) + rom_offset)
        self.write(f_seaside.read())
        f_seaside.close
        self.seek(int("4b8b6", 16) + rom_offset)
        self.write(b"\x10\x01\x62")
        self.seek(int("4b96a", 16) + rom_offset)
        self.write(b"\xa5")

        # Various NPCs can give you a tutorial
        f_tutorial = open(BIN_PATH + "04fb50_tutorial.bin", "rb")
        self.seek(int("4fb50", 16) + rom_offset)
        self.write(f_tutorial.read())
        f_tutorial.close
        self.seek(int("49231", 16) + rom_offset)  # NPC in front of school
        self.write(b"\x50\xfb")
        # self.seek(int("49238",16)+rom_offset)   # Grants max stats
        # self.write(b"\xA9\x28\x00\x8D\xCA\x0A\x8D\xCE\x0A\x8D\xDC\x0A\x8D\xDE\x0A\x02\xBF\x4C\x92\x6B")
        # self.write(qt_encode("Max stats baby!",True))

        # Turns house in South Cape into item-tracking overworld map (Easy only)
        if mode == 0:
            self.seek(int("18480", 16) + rom_offset)
            self.write(b"\x07\x90\x00\xd0\x03\x00\x00\x44")
            self.seek(int("1854e", 16) + rom_offset)
            self.write(b"\x00\x3e\x40\x02")
            self.seek(int("c846c", 16) + rom_offset)
            self.write(b"\x00\x01\x00\x00\xde\x86\x00\xFF\xCA")

            f_collectioncheck = open(BIN_PATH + "06dd30_collectioncheck.bin", "rb")
            self.seek(int("6dd30", 16) + rom_offset)
            self.write(f_collectioncheck.read())
            f_collectioncheck.close
        else:
            self.seek(int("491ed", 16) + rom_offset)
            self.write(qt_encode("This room is a lot cooler in Easy mode.", True))

        ##########################################################################
        #                       Modify Edward's Castle events
        ##########################################################################
        # Shorten Edward conversation
        self.seek(int("4c3d6", 16) + rom_offset)
        self.write(b"\x06\xc4")
        f_edward = open(BIN_PATH + "04c4fb_edward.bin", "rb")
        self.seek(int("4c4fb", 16) + rom_offset)
        self.write(f_edward.read())
        f_edward.close
        ...
        self.seek(int("4c746", 16) + rom_offset)
        self.write(b"\x10")

        # Move guard to allow roast location get
        f_castleguard = open(BIN_PATH + "04d1a0_castleguard.bin", "rb")
        self.seek(int("4d1a0", 16) + rom_offset)
        self.write(f_castleguard.read())
        f_castleguard.close
        self.seek(int("c8551", 16) + rom_offset)
        self.write(b"\x10")

        #  Update Large Roast event
        self.seek(int("4d0da", 16) + rom_offset)
        self.write(b"\x02\xc0\xe1\xd0\x02\xc1\x6b\x02\xd0\x46\x00\xe9\xd0\x02\xe0")
        self.write(b"\x02\xbf\x41\xd1\x02\xd4\x0a\xf6\xd0\x02\xcc\x46\x6b")
        self.write(INV_FULL)

        # Fix hidden guard text box
        self.seek(int("4c297", 16) + rom_offset)
        self.write(b"\xc0")
        self.seek(int("4c20e", 16) + rom_offset)
        self.write(b"\x02\xBF\x99\xC2\x02\xD4\x00\x1B\xC2\x02\xCC\xD8\x6B" + INV_FULL)

        ##########################################################################
        #                   Modify Edward's Prison/Tunnel events
        ##########################################################################
        # Skip long cutscene in prison
        self.seek(int("4d209", 16) + rom_offset)
        self.write(b"\x34\xd2")
        self.seek(int("4d234", 16) + rom_offset)
        self.write(b"\x02\xd0\x23\x00\xcf\xd2\x02\xe0")
        self.seek(int("4d335", 16) + rom_offset)
        self.write(b"\x6b")

        # Move Dark Space, allows player to exit area without key
        # Set new X/Y coordinates in exit table
        self.seek(int("18614", 16) + rom_offset)
        self.write(b"\x12\x07")
        # Set new X/Y coordinates in event table
        self.seek(int("C8635", 16) + rom_offset)
        self.write(b"\x12\x08")

        # Progression triggers when Lilly is with you
        self.seek(int("9aa45", 16) + rom_offset)
        self.write(b"\x6f\x00")
        self.seek(int("9aa4b", 16) + rom_offset)
        self.write(b"\x02")
        self.seek(int("9be74", 16) + rom_offset)
        self.write(b"\xd7\x18")

        # Give appearing Dark Space the option of handling an ability
        self.seek(int("9bf7f", 16) + rom_offset)
        self.write(b"\xAC\xD6\x88\xA8\x00\xC0\x04\x00\x0B\x4c\x10\xf7")
        self.seek(int("9f710", 16) + rom_offset)
        self.write(b"\xA5\x0E\x85\x24\xA9\x00\x20\x85\x0E\x02\xE0")
        self.seek(int("c8aa2", 16) + rom_offset)
        self.write(b"\x03")

        # Fix forced form change
        self.seek(int("9c037", 16) + rom_offset)
        self.write(FORCE_CHANGE + b"\x02\xe0")
        ...
        self.seek(int("4e2a3", 16) + rom_offset)
        self.write(f_itory.read())
        f_itory.close

        # Lilly now joins if you give her the Necklace
        f_lilly = open(BIN_PATH + "04e5ff_lilly.bin", "rb")
        self.seek(int("4e5ff", 16) + rom_offset)
        self.write(f_lilly.read())
        f_lilly.close
        self.seek(int("4e5a6", 16) + rom_offset)
        self.write(b"\x6f")
        self.seek(int("4e5ac", 16) + rom_offset)
        self.write(b"\xff\xe5\x02\x0b\x6b")

        # Shorten Inca Statue get
        self.seek(int("4f37b", 16) + rom_offset)
        self.write(b"\x02\xbf\x8d\xf3\x6b")

        # For elder to always give spoiler
        self.seek(int("4e933", 16) + rom_offset)
        self.write(b"\x10\x01")
        self.seek(int("4e97a", 16) + rom_offset)
        self.write(b"\x02\xbf\xff\xe9\x6b")
        ...
        self.seek(int("9d11e", 16) + rom_offset)
        self.write(f_moontribe.read())
        f_moontribe.close

        # Lilly event serves as an overworld exit
        self.seek(int("4f441", 16) + rom_offset)
        self.write(b"\x00\x00\x30\x02\x45\x14\x1c\x17\x1d\x4d\xf4\x6b")
        self.write(b"\x02\x40\x00\x04\x54\xf4\x6b\x02\x66\x90\x00\x60\x02\x01\x02\xC1\x6b")
        ...
        if self.settings.enemizer.value != Enemizer.NONE.value:
            ...
            if self.settings.enemizer.value != Enemizer.LIMITED.value:
                timer += 5
        self.seek(int("4f8b8", 16) + rom_offset)
        self.write(binascii.unhexlify(str(timer)))

        # Shorten Inca Statue get
        self.seek(int("4fae7", 16) + rom_offset)
        self.write(b"\x02\xbf\xf9\xfa\x6b")

        ##########################################################################
        #                          Modify Inca events
        ##########################################################################
        # Fix forced form change
        self.seek(int("9cfaa", 16) + rom_offset)
        self.write(FORCE_CHANGE + b"\xA9\xF0\xEF\x1C\x5A\x06\x02\xe0")

        # Put Gold Ship captain at Inca entrance
        self.seek(int("c8c9c", 16) + rom_offset)
        self.write(b"\x19\x1c\x00\x4e\x85\x85\x00")
        ...
        self.seek(int("c945c", 16) + rom_offset)
        self.write(b"\x0b\x24\x00\x3e\x96\x85\x00\xff\xca")
        self.seek(int("c805a", 16) + rom_offset)
        self.write(b"\x65")
        # Modify Seth event to ignore switch conditions
        self.seek(int("59643", 16) + rom_offset)
        self.write(b"\x10\x00")
        # Add in inventory full text
        self.seek(int("59665", 16) + rom_offset)
        self.write(INV_FULL)

        # Entering Gold Ship doesn't lock out Castoth
        self.seek(int("58188", 16) + rom_offset)
        self.write(b"\x02\xc1\x02\xc1")
        self.seek(int("18a09", 16) + rom_offset)
        self.write(b"\xd0\x00\x40\x02\x03")

        # Have ladder NPC move aside only if Mystic Statue has been acquired
        self.seek(int("583cb", 16) + rom_offset)
        self.write(b"\x10")

        # Modify queen switches
        self.seek(int("58a04", 16) + rom_offset)
        self.write(b"\x10\x00")
        self.seek(int("58a1f", 16) + rom_offset)
        self.write(b"\x10")
        ...
        self.seek(int("584a9", 16) + rom_offset)
        self.write(f_goldship.read())
        f_goldship.close

        # Sleeping sends player to Diamond Coast
        self.seek(int("586a3", 16) + rom_offset)
        self.write(b"\x02\x26\x30\x48\x00\x20\x00\x03\x00\x21")

        ##########################################################################
        #                        Modify Diamond Coast events
        ##########################################################################
        # Allow Turbo to contact Seth
        self.seek(int("c953e", 16) + rom_offset)
        self.write(b"\x01")
        self.seek(int("5aa76", 16) + rom_offset)
        self.write(b"\x00\xff")
        self.seek(int("5ff00", 16) + rom_offset)
        self.write(b"\x02\xCC\x01\x02\xD0\x11\x01\x0E\xFF\x02\xBF\x60\xFF\x6B\x02\xD0\x12\x01\x1B\xFF")
        self.write(b"\x02\xcc\x12\x02\xBF\x1F\xFF\x5C\xBD\xB9\x84")
        self.write(b"\xd3" + qt_encode("Woof woof!") + b"\xcb")
        self.write(qt_encode("(Oh good, you know Morse Code. Let's see what Seth's up to:)") + b"\xc0")
        self.write(b"\xd3" + qt_encode("Woof woof!") + b"\xcb")
        self.write(qt_encode("(You don't happen to know Morse Code, do you?)") + b"\xc0")

        # Kara event serves as an overworld exit
        self.seek(int("5aa9e", 16) + rom_offset)
        self.write(b"\x00\x00\x30\x02\x45\x03\x00\x06\x01\xaa\xaa\x6b")
        self.write(b"\x02\x40\x00\x08\xb1\xaa\x6b\x02\x66\x50\x02\x50\x03\x07\x02\xC1\x6b")
        ...
        self.seek(int("5cf85", 16) + rom_offset)
        self.write(f_freejia.read())
        f_freejia.close

        # Give full inventory text to trash can 2
        self.seek(int("5cf37", 16) + rom_offset)
        self.write(b"\x02\xBF\x49\xCF\x02\xD4\x12\x44\xCF\x02\xCC\x53\x6B" + INV_FULL)

        # Redirect event table to bypass deleted event
        # Changes locked door to a normal door
        self.seek(int("c95bb", 16) + rom_offset)
        self.write(b"\xf3\xc5\x80")

        # Update NPC dialogue to acknowledge change
        self.seek(int("5c331", 16) + rom_offset)
        self.write(b"\x42\xa2\x80\xa0\x2b\xac\x48\xac\xd6\xae\xa4\x87\x84\xac\xd7\x58\xcb\xa5")
        self.write(b"\x8d\x8b\x8e\x82\x8a\x84\x83\xac\x80\x86\x80\x88\x8d\x2a\x2a\x2a\xc0")

        # Add inventory full option to Creepy Guy event
        self.seek(int("5b6df", 16) + rom_offset)
        self.write(INV_FULL)

        # Alter laborer text
        self.seek(int("5bfdb", 16) + rom_offset)
        self.write(b"\xde\xbf")

        # Have some fun with snitch item text
        self.seek(int("5b8bc", 16) + rom_offset)
        self.write(b"\x62")
        self.seek(int("5b925", 16) + rom_offset)
        self.write(b"\x88\xa4\x84\x8c\x2a\xcb\xac\x69\x84\xa3\x2b\xac\x48\x0e\x8c\xac\x80\xac\x81")
        self.write(b"\x80\x83\xac\xa0\x84\xa2\xa3\x8e\x8d\xcb\xac\x63\x8d\x88\xa4\x82\x87\x84\xa3")
        self.write(b"\xac\x86\x84\xa4\xac\xa3\xa4\x88\xa4\x82\x87\x84\xa3\x2b\xac\xa9\x8e\xca")

        ##########################################################################
        #                        Modify Diamond Mine events
        ##########################################################################
        # Trapped laborer gives you an item instead of sending Jewels to Jeweler
        self.seek(int("5d739", 16) + rom_offset)
        self.write(b"\x4c\x5d\xd8\x85")
        f_trappedlaborer = open(BIN_PATH + "05d7e2_trappedlaborer.bin", "rb")
        self.seek(int("5d7e2", 16) + rom_offset)
        self.write(f_trappedlaborer.read())
        f_trappedlaborer.close

        # Shorten laborer items
        self.seek(int("aa753", 16) + rom_offset)
        self.write(b"\xef\xa7")
        self.seek(int("aa75a", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("aa773", 16) + rom_offset)
        self.write(b"\x5c\xa8")
        self.seek(int("aa77a", 16) + rom_offset)
        self.write(b"\x6b")

        # Shorten morgue item get
        self.seek(int("5d4d8", 16) + rom_offset)
        self.write(b"\x02\xbf\xeb\xd4\x02\xe0")

        # Cut out Sam's song
        self.seek(int("5d24f", 16) + rom_offset)
        self.write(b"\x6b")

        # Give appearing Dark Space the option of handling an ability
        self.seek(int("5d62f", 16) + rom_offset)
        self.write(b"\xAC\xD6\x88\x00\x2B\xA5\x0E\x85\x24\xA9\x00\x20\x85\x0E\x02\xE0")
        ...
        self.seek(int("5d89a", 16) + rom_offset)
        self.write(f_neilscottage.read())
        f_neilscottage.close

        # Invention event serves as an overworld exit
        self.seek(int("5e305", 16) + rom_offset)
        self.write(b"\x00\x00\x30\x02\x45\x07\x0d\x0a\x0e\x11\xe3\x6b")
        self.write(b"\x02\x40\x00\x04\x18\xe3\x6b\x02\x66\x70\x02\x70\x02\x07\x02\xC1\x6b")
        ...
        self.seek(int("5e647", 16) + rom_offset)
        self.write(f_nazca.read())
        f_nazca.close

        # Allow exit to world map
        self.seek(int("5e80c", 16) + rom_offset)
        self.write(b"\x02\x66\x10\x03\x90\x02\x07\x02\xC1\x6B")
        ...
        self.seek(int("5f356", 16) + rom_offset)
        self.write(f_skygarden.read())
        f_skygarden.close

        # Instant form change & warp to Seaside Palace if Viper is defeated
        self.seek(int("ace9b", 16) + rom_offset)
        self.write(b"\x4c\x90\xfd")
        self.seek(int("acecb", 16) + rom_offset)
        self.write(b"\x01\x02\x26\x5a\x90\x00\x70\x00\x83\x00\x14\x02\xc1\x6b")

        f_viperchange = open(BIN_PATH + "0afd90_viperchange.bin", "rb")
        self.seek(int("afd90", 16) + rom_offset)
        self.write(f_viperchange.read())
        f_viperchange.close
        ...
        self.seek(int("191de", 16) + rom_offset)
        self.write(b"\x04\x06\x02\x03\x69\xa0\x02\x38\x00\x00\x00\x13")  # Temporarily put exit one tile south
        # self.write(b"\x04\x05\x02\x03\x69\xa0\x02\x38\x00\x00\x00\x13")  # Change to this if map is ever updated

        # Replace Mu Passage map with one that includes a door to Angel Village
        f_mupassage = open(BIN_PATH + "1e28a5_mupassage.bin", "rb")
        self.seek(int("1e28a5", 16) + rom_offset)
        self.write(f_mupassage.read())
        f_mupassage.close

        # Shorten NPC item get
        self.seek(int("68b01", 16) + rom_offset)
        self.write(b"\x6b")

        # Purification event won't softlock if you don't have Lilly
        self.seek(int("69406", 16) + rom_offset)
        self.write(b"\x10")
        self.seek(int("6941a", 16) + rom_offset)
        self.write(b"\x01\x02\xc1\x02\xc1")

        # Remove Lilly text when Mu door opens
        self.seek(int("39174", 16) + rom_offset)
        self.write(b"\x60")
        self.seek(int("391d7", 16) + rom_offset)
        self.write(b"\xc0")

        # Allow player to open Mu door from the back
        f_mudoor = open(BIN_PATH + "069739_mudoor.bin", "rb")
        self.seek(int("69739", 16) + rom_offset)
        self.write(f_mudoor.read())
        f_mudoor.close

        # Remove fanfare from coffin item get
        self.seek(int("69232", 16) + rom_offset)
        self.write(b"\x9e\x93\x4c\x61\x92")
        self.seek(int("69267", 16) + rom_offset)
        self.write(b"\x80\x04")

        # Make coffin spoiler re-readable
        self.seek(int("68ff3", 16) + rom_offset)
        self.write(b"\xf5\x8f")
        self.seek(int("68ffb", 16) + rom_offset)
        self.write(b"\x70\xe5")
        self.seek(int("69092", 16) + rom_offset)
        self.write(b"\x02\xce\x01\x02\x25\x2F\x0A\x4c\xfd\x8f")
        self.seek(int("6e570", 16) + rom_offset)
        self.write(b"\x02\xD1\x3A\x01\x01\x8A\xE5\x02\xD0\x6F\x01\x82\xE5\x02\xBF\xA7\x90\x6B")
        self.write(b"\x02\xBF\xCF\x90\x02\xCC\x01\x6B\x02\xBF\x67\x91\x6B")

        ##########################################################################
        #                             Modify Mu events
        ##########################################################################
        # Add "inventory full" option to Statue of Hope items
        self.seek(int("698cd", 16) + rom_offset)
        self.write(INV_FULL)

        # Shorten Statue of Hope get
        self.seek(int("698b8", 16) + rom_offset)
        self.write(b"\x02\xBF\xD2\x98\x02\xD4\x28\xCD\x98\x02\xCC\x79\x6B")
        self.seek(int("69960", 16) + rom_offset)
        self.write(b"\x02\xBF\x75\x99\x02\xD4\x1E\xCD\x98\x02\xCC\x7F\x6B")

        # Shorten Rama statue event
        self.seek(int("69e50", 16) + rom_offset)
        self.write(b"\x10")
        self.seek(int("69f26", 16) + rom_offset)
        self.write(b"\x00")

        # Text in Hope Room
        self.seek(int("69baa", 16) + rom_offset)
        self.write(qt_encode("Hey.", True))

        # Spirits in Rama statue room can't lock you
        self.seek(int("6a07a", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("6a082", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("6a08a", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("6a092", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("6a09a", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("6a0a2", 16) + rom_offset)
        self.write(b"\x6b")

        # Move exits around to make Vampires required for Statue
        self.seek(int("193ea", 16) + rom_offset)
        self.write(b"\x5f\x80\x00\x50\x00\x03\x00\x44")
        self.seek(int("193f8", 16) + rom_offset)
        self.write(b"\x65\xb8\x00\x80\x02\x03\x00\x44")
        self.seek(int("69c62", 16) + rom_offset)
        self.write(b"\x67\x78\x01\xd0\x01\x80\x01\x22")
        self.seek(int("6a4c9", 16) + rom_offset)
        self.write(b"\x02\x26\x66\xf8\x00\xd8\x01\x00\x00\x22\x02\xc1\x6b")

        # Instant form change after Vamps are defeated
        self.seek(int("6a43b", 16) + rom_offset)
        self.write(b"\x4c\x00\xe5")

        f_vampchange = open(BIN_PATH + "06e500_vampchange.bin", "rb")
        self.seek(int("6e500", 16) + rom_offset)
        self.write(f_vampchange.read())
        f_vampchange.close
        ...
        self.seek(int("1941a", 16) + rom_offset)
        self.write(b"\x2a\x05\x01\x01\x5e\x48\x00\x90\x00\x01\x00\x14")  # Temporarily put exit one tile south
        # self.write(b"\x2a\x05\x01\x01\x5e\x48\x00\x80\x00\x01\x00\x14")  # Change to this if map is ever updated

        # Update sign to read "Passage to Mu"
        f_angelsign = open(BIN_PATH + "06ba36_angelsign.bin", "rb")
        self.seek(int("6ba36", 16) + rom_offset)
        self.write(f_angelsign.read())
        f_angelsign.close

        # Entering this area clears your enemy defeat count and forces change to Will
        self.seek(int("6bff7", 16) + rom_offset)
        self.write(b"\x00\x00\x30\x02\x40\x01\x0F\x01\xC0\x6b")
        self.write(b"\xA0\x00\x00\xA9\x00\x00\x99\x80\x0A\xC8\xC8\xC0\x20\x00\xD0\xF6")
        self.write(FORCE_CHANGE + b"\x02\xE0")
        ...
        self.seek(int("1a5a37", 16) + rom_offset)
        self.write(f_angelmap.read())
        f_angelmap.close

        # Ishtar's game never closes
        self.seek(int("6d9fc", 16) + rom_offset)
        self.write(b"\x10")
        self.seek(int("6cede", 16) + rom_offset)
        self.write(b"\x9c\xa6\x0a\x6b")
        self.seek(int("6cef6", 16) + rom_offset)
        self.write(b"\x40\x86\x80\x88\x8d\x4f\xc0")

        ##########################################################################
        #                           Modfy Watermia events
        ##########################################################################
        # Allow NPC to contact Seth
        self.seek(int("78542", 16) + rom_offset)
        self.write(b"\x50\xe9")
        self.seek(int("7e950", 16) + rom_offset)
        self.write(b"\x02\xD0\x11\x01\x5B\xE9\x02\xBF\xb8\xe9\x6B\x02\xD0\x12\x01\x68\xE9")
        self.write(b"\x02\xcc\x12\x02\xBF\x6c\xE9\x5C\xBD\xB9\x84")
        self.write(b"\xd3" + qt_encode("Oh, you know Bagu? Then I can help you cross.") + b"\xcb")
        self.write(qt_encode("(And by Bagu I mean Morse Code.)") + b"\xc0")
        self.write(b"\xd3" + qt_encode("Only town folk may cross this river!") + b"\xcb")
        self.write(qt_encode("(Or, if you can talk to fish, I guess.)") + b"\xc0")

        # Allow for travel from  Watermia to Euro
        # Update address pointer
        self.seek(int("78544", 16) + rom_offset)
        self.write(b"\x69")

        # Update event address pointers
        f_watermia1 = open(BIN_PATH + "078569_watermia1.bin", "rb")
        self.seek(int("78569", 16) + rom_offset)
        self.write(f_watermia1.read())
        f_watermia1.close

        # Change textbox contents
        f_watermia2 = open(BIN_PATH + "0786c1_watermia2.bin", "rb")
        self.seek(int("786c1", 16) + rom_offset)
        self.write(f_watermia2.read())
        f_watermia2.close

        # Russian Glass NPC just gives you the item
        f_russianglass = open(BIN_PATH + "079237_russianglass.bin", "rb")
        self.seek(int("79237", 16) + rom_offset)
        self.write(f_russianglass.read())
        f_russianglass.close

        # Fix Lance item get text
        self.seek(int("7ad28", 16) + rom_offset)
        self.write(INV_FULL)
        ...
        self.seek(int("7b59e", 16) + rom_offset)
        self.write(f_necklace.read())
        f_necklace.close

        # Update event table instance to point to new events
        # New pointers include leading zero byte to negate event parameters
        self.seek(int("cb822", 16) + rom_offset)  # First stone, map 130
        self.write(b"\x00\x9e\xb5\x87")
        self.seek(int("cb94a", 16) + rom_offset)  # Second stone, map 131
        self.write(b"\x00\xfe\xb5\x87")

        # Entering wrong door in 2nd map area doesn't softlock you
        self.seek(int("19a4c", 16) + rom_offset)
        self.write(b"\x84")

        # Give appearing Dark Space the option of handling an ability
        self.seek(int("7be0b", 16) + rom_offset)
        self.write(b"\xAC\xD6\x88\xC8\x01\x80\x02\x00\x2B\xA5\x0E\x85\x24\xA9\x00\x20\x85\x0E\x02\xE0")
        self.seek(int("cbc60", 16) + rom_offset)
        self.write(b"\x03")

        # Exit after Sand Fanger takes you back to start
        self.seek(int("19c84", 16) + rom_offset)
        self.write(b"\x82\x10\x00\x90\x00\x07\x00\x18")
        ...
        self.seek(int("7c432", 16) + rom_offset)
        self.write(f_euro1.read())
        f_euro1.close
        # Change textbox contents
        f_euro2 = open(BIN_PATH + "07c4d0_euro2.bin", "rb")
        self.seek(int("7c4d0", 16) + rom_offset)
        self.write(f_euro2.read())
        f_euro2.close
        self.seek(int("7c482", 16) + rom_offset)
        self.write(qt_encode("A moose once bit my sister.", True))

        # Neil in Euro
        f_euroneil = open(BIN_PATH + "07e398_euroneil.bin", "rb")
        self.seek(int("7e398", 16) + rom_offset)
        self.write(f_euroneil.read())
        f_euroneil.close
        self.seek(int("7e37f", 16) + rom_offset)
        self.write(b"\x14\x00")
        self.seek(int("7e394", 16) + rom_offset)
        self.write(b"\x10\x00")

        # Hidden house replaces STR upgrade with item acquisition
        f_euroitem = open(BIN_PATH + "07e517_euroitem.bin", "rb")
        self.seek(int("7e517", 16) + rom_offset)
        self.write(f_euroitem.read())
        f_euroitem.close

        # Speed up store line
        self.seek(int("7d5e1", 16) + rom_offset)
        self.write(b"\x00\x01")

        # Change vendor event, allows for only one item acquisition
        # Note: this cannibalizes the following event
        self.seek(int("7c0a7", 16) + rom_offset)
        self.write(b"\x02\xd0\x9a\x01\xba\xc0\x02\xbf\xf3\xc0\x02\xd4\x28\xbf\xc0")
        self.write(b"\x02\xcc\x9a\x6b\x02\xbf\xdd\xc0\x6b")
        self.write(INV_FULL)
        # Change pointer for cannibalized event
        self.seek(int("7c09b", 16) + rom_offset)
        self.write(b"\xc4")

        # Store replaces status upgrades with item acquisition
        self.seek(int("7cd03", 16) + rom_offset)  # HP upgrade
        self.write(b"\x02\xd0\xf0\x01\x32\xcd\x02\xc0\x10\xcd\x02\xc1\x6b")
        self.write(b"\x02\xd4\x29\x20\xcd\x02\xcc\xf0\x02\xbf\x39\xcd\x02\xe0")
        self.seek(int("7cdf7", 16) + rom_offset)  # Dark Friar upgrade
        self.write(b"\x02\xd4\x2d\x05\xce\x02\xcc\xf1\x02\xbf\x28\xce\x02\xe0")
        self.write(b"\x02\xbf\x3e\xce\x6b")

        # Old men text no longer checks for Teapot
        self.seek(int("7d60a", 16) + rom_offset)
        self.write(b"\x14\xd6")
        self.seek(int("7d7a5", 16) + rom_offset)
        self.write(b"\xbd\xd7")

        # Various NPC dialogue
        self.seek(int("7d6db", 16) + rom_offset)
        self.write(b"\x2A\xD0\xC8\xC9\x1E\xC2\x0B\xC2\x03" + qt_encode("It could be carried by an African swallow!"))
        self.write(b"\xCF\xC2\x03\xC2\x04" + qt_encode("Oh yeah, an African swallow maybe, but not a European swallow, that's my point!") + b"\xc0")
        self.seek(int("7d622", 16) + rom_offset)
        self.write(qt_encode("Rofsky: Wait a minute, supposing two swallows carried it together?...") + b"\xc0")
        self.seek(int("7c860", 16) + rom_offset)
        self.write(b"\xce" + qt_encode("Nobody expects the Spanish Inquisition!") + b"\xc0")
        self.seek(int("7c142", 16) + rom_offset)
        self.write(qt_encode("I am no longer infected.", True))
        self.seek(int("7c160", 16) + rom_offset)
        self.write(qt_encode("My hovercraft is full of eels.", True))
        self.seek(int("7c182", 16) + rom_offset)
        self.write(qt_encode("... w-i-i-i-i-ith... a herring!!", True))
        self.seek(int("7c1b6", 16) + rom_offset)
        self.write(qt_encode("It's only a wafer-thin mint, sir...", True))
        self.seek(int("7c1dc", 16) + rom_offset)
        self.write(b"\xd3" + qt_encode("The mill's closed. There's no more work. We're destitute.|"))
        self.write(qt_encode("I've got no option but to sell you all for scientific experiments.") + b"\xc0")
        self.seek(int("7c3d4", 16) + rom_offset)
        self.write(qt_encode("You're a looney.", True))
        ...
        self.seek(int("88fc4", 16) + rom_offset)
        self.write(f_natives.read())
        f_natives.close

        # Change event pointers for cannibalized NPC code
        self.seek(int("cca93", 16) + rom_offset)
        self.write(b"\x88\x8f\x88")
        ...
        self.seek(int("89abf", 16) + rom_offset)
        self.write(f_ankorwat1.read())
        f_ankorwat1.close

        # Shorten black glasses get
        self.seek(int("89fa9", 16) + rom_offset)
        self.write(b"\x02\xe0")

        # Bright room looks at switch 4f rather than equipment
        f_ankorwat2 = open(BIN_PATH + "089a31_ankorwat.bin", "rb")
        self.seek(int("89a31", 16) + rom_offset)
        self.write(f_ankorwat2.read())
        f_ankorwat2.close
        ...
        self.seek(int("8b010", 16) + rom_offset)
        self.write(f_snakegame.read())
        f_snakegame.close

        # Neil in Dao
        f_daoneil = open(BIN_PATH + "08a5bd_daoneil.bin", "rb")
        self.seek(int("8a5bd", 16) + rom_offset)
        self.write(f_daoneil.read())
        f_daoneil.close
        self.seek(int("8a5b3", 16) + rom_offset)
        self.write(b"\x14\x00")

        # Allow travel back to Natives' Village
        self.seek(int("8b16d", 16) + rom_offset)
        self.write(b"\x4c\x50\xfe")
        self.seek(int("8fe50", 16) + rom_offset)
        self.write(b"\x02\xBF\x71\xFE\x02\xBE\x02\x01\x5A\xFE\x60\xFE\x60\xFE\x65\xFE")
        self.write(b"\x02\xBF\x93\xFE\x6B\x02\x26\xAC\xC0\x01\xD0\x01\x06\x00\x22\x02\xC5")
        self.write(b"\xd3" + qt_encode("Go to Natives' Village?") + b"\xcb\xac")
        self.write(qt_encode("No") + b"\xcb\xac" + qt_encode("Yes") + b"\xca")
        self.write(b"\xce" + qt_encode("Come back anytime!") + b"\xc0")

        # Modify two-item acquisition event
        self.seek(int("8b1bb", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("8b189", 16) + rom_offset)
        self.write(b"\xe0\xfd")
        self.seek(int("8fde0", 16) + rom_offset)
        self.write(b"\xD3\x4b\x8e\x8b\x80\x0e\xa3\xac\x4b\x84\xa4\xa4\x84\xa2\xCB")
        self.write(b"\x49\x8e\xa5\xa2\x8d\x80\x8b\xac\xac\xac\xac\xac\xac\xCF\xCE")
        self.write(qt_encode("If you want a guide to take you to the Natives' Village, I can get one for you.") + b"\xc0")

        # Spirit appears only after you defeat Mummy Queen or Solid Arm
        self.seek(int("980cb", 16) + rom_offset)
        self.write(b"\x4c\xb0\xf6")
        self.seek(int("9f6b0", 16) + rom_offset)
        self.write(b"\x02\xd0\xf6\x01\xd1\x80\x02\xd1\x79\x01\x01\xd1\x80\x02\xe0")

        ##########################################################################
        #                           Modify Pyramid events
        ##########################################################################
        # Can give journal to the guide in hieroglyph room
        self.seek(int("8c207", 16) + rom_offset)
        self.write(b"\x0E\xC2\x02\x0B\x02\xC1\x6B\x02\xD0\xEF\x01\x1E\xC2\x02\xD6\x26\x22\xC2")
        self.write(b"\x02\xBF\x2D\xC2\x6B\x5C\x08\xF2\x83\x02\xCC\xEF\x02\xD5\x26\x02\xBF\x7F\xC2\x6B")
        self.write(qt_encode("If you have any information about the pyramid, I'd be happy to hold onto it for you.", True))
        self.write(qt_encode("I'll hold onto that journal for you. Come back anytime if you want to read it.", True))
        self.seek(int("3f208", 16) + rom_offset)
        self.write(b"\x02\xbf\x1a\x9e\x6b")

        # Shorten hieroglyph get
        self.seek(int("8c7b8", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("8c87f", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("8c927", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("8c9cf", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("8ca77", 16) + rom_offset)
        self.write(b"\x6b")
        self.seek(int("8cb1f", 16) + rom_offset)
        self.write(b"\x6b")
        ...
        self.seek(int("183f4", 16) + rom_offset)
        self.write(b"\xea")
        # Move Dark Space exit data to Map 223
        f_babel1 = open(BIN_PATH + "01a7c2_babel.bin", "rb")
        self.seek(int("1a7c2", 16) + rom_offset)
        self.write(f_babel1.read())
        f_babel1.close
        # Modify address pointer for Maps 224-227 in event table
        self.seek(int("c81c0", 16) + rom_offset)
        self.write(b"\xa2\xe0\xe3\xe0\x0f\xe1\x2d\xe1")
        # Assign new Dark Space correct overworld name
        self.seek(int("bf8c4", 16) + rom_offset)
        self.write(b"\x47\xfa")
        # Move Dark Space event data to Map 223
        # Also move spirits for entrance warp
        f_babel2 = open(BIN_PATH + "0ce099_babel.bin", "rb")
        self.seek(int("ce099", 16) + rom_offset)
        self.write(f_babel2.read())
        f_babel2.close

        # Spirits can warp you back to start
        self.seek(int("99b69", 16) + rom_offset)
        self.write(b"\x76\x9B\x76\x9B\x76\x9B\x76\x9B")
        self.seek(int("99b7a", 16) + rom_offset)
        self.write(b"\x02\xBE\x02\x01\x80\x9B\x86\x9B\x86\x9B\x16\x9D\x02\xBF\x95\x9C\x6B")
        self.seek(int("99c1e", 16) + rom_offset)
        self.write(b"\xd3" + qt_encode("What'd you like to do?") + b"\xcb\xac")
        self.write(qt_encode("Git gud") + b"\xcb\xac" + qt_encode("Run away!") + b"\xca")
        self.seek(int("99c95", 16) + rom_offset)
        self.write(b"\xce" + qt_encode("Darn straight.") + b"\xc0")
        self.seek(int("99d16", 16) + rom_offset)
        self.write(b"\x02\x26\xDE\x78\x00\xC0\x00\x00\x00\x11\x02\xC5")

        # Change switch conditions for Crystal Ring item
        self.seek(int("9999b", 16) + rom_offset)
        self.write(b"\x4c\xd0\xf6")
        self.seek(int("9f6d0", 16) + rom_offset)
        self.write(b"\x02\xd0\xdc\x00\xa0\x99\x02\xd0\x3e\x00\xa0\x99\x02\xd0\xb4\x01\x9a\x99\x4c\xa0\x99")
        self.seek(int("999aa", 16) + rom_offset)
        self.write(b"\x4c\xf0\xf6")
        self.seek(int("9f6f0", 16) + rom_offset)
        self.write(b"\x02\xd0\xdc\x01\x9a\x99\x4c\xaf\x99")
        self.seek(int("99a49", 16) + rom_offset)
        self.write(b"\x02\xbf\xe4\x9a\x02\xd4\x27\x57\x9a\x02\xcc\xdc\x02\xe0" + INV_FULL)
        # Change text
        self.seek(int("99a70", 16) + rom_offset)
        self.write(qt_encode("Well, lookie there.", True))

        # Olman event no longer warps you out of the room
        self.seek(int("98891", 16) + rom_offset)
        self.write(b"\x02\x0b\x6b")

        # Shorten Olman text boxes
        self.seek(int("9884c", 16) + rom_offset)
        self.write(b"\x01\x00")
        self.seek(int("98903", 16) + rom_offset)
        self.write(qt_encode("heya.", True))
        self.seek(int("989a2", 16) + rom_offset)
        self.write(qt_encode("you've been busy, huh?", True))

        # Speed up roof sequence
        self.seek(int("98fad", 16) + rom_offset)
        self.write(b"\x02\xcc\x0e\x02\xcc\x0f\x6b")

        ##########################################################################
        #                      Modify Jeweler's Mansion events
        ##########################################################################
        # Set exit warp to Dao
        self.seek(int("8fcb2", 16) + rom_offset)
        self.write(b"\x02\x26\xc3\x40\x01\x88\x00\x00\x00\x23")

        # Set flag when Solid Arm is killed
        self.seek(int("8fa25", 16) + rom_offset)
        self.write(b"\x4c\x20\xfd")
        self.seek(int("8fd20", 16) + rom_offset)
        self.write(b"\x02\xcc\xf6\x02\x26\xe3\x80\x02\xa0\x01\x80\x10\x23\x02\xe0")

        # Solid Arm text
        self.seek(int("8fa32", 16) + rom_offset)
        self.write(qt_encode("Weave a circle round him") + b"\xcb" + qt_encode("  thrice,"))
        self.write(b"\xcb" + qt_encode("And close your eyes with") + b"\xcb" + qt_encode("  holy dread,"))
        self.write(b"\xcf" + qt_encode("For he on honey-dew hath") + b"\xcb" + qt_encode("  fed,"))
        self.write(b"\xcb" + qt_encode("And drunk the milk of ") + b"\xcb" + qt_encode("  Paradise.") + b"\xc0")

        self.seek(int("8fbc9", 16) + rom_offset)
        self.write(b"\xd5\x02" + qt_encode("Ed, what an ugly thing to say... does this mean we're not friends anymore?|"))
        self.write(qt_encode("You know, Ed, if I thought you weren't my friend, I just don't think I could bear it.") + b"\xc0")
        ...
        self.seek(int("bd566", 16) + rom_offset)
        self.write(b"\xCB" + qt_encode("    Thanks a million.") + str_endpause)
        self.seek(int("bd5ac", 16) + rom_offset)
        self.write(b"\xCB" + qt_encode(" Extra special thanks to") + b"\xCB" + qt_encode("       manafreak"))
        self.write(b"\xC9\x78\xCE\xCB" + qt_encode(" This project would not") + b"\xCB" + qt_encode("  exist without his work."))
        self.write(b"\xC9\x78\xCE\xCB" + qt_encode("     gaiathecreator") + b"\xCB" + qt_encode("      .blogspot.com") + str_endpause)
        self.seek(int("bd71c", 16) + rom_offset)
        self.write(qt_encode("    Created by") + b"\xCB" + qt_encode("       DontBaguMe") + str_endpause)
        self.seek(int("bd74f", 16) + rom_offset)
        self.write(qt_encode("Additional Development By") + b"\xCB" + qt_encode("    bryon w and Raeven0"))
        self.write(b"\xCB" + qt_encode("  EmoTracker by Apokalysme"))
        self.write(b"\xC9\x78\xCE\xCB" + qt_encode("   Thanks to all the") + b"\xCB" + qt_encode("  amazing playtesters!") + str_endpause)
        self.seek(int("bdee2", 16) + rom_offset)
        # self.write(b"\xCB" + qt_encode("  Thanks RPGLimitBreak!") + str_endpause)
        self.write(b"\xCB" + qt_encode(" That's it, show's over.") + str_endpause)
        self.seek(int("bda09", 16) + rom_offset)
        self.write(b"\xCB" + qt_encode("   Thanks for playing!") + str_endpause)
        self.seek(int("bdca5", 16) + rom_offset)
        self.write(qt_encode("Wait a minute...") + b"\xCB" + qt_encode("what happened to Hamlet?") + str_endpause)
        self.seek(int("bdd48", 16) + rom_offset)
        self.write(qt_encode("Um...") + b"\xCB" + qt_encode("I wouldn't worry about") + b"\xCB" + qt_encode("that too much...") + str_endpause)
        self.seek(int("bddf6", 16) + rom_offset)
        self.write(qt_encode("Well, but...") + str_endpause)
        self.seek(int("bde16", 16) + rom_offset)
        self.write(qt_encode("Shh... here,") + b"\xCB" + qt_encode("have some bacon.") + str_endpause)

        # Thank the playtesters
        self.seek(int("be056", 16) + rom_offset)
        self.write(b"\x80\xfa")
        self.seek(int("bfa80", 16) + rom_offset)
        self.write(b"\xD3\xD2\x00\xD5\x00" + qt_encode("Contributors and Testers:") + b"\xCB")
        self.write(qt_encode("-Alchemic   -Austin21300") + b"\xCB")
        self.write(qt_encode("-Atlas      -BOWIEtheHERO") + b"\xCB")
        self.write(qt_encode("-Bonzaibier -BubbaSWalter") + b"\xC9\xB4\xCE")

        self.write(qt_encode("-Crazyhaze  -DerTolleIgel") + b"\xCB")
        self.write(qt_encode("-DoodSF     -djtifaheart") + b"\xCB")
        self.write(qt_encode("-Eppy37     -Keypaladin") + b"\xCB")
        self.write(qt_encode("-Lassic") + b"\xC9\xB4\xCE")

        self.write(qt_encode("-Le Hulk    -Plan") + b"\xCB")
        self.write(qt_encode("-manafreak  -Pozzum Senpai") + b"\xCB")
        self.write(qt_encode("-Mikan      -roeya") + b"\xCB")
        self.write(qt_encode("-Mr Freet") + b"\xC9\xB4\xCE")

        self.write(qt_encode("-Scheris    -SmashManiac") + b"\xCB")
        self.write(qt_encode("-SDiezal    -solarcell007") + b"\xCB")
        self.write(qt_encode("-Skarsnik   -steve hacks") + b"\xCB")
        self.write(qt_encode("-Skipsy") + b"\xC9\xB4\xCE")

        self.write(qt_encode("-Sye990     -Verallix") + b"\xCB")
        self.write(qt_encode("-Tsurana    -Volor") + b"\xCB")
        self.write(qt_encode("-Tymekeeper -Veetorp") + b"\xC9\xB4\xCE")

        self.write(qt_encode("-Voranthe   -Xyrcord") + b"\xCB")
        self.write(qt_encode("-Wilddin    -Z4t0x") + b"\xCB")
        self.write(qt_encode("-wormsofcan -ZockerStu") + b"\xC9\xB4\xCE")

        self.write(b"\xCB" + qt_encode("  Thank you all so much!"))
        self.write(b"\xCB" + qt_encode("     This was so fun!"))

        self.write(b"\xC9\xF0\xC8\xCA")
        ...
        self.seek(int("8cec9", 16) + rom_offset)
        self.write(f_jeweler.read())
        f_jeweler.close

        # Allow jeweler to take 2- and 3-jewel items
        f_jeweler2 = open(BIN_PATH + "08fd90_jeweler2.bin", "rb")
        self.seek(int("8fd90", 16) + rom_offset)
        self.write(f_jeweler2.read())
        f_jeweler2.close

        # Jeweler doesn't disappear when defeated
        self.seek(int("8cea5", 16) + rom_offset)
        self.write(b"\x10\x00")

        # Jeweler warps you to credits for Red Jewel hunts
        if self.settings.goal.value == Goal.RED_JEWEL_HUNT.value:
            self.seek(int("8d32a", 16) + rom_offset)
            self.write(b"\xE5\x00\x00\x00\x00\x00\x00\x11")
            self.seek(int("8d2d8", 16) + rom_offset)
            self.write(qt_encode("Beat the game"))
        ...
        self.seek(int("8db07", 16) + rom_offset)
        self.write(f_darkspace.read())
        f_darkspace.close

        # Shorten ability acquisition text
        self.seek(int("8eb81", 16) + rom_offset)
        self.write(b"\xc0")

        # Cut out ability explanation text
        self.seek(int("8eb15", 16) + rom_offset)
        self.write(b"\xa4\x26\xa9\x00\x00\x99\x24\x00\x02\x04\x16")
        self.write(b"\x02\xda\x01\xa9\xf0\xff\x1c\x5a\x06\x02\xe0")

        # Remove abilities from all Dark Spaces
        self.seek(int("c8b34", 16) + rom_offset)  # Itory Village (Psycho Dash)
        self.write(b"\x01")
        self.seek(int("c9b49", 16) + rom_offset)  # Diamond Mine (Dark Friar)
        self.write(b"\x03")
        self.seek(int("caa99", 16) + rom_offset)  # Mu (Psycho Slide)
        self.write(b"\x03")
        self.seek(int("cbb80", 16) + rom_offset)  # Great Wall (Spin Dash)
        self.write(b"\x03")
        self.seek(int("cc7b8", 16) + rom_offset)  # Mt. Temple (Aura Barrier)
        self.write(b"\x03")
        self.seek(int("cd0a2", 16) + rom_offset)  # Ankor Wat (Earthquaker)
        self.write(b"\x03")

        # Insert subroutine that can force change back to Will
        f_forcechange = open(BIN_PATH + "08fd30_forcechange.bin", "rb")
        self.seek(int("8fd30", 16) + rom_offset)
        self.write(f_forcechange.read())
        f_forcechange.close
        ...
        self.seek(int("2b871", 16) + rom_offset)
        self.write(b"\x30")

        # Insert new code to explicitly check for Psycho Dash and Friar
        self.seek(int("2f090", 16) + rom_offset)
        self.write(b"\xAD\xA2\x0A\x89\x01\x00\xF0\x06\xA9\xA0\xBE\x20\x26\xB9\x4C\x01\xB9")  # Psycho Dash @2f090
        self.write(b"\xAD\xA2\x0A\x89\x10\x00\xF0\x06\xA9\x3B\xBB\x20\x26\xB9\x4C\x01\xB9")  # Dark Friar @2f0a1

        # Insert jumps to new code
        self.seek(int("2b858", 16) + rom_offset)  # Psycho Dash
        self.write(b"\x4c\x90\xf0")
        self.seek(int("2b8df", 16) + rom_offset)  # Dark Friar
        self.write(b"\x4c\xa1\xf0")

        ##########################################################################
        #                      Disable NPCs in various maps
        ##########################################################################
        # School
        self.seek(int("48d25", 16) + rom_offset)  # Lance
        self.write(b"\xe0\x6b")
        self.seek(int("48d72", 16) + rom_offset)  # Seth
        self.write(b"\xe0\x6b")
        self.seek(int("48d99", 16) + rom_offset)  # Erik
        self.write(b"\xe0\x6b")

        # Seaside cave
        self.seek(int("4afb6", 16) + rom_offset)  # Playing cards
        self.write(b"\xe0\x6b")
        self.seek(int("4b06c", 16) + rom_offset)  # Lance
        self.write(b"\xe0\x6b")
        self.seek(int("4b459", 16) + rom_offset)  # Seth
        self.write(b"\xe0\x6b")

        # Edward's Castle
        self.seek(int("4cb5e", 16) + rom_offset)  # Kara
        self.write(b"\xe0\x6b")
        self.seek(int("9bc37", 16) + rom_offset)  # Lilly's flower
        self.write(b"\x02\xe0\x6b")

        # Itory Village
        self.seek(int("4e06e", 16) + rom_offset)  # Kara
        self.write(b"\xe0\x6b")
        self.seek(int("4f0a7", 16) + rom_offset)  # Lola
        self.write(b"\xe0\x6b")
        self.seek(int("4ef78", 16) + rom_offset)  # Bill
        self.write(b"\xe0\x6b")

        # Lilly's House
        self.seek(int("4e1b7", 16) + rom_offset)  # Kara
        self.write(b"\xe0\x6b")

        # Moon Tribe
        # self.seek(int("4f445",16)+rom_offset)   # Lilly
        # self.write(b"\xe0\x6b")

        # Inca Ruins Entrance
        self.seek(int("9ca03", 16) + rom_offset)  # Lilly
        self.write(b"\xe0\x6b")
        self.seek(int("9cea7", 16) + rom_offset)  # Kara
        self.write(b"\xe0\x6b")

        # Diamond Coast
        # self.seek(int("5aaa2",16)+rom_offset)   # Kara
        # self.write(b"\xe0\x6b")

        # Freejia Hotel
        self.seek(int("5c782", 16) + rom_offset)  # Lance
        self.write(b"\xe0\x6b")
        self.seek(int("5cb34", 16) + rom_offset)  # Erik
        self.write(b"\xe0\x6b")
        self.seek(int("5c5b7", 16) + rom_offset)  # Lilly??
        self.write(b"\xe0\x6b")
        self.seek(int("5c45a", 16) + rom_offset)  # Kara??
        self.write(b"\xe0\x6b")

        # Nazca Plain
        self.seek(int("5e845", 16) + rom_offset)
        self.write(b"\xe0\x6b")
        self.seek(int("5ec8f", 16) + rom_offset)
        self.write(b"\xe0\x6b")
        self.seek(int("5eea5", 16) + rom_offset)
        self.write(b"\xe0\x6b")
        self.seek(int("5efed", 16) + rom_offset)
        self.write(b"\xe0\x6b")
        self.seek(int("5f1fd", 16) + rom_offset)
        self.write(b"\xe0\x6b")

        # Seaside Palace
        self.seek(int("68689", 16) + rom_offset)  # Lilly
        self.write(b"\xe0\x6b")

        # Mu
        self.seek(int("6a2cc", 16) + rom_offset)  # Erik
        self.write(b"\xe0\x6b")

        # Watermia
        self.seek(int("7a871", 16) + rom_offset)  # Lilly
        self.write(b"\xe0\x6b")

        # Euro
        self.seek(int("7d989", 16) + rom_offset)  # Kara
        self.write(b"\xe0\x6b")
        self.seek(int("7daa1", 16) + rom_offset)  # Erik
        self.write(b"\xe0\x6b")
        self.seek(int("7db29", 16) + rom_offset)  # Neil
        self.write(b"\xe0\x6b")

        # Natives' Village
        # self.seek(int("8805d",16)+rom_offset)   # Kara
        # self.write(b"\xe0\x6b")
        # self.seek(int("8865f",16)+rom_offset)   # Hamlet
        # self.write(b"\xe0\x6b")
        # self.seek(int("8854a",16)+rom_offset)   # Erik
        # self.write(b"\xe0\x6b")

        # Dao
        self.seek(int("8a4af", 16) + rom_offset)  # Kara
        self.write(b"\xe0\x6b")
        self.seek(int("8a56b", 16) + rom_offset)  # Erik
        self.write(b"\xe0\x6b")
        # self.seek(int("980cc",16)+rom_offset)   # Spirit
        # self.write(b"\xe0\x6b")

        # Pyramid
        self.seek(int("8b7a1", 16) + rom_offset)  # Kara
        self.write(b"\xe0\x6b")
        self.seek(int("8b822", 16) + rom_offset)  # Jackal
        self.write(b"\xe0\x6b")

        # Babel
        self.seek(int("99e90", 16) + rom_offset)  # Kara 1
        self.write(b"\xe0\x6b")
        self.seek(int("98519", 16) + rom_offset)  # Kara 2
        self.write(b"\xe0\x6b")
        ...
        self.seek(int("1aade", 16) + rom_offset)
        self.write(f_roomrewards.read())
        f_roomrewards.close

        # Make room-clearing HP rewards grant +3 HP each
        self.seek(int("e041", 16) + rom_offset)
        self.write(b"\x03")

        # Make boss rewards also grant +3 HP per unclaimed reward
        self.seek(int("c381", 16) + rom_offset)
        self.write(b"\x20\x90\xf4")
        self.seek(int("f490", 16) + rom_offset)
        self.write(b"\xee\xca\x0a\xee\xca\x0a\xee\xca\x0a\x60")

        # Change boss room ranges
        self.seek(int("c31a", 16) + rom_offset)
        self.write(b"\x67\x5A\x73\x00\x8A\x82\xA8\x00\xDD\xCC\xDD\x00\xEA\xB0\xBF\x00")
        # self.write(b"\xF6\xB0\xBF\x00")   # If Solid Arm ever grants Babel rewards

        # Add boss reward events to Babel and Jeweler Mansion
        # self.seek(int("ce3cb",16)+rom_offset)  # Solid Arm
        # self.write(b"\x00\x01\x01\xDF\xA5\x8B\x00\x00\x01\x01\xBB\xC2\x80\x00\xFF\xCA")
        self.seek(int("ce536", 16) + rom_offset)  # Mummy Queen (Babel)
        self.write(b"\x00\x01\x01\xBB\xC2\x80\x00\xFF\xCA")

        # Black Glasses allow you to "see" which upgrades are available
        f_startmenu = open(BIN_PATH + "03fdb0_startmenu.bin", "rb")
        self.seek(int("3fdb0", 16) + rom_offset)
        self.write(f_startmenu.read())
        f_startmenu.close
        self.seek(int("381d6", 16) + rom_offset)
        self.write(b"\x4C\xB0\xFD")

        # Change start menu "FORCE" text
        self.seek(int("1ff70", 16) + rom_offset)
        self.write(b"\x01\xC6\x01\x03\x14\x2D\x33\x48\x50\x00")  # "+3HP"
        self.seek(int("1ff80", 16) + rom_offset)
        self.write(b"\x01\xC6\x01\x03\x14\x2D\x31\x53\x54\x52\x00")  # "+1STR"
        self.seek(int("1ff90", 16) + rom_offset)
        self.write(b"\x01\xC6\x01\x03\x14\x2D\x31\x44\x45\x46\x00")  # "+1DEF"
        ...
        if mode < 4:
            self.seek(int("1abf0", 16) + rom_offset)
            self.write(f_enemies.read())
            f_enemies.close
        ...
        self.seek(int("9c683", 16) + rom_offset)
        self.write(binascii.unhexlify(inca_str))

        # Remove Wind Melody when door opens
        self.seek(int("9c660", 16) + rom_offset)
        self.write(b"\x02\xcd\x12\x01\x02\xd5\x08\x02\xe0")
        self.seek(int("9c676", 16) + rom_offset)
        self.write(b"\x67")
        self.seek(int("9c6a3", 16) + rom_offset)
        self.write(b"\x02\xd0\x10\x01\x60\xc6")
        ...
        # self.seek(int("1f38db",16)+rom_offset)
        self.seek(int("1f3ea0", 16) + rom_offset)
        self.write(b"\x02\x02")
        self.write(f_incamapcomp.read())
        f_incamapcomp.close

        # Direct map arrangement pointer to new data - NO LONGER NECESSARY
        # self.seek(int("d8703",16)+rom_offset)
        # self.write(b"\xa0\x3e")
        ...
        self.seek(int("39e9a", 16) + rom_offset)
        for x in hieroglyph_order:
            if x == 1:
                self.write(b"\xc0\xc1")
            elif x == 2:
                self.write(b"\xc2\xc3")
            elif x == 3:
                self.write(b"\xc4\xc5")
            elif x == 4:
                self.write(b"\xc6\xc7")
            elif x == 5:
                self.write(b"\xc8\xc9")
            elif x == 6:
                self.write(b"\xca\xcb")

        # Update sprite pointers for hieroglyph items, Item 1e is @10803c
        self.seek(int("10803c", 16) + rom_offset)
        for x in hieroglyph_order:
            if x == 1:
                self.write(b"\xde\x81")
            if x == 2:
                self.write(b"\xe4\x81")
            if x == 3:
                self.write(b"\xea\x81")
            if x == 4:
                self.write(b"\xf0\x81")
            if x == 5:
                self.write(b"\xf6\x81")
            if x == 6:
                self.write(b"\xfc\x81")

        # Update which tiles are called when hieroglyph is placed
        i = 0
        for x in hieroglyph_order:
            self.seek(int("39b89", 16) + 5 * i + rom_offset)
            if x == 1:
                self.write(b"\x84")
            elif x == 2:
                self.write(b"\x85")
        ...
        self.seek(int("8cb94", 16) + rom_offset)
        for x in hieroglyph_order:
            if x == 1:
                self.write(b"\x84")
            elif x == 2:
        ...
        self.seek(int("8cee0", 16) + rom_offset)
        self.write(binascii.unhexlify(gem_str[0]))

        gem_str.append(format(int(gem[1] / 10), "x") + format(gem[1] % 10, "x"))
        self.seek(int("8cef1", 16) + rom_offset)
        self.write(binascii.unhexlify(gem_str[1]))

        gem_str.append(format(int(gem[2] / 10), "x") + format(gem[2] % 10, "x"))
        self.seek(int("8cf02", 16) + rom_offset)
        self.write(binascii.unhexlify(gem_str[2]))

        gem_str.append(format(int(gem[3] / 10), "x") + format(gem[3] % 10, "x"))
        self.seek(int("8cf13", 16) + rom_offset)
        self.write(binascii.unhexlify(gem_str[3]))

        gem_str.append(format(int(gem[4] / 10), "x") + format(gem[4] % 10, "x"))
        self.seek(int("8cf24", 16) + rom_offset)
        self.write(binascii.unhexlify(gem_str[4]))

        gem_str.append(format(int(gem[5] / 10), "x") + format(gem[5] % 10, "x"))
        self.seek(int("8cf35", 16) + rom_offset)
        self.write(binascii.unhexlify(gem_str[5]))

        gem_str.append(format(int(gem[6] / 10), "x") + format(gem[6] % 10, "x"))
        self.seek(int("8cf40", 16) + rom_offset)
        self.write(binascii.unhexlify(gem_str[6]))
        ...
        while i < statues_required:
            if statueOrder[i] == 1:
                ...
                self.seek(int("8dd19", 16) + rom_offset)
                self.write(b"\xf8")
            ...
            if statueOrder[i] == 2:
                ...
                # Check for Mystic Statue possession at end game state
                self.seek(int("8dd1f", 16) + rom_offset)
                self.write(b"\xf9")
            ...
            if statueOrder[i] == 3:
                ...
                # Check for Mystic Statue possession at end game state
                self.seek(int("8dd25", 16) + rom_offset)
                self.write(b"\xfa")

                # Restrict removal of Rama Statues from inventory
                self.seek(int("1e12c", 16) + rom_offset)
                self.write(b"\x9f")
            ...
            if statueOrder[i] == 4:
                ...
                # Check for Mystic Statue possession at end game state
                self.seek(int("8dd2b", 16) + rom_offset)
                self.write(b"\xfb")
            ...
            if statueOrder[i] == 5:
                ...
                # Check for Mystic Statue possession at end game state
                self.seek(int("8dd31", 16) + rom_offset)
                self.write(b"\xfc")

                # Restrict removal of Hieroglyphs from inventory
                self.seek(int("1e12d", 16) + rom_offset)
                self.write(b"\xf7\xff")
            ...
            if statueOrder[i] == 6:
                ...
                # Check for Mystic Statue possession at end game state
                self.seek(int("8dd37", 16) + rom_offset)
                self.write(b"\xfd")
        ...
        if self.settings.goal.value != Goal.DARK_GAIA.value:
            self.seek(int("8dd0d", 16) + rom_offset)
            self.write(b"\x10\x01")
        ...
        self.seek(int("48aab", 16) + rom_offset)
        self.write(statue_str)
        ...
        self.seek(int("6d153", 16) + rom_offset)
        self.write(b"\x8a")
        self.seek(int("6d169", 16) + rom_offset)
        self.write(b"\x02\xd2\x8a\x01\x02\xe0")
        self.seek(int("6d25c", 16) + rom_offset)
        self.write(b"\x8a")
        self.seek(int("6d27e", 16) + rom_offset)
        self.write(qt_encode("Hurry boy, she's waiting there for you!") + b"\xc0")
        self.seek(int("6d305", 16) + rom_offset)
        self.write(qt_encode("Kara's portrait. If only you had Magic Dust...") + b"\xc0")

        if kara_location == KARA_ANGEL:
            # Set spoiler for Kara's location in Lance's Letter
            self.seek(int("39521", 16) + rom_offset)
            self.write(b"\x40\x8d\x86\x84\x8b\xac\x66\x88\x8b\x8b\x80\x86\x84")

        else:
            # Remove Kara's painting from Ishtar's Studio
            self.seek(int("cb397", 16) + rom_offset)
            self.write(b"\x18")

            if kara_location == KARA_EDWARDS:  # Underground tunnel exit, map 19 (0x13)
                # Set spoiler for Kara's location in Lance's Letter
                self.seek(int("39521", 16) + rom_offset)
                self.write(b"\x44\x83\xa7\x80\xa2\x83\x0e\xa3\xac\x60\xa2\x88\xa3\x8e\x8d")

                # Set map check ID for Magic Dust item event
                self.seek(int("393a9", 16) + rom_offset)
                self.write(b"\x13\x00\xD0\x08\x02\x45\x0b\x0b\x0d\x0d")

                # Set Kara painting event in appropriate map
                self.seek(int("c8ac5", 16) + rom_offset)
                self.write(b"\x0b\x0b\x00\x4e\xd1\x86\xff\xca")  # this is correct

                # Adjust sprite palette
                self.seek(int("6d15b", 16) + rom_offset)
                self.write(b"\x02\xb6\x30")

            elif kara_location == KARA_MINE:
                # Set spoiler for Kara's location in Lance's Letter
                self.seek(int("39521", 16) + rom_offset)
                self.write(b"\x43\x88\x80\x8c\x8e\x8d\x83\xac\x4c\x88\x8d\x84")

                # Set map check ID for Magic Dust item event
                self.seek(int("393a9", 16) + rom_offset)
                self.write(b"\x47\x00\xD0\x08\x02\x45\x0b\x24\x0d\x26")

                # Change "Sam" to "Samlet"
                self.seek(int("5fee0", 16) + rom_offset)
                self.write(b"\x1a\x00\x10\x02\xc0\x9e\xd2\x02\x0b\x02\xc1\x6b")
                self.seek(int("5d2bd", 16) + rom_offset)
                self.write(b"\xf0\xd2")
                self.seek(int("5d2f0", 16) + rom_offset)
                self.write(b"\xd3\xc2\x05" + qt_encode("Samlet: I'll never forget you!") + b"\xc0")
                self.seek(int("c9c78", 16) + rom_offset)
                self.write(b"\x03\x2a\x00\xe0\xfe\x85")

                # Disable Remus
                self.seek(int("5d15e", 16) + rom_offset)
                self.write(b"\xe0")

                # Assign Kara painting spriteset to appropriate Map
                self.seek(0)
                addr = self.find(b"\x15\x0C\x00\x49\x00\x02", int("d8000", 16) + rom_offset)
                if addr < 0:
                    print("ERROR: Could not change spriteset for Diamond Mine")
                else:
                    self.seek(addr)
                    self.write(b"\x15\x25")

                # Set Kara painting event in appropriate map
                self.seek(int("c9c6a", 16) + rom_offset)
                self.write(b"\x0b\x24\x00\x4e\xd1\x86")

                # Adjust sprite
                # self.seek(int("6d14e",16)+rom_offset)
                # self.write(b"\x2a")
                self.seek(int("6d15b", 16) + rom_offset)
                self.write(b"\x02\xb6\x30")
                ...
                # Set spoiler for Kara's location in Lance's Letter
                self.seek(int("39521", 16) + rom_offset)
                self.write(b"\x4c\xa4\x2a\xac\x4a\xa2\x84\xa3\xa3")

                # Set map check ID for Magic Dust item event
                self.seek(int("393a9", 16) + rom_offset)
                self.write(b"\xa9\x00\xD0\x08\x02\x45\x12\x06\x14\x08")

                # Set Kara painting event in appropriate map
                # Map #169, written into unused Map #104 (Seaside Tunnel)
                self.seek(int("c8152", 16) + rom_offset)
                self.write(b"\x42\xad")
                self.seek(int("cad42", 16) + rom_offset)
                self.write(b"\x05\x0a\x00\x8c\xc3\x82\x00\x00\x00\x00\xed\xea\x80\x00\xdf")
                self.write(b"\xdf\x00\x4d\xe9\x80\x00\x12\x06\x00\x4e\xd1\x86\x00\xff\xca")
                ...
                # Set spoiler for Kara's location in Lance's Letter
                self.seek(int("39521", 16) + rom_offset)
                self.write(b"\x40\x8d\x8a\x8e\xa2\xac\x67\x80\xa4")

                # Set map check ID for Magic Dust item event
                self.seek(int("393a9", 16) + rom_offset)
                self.write(b"\xbf\x00\xD0\x08\x02\x45\x1a\x10\x1c\x12")

                # Set Kara painting event in appropriate map (Map #191)
                # Map #191, written into unused Map #104 (Seaside Tunnel)
                self.seek(int("c817e", 16) + rom_offset)
                self.write(b"\x42\xad")
                self.seek(int("cad42", 16) + rom_offset)
                self.write(b"\x05\x0a\x02\x8c\xc3\x82\x00\x00\x00\x00\xed\xea\x80\x00\x0f")
                self.write(b"\x0b\x00\xa3\x9a\x88\x00\x1a\x10\x00\x4e\xd1\x86\x00\xff\xca")
        ...
        # Will death text
        self.seek(int("d7c3", 16) + rom_offset)
        self.write(death_list[random.randint(0, len(death_list) - 1)])
        # self.write(death_list[0])

        # Change Fredan and Shadow death pointers
        self.seek(int("d7b2", 16) + rom_offset)
        self.write(b"\xc2\xd7")
        self.seek(int("d7b8", 16) + rom_offset)
        self.write(b"\xc2\xd7")
        ...
        self.seek(int("98ebe", 16) + rom_offset)
        self.write(superhero_list[random.randint(0, len(superhero_list) - 1)])
        ...
        seed_adj = 0
        self.w = World(self.settings, statues, kara_location, gem, [inca_x + 1, inca_y + 1], hieroglyph_order)
        while not done:
            if seed_adj > 10:
        ...
        self.w.write_to_rom(self.patch, rom_offset)

        ##########################################################################
        #                        Randomize Ishtar puzzle
        ##########################################################################
        # Add checks for Will's hair in each room
        self.seek(int("6dc53", 16) + rom_offset)
        self.write(b"\x4c\x00\xdd\x86")
        self.seek(int("6dd00", 16) + rom_offset)
        self.write(b"\x02\x45\x10\x00\x20\x10\x66\xdc\x02\x45\x30\x00\x40\x10\x66\xDC")
        self.write(b"\x02\x45\x50\x00\x60\x10\x66\xdc\x02\x45\x70\x00\x80\x10\x66\xDC")
        self.write(b"\x4c\x66\xdc\x86")

        # Generalize success text boxes
        # Make Rooms 1 and 3 equal to Room 2 text (which is already general)
        self.seek(int("6d978", 16) + rom_offset)  # Room 1
        self.write(b"\x2c\xdb")
        self.seek(int("6d9ce", 16) + rom_offset)  # Room 3
        self.write(b"\x2c\xdb")
        # Generalize Room 4 text
        self.seek(int("6d9f8", 16) + rom_offset)
        self.write(b"\xc6\xdb")
        self.seek(int("6dbc6", 16) + rom_offset)
        self.write(b"\xc2\x0a")
        ...
        if changes[0] == 1:  # Change right vase to light (vanilla)
            ...
            self.seek(int("6dd06", 16) + rom_offset)
            self.write(b"\x5d")
            coords = [b"\xa0\x01", b"\xc0\x01", b"\xb0\x00", b"\xd0\x00"]

        # Update cursor check ranges for Room 1
        for i in range(4):
            self.seek(int(room_offsets[0], 16) + coord_offsets[i] + rom_offset)
            self.write(coords[i])
        ...
        if changes[1] == 1:  # Change both pots to dark (vanilla)
            ...
            self.seek(int("6dd0e", 16) + rom_offset)
            self.write(b"\x5d")
        ...
        for i in range(4):
            self.seek(int(room_offsets[1], 16) + coord_offsets[i] + rom_offset)
            self.write(coords[i])
        ...
        if self.w.item_locations[80][3] == self.w.item_locations[81][3]:
            if changes[2] == 1:  # Remove rock
                ...
                self.seek(int("6dd16", 16) + rom_offset)
                self.write(b"\x5d")
            ...
            for i in range(4):
                self.seek(int(room_offsets[2], 16) + coord_offsets[i] + rom_offset)
                self.write(coords[i])

        # Set change for Room 4
        if changes[3] == 1:  # Will's hair (vanilla)
            self.seek(int("6dd1e", 16) + rom_offset)
            self.write(b"\x5d")
            ...
            for i in range(4):
                self.seek(int(room_offsets[3], 16) + coord_offsets[i] + rom_offset)
                self.write(coords[i])
        ...
        # Insert new compressed map data
        # self.seek(int("193d25",16)+rom_offset)
        self.seek(int("1f4100", 16) + rom_offset)
        self.write(b"\x08\x02")
        self.write(f_ishtarmapcomp.read())
        f_ishtarmapcomp.close()

        # Direct map arrangement pointer to new data - NO LONGER NECESSARY
        # self.seek(int("d977e",16)+rom_offset)
        # self.write(b"\x00\x41")

        return json.dumps(self.patch.data)

    def generate_spoiler(self) -> str:
        #  self.w.print_spoiler()
    ...
    def find(self, d, start=None, end=None):
        return self.patch.find(d, start, end)
    ...
    def __copy_original_rom__(self):
        ...
        self.patch = Patch(self.original_rom_data, self.logger)
        self.__set_offset__()
    ...
    def __set_offset__(self):
        ...
        self.seek(0)
        h_addr = self.find(header)
        if h_addr < 0:
            raise OffsetError

        self.offset = h_addr - int("ffc0", 16)

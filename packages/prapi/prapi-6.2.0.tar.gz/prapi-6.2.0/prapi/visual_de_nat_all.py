...
######################
def visual_de_nat_all(clu,allgene,genename,refGenes,path,apapos,atipos,apapos_r,atipos_r,ir,es,alta,altd,altp,nat,difflib,pairs,fpkmcal,ir_r,\
	es_r,alta_r,altd_r,altp_r,de_nat_lib,mapdict,conf,cf,bamlist,cuffid,cuffd):
	...
	##Normailiation
		...
		###Normaliation END	
		return wig
	def transcripts(cr,h,scale,line,arrayline,sort_dict,refGenes,minpos,maxpos,genename,allgene,nat,choice):
		global hh
		think=dict()
		for j in sort_dict:
			info=j.split()
...
#			apapos=difflib[0][-1]
		...
		if True:
			...
			for i in conf:
				...
				if nat!="True" or (nat=="True" and Strand_Specificity!="True"):	
					...
					if wigggforward:
						...
						cr.fill()
						if difflib!="NULL":
							libpos[tpp+"_+"]=hh-maxacnum*scaleh/2
						#######
						hh+=50
					if wigggreverse:
						scaleh=32/(max(eval(wiggdict[i])["-"].keys()))
			...
			cr.set_source_rgb(1,0,0)
			cr.set_line_width(1.5*arrayline)
			for lib1,lib2,strand in difflib:
				xlen+=6
			...
			### apa differentcial site
				...
				##################
				cr.move_to(50+(pos1-minpos)*scale,hh)
				cr.line_to(50+(pos1-minpos)*scale,high1)
				cr.stroke()
				##################3
				cr.move_to(50+(pos2-minpos)*scale,hh)
				cr.line_to(50+(pos2-minpos)*scale,high1)
				cr.stroke()
				##################
			####
			"""
	...
	for i in [clu]:
		...
		#######################################################
		if nat=="True":
			hight_of=70+8+200*(len(sort_dict)+angene)/(len(sort_dict)+23)+90*len(libnum)+10+10*max(len(atinum),len(apanum))+30*(asnum+1)+40*3
		else:
		...
		################
		transcripts(cr,h,scale,line,arrayline,sort_dict,refGenes,minpos,maxpos,genename,allgene,nat,"True")
		if cuffid!="NULL":
			cuff_sort_dict= sorted(cuffd[cuffid], key=lambda d:(d.split()[4]), reverse = False)
			transcripts(cr,h,scale,line,arrayline,cuff_sort_dict,refGenes,minpos,maxpos,genename,allgene,nat,"False")
		else:
			pass

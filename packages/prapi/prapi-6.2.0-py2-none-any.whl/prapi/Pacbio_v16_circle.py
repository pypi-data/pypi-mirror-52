...
#from asas_v3 import *
###############
#sys.path.append("scripts/viusal")
...
def Control(Genome_Annotion_format):
	...
	for i in open(Genome_Annotion_format,"r"):
		...
		ele=i.split()
		allgene[ele[4]]=i
	for itelfile in ("Annotation","Annotation_Miss","Novel_Gene"):
	...
	if Multile_processing=="True":
		...
		for clu in mapdict:
			if (cf.get("DE AS option","DE")=="True") and (clu not in RI) and (clu not in SE) and (clu not in A3SS) and (clu not in A5SS):
				continue
...
def subcon(clu,allgene,RI,SE,A3SS,A5SS,APA,ATI,DEAPA,cuffd,cufflim,RI_cuff,SE_cuff,A3SS_cuff,A5SS_cuff,DENAT):
	...
	DE_NAT=cf.get("DE NAT option","DE")
	for i in [clu]:
		...
		if len(strand)==2:
			...
			if cf.has_option("input file","Pacbio_reads"):
				#apapos=APA[clu,strand.keys()[0]]
				#atipos=ATI[clu,strand.keys()[0]]
				#if apapos==None:
				#	apapos="NULL"
				#if atipos==None:
				#	atipos="NULL"
				#(ir,es,altd,alta,altp,seqir,seqes,seqalta,seqaltd)=AS_v3(clu,"pacbio",gene,strand,genename)
				#output=open("%s/temp/%s.csv"%(Output_dir,genename),"w")
				...
				"""
				if DE_AS=="True":
					if ir!="NULL" or es!="NULL" or alta!="NULL" or altd!="NULL":
						print "IR\n",ir,seqir
						print "ES\n",es,seqes
						print "ALtA\n",alta,seqalta
						print "ALtD\n",altd,seqaltd
						write_mats_single(seqir,seqes,seqalta,seqaltd)
						print "ok"
					else:
						continue
				"""
				...
				if DE_AS=="True":
					...
					if cf.has_option("input file","Cuffmerge_file") and cf.has_option("input file","Pacbio_reads"):
						...
						visual_basic_pacbio(clu,allgene,genename,refGenes,path,apapos,atipos,nat,difflib,mapdict,conf,cf,bamlist,strand.keys()[0],RI,SE,A3SS,A5SS)
			"""
			elif cf.has_option("input file","Cuffmerge_file"):
...
if  __name__ == '__main__':
	...
	if not os.path.exists("temp"):
		...
		os.makedirs("temp")
	######
	if (cf.get("DE AS option","DE")=="True" and cf.get("DE APA option","DE")=="True") or(cf.get("DE NAT option","DE")=="True" and cf.get("DE APA option","DE")=="True"):
	...
	if cf.has_option("input file","Pacbio_reads"):
		...
		cmd="alignPacBio.py -v -o %s/fix %s %s %s %s"%(Output_dir,GMAP_IndexesDir,GMAP_IndexesName,fa,Reads)
		os.system(cmd)
	...
	if   cf.get("DE AS option","DE")=="True":
		...
		#cmd="mats_as.py %s/merge.format.cluster.gtf %s/AS/gtf/as >%s/AS.log"%(Output_dir,Output_dir,Output_dir)
		#os.system(cmd)
		readLength=int(cf.get("Wig option","readLength"))

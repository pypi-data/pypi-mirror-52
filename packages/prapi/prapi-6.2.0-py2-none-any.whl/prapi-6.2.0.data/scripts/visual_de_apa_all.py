...
def visual_de_apa_all(clu,allgene,genename,refGenes,path,apapos,atipos,nat,difflib,mapdict,conf,cf,bamlist,strand,RI,SE,A3SS,A5SS,cuffid,cuffd,RI_cuff,SE_cuff,A3SS_cuff,A5SS_cuff):
			...
			for st in range(6,len(info)):
				...
				if strand == '+':
					if nat=="True":
						...
						if info[5].startswith("cufflinks"):
							(r,g,b)=hex2rgb(conf["cufflinksexon"])
							cr.set_source_rgb(r,g,b)
						else:
							(r,g,b)=hex2rgb(conf["exon"])
							cr.set_source_rgb(r,g,b)
		...
		if True:
			...
			for i in conf:
				...
				if nat!="True" or (nat=="True" and Strand_Specificity!="True"):	
					...
					if difflib!="NULL":
						libpos[tpp]=hh-maxacnum*scaleh/2
					####
				elif nat=="True" and Strand_Specificity=="True":
					if wigggforward:
						scaleh=32/(max(eval(wiggdict[i])["+"].keys()))
			...
			cr.set_source_rgb(1,0,0)
			cr.set_line_width(1.5*arrayline)
			for lib1,lib2,pos1,pos2 in difflib:
				xlen+=6

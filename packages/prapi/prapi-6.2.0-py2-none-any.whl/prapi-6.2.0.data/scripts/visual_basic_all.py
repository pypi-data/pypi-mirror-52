...
def visual_basic_all(clu,allgene,genename,refGenes,path,apapos,atipos,nat,difflib,mapdict,conf,cf,bamlist,strand,RI,SE,A3SS,A5SS,cuffid,cuffd,RI_cuff,SE_cuff,A3SS_cuff,A5SS_cuff):
	def ass_cuff(cr,h,scale,line,arrayline,sort_dict,refGenes,minpos,maxpos,genename,allgene,Gene_coordinate,nat,posnum,tpd):
		#########################example
		global hh,ashigh
		showhigh=10
		#hh+=10+1*h/2
		...
		cr.set_font_size(8)
		#print RI_cuff.keys(),SE_cuff[clu].keys(),A3SS_cuff[clu].keys(),A5SS_cuff[clu].keys()
		#RI,SE,A3SS,A5SS
		######################
		clu_change=";".join(clu.split("."))
		######################
		if RI_cuff[clu]:
			ashigh+=30
	...
	def atiapa(atinum,apanum,cr,h,scale,line,arrayline,sort_dict,refGenes,minpos,maxpos,genename,allgene,Gene_coordinate,nat,types):
		global hh
		strand=types
	...
	def example(cr,h,scale,line,arrayline,bar_high,nat,genename,de_nat_lib):
		...
		if True and cf.has_option("input file","Pacbio_reads"):
	...
	for i in [clu]:
		...
		libnum=[]
		for libsub in conf:
			if libsub.startswith("lib") and Wig_plot=="True":
		...
		#################################################
		#print "%s/%s/%s.pdf"%(Output_dir,path,genename)
		######
		...
		if cuffid!="NULL":
			cuffmin=min(int(x.split()[2]) for x in cuffd[cuffid])
			cuffmax=max(int(x.split()[3]) for x in cuffd[cuffid])
			minpos=min(cuffmin,minpos)
			maxpos=max(cuffmax,maxpos)
		scale=400/(maxpos-minpos+1)
		posnum=(maxpos-minpos+1)//10
		...
		if cuffid!="NULL":
			cuff_sort_dict= sorted(cuffd[cuffid], key=lambda d:(int(d.split()[3])-int(d.split()[2])), reverse = False)
			transcripts(cr,h,scale,line,arrayline,cuff_sort_dict,refGenes,minpos,maxpos,genename,allgene,nat,"NULL")
		if Wig_plot=="True":
			wig(cr,h,scale,line,arrayline,sort_dict,refGenes,minpos,maxpos,genename,allgene,Gene_coordinate,nat,"NULL","NULL")
		atiapa(atinum,apanum,cr,h,scale,line,arrayline,sort_dict,refGenes,minpos,maxpos,genename,allgene,Gene_coordinate,nat,strand)
		ashigh=hh
		ass(cr,h,scale,line,arrayline,sort_dict,refGenes,minpos,maxpos,genename,allgene,Gene_coordinate,nat,posnum,"P")
		if cuffid!="NULL":
			ass_cuff(cr,h,scale,line,arrayline,sort_dict,refGenes,minpos,maxpos,genename,allgene,Gene_coordinate,nat,posnum,"C")

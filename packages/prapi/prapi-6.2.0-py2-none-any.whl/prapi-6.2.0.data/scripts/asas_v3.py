...
def AltD(exondict,introndict,gene,strand,genename):
	...
	for i in introndict:
		...
		for j in introndict:
			...
			if from2<from1 and abs(to2-to1)<5:
				"""
				if abs(int(introndict[i].split(":")[0])-int(introndict[j].split(":")[0]))<10 and abs(int(introndict[i].split(":")[-1])-int(introndict[j].split(":")[-1]))<10:
				...
				"""
				#if int(introndict[j].split(":")[0])<=int(introndict[i].split(":")[0])<from2:
				if from2>int(introndict[i].split(":")[0]):
					string+="%s\tNA\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t\n"%(genename,gene,strand,int(introndict[i].split(":")[0])-1,from1,int(introndict[i].split(":")[0])-1,from2,to1-1,int(introndict[i].split(":")[-1]))
	return string
def AltA(exondict,introndict,gene,strand,genename):
	...
	for i in introndict:
		...
		for j in introndict:
			(from2,to2)=j.split(':')
			(from2,to2)=(int(from2),int(to2))
			if abs(from2-from1)<5 and to2<to1:
				
				"""
				if abs(int(introndict[i].split(":")[0]) - int(introndict[j].split(":")[0]))<10 and abs(int(introndict[i].split(":")[-1])-int(introndict[j].split(":")[-1]))<10:
					string+="%s\tNA\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t\n"%(genename,gene,strand,to2-1,int(introndict[i].split(":")[-1]),to1-1,int(introndict[i].split(":")[-1]),int(introndict[i].split(":")[0])-1,from1)
				"""
				#if to1<int(introndict[j].split(":")[-1])<=int(introndict[i].split(":")[-1]):
				if int(introndict[j].split(":")[-1])>to1:
					string+="%s\tNA\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t\n"%(genename,gene,strand,to2-1,int(introndict[i].split(":")[-1]),to1-1,int(introndict[i].split(":")[-1]),int(introndict[i].split(":")[0])-1,from1)
				#string+="%s\tNA\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t\n"%(genename,gene,strand,to2-1,int(introndict[i].split(":")[-1]),to1-1,int(introndict[i].split(":")[-1]),int(introndict[i].split(":")[0])-1,from1)
	return string
#################################################

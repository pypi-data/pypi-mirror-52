...
##### Getting Start Time ######
#print('Start the program with [%s]\n', listToString(sys.argv));
#startTime = time.time();
#
myVer="3.2.5";
#
...
if True:
    value="NULL"
    readLength=int(sys.argv[1]);
    junctionLength=int(sys.argv[2]);
    SE = "%s/as.SE.txt"%(sys.argv[3]);
    MXE = "NULL";
    A5SS = "%s/as.A3SS.txt"%(sys.argv[3]);
    A3SS = "%s/as.A5SS.txt"%(sys.argv[3]);
    AFE = "NULL";
    ALE = "NULL";
    RI = "%s/as.RI.txt"%(sys.argv[3]);
    experiment = "RNASeq";
    base_1 = value;
    base_2 = value;
    dataType = "paired";
...
#
print("Effective junction length is: %d" % ejLength); 
#
print("There are %d replicates in sample_1: %s" % (numRep_1,input_1));
print("There are %d replicates in sample_2: %s" % (numRep_2,input_2));
#
print("==================================================================");
#
...
#
print("populating AS events dictionary");
#
...
for line in seFile: ## process skipped exon events file
  ...
  if chr in se: ## already processed this chromosome
    ...
    for i in group: ## for each possible group
      ...
      se[chr][i][id] = [tS,tE,uS,uE,dS,dE,strand]; ## skipping event with coords
      numSE+=1;
print("Processed %d skipped exon events from input AS event file" % c);
print("Done populating skipped exon dictionary with %d items. %d have duplicate ids" % (numSE, numSEDup));
print("There are %d se ids in count dictionary and %d se ids in supple dictionary" % (len(c_se),len(s_se)));
#
logline();
#
#### MXE ####
...
for line in mxeFile: ## process mxe events file
  ...
  if chr in mxe: ## already processed this chromosome
    ...
    for i in group: ## for each possible group
      ...
      mxe[chr][i][id] = [tS,tE,sS,sE,uS,uE,dS,dE,strand]; ## mxe event with coords
      numMXE+=1;
print("Processed %d mxe events from input AS event file" % c);
print("Done populating mxe event dictionary with %d items. %d have duplicate ids" % (numMXE, numMXEDup));
print("There are %d mxe ids in count dictionary and %d mxe ids in supple dictionary" % (len(c_mxe),len(s_mxe)));
#
logline();
#
"""
...
for line in a5ssFile: ## process a5ss events file
  ...
  if chr in a5ss: ## already processed this chromosome
    ...
    for i in group: ## for each possible group
      ...
      a5ss[chr][i][id] = [lS,lE,sS,sE,fS,fE,strand]; ## a5ss event with coords
      numA5SS+=1;
print("Processed %d a5ss events from input AS event file" % c);
print("Done populating a5ss dictionary with %d items. %d have duplicate ids" % (numA5SS, numA5SSDup));
print("There are %d a5ss ids in count dictionary and %d a5ss ids in supple dictionary" % (len(c_a5ss),len(s_a5ss)));
#
logline();
#
#### A3SS ####
...
for line in a3ssFile: ## process a3ss events file
  ...
  if chr in a3ss: ## already processed this chromosome
    ...
    for i in group: ## for each possible group
      ...
      a3ss[chr][i][id] = [lS,lE,sS,sE,fS,fE,strand]; ## a3ss event with coords
      numA3SS+=1;
print("Processed %d a3ss events from input AS event file" % c);
print("Done populating a3ss dictionary with %d items. %d have duplicate ids" % (numA3SS, numA3SSDup));
print("There are %d a3ss ids in count dictionary and %d a3ss ids in supple dictionary" % (len(c_a3ss),len(s_a3ss)));
#
logline();
#### RI ####
#
...
for line in riFile: ## process ri events file
  ...
  if chr in ri: ## already processed this chromosome
    ...
    for i in group: ## for each possible group
      ...
      ri[chr][i][id] = [rS,rE,uS,uE,dS,dE,strand]; ## ri event with coord
      numRI+=1;
print("Processed %d ri events from input AS event file" % c);
print("Done populating ri dictionary with %d items. %d have duplicate ids" % (numRI, numRIDup));
print("There are %d ri ids in count dictionary and %d ri ids in supple dictionary" % (len(c_ri),len(s_ri)));
#
logline();
#
def is_unique(r):
...
def processSample(sample, sInd): ## call it with processSample(sample_1, S1) something like this
  ...
  for s1 in sample: ## for each sam file 
    ...
    e1 = {}; ## edge count here
    #for read in sFile.fetch(chro,s,e):
...
####################### for Zhijie: START WORKING FROM HERE ###################
    ...
    print("Done populating edeg count for %s with %d junctions" % (s1.strip(), len(e1)));
    sFile.close();  
...
if dataType=="skipThisForNow": ## call paired..
  print("Start processing sample_1: %s" % base_1);
  processSample_PE(sample_1, S1, 72, 60);
  print("Done processing %s" % base_1);
  print("Start processing sample_2: %s" % base_2);
  processSample_PE(sample_2, S2, 75, 62);
  print("Done processing %s" % base_2);
...
#
print("Writing out the rMATS input file..");
#
...
print("Program ran %.2d:%.2d:%.2d" % (runningTime/3600, (runningTime%3600)/60, runningTime%60));
##############################################
sys.exit(0);
#

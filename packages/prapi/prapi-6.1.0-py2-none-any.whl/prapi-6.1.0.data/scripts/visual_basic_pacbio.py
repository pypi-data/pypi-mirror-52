...
def visual_basic_pacbio(clu,allgene,genename,refGenes,path,apapos,atipos,nat,difflib,mapdict,conf,cf,bamlist,strand,RI,SE,A3SS,A5SS,circle_item):
	...
	#################
		#Mapping position
			if 'D' in cigar or 'I' in cigar or "S" in cigar:
				continue
			if 'N' in cigar:
	...
	for i in [clu]:
		...
		if len(refGenes)!=0: 
			...
			annomin=min(reg[1] for reg in refGenes)
			annomax=max(reg[2] for reg in refGenes)

...
def as_cufflinks_dict_de():
	...
	for i in ["RI","SE","A3SS","A5SS"]:
		#files="%s/AS_cufflinks/gtf/gtf/as.%s.txt"%(Output_dir,i)
		files="%s/JC_cuff/%s.MATS.ReadsOnTargetAndJunctionCount.txt"%(Output_dir,i)
		for j in open(files,"r"):
			ele=j.rstrip().split()
			#genename=ele[1].split("\"")[1]
			if j.startswith("ID"):
...
def Control(Genome_Annotion_format):
	...
	if cf.has_section("DE APA option"):
		...
		if cf.has_section("Circle") or cf.has_section("DE Circle"):
			deapa="NULL"
			cuffd,cufflim="NULL","NULL"
			RI_cuff,SE_cuff,A3SS_cuff,A5SS_cuff="NULL","NULL","NULL","NULL"
		else:
	...
	if cf.has_section("DE AS option"):
		...
		RI_cuff,SE_cuff,A3SS_cuff,A5SS_cuff=as_cufflinks_dict_de()
	else:
...
def subcon(clu,allgene,RI,SE,A3SS,A5SS,APA,ATI,DEAPA,cuffd,cufflim,RI_cuff,SE_cuff,A3SS_cuff,A5SS_cuff,DENAT):
	...
	for i in [clu]:
		...
		if len(strand)==2:
			...
			if cufflinks=="True" and cf.has_option("input file","Pacbio_reads"):
				cuffid="NULL"
				if gene in cufflim:
						...
						if gene in cufflim:
							for x in cufflim[gene]:
								...
								if (maxpos<s)or(e<minpos):
									...
									cuffid=cufflim[gene][x]
									break
						#print RI_cuff.keys(),SE_cuff.keys(),A3SS_cuff.keys(),A5SS_cuff.keys()
...
def rmsummary(Output_dir):
	...
	if  cf.get("DE NAT option","DE")=="True":
		...
		files="%s/NAT/de_nat.txt"%(Output_dir)
		os.system("mv %s %s/temp"%(files,Output_dir))
def circle_return(libs,Output_dir):
	gene_count=defaultdict(dict)
	for i in libs:
		fl="%s/circle/%s_circ.txt"%(Output_dir,i)
		for j in open(fl,"r"):
			ele=j.rstrip().split()
			gene_count[ele[14].split("|")[0]]["%s:%s-%s"%(ele[0],ele[1],ele[2])]=ele[12]
			#gene_count["%s|%s:%s-%s"%(ele[14].split("|")[0],ele[0],ele[1],ele[2])][i]=ele[12]
	return gene_count
        ...
        for i in open(fl,"r"):
                ...
                if ele[2]=="mRNA":
                        ...
                        if not ele[0]  in chrom:
                                ...
                                chrom[ele[0]]+=1
                        genename=ele[-1].split(".")[0].split("=")[-1]
...
if  __name__ == '__main__':
	...
	################
	idps=cf.get("input file","IDP_isoform")
	################
	if cf.has_option("input file","Pacbio_reads"):
		...
		os.system(cmd)
		################################################################
		if_tapis=cf.get("Global option","Genome_correction")
		if if_tapis=="True":
			cmd="sam_to_gff_trim_none.py %s/fix/aligned.bam >%s/merge.format.none"%(Output_dir,Output_dir)
			os.system(cmd)
		else:
			cmd="gmap -D %s -d %s --no-chimeras --cross-species -K 8000 -f samse -n 1 -t 4 %s -f 2 >%s/fix/raw.gff"%(GMAP_IndexesDir,GMAP_IndexesName,Reads,Output_dir)
			os.system(cmd)
			Format_gff3("%s/fix/raw.gff"%(Output_dir))
			cmd="mv %s/fix/raw.gff.format %s/merge.format.none"%(Output_dir,Output_dir)
			os.system(cmd)
	...
	if  cf.has_option("input file","Pacbio_reads"):
		...
		######
	if not cf.has_section("DE APA option") and not cf.has_section("DE Circle") and not cf.has_section("Circle"):
		if not os.path.exists("%s/cufflinks"%(Output_dir)):
			sys.stderr.write("Making cufflinks temportery dir\n")
			os.makedirs("%s/cufflinks"%(Output_dir))
		output=open("%s/cufflinks/assembly_list.txt"%(Output_dir),"w")
		for x in cf.items("Wig option"):
			if x[0].startswith("lib"):
				ele=x[1].split()
				##
				# cufflinks
				cmd="cufflinks %s -o %s/cufflinks/%s"%(ele[1],Output_dir,ele[0])
				os.system(cmd)
				output.write("%s/cufflinks/%s/transcripts.gtf\n"%(Output_dir,ele[0]))
				#cmd="echo \"$output/cufflink/$i/transcripts.gtf\" >>$output/cuffmerge/assembly_list.txt"
		output.close()
		cmd="cuffmerge -o %s/cufflinks/merge -s %s %s/cufflinks/assembly_list.txt"%(Output_dir,fa,Output_dir)
		print cmd
		os.system(cmd)
		#parrse_cufflinks(cf)
		parrse_cufflinks_v2(cf)
		...
		######
	if idps:
		shutil.copy(idps,Output_dir)
		gff_gpd_format("%s/%s"%(Output_dir,os.path.split(idps)[1]))
		Cluster_Reads("%s/%s.format"%(Output_dir,os.path.split(idps)[1]))
		if not os.path.exists("%s/IDP"%(Output_dir)):
			sys.stderr.write("Making alternative splicing  temportery dir\n")
			os.makedirs("%s/IDP"%(Output_dir))
		cmd="asas_v3.py %s/%s.format.cluster %s/IDP %s pacbio"%(Output_dir,os.path.split(idps)[1],Output_dir,Output_dir)
		os.system(cmd)
		#sys.exit()
	#if cf.has_section("DE AS option"):
	#	merge()
	if   cf.has_section("DE AS option"):
		...
		## pacbio as ended
		## cufflinks as
		if not os.path.exists("%s/JC_cuff/temp"%(Output_dir)):
			sys.stderr.write("Making cufflinks alternative splicing  temportery dir\n")
			os.makedirs("%s/JC_cuff/temp"%(Output_dir))
		readLength=int(cf.get("Wig option","readLength"))
		junctionLength=check_jucton_length()
		libs=cf.get("DE AS option","delib")
		group1= [conf[x.lower()].strip('\n').split()[1] for x in libs.split()[0].split("_")]
		...
		group2= [conf[x.lower()].strip('\n').split()[1] for x in libs.split()[1].split("_")]
		group2str=",".join(group2)
		cmd="Pacbio_mats.py %d %d %s/AS_cufflinks/gtf/gtf %s %s %s/JC_cuff/temp"%(readLength,junctionLength,Output_dir,group1str,group2str,Output_dir)
		os.system(cmd)
		cmd="Pacbio_mats_cuff.sh %s"%(Output_dir)
		os.system(cmd)
		###### add gene info
		for i in ["RI","SE","A5SS","A3SS"]:
			files="%s/JC_cuff/%s.MATS.ReadsOnTargetAndJunctionCount.txt"%(Output_dir,i)
	...
	if  (not cf.has_section("DE Circle")) and (cf.has_section("Circle")):
		...
		for i in libs:
			...
			os.system(cmd)
		name_lib=circle_return(libs,Output_dir)
	#################################
	if  cf.has_section("DE Circle"):
		if not os.path.exists("%s/circle"%(Output_dir)):
			sys.stderr.write("Making circle temportery dir\n")

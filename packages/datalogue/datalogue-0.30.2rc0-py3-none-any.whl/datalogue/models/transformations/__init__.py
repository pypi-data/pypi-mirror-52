...
class Classify(Transformation):
    ...
    def __init__(self, model_identifier: UUID, paths: Optional[List[List[str]]] = None, use_context: bool = False, include_classes: bool = False,
    ...
    def _as_payload(self) -> dict:
        ...
        base["modelIdentifier"] = str(self.model_identifier)
...
def parse_uuid(json: dict, key: str) -> Union[DtlError, UUID]:
    maybe_value = json.get(key)
    if maybe_value is None:
        return _property_not_found(key, json)
    else:
        return UUID(maybe_value)
...
def _classify_from_payload(json: dict) -> Union[DtlError, Classify]:
    ...
    model_identifier = parse_uuid(json, "modelIdentifier")
    if isinstance(model_identifier, DtlError):
    ...
    return Classify(model_identifier, paths, use_context, include_classes, include_score, replace_class, datastore_collection_name)
...
class RecognizeEntities(Transformation):
    ...
    def __init__(self, model_url: Optional[str] = None, include_classes: bool = False, include_scores: bool = False):
        """
    ...
    def __repr__(self):
        ...
        if self.include_scores:
            options.append(self.include_scores_type)
        model_url_rep = '' if self.model_url is None else self.model_url
        return "RecognizeEntities(" + \
               f"model_url: {model_url_rep}, " \
               f"options: {'' if options is None else ','.join(options)})"
...
class ClassifyOrRecognizeEntities(Transformation):
    ...
    def __init__(self, model_url: Optional[str], max_value_length_for_cfc: int):
        self.model_url = model_url
...
def _classify_recognize_entities_from_payload(json: dict) -> Union[DtlError, ClassifyOrRecognizeEntities]:
    ...
    return ClassifyOrRecognizeEntities(model_url, max_value_length_for_cfc)
...
class StandardizeDates(Transformation):
    ...
    def __init__(self, model_url: Optional[str] = None, paths: Optional[List[List[str]]] = None, include_new_nodes: bool = False):
...
def _entity_recognition_from_payload(json: dict) -> Union[DtlError, RecognizeEntities]:
    ...
    return RecognizeEntities(model_url, include_classes, include_score)
...
class ParseAddresses(Transformation):
    ...
    def __repr__(self):
        return f"ParseAddresses(model_url: {'' if self.model_url is None else self.model_url}, paths: {'' if self.paths is None else ','.join(map(lambda path: '.'.join(path), self.paths))})"

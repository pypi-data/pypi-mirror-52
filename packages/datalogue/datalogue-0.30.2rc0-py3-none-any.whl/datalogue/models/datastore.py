...
from datalogue.models.schema import AbstractDataSchema, SchemaNode
from datalogue.utils import _parse_list, _parse_string_list, SerializableStringEnum



class NodeClassification: 
    def __init__(self, schema_node_id: UUID, ontology_node_id: UUID):
    ...
    def _as_payload(self) -> Union[DtlError, dict]:
        ...
        return {
            ...
            "userClassifications": [
                {
                    "ontologyNodeId": str(self.ontology_node_id)
                }
            ]
        }
...
class Datastore:
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(id: {self.id}, name: {self.name!r}, alias: {self.alias!r}, ' \
               ...
               f'samples: {self.samples!r}, schema_paths: {self.schema_paths!r}, schema_labels: {self.schema_labels!r}, schema_nodes: {self.schema_nodes!r})'

...
class sim(object):
    ...
    def __init__(self):
        ...
        try:
            ...
            if sys.platform in ('win32', 'cygwin'):
                self._ramseslib = ctypes.CDLL(os.path.join(__libdir__, "ramses.dll"))
            else:
                ...
                self._ramseslib = ctypes.CDLL(os.path.join(__libdir__, "ramses.so"))
        except OSError as e:
    ...
    def _c_func_wrapper(self, cdecl_text):
        ...
        try:
            ...
            raise AttributeError(
                'RAMSES: Function %s is listed in ramses.h but cannot be found in the library.' + e % (name))
    ...
    def execSim(self, cmd, pause=None):
        ...
        if (retval != 0) and (retval != 112):
            raise RAMSESError('RAMSES: Function execSim(%s) failed with the flag %i. Last message was: %s' % (cmdfilename, retval, self.getLastErr()))
        return 0

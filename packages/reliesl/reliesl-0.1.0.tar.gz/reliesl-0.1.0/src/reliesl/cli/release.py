...
def upload_file(file: bytes, project_api_url: str, auth: Header) -> Json:
    uploads_url = project_api_url + "/uploads"
    result = requests.post(uploads_url, files={"file": file}, headers=auth)
...
def upload_wheel(
    ...
    with wheel_path.open(encoding="latin-1") as wheel_file:
        wheel_reply = upload_file(
            file=wheel_file, project_api_url=project_api_url, auth=auth
        )

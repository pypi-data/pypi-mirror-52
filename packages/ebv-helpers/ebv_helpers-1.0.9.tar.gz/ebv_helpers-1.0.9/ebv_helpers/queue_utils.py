...
from pika.exceptions import ProbableAccessDeniedError, ConnectionClosedByBroker, ChannelClosed, ConnectionClosed
...
class RabbitmqManagement(object):
    ...
    def request_with_credentials(self, url, data=None):
    ...
    def create_vhost(self, vhost):
        url = "http://%s:%s/api/vhosts/%s" % (self.host, self.MANAGEMENT_PORT, vhost)

        try:
            self.request_with_credentials(url)
        except Exception:
            LOGGER.exception("Attempt to create temporary vhost=%s failed. url=%r", vhost, url)
            raise

    def create_permission(self, vhost):
        url = "http://%s:%s/api/permissions/%s/%s" % (self.host, self.MANAGEMENT_PORT, vhost, self.username)
        try:
            self.request_with_credentials(url, json.dumps({"configure": ".*", "write": ".*", "read": ".*"}))
        except Exception:
...
class Queue(object):
    ...
    def publish(self, body, properties=None):
        ...
        except (ChannelClosed, ConnectionClosed) as cex:

...
def main():
    ...
    process_dir(args.src, args.dst, {path.join(args.src, exfile) for exfile in args.x})


def process_dir(src_dir: str, dst_dir: str, excluded: Set):
    ...
    for entry_name in os.listdir(src_dir):
        ...
        if path.isdir(entry_src):
            process_dir(entry_src, entry_dst, excluded)

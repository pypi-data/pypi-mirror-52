...
class AgentCom(object):
    ...
    def _connect(self):
        ...
        while self._running:
            try:
                try:
                    ...
                    if e.status_code == 403:  # invalid token
                        ...
                        logger.error('Connection failed; %s', self._connection_error.message)
                        return
                    raise
            ...
            def signal_stop_incoming_thread():
                logger.debug("calling stop incoming thread")
                try:
            ...
            def signal_stop_outgoing_thread():
                logger.debug("calling stop outgoing thread")
                try:
    ...
    def _incoming(self, stop_socket, on_exit):
        try:
            while True:
                try:
                    ...
                    if stop_socket in rlist or stop_socket in xlist or self._connection in xlist:
                        logger.debug("Incoming thread signaled to stop")
            ...
            logger.debug("Incoming thread exiting")
            on_exit()
    ...
    def _outgoing(self, outgoing_exit_sentinel, on_exit):
        try:
            ...
            logger.debug("Outgoing thread exiting")
            on_exit()

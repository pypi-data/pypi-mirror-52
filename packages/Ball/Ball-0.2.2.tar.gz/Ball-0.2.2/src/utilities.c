...
void merge(double *vector, int *index, int *number, int start, int mid, int end) {
    ...
    double *left = malloc(left_size * sizeof(double)), *right = malloc(right_size * sizeof(double));
    int *left_index = malloc(left_size * sizeof(int)), *right_index = malloc(right_size * sizeof(int));
    #endif
    ...
    free(left_index); free(right_index);
    #endif
}
...
void quick_rank_max(const double *x, int *r, int n) {
    ...
    int i, j;
    for (j = 0; j < n; j++) { x_index[j] = j; }
    for (j = 0; j < n; j++) { x_cpy[j] = x[j]; }

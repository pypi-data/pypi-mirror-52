...
def out_t_gg_all(quyu,conp):
    arr=quyu.split("_")
    sql="""
    CREATE unlogged TABLE if not exists unlogged %s."t_gg_hawq" (
    "rowid" int8,
...
def out_t_gg_cdc(quyu,conp):
    ...
    sql="""
    into %s.t_gg_hawq(rowid ,guid ,   gg_name, fabu_time ,  href   , ggtype , jytype , diqu   , quyu ,   info  ,  page)
    with b as(%s)
        , b1 as (
...
def out_t_gg_all_zlsys(quyu,conp):
    ...
    sql="""
    insert into zlsys_yunnan_qujingshi.t_gg_hawq(rowid ,guid ,   gg_name, fabu_time ,  href   , ggtype , jytype , diqu   , quyu ,   info  ,  page)
    select row_number() over() as rowid
    ,guid    ,gg_name ,coalesce(href ,'暂无') as href   ,fabu_time   ,coalesce(ggtype,'其它') ggtype ,quyu    ,diqu    ,jytype  ,create_time ,info   , page

...
def out_t_gg_all_zljianzhu(quyu,conp):
    arr=quyu.split("_")
    sql="""
    CREATE unlogged TABLE if not exists  zljianzhu."t_gg_hawq" (
    "rowid" int8,
    "guid" text COLLATE "default",
    "gg_name" text COLLATE "default",
    "fabu_time" text COLLATE "default",
    ...
    db_command(sql=sql,dbtype="postgresql",conp=conp)
    sql="truncate table  zljianzhu.t_gg_hawq;"

    print(sql)
    db_command(sql=sql,dbtype="postgresql",conp=conp)

    sql="""
    insert into  zljianzhu.t_gg_hawq(rowid ,guid ,   gg_name, fabu_time ,  href   , ggtype , jytype , diqu   , quyu ,   info  ,  page)
        with a as (
        select   encode(digest(concat(zljianzhu.zljianzhu_name(xm_page),xm_href),'md5'),'hex') as guid
         , concat(zljianzhu.zljianzhu_name(xm_page) ,xm_code) as gg_name 
        ,zljianzhu.zljianzhu_time(xm_page) as fabu_time
        , xm_href as href 
        ,'中标公告'  as ggtype
        ,'其它类型'  as jytype 
        ,zljianzhu.zljianzhu_diqu(xm_page) as diqu
        ,'zljianzhu' as quyu 
        ,json_build_object('xm_code',xm_code,'xmxx_href',xmxx_href)::text  as info
        ,xm_page    as page 
        from zljianzhu.jianzhu_xm_detail_html where xm_page !='None' and length(xm_page)>10  )
...
def out_t_gg_cdc_zljianzhu(quyu,conp):
    ...
    insert into  zljianzhu.t_gg_hawq(rowid ,guid ,   gg_name, fabu_time ,  href   , ggtype , jytype , diqu   , quyu ,   info  ,  page)
        ...
        ,json_build_object('xm_code',xm_code,'xmxx_href',xmxx_href)::text  as info
        ,xm_page    as page 
        from jianzhu_xm_detail_html where xm_page !='None' and length(xm_page)>10 )

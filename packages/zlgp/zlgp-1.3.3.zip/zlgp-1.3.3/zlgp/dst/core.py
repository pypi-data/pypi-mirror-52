...
def update_gg_meta_prt4(quyu,conp):
    sql="""
    insert into dst.gg_meta_1_prt_zljianzhu(html_key   ,guid   ,gg_name,   href    ,fabu_time, ggtype  ,jytype,    diqu    ,quyu,  info
    ,   create_time ,xzqh   ,ts_title   
    ,bd_key ,person ,price  ,zhongbiaoren   ,zhaobiaoren    ,zbdl   ,zhongbiaojia   ,kzj    ,xmmc   ,xmjl   ,xmjl_zsbh  ,xmdz   
    ,zbfs   ,xmbh   ,mine_info)
    with a as (select html_key, guid,   gg_name,    href,   fabu_time,  ggtype, jytype, diqu,   quyu,   info,   create_time 
    ,dst.parse_diqu_code(quyu,page) as xzqh,dst.title2ts(gg_name) as ts_title
    from src.t_gg as t1  where quyu='zljianzhu' 

    and not exists(select 1 from dst.gg_meta_1_prt_zljianzhu  as t2 where t2.html_key=t1.html_key )
    )
    ...
    ,algo.get_js_v(minfo,'xmbh') as xmbh
     ...
     from algo.m_gg where quyu='zljianzhu' )
    ,d as (
    select a.*
    ,t3.bd_key  
    ,coalesce(zhongbiaoren,zhaobiaoren,zbdl) person 
    ...
    ,xmbh   
    ,null::text  mine_info
     from a  left join bid.t_bd_gg  as t3 on a.html_key=t3.html_key and t3.quyu='zljianzhu' left join c  on a.html_key=c.html_key )
    select * from d  
         """

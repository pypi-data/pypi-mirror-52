...
class SystemResourceUsageTelemetry:
    ...
    def _get_psutil_mem_stats(self, debug_mode: bool = False) -> Tuple[Any, Any]:
        if debug_mode:
            try:
                import psutil
...
class _NonWindowsSystemResourceUsageTelemetry(SystemResourceUsageTelemetry):
    ...
    def _get_usage(self, prefix_message=None):
        ...
        try:
            ...
            if psutil_parent_mem is not None:
                self._log_memory_usage(psutil_parent_mem, prefix_message)
                self._log_memory_usage(psutil_child_mem, '{}child '.format(prefix_message))
            else:

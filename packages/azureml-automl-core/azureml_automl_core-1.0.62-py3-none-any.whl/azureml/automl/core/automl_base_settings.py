...
class AutoMLBaseSettings:
    ...
    def __init__(self,
                 ...
                 use_incremental_learning: Optional[bool] = None,
                 **kwargs: Any):
        """
        ...
        :param use_incremental_learning: Flag to enable/disable incremental learning, through data streaming.
        :param kwargs:
        """
        self.path = path
        ...
        if is_timeseries:
            ...
            if not isinstance(self.max_horizon, int):
                raise ConfigException.create_without_pii(
                    "{} must be an integer value.".format(constants.TimeSeries.MAX_HORIZON),
                    target=constants.TimeSeries.MAX_HORIZON)
            self.dropna = False
            ...
            if isinstance(lags, int):
                target_lags = [lags]  # type: Optional[List[int]]
            else:
                # Get unique values.
                if lags is None:
                    target_lags = None
                ...
                for lag in target_lags:
                    ...
                    if lag < 1 or lag > AutoMLBaseSettings.MAX_LAG_LENGTH:
                        raise ValueError("The {} must be between 1 and {} inclusive."
                                         .format(constants.TimeSeries.TARGET_LAGS, AutoMLBaseSettings.MAX_LAG_LENGTH))
                self.lags =\
                    {constants.TimeSeriesInternal.DUMMY_TARGET_COLUMN:
                     target_lags}  # type: Optional[Dict[str, List[int]]]
            else:
                self.lags = None
        ...
        if not enable_cache:
            logging.warning("Parameter 'enable_cache' will be deprecated. Azure blob store / local disk based "
        ...
        self.use_incremental_learning = False if use_incremental_learning is None else use_incremental_learning

        self.label_column_name = kwargs.get('label_column_name', None) if self.use_incremental_learning else None
        self.weight_column_name = kwargs.get('weight_column_name', None) if self.use_incremental_learning else None
    ...
    def _verify_settings(self):
        ...
        if self.is_timeseries:
            if self.preprocess:
                logging.warning('Timeseries use its own preprocess, disable preprocess to run, '
                                'ignoring preprocess parameter.')

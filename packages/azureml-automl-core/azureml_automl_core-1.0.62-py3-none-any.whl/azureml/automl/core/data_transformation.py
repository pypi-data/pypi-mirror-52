...
def transform_data_streaming(raw_data_context: RawDataContext,
    ...
    if preprocess:
        ...
        # workaround for the variable name is assigned but never used.
        # this code is going to go away soon so not going to change this.
        X.head(1)
        X_valid.head(1)

    return StreamingTransformedDataContext(
        x_raw_column_names=raw_data_context.x_raw_column_names,
    ...
    try:
        if isinstance(data, pd.DataFrame):
            if is_forecasting:
                ...
                df_str = _get_data_snapshot_helper(data[:1])

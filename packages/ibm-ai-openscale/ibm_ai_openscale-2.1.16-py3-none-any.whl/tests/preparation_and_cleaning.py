...
def get_performance_timeout():
    if "E2E_SANITY" in get_env():
        json.loads(config.get(suite_config, 'performance_timeout')) * 6
...
def wait_for_payload_table(subscription, payload_records):
    print("Waiting {} seconds for payload records...".format(get_payload_timeout()))
...
def wait_for_monitor_instance(run_url, headers, waiting_timeout=15):
    ...
    status = get_run_details(run_url, headers)['entity']['status']['state']
    ...
    if elapsed_time >= waiting_timeout:
        print("Waiting for correlation calculation more than {} seconds - TIMEOUT".format(elapsed_time))

    return get_run_details(run_url, headers)
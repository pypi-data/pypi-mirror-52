...
class TestAIOpenScaleClient(unittest.TestCase):
    ...
    data_df = pd.read_csv(
        "./datasets/diabetes/diabetes_train.csv",
        dtype={'x1': float, 'x2': float, 'x3': float, 'x4': float,
               'x5': float, 'x6': float, 'x7': float, 'x8': float, 'x9': float, 'x10': float})
    ...
    def score(self, subscription_details):
        ...
        data = {"input": [{'x1': 0.0380759064334241,
                           'x2': 0.0506801187398187,
                           'x3': 0.0616962065186885,
                           'x4': 0.0218723549949558,
                           'x5': -0.0442234984244464,
                           'x6': -0.0348207628376986,
                           'x7': -0.0434008456520269,
                           'x8': -0.00259226199818282,
                           'x9': 0.0199084208763183,
                           'x10': -0.0176461251598052},
                          {'x1': -0.0018820165277910398,
                           'x2': -0.044641636506989005,
                           'x3': -0.0514740612388061,
                           'x4': -0.0263278347173518,
                           'x5': -0.00844872411121698,
                           'x6': -0.019163339748222,
                           'x7': 0.0744115640787594,
                           'x8': -0.0394933828740919,
                           'x9': -0.0683297436244215,
                           'x10': -0.09220404962683}]}
    ...
    def test_17_send_feedback_data(self):
        ...
        feedback_data = pd.read_csv('datasets/diabetes/diabetes_train.csv', sep=",")
    ...
    def test_19_feedback_logging_data_distribution(self):
        ...
        self.assertIn("x1", data_columns)
        self.assertIn("x2:avg", data_columns)
        self.assertIn("x3:avg", data_columns)
    ...
    def test_29_setup_fairness_monitoring(self):
        ...
        TestAIOpenScaleClient.subscription.fairness_monitoring.enable(
            ...
            features=[
                Feature("x9", majority=[[0.12,0.3]], minority=[[0.003,0.1]], threshold=0.95)
            ],
            deployment_uid=None,
            favourable_classes=[[50,150]],
            unfavourable_classes=[[151,310]],
            min_records=5
        )
    ...
    def test_34_disable_payload_logging_and_performance_monitoring(self):
        self.subscription.payload_logging.disable()
        self.subscription.performance_monitoring.disable()
        self.subscription.quality_monitoring.disable()

    def test_34b_disable_explainability_and_fairness_monitoring(self):
        skip_on_sanity_run()
    ...
    def test_35_unsubscribe(self):
        TestAIOpenScaleClient.ai_client.data_mart.subscriptions.delete(TestAIOpenScaleClient.subscription.uid)

    def test_36_unbind(self):

...
class Qiwi:
    ...
    def pay_bill(self, uid: int, currency: Currency = Currency.RUB):

        url = self._host + 'checkout/invoice/pay/wallet'
    ...
    def get_payments(self, rows: int = 25, operation: OperationType = OperationType.ALL):
        ...
        if 'error' in data:
            ...
            return [Payment(i) for i in data['data']]

    def get_bills(self, rows: int = 25):

        url = self._host + 'checkout/api/bill/search'
        ...
        if 'error' in data:
            ...
            return [Bill(i) for i in data['bills']]
    ...
    def on_bill_func(self, interval: int = 3, amount: int = 3):

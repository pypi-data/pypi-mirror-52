...
class Session:
    ...
    _default: dict = {
        ...
        'default': {
            ...
            'filetype': {
                ...
                'script': 'py'},
            'path': {
                'baseconf': ('%user_config_dir%', 'rian.ini'),
                ...
                'logfile':
                    ('%basepath%', '%workspace%', 'rian_old.log')}},
        'register': {
            'dataset': {},
            'model': {},
            'network': {},
    ...
    def create(self, key: str, *args, **kwds):
        ...
        if key == 'model':
            from rian import model
            return model.create(*args, **kwds)
        if key == 'network':
            from rian import network
            return network.create(*args, **kwds)
        if key == 'dataset':
            from rian import dataset
            return dataset.create(*args, **kwds)
        if key == 'system':
            from rian import system
    ...
    def _get_about(self, key='about', *args, **kwds):
        """Get rian meta information."""

        if key == 'about':
            return rian.__description__
        if key == 'version':
            return rian.__version__
        if key == 'status':
            return rian.__status__
        if key == 'description':
            return rian.__doc__.strip()
        if key == 'url':
            return rian.__url__
        if key == 'license':
            return rian.__license__
        if key == 'copyright':
            return rian.__copyright__
        if key == 'author':
            return rian.__author__
        if key == 'email':
            return rian.__email__
        if key == 'maintainer':
            return rian.__maintainer__
        if key == 'credits':
            return rian.__credits__
    ...
    def open(self, key=None, *args, **kwds):
        ...
        if len(args) == 1:
            ...
            if key == 'model':
                return rian.model.open(args[0], **kwds)
            if key == 'dataset':
                return rian.dataset.open(args[0], **kwds)
            if key == 'network':
                return rian.network.open(args[0], **kwds)
            if key == 'system':
                return rian.system.open(args[0], **kwds)

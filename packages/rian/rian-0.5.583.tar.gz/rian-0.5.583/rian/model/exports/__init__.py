...
import rian.model.exports.archive
import rian.model.exports.image
...
def filetypes(filetype = None):
    ...
    archive_types = rian.model.exports.archive.filetypes()
    for key, val in list(archive_types.items()):
        type_dict[key] = ('archive', val)

    # get supported image filetypes
    image_types = rian.model.exports.image.filetypes()
...
def save(model, path = None, filetype = None, workspace = None,
    ...
    Args:
        model (object): rian model instance
    ...
    if isinstance(workspace, str) and not workspace == 'None':
        directory = rian.path('models', workspace = workspace, base = base)
    ...
    if module_name == 'archive':
        return rian.model.exports.archive.save(
            model, path, filetype, **kwds)
    if module_name == 'image':
        return rian.model.exports.image.save(
            model, path, filetype, **kwds)
...
def show(model, *args, **kwds):
    """ """
    return rian.model.exports.image.show(model, *args, **kwds)

...
class ANN(rian.system.classes.base.System):
    """Artificial Neuronal Network (ANN).
    ...
    def _configure_test_units(self, params):
        ...
        for layer_id in range(len(params['units'])):
            ...
            if layer['class'] == 'gauss' \
                and not rian.system.commons.units.Gauss.check(layer):
                return False
            elif layer['class'] == 'sigmoid' \
                and not rian.system.commons.units.Sigmoid.check(layer):
                return False
    ...
    def _remove_units(self, layer = None, label = []):
        ...
        if layer['class'] == 'gauss':
            rian.system.commons.units.Gauss.remove(layer, select)
        elif layer['class'] == 'sigmoid':
            rian.system.commons.units.Sigmoid.remove(layer, select)
    ...
    def _get_links_energy(self, data, mapping = None, **kwds):
        ...
        if (sid, tid) in self._params['links']:
            ...
            return rian.system.commons.links.Links.energy(

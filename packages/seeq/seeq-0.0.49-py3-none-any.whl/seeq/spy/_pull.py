...
def pull(items, *, start=None, end=None, grid='15min', header='__auto__', capsules_as='auto', tz_convert=None,
         calculation=None, bounding_values=False, return_status_df=False, errors='raise', quiet=False):
    """
    ...
    :param return_status_df: If True, the return value will be a (data_df, status_df) tuple where data_df is the
    normal DataFrame of results and status_df is a copy of the items parameter with extra information about the pull
    operations, including a 'Result' column with error messages when errors='catalog'.
    :type return_status_df: bool
    :param errors: If 'raise' (default), any errors encountered will cause an exception. If 'catalog', errors will be
    added to a 'Result' column in the returned status_df DataFrame (errors='catalog' must be combined with
    ...
    if errors == 'catalog' and not return_status_df:
        raise RuntimeError("errors='catalog' must be combined with return_status_df=True")
            ...
            for index, row in query_df.iterrows():
                ...
                try:
                    output_df, at_least_one_signal = _process_query_row(
                        ...
                        status_df, status_updates, threads, tz_convert, bounding_values)

                except BaseException:
                    if errors == 'raise':
                        raise

                    status_df.at[index, 'Result'] = _common.format_exception()
    ...
    return (output_df, status_df) if return_status_df else output_df
...
def _process_query_row(_drain_status_updates, at_least_one_signal, calculation, capsules_as, column_names,
                       ...
                       phase, pulled_series, row, status_df, status_updates, threads, tz_convert, bounding_values):
...
def _pull_condition(capsules_as, formulas_api, formula, parameters, item_id, item_name, output_df, pd_start, pd_end,
    ...
    try:
        ...
        status_df.at[status_index, 'Result'] = _common.format_exception()

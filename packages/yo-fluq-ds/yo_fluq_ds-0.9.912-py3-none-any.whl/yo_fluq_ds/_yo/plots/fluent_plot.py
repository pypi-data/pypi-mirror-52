...
class FluentPlot:
    ...
    def _make_final_call(self, item, ax):
        if self._method is not None:
            self._method(item, ax, *self._make_args(item), **self._make_kwargs(item))
        elif self._direct_method is not None:
            self._direct_method(item,ax)
        else:
            raise ValueError('No call method is formed. Check for .call, .call_static, .call_obj or .call_ax calls')


    def _draw(self, obj, ax):
        if self._iterate:
            for item in obj:
                self._make_final_call(item, ax)
    ...
    def call_static(self, method, ax_argument_name='ax') -> 'FluentPlot':
        self._method = _method_wrap(method, ax_argument_name)
        return self

    def call(self, method: Callable) -> 'FluentPlot':

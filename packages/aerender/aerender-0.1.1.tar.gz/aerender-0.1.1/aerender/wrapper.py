...
class AERenderWrapper:
    ...
    async def run(
            self, reuse: bool = None, project_path: str = None, project_name: str = None,
            comp_name: str = None, index_in_render_queue: str = None,
            render_settings_template: str = None, output_module_template: str = None,
            output_path: str = None, logfile_path: str = None, start_frame: int = None,
        ...
        Args:
            ...
            project_path: a file path or URI specifying a project file to open. If none is
                provided, aerender will work with the currently open project. If no project
                is open and no project is provided, an error will result.
            ...
            comp_name: specifies a comp to be rendered. If the comp is in the render queue
                already, and in a queueable state, then (only) the first queueable instance
                of that comp on the render queue will be rendered. If the comp is in the
                project but not in the render queue, then it will be added to the render
                queue and rendered. If no -comp argument is provided, aerender will render
                the entire render queue as is. In this case (no -comp), the only other
                arguments used will be -project, -log, -v, -mem_usage, and -close;
                the -RStemplate, -OMtemplate, -output, -s, -e, and -i arguments will be
                ignored.
            index_in_render_queue: specifies a render queue item to be rendered. Options
                that make sense when rendering a single render queue item are available like
                with the -comp flag.
            ...
            end_frame: the last frame to render. Note, this is "inclusive;" the final frame
                will be rendered. Default is the end frame in the file
        ...
        command = await self.get_command(
                aerender_abs_path=self.exe_path, reuse=reuse,
    ...
    async def get_command(
            aerender_abs_path: str, *, reuse: bool = None, project_path: str = None,
            project_name: str = None, comp_name: str = None,
            index_in_render_queue: str = None, render_settings_template: str = None,
            output_module_template: str = None, output_path: str = None,
            logfile_path: str = None, start_frame: int = None, end_frame: int = None,

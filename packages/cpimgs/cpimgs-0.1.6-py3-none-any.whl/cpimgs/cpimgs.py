...
def getPlateWellByCpdName(name):
    ...
    def checkDistance(cpd_name):
        distance_array = np.array(list(map(lambda x: edit_distance(x, cpd_name), df_name2id.index.dropna())))
        index = distance_array.argsort()[:5]
        return df_name2id.index[index]
    ...
    if not name.startswith("BRD"):
        ...
        #         print(brd_id)
            ...
            suggested_words = checkDistance(name)
            raise KeyError(f"{name} not found.\n"
                           f"Do you mean any of the words below:\n"
                           f"{list(suggested_words)}")
    else:
        brd_id = name

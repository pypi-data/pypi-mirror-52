...
def import_auth():
 try:
  ...
  raise NzeTx('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=NzeTk)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=NzeTV)
def get_params(args,names=[],mandatory=NzeTk):
 result=()
 for name in names:
  value=NzeTF
  if name in args:
  ...
  if not value and mandatory:
   raise NzeTx('Please provide %s=... parameter'%name)
  result+=(value)
 if NzeTd(result)==1:
  return result[0]
 return result

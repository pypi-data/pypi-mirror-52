...
def import_auth():
 try:
  ...
  raise XTqSG('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=XTqSA)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=XTqSd)
def get_params(args,names=[],mandatory=XTqSA):
 result=()
 for name in names:
  value=XTqSF
  if name in args:
  ...
  if not value and mandatory:
   raise XTqSG('Please provide %s=... parameter'%name)
  result+=(value)
 if XTqSJ(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise frsoi('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=frsow)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=frsoj)
def get_params(args,names=[],mandatory=frsow):
 result=()
 for name in names:
  value=frsoB
  if name in args:
  ...
  if not value and mandatory:
   raise frsoi('Please provide %s=... parameter'%name)
  result+=(value)
 if frsoI(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise KRaNo('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=KRaNC)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=KRaNB)
def get_params(args,names=[],mandatory=KRaNC):
 result=()
 for name in names:
  value=KRaNn
  if name in args:
  ...
  if not value and mandatory:
   raise KRaNo('Please provide %s=... parameter'%name)
  result+=(value)
 if KRaNp(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise iuaXI('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=iuaXW)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=iuaXh)
def get_params(args,names=[],mandatory=iuaXW):
 result=()
 for name in names:
  value=iuaXf
  if name in args:
  ...
  if not value and mandatory:
   raise iuaXI('Please provide %s=... parameter'%name)
  result+=(value)
 if iuaXk(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise LtRoH('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=LtRoI)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=LtRoT)
def get_params(args,names=[],mandatory=LtRoI):
 result=()
 for name in names:
  value=LtRoh
  if name in args:
  ...
  if not value and mandatory:
   raise LtRoH('Please provide %s=... parameter'%name)
  result+=(value)
 if LtRoC(result)==1:
  return result[0]
 return result

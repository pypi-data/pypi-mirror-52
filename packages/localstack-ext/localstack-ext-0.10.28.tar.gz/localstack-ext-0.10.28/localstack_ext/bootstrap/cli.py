...
def import_auth():
 try:
  ...
  raise tJKIf('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=tJKIl)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=tJKIQ)
def get_params(args,names=[],mandatory=tJKIl):
 result=()
 for name in names:
  value=tJKIz
  if name in args:
  ...
  if not value and mandatory:
   raise tJKIf('Please provide %s=... parameter'%name)
  result+=(value)
 if tJKIy(result)==1:
  return result[0]
 return result

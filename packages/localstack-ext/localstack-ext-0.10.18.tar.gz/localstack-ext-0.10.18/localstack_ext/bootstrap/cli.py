...
def import_auth():
 try:
  ...
  raise sBcxu('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=sBcxj)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=sBcxl)
def get_params(args,names=[],mandatory=sBcxj):
 result=()
 for name in names:
  value=sBcxT
  if name in args:
  ...
  if not value and mandatory:
   raise sBcxu('Please provide %s=... parameter'%name)
  result+=(value)
 if sBcxw(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise mznId('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=mznIt)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=mznIq)
def get_params(args,names=[],mandatory=mznIt):
 result=()
 for name in names:
  value=mznIQ
  if name in args:
  ...
  if not value and mandatory:
   raise mznId('Please provide %s=... parameter'%name)
  result+=(value)
 if mznIE(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise MqtIX('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=MqtIv)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=MqtIe)
def get_params(args,names=[],mandatory=MqtIv):
 result=()
 for name in names:
  value=MqtIB
  if name in args:
  ...
  if not value and mandatory:
   raise MqtIX('Please provide %s=... parameter'%name)
  result+=(value)
 if MqtId(result)==1:
  return result[0]
 return result

...
def import_auth():
 try:
  ...
  raise cWxHa('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=cWxHP)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=cWxHt)
def get_params(args,names=[],mandatory=cWxHP):
 result=()
 for name in names:
  value=cWxHy
  if name in args:
  ...
  if not value and mandatory:
   raise cWxHa('Please provide %s=... parameter'%name)
  result+=(value)
 if cWxHT(result)==1:
  return result[0]
 return result

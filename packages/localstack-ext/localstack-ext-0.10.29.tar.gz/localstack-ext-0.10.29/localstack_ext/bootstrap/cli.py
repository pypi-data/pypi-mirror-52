...
def import_auth():
 try:
  ...
  raise oqIjx('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=oqIjd)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=oqIjC)
def get_params(args,names=[],mandatory=oqIjd):
 result=()
 for name in names:
  value=oqIje
  if name in args:
  ...
  if not value and mandatory:
   raise oqIjx('Please provide %s=... parameter'%name)
  result+=(value)
 if oqIjB(result)==1:
  return result[0]
 return result

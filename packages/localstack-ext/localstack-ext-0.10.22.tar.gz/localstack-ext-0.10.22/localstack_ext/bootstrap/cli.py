...
def import_auth():
 try:
  ...
  raise BYqPn('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=BYqPc)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=BYqPx)
def get_params(args,names=[],mandatory=BYqPc):
 result=()
 for name in names:
  value=BYqPi
  if name in args:
  ...
  if not value and mandatory:
   raise BYqPn('Please provide %s=... parameter'%name)
  result+=(value)
 if BYqPv(result)==1:
  return result[0]
 return result

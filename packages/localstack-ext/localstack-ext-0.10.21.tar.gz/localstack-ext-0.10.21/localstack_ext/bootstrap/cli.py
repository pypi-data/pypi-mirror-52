...
def import_auth():
 try:
  ...
  raise bkjlF('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=bkjlN)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=bkjlQ)
def get_params(args,names=[],mandatory=bkjlN):
 result=()
 for name in names:
  value=bkjlV
  if name in args:
  ...
  if not value and mandatory:
   raise bkjlF('Please provide %s=... parameter'%name)
  result+=(value)
 if bkjlr(result)==1:
  return result[0]
 return result

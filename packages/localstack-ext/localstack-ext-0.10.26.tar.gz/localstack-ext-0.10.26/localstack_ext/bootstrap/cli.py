...
def import_auth():
 try:
  ...
  raise RgsPp('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=RgsPb)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=RgsPD)
def get_params(args,names=[],mandatory=RgsPb):
 result=()
 for name in names:
  value=RgsPd
  if name in args:
  ...
  if not value and mandatory:
   raise RgsPp('Please provide %s=... parameter'%name)
  result+=(value)
 if RgsPU(result)==1:
  return result[0]
 return result

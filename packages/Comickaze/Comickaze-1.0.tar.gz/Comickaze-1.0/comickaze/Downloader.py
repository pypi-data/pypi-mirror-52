...
class Downloader:
    def __init__(self, comic, dl_range, session=None, dl_path=config.DEFAULT_DL_PATH):
        self.COMIC = comic
        self.DL_PATH = path.normpath(dl_path) + path.sep + self.cleanPath(self.COMIC["title"])
    ...
    def downloadPage(self, dl_path, page, pbar):
        ...
        fileDir = path.normpath(dl_path + path.sep + fileName)
    ...
    def startDownload(self):
        ...
        for chapter in self.TO_DL:
            ...
            for page in pages:
                downloadFolder = self.DL_PATH + path.sep + self.cleanPath(chapterTitle)

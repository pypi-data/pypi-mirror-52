...
class DaxServiceError(BotoClientError):
    def __init__(self, operation_name, message, dax_codes, request_id, ddb_code, http_status, cancellation_reasons=[]):
    ...
    def from_no_more_data_exception(cls, operation_name, exc):
        ...
        Necessary to avoid cluttering up the DaxServiceException ctor, which
        assumes the presence of error codes from DAX.
        '''

        dse = DaxServiceError(operation_name, "remote socket is closed", (), None, None, None)
        dse.__cause__ = exc
        dse.wait_for_recovery_before_retry = True
        dse._tube_invalid = True # not used
        return dse

...
class Subject:
	...
	def stimulusDictInitialisation(self, stimuli_names, columns, json_file, sensors, database, reading_method):
		...
		try:
			...
			aoi_dict = dict(zip(csv_df["Stim_Key"], csv_df["Stim_presented_rect_CoOr"]))
		...
		for category in stimuli_names:
			...
			for stimulus_name in stimuli_names[category]:
				...
				if aoi_dict != None and stimulus_name[:-2] in aoi_dict:
					aoi_coords = str(aoi_dict[stimulus_name[:-2]])
					aoi_coords = aoi_coords.strip('][').split(',')
					aoi_coords = [float(x) for x in aoi_coords]
				else:
					aoi_coords = self.aoi

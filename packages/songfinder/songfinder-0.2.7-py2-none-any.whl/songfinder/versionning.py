...
class AddRepo(object): # TODO integrate to repo class
	def __init__(self, paths, exe, screens=None, updateData=None, **kwargs):
		...
		with guiHelper.SmoothWindowCreation(fenetre, screens=screens):
			...
			self._pathEntryFrame = inputFrame.entryField(fenetre, packing=tk.LEFT, text='Chemin', width=60)
			self._userEntryFrame = inputFrame.entryField(fenetre, packing=tk.LEFT, text='Utilisateur', width=30)
			self._passwordEntryFrame = inputFrame.entryField(fenetre, packing=tk.LEFT, text='Mot de passe', width=30)
...
"""%(repo, prefix, user, mdp, name)
	...
	def update_fen(self, message):
		...
		self.fenetre.update()
		self.fenetre.geometry("%dx%d"%(self.fenetre.winfo_reqwidth(), self.fenetre.winfo_reqheight()))
...
class Repo(object):
	...
	def __getRemotes(self):
		...
		if self.__remotes == []:
			return self.__showError('La connection a échoué. Verifiez votre connexion à internet.\n'
								'Aucune clé USB n\'as été trouvé')
		return 0
	...
	def send(self, what='all', showGui=True):
		...
		if out.find('nothing changed') != -1 or out.find('aucun changement') != -1 or out.find('nothing to commit') != -1:
			return self.__showInfo('Rien à envoyer') or receiveReturn
		elif code != 0:
			self.__update_fen("    échec\n")

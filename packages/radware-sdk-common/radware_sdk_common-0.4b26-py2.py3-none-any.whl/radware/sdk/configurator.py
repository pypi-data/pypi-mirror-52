...
from radware.sdk.beans_common import BeanUtils, READ_PROP, BaseBeanEnum
from radware.sdk.common import RadwareParametersStruct,  PasswordArgument, \
    get_annotation_class, is_annotation_type_list_incl_optional_lookup, is_annotation_type_list
...
class DeviceConfigurator(Configurator, ConfiguratorAPI):
    ...
    @abstractmethod
    def update(self, parameters: RadwareParametersStruct, dry_run=False, **kw) -> str:
        pass
    ...
    def _struct_load(self, parameters):
        if self._beans:
            ...
            annotations = type(parameters).__annotations__
    ...
    def _write_device_beans(self, parameters, dry_run=False):
        for bean_class, bean_keys in self._bean_map.items():
            if bean_keys['direct']:
                if bean_keys['struct'] == type(parameters):
...
class DeviceConfigurationManager(object):
    ...
    def _execute_write_function(self, configurator, func_name, parameters, dry_run=False, differential=False,
        ...
        def _get_diff():
            if get_diff:
                if dry_run:
                    if differential and self._get_config_function(configurator, func_name) == configurator.update:
                        return {self.DIFF_APPEND_KEY: self._get_dry_run_diff_append(dry_run_diff)}
                    else:
                        return dry_run_diff
                else:
        ...
        if differential:
            if self._get_config_function(configurator, func_name) == configurator.update:
                ...
                if diff_append:
                    new_parameters = type(parameters)()
                    try:
                        new_parameters.copy_parameters_required_fields(parameters)
                        new_parameters.set_attributes(**diff_append)
                    except TypeError:
                        raise ArgumentTypeError(RadwareParametersStruct, new_parameters)
                    result = self._execute_function(configurator, func_name, new_parameters, dry_run=dry_run, **kw)
                    diff = _get_diff()
                    return result, diff
                else:
    ...
    def _dry_run_delete_diff_normalization(self, configurator, diff):
        ...
        if result:
            ...
            if result.ignore_all_props:
                ...
                if result.ignore_prop_by_value:
                    self._remove_by_value(diff, result.ignore_prop_by_value)
                if result.non_removable_bean_map_attrs_list:
                    for item in result.non_removable_bean_map_attrs_list:
                        self.remove_non_removable_by_bean_map_attrs(diff, item, result.ignore_prop_by_value)

    def _dry_run_evaluation(self, configurator, func_name, parameters, differential):

        def _dry_run_update():
            updated_attrs = self._dry_run_result_normalization(parameters, before_config)
    ...
    def _dry_run_result_normalization(self, parameters, current_config):
        ...
        for k, v in attrs_dict.items():
            ...
            if type(v) == dict:
                dict_result.update({k: self._dry_run_result_normalization(v, cur_cfg_attr_val)})
            elif type(v) == list:
                ...
                if current_config is not None:
                    annotations = type(current_config).__annotations__
                ...
                if cur_cfg_attr_val is not None:
                    if isinstance(cur_cfg_attr_val, BaseBeanEnum):
                        dict_result.update({k: cur_cfg_attr_val.name})
                    else:
                        dict_result.update({k: cur_cfg_attr_val})
                else:
    ...
    @staticmethod
    def _dry_run_array_normalization(arr: list, cur_arr: list):
        new_arr = list()
        ...
        for item in cur_arr:
            if isinstance(item, RadwareParametersStruct):
                ...
                if item in arr and item not in new_arr:
                    new_arr.append(item)
    ...
    @staticmethod
    def remove_non_removable_by_bean_map_attrs(diff_dict, non_removable_bean_attrs, exclude_map):
        if not exclude_map:
            exclude_map = dict()
    ...
    @staticmethod
    def _remove_by_value(diff_dict, value_dict):

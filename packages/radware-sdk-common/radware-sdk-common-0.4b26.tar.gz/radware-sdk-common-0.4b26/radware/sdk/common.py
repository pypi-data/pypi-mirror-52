...
from typing_inspect import is_generic_type, get_origin, get_args, is_optional_type
...
class RadwareParametersStruct(object):
    __metaclass__ = ABCMeta
            ...
            if v is not None:
                if k in annotations:
                    ...
                    if list_mode:
                        ...
                        elif issubclass(annotation_class, BaseBeanEnum):
                            if type(v) != annotation_class:
        ...
        for item in arr:
            if isinstance(item, RadwareParametersStruct):
                if item == self:
                    ...
                    if partial_dict_match(item.translate_to_dict(), struct_dict):
                        _add_match_item(item)
                        return True
            elif type(item) == dict:
                if item == struct_dict:
                    _add_match_item(item)
                    return True
                if partial_dict_match(item, struct_dict):
                    _add_match_item(item)
                    return True
        return False

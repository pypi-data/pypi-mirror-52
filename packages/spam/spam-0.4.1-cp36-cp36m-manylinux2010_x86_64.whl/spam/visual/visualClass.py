...
class ereg(QWidget):
    def __init__(self, _images, enterPhi, binning, names):
        ...
        sliceIm1 = [int(_images[0].shape[0] / 2), int(_images[0].shape[1] / 2), int(_images[0].shape[2] / 2)]
        sliceIm2 = [int(_images[1].shape[0] / 2), int(_images[1].shape[1] / 2), int(_images[1].shape[2] / 2)]
        ...
        self.iterations = 0
        self.images[1] = transf.applyPhi(self.images[2], Phi=self.Phis[1])
        ...
        self.resetButton = QPushButton("TSV")
        ...
        blankLabel = QLabel()
        self.currentAxeLabel = QComboBox()
        self.currentAxeLabel.addItems(["z", "y", "x"])
        rightArrow = QLabel("→")
        self.rightAxe = QLabel("x")
        bottomArrow = QLabel("↓")
        self.bottomAxe = QLabel("y")
        self.currentAxeLabel.currentIndexChanged.connect(self.changeAxe)
        # viewer grid
        ...
        # self.zRadioButton.toggled.connect(partial(self.changeAxe, "z"))
        # self.yRadioButton.toggled.connect(partial(self.changeAxe, "y"))
        # self.xRadioButton.toggled.connect(partial(self.changeAxe, "x"))
        ...
        self.labelSlice1 = QLabel("{}: {}".format(self.axe, self.im1Slider.value()))
        self.labelSlice2 = QLabel("{}: {}".format(self.axe, self.im2Slider.value()))
        ...
        self.delta = QLabel(u"\u0394" + self.axe + ": " + str(d))
        self.equalizerButton = QPushButton(u"Apply \u0394{}={}".format(self.axe, d))
        ...
        self.nameEntry = QLineEdit("mmr-phi-bin{}.tsv".format(self.binning))
        ...
        self.bmGrid.addWidget(self.currentAxeLabel, 2, 0)
        self.bmGrid.addWidget(self.rightAxe, 2, 2)
        self.bmGrid.addWidget(self.bottomAxe, 4, 0)

        self.currentAxeLabel.setFixedWidth(50)
        self.currentAxeLabel.setFixedHeight(25)
        self.rightAxe.setFixedWidth(25)
        self.rightAxe.setFixedHeight(25)
        self.bottomAxe.setFixedWidth(25)
        self.bottomAxe.setFixedHeight(25)
    ...
    def slideIm1(self, value):
        if value < self.im1Slider.maximum() and value > self.im1Slider.minimum():
            ...
            self.labelSlice1.setText("{}: {}".format(self.axe, self.im1Slider.value()))
            ...
            if self.slaveBox.isChecked():
                ...
                self.labelSlice2.setText("{}: {}".format(self.axe, self.im2Slider.value()))
            self.showImages()  # call the function to display images
            # change the delta string
            # self.delta.setText("Delta " + self.axe + ": " + str(self.im2Slider.value() - self.im1Slider.value()))
            d = self.im2Slider.value() - self.im1Slider.value()
            self.delta.setText(u"\u0394" + self.axe + ": " + str(d))
            self.equalizerButton.setText(u"Apply \u0394{}={}".format(self.axe, d))
    ...
    def modifyPhi(self, mod, value):
        ...
        # phiPrev = transf.computePhi({'t': [self.tArray[0], self.tArray[1], self.tArray[2]],
        #                              'r': [self.rArray[0], self.rArray[1], self.rArray[2]],
        #                              'z': [self.zArray[0], self.zArray[1], self.zArray[2]]})
        phiRotPrev = transf.computePhi(
            {'r': [self.rArray[0], self.rArray[1], self.rArray[2]]})
        ...
        if deltaRot.sum() != 0:
            newRotPhi = transf.computePhi({'r': deltaRot})
            totalPhi = numpy.dot(newRotPhi, phiRotPrev)
            # totalPhi = numpy.dot(phiRotPrev, newRotPhi)
            decomposedPhi = transf.decomposePhi(totalPhi)
    ...
    #                                  'r': [self.rArray[0], self.rArray[1], self.rArray[2]],
    #                                  'z': [self.zArray[0], self.zArray[1], self.zArray[2]]})
    #     phiRotPrev = transf.computePhi({'r': [self.rArray[0], self.rArray[1], self.rArray[2]]})
    #     deltaRot = numpy.zeros(3, dtype='<f8')
    #
    #     if mod == "tZ":  # check which value to increase and then increase the good one to load the new image
    #         self.tArray[0] = (self.tArray[0] + value)
    ...
    #     transf.computePhi(transformation)
    #
    #     # modify phi that has to be applied then apply it
    #     self.Phis[1]   = transf.computePhi(transformation)
    #     self.images[1] = transf.applyPhi(self.images[2], Phi=self.Phis[1])
    ...
    def saveTSV(self):
        ...
        if self.nameEntry.text() != "":
            ...
            TSVheader = TSVheader = "F11\tF12\tF13\tZdisp\tF21\tF22\tF23\tYdisp\tF31\tF32\tF33\tXdisp\tbin"
            slices = numpy.array([[self.Phis[1][0, 0]], [self.Phis[1][0, 1]],
                                  [self.Phis[1][0, 2]], [self.Phis[1][0, 3]],
                                  [self.Phis[1][1, 0]], [self.Phis[1][1, 1]],
                                  [self.Phis[1][1, 2]], [self.Phis[1][1, 3]],
                                  [self.Phis[1][2, 0]], [self.Phis[1][2, 1]],
                                  [self.Phis[1][2, 2]], [self.Phis[1][2, 3]],
                                  [self.binning]])
            numpy.savetxt(fileName, slices.T, fmt='%.7f', delimiter='\t', newline='\n', comments='', header=TSVheader)
...
class QtCropWidget(QWidget):
    def __init__(self, images, phi, crop, binning, names):
        QWidget.__init__(self)
        ...
        self.nameEntry = QLineEdit("mmr-crop-bin{}.tsv".format(self.binning))
        self.resultLabel = QLabel()
    ...
    def saveTSVCrop(self):
        if self.nameEntry.text() != "":
            ...
            slices = numpy.array([[self.z[0]], [self.z[1]], [self.y[0]], [
                self.y[1]], [self.x[0]], [self.x[1]], [self.binning]])
...
class JHist(QWidget):
    def __init__(self, images, phi, crop, fontColor, names):
        import spam.helpers
        QWidget.__init__(self)
        self.images = images
        self.Phi = phi
        self.images[0] = transf.applyPhi(self.images[0], Phi=self.Phi)
        self.crop = crop
        ...
        self.distanceMinSliderLabelValue = QLabel(
...
class PhaseDiagram(QWidget):
    def __init__(self, gaussianParameters, bins, jointHistogram, names):
        ...
        self.selectDistanceType.currentIndexChanged.connect(
            self.changeDistanceType)
...
class FinalStep(QWidget):
    def __init__(self, images, crop, phaseDiagram, phi, GP, bins, greyLimitsCrop, binning, name):
        import spam.helpers
        QWidget.__init__(self)
        self.images = images
        self.crop = crop
        ...
        self.binning = binning
        self.rootFileName = "".join(name.split(".")[:-1])
        ...
        # scale to 0-255
        tmp = transf.applyPhi(self.images[0], Phi=self.phi)
        ...
        DICToolkit.computeGMresidualAndPhase(self.imagesCrop[0][margin], self.imagesCrop[1][margin], self.phaseDiagram, self.gaussianParameters, residualField, phaseField)
        ...
        self.viewerIm2.setImage(qimage2ndarray.array2qimage(self.imagesCrop[1][margin][middleSlice, :, :], normalize=True))
        self.viewerCheckerBoard.setImage(qimage2ndarray.array2qimage(cGM.checkerBoard(self.imagesCrop[0][margin][middleSlice, :, :], self.imagesCrop[1][margin][middleSlice, :, :]), normalize=True))
        ...
        im1Label = QLabel("Image 1 (def)")
        im2Label = QLabel("Image 2")
    ...
    def starCorrelation(self):
        ...
        if float(self.minPhiEntry.text()):
            self.minDeltaPhi = float(self.minPhiEntry.text())

        m = self.margin  # margin
        maxIterations = self.maxIterations
        minDeltaPhi = self.minDeltaPhi

        # set local variables (can be removed later, it's just for sake of clarity)
        # Here we deform imF There is no initial phi but imagesCrop[0] has been deformed previously from ereg
        imF = self.imagesCrop[0]
        imG = self.imagesCrop[1]
        BINS = self.BINS
        ...
        margin = (slice(m, imF.shape[0] - m),
                  slice(m, imF.shape[1] - m),
                  slice(m, imF.shape[2] - m))
        ...
        p, _, _ = numpy.histogram2d(imF[margin].ravel(), imG[margin].ravel(), bins=BINS, range=[[0.0, BINS], [0.0, BINS]], normed=False, weights=None)
        LL = numpy.sum(numpy.log(p[numpy.where(p > 0)]))

        print("[finalStep.startCorrelation] Initial state        LL = {:0.2f}".format(LL))

        while (iterationNumber <= maxIterations and deltaPhiNorm > minDeltaPhi) or iterationNumber <= 1:
            # previousLL = LL  # need if divergence criterion

            # compute DIC operators
            M = numpy.zeros((12, 12), dtype='<f8')
            A = numpy.zeros((12), dtype='<f8')
            imFdefGradZ, imFdefGradY, imFdefGradX = [g.astype('<f4') for g in numpy.gradient(imFdef)]
            DICToolkit.computeDICoperatorsGM(imFdef[margin].astype("<f4"),
                                             imG[margin].astype("<f4"),
                                             imFdefGradZ[margin].astype("<f4"),
                                             imFdefGradY[margin].astype("<f4"),
                                             imFdefGradX[margin].astype("<f4"),
                                             phaseDiagram.astype("<u1"), gaussianParameters.astype("<f8"), M, A)
            ...
            # add deltaPhi to phiCrop and compute current transformation
            ...
            phiCrop = numpy.dot((numpy.eye(4) - deltaPhi), phiCrop)
            currentTransformation = transf.decomposePhi(phiCrop)

            # transform imF
            imFdef = transf.applyPhi(imF, Phi=phiCrop, interpolationOrder=1)

            # compute residuals
            residualField = numpy.zeros_like(imG[margin], dtype="<f4")
            phaseField = numpy.zeros_like(imG[margin], dtype="<u1")
            DICToolkit.computeGMresidualAndPhase(imFdef[margin].astype("<f4"),
                                                 imG[margin].astype("<f4"),
            ...
            p, _, _ = numpy.histogram2d(imFdef[margin].ravel(), imG[margin].ravel(), bins=BINS, range=[[0.0, BINS], [0.0, BINS]], normed=False, weights=None)
            LL = numpy.sum(numpy.log(p[numpy.where(p > 0)]))
            print("[finalStep.startCorrelation] Iteration Number {:03d} ".format(iterationNumber), end="")
            ...
            print("Zo = {: .3f}, {: .3f}, {: .3f}".format(phiCrop[0, 0], phiCrop[1, 1], phiCrop[2, 2]))

            # plot
            self.informationLabel.setText("Gaussian Mixture iteration number {} "
                                          "|deltaPhi|={:.5f} \tT = [{: 2.4f} {: 2.4f} {:.4f}]\t"
                                          "R = [{: 2.4f} {: 2.4f} {: 2.4f}]\t"
                                          "Z = [{: 2.4f} {: 2.4f} {: 2.4f}]".format(iterationNumber, deltaPhiNorm,
                                                                                    ...
                                                                                    phiCrop[0, 0], phiCrop[1, 1], phiCrop[2, 2]))
            middleSlice = imF[margin].shape[0] // 2
            ...
            self.viewerIm1.setImage(qimage2ndarray.array2qimage(imFdef[margin][middleSlice, :, :], normalize=True))
            self.viewerIm2.setImage(qimage2ndarray.array2qimage(imG[margin][middleSlice, :, :], normalize=True))
            self.viewerCheckerBoard.setImage(qimage2ndarray.array2qimage(cGM.checkerBoard(imFdef[margin][middleSlice, :, :], imG[margin][middleSlice, :, :]), normalize=True))
            #
            # tmp1 = transf.applyPhi(self.images[0], Phi=self.phi)
            # # tmp1 = transf.applyPhi(self.images[0], Phi=self.phi)
            # tmp2 = self.images[1]
        ...
        print("[finalStep.startCorrelation] End of the loop after {}/{} iterations: ".format(iterationNumber, maxIterations), end="")
        print("dPhi = {:0.4f} (criterion at {}) ".format(deltaPhiNorm, minDeltaPhi))

        # compute full phi
        # get centers
        cropCentre = [0.5 * (self.crop[i].start + self.crop[i].stop - 1) for i in range(3)]
        imCentre = [0.5 * (self.images[0].shape[i] - 1) for i in range(3)]
        # define phiCrop at the centre of the full image (change the translation part)
        phiCropInitialCentre = phiCrop.copy()
        phiCropInitialCentre[:3, -1] = transf.decomposePhi(phiCrop, PhiCentre=imCentre, PhiPoint=cropCentre).get('t', [0., 0., 0.])
        # compute the total Phi = phiEreg + phiCrop
        phiTotal = numpy.dot(phiCropInitialCentre, self.phi)

        print("[finalStep.startCorrelation] Total transformation (eye registration + lucas and kanade on the crop): ")
        for k, v in transf.decomposePhi(phiTotal).items():
        ...
        im1Def = transf.applyPhi(self.images[0], Phi=phiTotal)
        ...
        self.result = {'transformation': transf.decomposePhi(phiTotal),
                       'Phi': phiTotal,
                       # 'PhiCentre': PhiCentre,
    ...
    def saveFinalResults(self):
        if self.nameEntry.text() != "":
            ...
            # transformationInitialGuess = transf.decomposePhi(self.phi)
            ...
            Phi = self.result['Phi']
            TSVheader = TSVheader = "F11\tF12\tF13\tZdisp\tF21\tF22\tF23\tYdisp\tF31\tF32\tF33\tXdisp\tbin"
            slices = numpy.array([[Phi[0, 0]], [Phi[0, 1]], [Phi[0, 2]],

...
from zlairflow.dag.data.data_common_cdc import get_para
...
def write_dags_a(dirname, loc="aliyun",pool="abc_a1", data_cdc_mode="data_cdc",**krg):
    for w in get_para(DB_A,data_cdc_mode):
...
def write_dags_b(dirname, loc="aliyun", pool="abc_a2",data_cdc_mode="data_cdc",**krg):
    for w in get_para(DB_B,data_cdc_mode):
...
def write_dags_c(dirname, loc="aliyun",pool="abc_a3",data_cdc_mode="data_cdc", **krg):
    for w in get_para(DB_C,data_cdc_mode):
...
def write_dags_common(dirname, loc="aliyun",pool="abc_a4",data_cdc_mode="data_cdc",**krg):
    for w in get_para(DB_D,data_cdc_mode):

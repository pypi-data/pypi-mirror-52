...
class Config(ABC):
    """
    Base class which standardizes configuration for PyTorch modules. For a
    specific configuration subclass, the configuration values are automatically
    deduced from the class's annotations. All annotations which do not define
    a default value are considered required.
    """
    ...
    def _get_fields(cls):
        """
        Aggregates all available fields and their types from the class itself
        and its superclass (assuming it is a configuration as well).
    ...
    def __init__(self, **kwargs):
        ...
        # 5) Finally, set parameters on self
        for k, v in params.items():
            setattr(self, k, v)
    ...
    def is_valid(self):
        ...
        overridden if required. The default implementation implies validity for
        all inputs matching the specified types.
...
class Configurable(ABC):
    ...
    to save and load the module in a consistent way. The binding to the
    configuration class is achieved by setting the __
    ...
    def __init__(self, *args, **kwargs):
        """
        Initializes the configurable model. You can either pass an instance of
        the model's configuration class or the keyword arguments which you
        would normally pass to initialize the configuration instance. The
        configuration is then created implicitly.

        Parameters:
        -----------
        - args: variadic arguments
            Either zero or one parameters. If one is given, it must be an
            instance of the model's configuration class.
        - kwargs: keyword arguments
            The arguments passed to the initializer of the configuration class.
            Must only be given if the configuration instance is not defined.
        """

...
class Cache(object):
    ...
    def get(self, key, prime_on_cache_miss=False):
        ...
        queryset = DataProduct.objects.filter(key=key)
        try:
            return self.cache_entry(queryset=queryset, key=key)
        ...
        # try to prime
            ...
            if primer_cls.prime_on_cache_miss or prime_on_cache_miss:
                return primer_cls(key).prime(cache=self)

            # failing all else, raise KeyError
            raise KeyError(key)

    def set(self, key, value, primer_name=None, version=None, soft_expire=None, hard_expire=None):
        params = dict(
            key=key,
            value=value,
        ...
        data_product.save()
        return self.cache_entry(data_product=data_product)

...
class BedGraph:
    ...
    def __init__(self, chrom_size_file_name, data_file_name, chrom_wanted=None,
        ...
        found_wanted_chrom = False
        log.info(f"Reading in {data_file_name} ...")
        with open(data_file_name) as data_file:
            # pool = multiprocessing.Pool(2)
            # for result in pool.imap_unordered(get_data, data_file, chunksize=100000):
            for line in data_file:

                if chrom_wanted is not None:
                    if chrom_wanted not in line:
                        if found_wanted_chrom:
                            break
                        continue
                    else:
                        found_wanted_chrom = True
                ...
                # self.chromosome_map[chrom_name].add_processed_data(result)
                self.chromosome_map[chrom_name].add_data(data)

        # Clean up chromosomes
        for chrom_name in self.chromosome_map:
            self.chromosome_map[chrom_name].finish_init()

...
class Chrom_Data:
    ...
    def add_processed_data(self, data):
        ...
        self.value_map.append(value)
        self.intervals[0].append(start)
        self.intervals[1].append(end)
    ...
    def finish_init(self):
        ...
        self.value_map = np.array(self.value_map, dtype=np.float64)
        self.intervals = [np.array(self.intervals[0], dtype=np.uint32),
                          np.array(self.intervals[1], dtype=np.uint32)]

        if self.num_intervals == 0:
            # log.warning(f"No intervals in {self.name}")
            return

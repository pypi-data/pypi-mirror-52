...
def image_preprocess(data):
  for i in range(0,len(data)):
    for j in range(0,len(data[i])):
      if isinstance(data[i][j], collections.Sequence):
        if len(data[i][j]) < 3:
...
def image_show(data):
    if not (isinstance(data, collections.Sequence) and
...
def animation_show(data):
    if not (isinstance(data, collections.Sequence) and
            isinstance(data[0], collections.Sequence) and
            isinstance(data[0][0], collections.Sequence)):
...
def plotdata(data, line=False, **kwargs):
  ...
  if isinstance(data[0], collections.Sequence):
    if isinstance(data[0][0], collections.Sequence):
      plot_clusters(data)
    else:
      xdata = [ i[0] for i in data ]
      ydata = [ i[1] for i in data ]

...
def optim_feature(optimname="", modelname="model.randomForest", file_train, file_test, colX, coly, params={}) :
   """
   ...
   """
   p = dict2(params)
   clf = model_load(modelname)

   df = data_load(file_train)
   dftest = data_load(file_test)


   if optimname == "genetic" :
      optim_feature_genetic( clf, df, dftest, colX, coly,   params) :
...
def optim_(modelname="model_dl.1_lstm.py", 
          pars= {},      
          df = None,
          optim_engine="optuna",

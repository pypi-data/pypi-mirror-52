...
See the following documents  for details on pyfeeds:
  ...
  - The pyfeeds configuration file: ``doc/pyfeeds_configuration.md``
...
class Pyfeeds(object):
    ...
    def __init__(self, args, cfg):
        ...
        self.skipInputDir     = False
        self.skipBenchmarkDir = False
    ...
    def __findTests(self, testDirs):
        ...
        for td in testDirs:
            try:
                tests.append(testing.Test(td, rootDir))
            except Exception:
                log.warning('Error loading test {}'.format(td))
    ...
    def __validate(self):
        ...
        nooutputfile = 'Output file not specified!'
        ...
        skipInputDir     = self.skipInputDir
        skipBenchmarkDir = self.skipBenchmarkDir
        ...
        if command == 'run':
            ...
            if outputDir    is None: raise RuntimeError(nooutputdir)
            ...
            if     op.exists(outputDir) and \
                   not ovw:                 raise RuntimeError(outputexist)
            ...
            if inputDir     is None:        raise RuntimeError(noinput)
            if outputFile   is None:        raise RuntimeError(nooutputfile)
            if benchmarkDir is None:        raise RuntimeError(nobenchmark)
            if not op.exists(inputDir):     raise RuntimeError(noinput)
            if not op.exists(benchmarkDir): raise RuntimeError(nobenchmark)
...
def parseArgs(argv):
    ...
    usages = {
        'mainParser' :
        'pyfeeds [-v|-q] command [options] testDir [testDir ...]\n'
        'pyfeeds -c cfgfile command',

        'list' :
        'pyfeeds [-v|-q] list testDir [testDir ...]',
        ...
        'genhash' :
        'pyfeeds [-v|-q] genhash -i inputDir [options] testDir [testDir ...]',

        'checkhash' :
        'pyfeeds [-v|-q] checkhash -i inputDir [options] testDir '
        '[testDir ...]',
    ...
    flags = {
        ...
        # bundle options
        'skipInputDir'     : ('-si', '--skipInputDir'),
        'skipBenchmarkDir' : ('-sb', '--skipBenchmarkDir'),
    ...
    helps = {
        ...
        # Bundle options
        'skipInputDir'     : 'Do not bundle test input data',
        'skipBenchmarkDir' : 'Do not bundle test benchmark data',
        'genHashes'        : 'Generate hashes for bundled tests. The '
                             'hashes are saved in '
                             '[outputDir]/[inputDir]/.feedsHashes',

        # Compare options
        'outputFile'       : 'Save results to this file.',
    }
    ...
    # Global options
    mainParser.add_argument(*flags['version'],
                            action='version',
                            version=pyfeeds_version)
    mainParser.add_argument(*flags['config'],
                            help=helps['config'])
    mainParser.add_argument(*flags['logFile'],
                            help=helps['logFile'])
    mainParser.add_argument(*flags['includeTests'],
                            help=helps['includeTests'],
                            action='append')
    mainParser.add_argument(*flags['excludeTests'],
                            help=helps['excludeTests'],
                            action='append')
    mainParser.add_argument(*flags['cacheSize'],
                            help=helps['cacheSize'],
                            type=int)

    baseParser = mainParser.add_mutually_exclusive_group()
    baseParser.add_argument(*flags['verbose'],
                            help=helps['verbose'],
                            action='store_const',
                            const=True)
    baseParser.add_argument(*flags['quiet'],
                            help=helps['quiet'],
                            action='store_const',
                            const=True)
    ...
    required       = testBaseParser.add_argument_group('required arguments')
    required.add_argument(flags['testDir'],
                          help=helps['testDir'],
                          nargs='*')
    ...
    required = runbungencheckParser.add_argument_group('required arguments')
    required.add_argument(*flags['inputDir'],
                          help=helps['inputDir'])

    # Options shared by the run/bundle sub-commands
    runbunParser = argparse.ArgumentParser(add_help=False)
    required     = runbunParser.add_argument_group('required arguments')
    required.add_argument(*flags['outputDir'],
                          help=helps['outputDir'])
    required.add_argument(*flags['overwrite'],
                          help=helps['overwrite'],
                          action='store_true')
    required.add_argument(*flags['benchmarkDir'],
    ...
    rungencheckParser.add_argument(*flags['hashFile'], help=helps['hashFile'])

    # Options for the compare sub-command
    compareParser = argparse.ArgumentParser(add_help=False)
    required      = compareParser.add_argument_group('required arguments')
    required.add_argument(*flags['inputDir'],
                          help=helps['inputDir'])
    required.add_argument(*flags['benchmarkDir'],
                          help=helps['benchmarkDir'])
    required.add_argument(*flags['outputFile'],
    ...
    parents = {
        'list'      : [testBaseParser],
        'run'       : [testBaseParser, runbungencheckParser, runbunParser,
                       rungencheckParser, runcmpParser],
        'bundle'    : [testBaseParser, runbungencheckParser, runbunParser],
        'genhash'   : [testBaseParser, runbungencheckParser,
                       rungencheckParser],
        'checkhash' : [testBaseParser, runbungencheckParser,
                       rungencheckParser],
        'compare'   : [compareParser, runcmpParser]
    }
    ...
    bundleParser.add_argument(*flags['skipInputDir'],
                              help=helps['skipInputDir'],
                              action='store_const',
                              const=True)
    bundleParser.add_argument(*flags['skipBenchmarkDir'],
                              help=helps['skipBenchmarkDir'],
                              action='store_const',
                              const=True)
    ...
    # set args.testDir to None
    if not hasattr(args, 'testDir') or len(args.testDir) == 0:
        args.testDir = None
...
def loadPyfeedsConfig(cfgfile):
    ...
    types = {
        ...
        'skipInputDir'     : strbool,
        'skipBenchmarkDir' : strbool,
        'genHashes'        : strbool,

...
def genHashes(pyf):
    ...
    for test in tests:
        ...
        inputHashes.append(test.hashInputs(inputDir))
...
def checkHashes(pyf):
    ...
    results = [checkTestHash(pyf, t, inputDir, savedHashes, False)
               for t in tests]
...
"""Return code for the :func:`checkTestHash` function. This code indicates
that the test verification was skipped , as the test input data had been
...
def checkTestHash(pyf, test, inputDir, testHashes, updateHashes):
...
def hashPath(path, hashObj):
    ...
    the :meth:`__hashFile` method). Otherwise, if ``path`` is a directory,

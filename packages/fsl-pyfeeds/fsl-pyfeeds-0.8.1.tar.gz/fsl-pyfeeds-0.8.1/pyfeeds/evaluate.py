...
def evaluateTest(pyf, test, retcode, outputDir, benchmarkDir):
    ...
    return compareAllFiles(pyf, test, outputDir, benchmarkDir)
...
def compareAllFiles(pyf, test, outputDir, benchmarkDir, logFile=None):
    ...
    If ``logFile`` is not provided, it is set to
    ``outputDir/feedsResults.log``.
    """

    if logFile is None:
        logFile = op.join(outputDir, 'feedsResults.log')
    ...
    def fmtlog(logf, fname, status, routine, error, tolerance):
    ...
    with open(logFile, 'wt') as f:
        f.write('FILE,RESULT,ROUTINE,ERROR,TOLERANCE\n')
...
def cmpArrays(arr1, arr2):
    ...
    denom    = np.max((np.abs(arr1), np.abs(arr2)), axis=0)

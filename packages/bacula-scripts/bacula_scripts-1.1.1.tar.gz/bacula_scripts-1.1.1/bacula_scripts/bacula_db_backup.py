...
def createbackup(dbname, dbtype, pg_dump_compression="1", dbbackupdir="/tmp/dbbackupdir/"):
    """Creates backup"""
    ...
    if dbtype == "postgresql":
        if pg_dump_compression:
            if re.match("^[0-9]$", pg_dump_compression):
                print("pg_dump with -Z%s compression" % pg_dump_compression)
                cmd = ["pg_dump -Z%s -Fc" % pg_dump_compression, dbname]
            else:
                print("(WARNING) Compression has to be a number from 0-9. Falling back to "\
                      "compression 1.")
                cmd = ["pg_dump -Z1 -Fc" % pg_dump_compression, dbname]
        else:
            cmd = ["pg_dump", dbname]
...
def main():
    ...
    p.add_argument(
        "-pg_dump_compression",
        choices=["None"] + [str(x) for x in range(0,10)],
        default="1",
        help="Specify the compression of the postgresql dump' ",
        required=False
    )
    ...
    if args.c:
        check_services()
        if args.pg_dump_compression:
            createbackup(args.c[0], args.t, pg_dump_compression=pg_dump_compression, dbbackupdir=dbbackupdir)
        else:

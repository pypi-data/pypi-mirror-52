...
def load(
    ...
    for source_name in source_names:
        source_query = queries.get(source_name, None)
        if source_query and isinstance(feed.get_source(source_name), Queryable):
            feed.sources[source_name].query(source_query)
        elif source_query:
            raise SourceTypeException(f"{source_name} is not queryable.")
        else:
            feed.get_source(source_name).load()
        source_dfs[source_name] = feed.get_source(source_name).to_df()

...
def search(keywords=None):
    """
    Function to allow a user to search for their feeds with keywords.
    ...
    mapping feed_ids to a list of source names for that feed
    """
    if not keywords:
        keywords = ""
    ...
    try:
        ...
        for feed_result in json.loads(f.content):
            ...
            search_results[feed.id] = list(feed.sources.keys())
        config.logger.info(
        ...
        raise MobikitException(
            f"Recieved invalid response status code {response_status}."
        ) from e

        ...
        for item, val in cur.iteritems():
            if isinstance(val, dict):
                ...
                if cur[item] != pre[item] and item != 'hid':
                    print '%s(%s..) %s:  %s' % (
                        cur['name'], cur['hid'][:5], item, watcher._format_item(item, val)
                    )
                    found_change = True
                else:
                    if trace is True:
            ...
            for item, val in cur.iteritems():
                ...
                if isinstance(val, dict):
                    ...
                    if item in cur and item in pre and cur[item] != pre[item]:
                        print '%s(%s..) %s: %s:  %s' % (
                            rname, hid, item_print_string, item,
                            watcher._format_item(item, cur[item])
                        )
                        found_change = True
                    else:
                        if trace is True:
                            print 'Trace: %s(%s..) %s: %s:  %s' % (
                                rname, hid, item_print_string, item, cur[item]
                            )
        else:
            if pre and cur:

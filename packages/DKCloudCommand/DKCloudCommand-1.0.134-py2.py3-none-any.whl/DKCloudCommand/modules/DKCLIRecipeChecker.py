...
class DKCLIRecipeChecker:
    ...
    def _check_node(self, node_path):
        ...
        if description_json_dict['type'] not in DKCLIRecipeChecker.NODE_TYPE_LIST:
            error_message = 'Bad node type: "%s". Needs to be one of this: %s' % (
                description_json_dict['type'], DKCLIRecipeChecker.NODE_TYPE_LIST
            )
        ...
        if 'tests' in json_dict:
            for test_key, test_value in json_dict['tests'].iteritems():
                if 'action' in test_value and test_value[
                    ...
                    error_message += 'Bad value "%s", needs to be one of this list: %s' % (
                        test_value['action'], DKCLIRecipeChecker.TEST_ACTION_TYPE_LIST
                    )
                    self._display_error(
    ...
    def _check_dict_key(
        ...
        if expected_values is not None and the_dict[the_key] not in expected_values:
            error_message = 'Bad value "%s", needs to be one of this list: %s' % (
                the_dict[the_key], expected_values
            )
    ...
    def _display_error(self, message, severity, file_path):
        formatted_message = '%s---- %s%s' % (
            os.linesep, DKCLIRecipeChecker.EXCEPTION_MESSAGE_HEADER, os.linesep
        )

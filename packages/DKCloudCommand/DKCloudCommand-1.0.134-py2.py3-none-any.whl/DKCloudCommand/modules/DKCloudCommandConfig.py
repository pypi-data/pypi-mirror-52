...
class DKCloudCommandConfig(object):
    ...
    def __str__(self):
        ...
        output_string += 'Merge Tool:\t\t\t%s\n' % self._config_dict[
            DKCloudCommandConfig.DK_CLOUD_MERGE_TOOL]
        output_string += 'Diff Tool:\t\t\t%s\n' % self._config_dict[
            DKCloudCommandConfig.DK_CLOUD_DIFF_TOOL]
        output_string += 'Check Working Path:\t\t%s\n' % self._config_dict[
            DKCloudCommandConfig.DK_CHECK_WORKING_PATH]
        output_string += 'Hide Context Legend:\t\t%s\n' % self.get_hide_context_legend()
        output_string += 'Skip Recipe Checker:\t\t%s\n' % self.get_skip_recipe_checker()
    ...
    def context_exists(self, context_name):
        ...
        for context in context_list:
            if context != context_name and context.lower() == context_name.lower():
                raise Exception('You probably meant context: %s' % context)
    ...
    def get_merge_tool(self):
        if DKCloudCommandConfig.DK_CLOUD_MERGE_TOOL not in self._config_dict or \
            ...
            raise Exception(
                'Merge tool was not properly configured. Please run \'dk config-list\' to check current\
    ...
    def get_diff_tool(self):
        if DKCloudCommandConfig.DK_CLOUD_DIFF_TOOL not in self._config_dict or \
            ...
            raise Exception(
                'Diff tool was not properly configured. Please run \'dk config-list\' to check current\
                configuration and \'dk config\' to change it.'
            )
                ...
                if statinfo.st_size > 0:
                    with open(full_path) as data_file:
                        try:
                            ...
                            print(
                                'DKCloudCommandConfig: failed json.load check syntax %s. %s' %
                                (full_path, e)
                            )
                            rv = False
                        else:
    ...
    def validate_config(self):
        for v in self._required_config_attributes:
            if v not in self._config_dict:
                print(
                    "DKCloudCommandConfig: failed to find %s in %s" %
                    (v, DKCloudCommandConfig.CONFIG_FILE_NAME)
                )
                return False
        return True

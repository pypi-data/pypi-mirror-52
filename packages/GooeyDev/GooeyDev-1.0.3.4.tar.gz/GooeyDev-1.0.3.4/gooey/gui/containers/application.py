...
class GooeyApplication(wx.Frame):
    ...
    def __init__(self, buildSpec, *args, **kwargs):
        ...
        pub.subscribe(events.WINDOW_SAVE, self.onSave)
    ...
    def onSave(self, *args,**kwarg):

        with wx.FileDialog(self, "Save console output into textfile", wildcard="Textfile (*.txt)|*.txt",
                           style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT, defaultFile="cryolo_console_output.txt") as fileDialog:
            ...
            txt = self.console.getText()
            text_file = open(pathname, "w")
            text_file.write(txt)


        print("Call save with", args, kwarg)
    ...
    def showComplete(self):
        self.navbar.Show(False)
        self.console.Show(True)
        buttons = (['save_button', 'edit_button', 'restart_button', 'close_button']
                   if self.buildSpec.get('show_restart_button', True)
                   else ['save_button', 'edit_button', 'close_button'])

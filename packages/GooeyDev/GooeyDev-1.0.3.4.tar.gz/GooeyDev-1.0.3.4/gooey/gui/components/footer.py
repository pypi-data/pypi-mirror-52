...
class Footer(wx.Panel):
    ...
    def _init_components(self):
        ...
        self.save_button = self.button(_('save'), wx.ID_OK, event_id=int(events.WINDOW_SAVE))
        self.edit_button = self.button(_('back'), wx.ID_OK, event_id=int(events.WINDOW_EDIT))

        self.progress_bar = wx.Gauge(self, range=100)

        self.buttons = [self.cancel_button, self.start_button,
                        self.stop_button, self.close_button,
                        self.restart_button, self.edit_button, self.save_button]
    ...
    def _do_layout(self):
        ...
        h_sizer.Add(self.save_button, 0, wx.ALIGN_RIGHT | wx.RIGHT, 10)

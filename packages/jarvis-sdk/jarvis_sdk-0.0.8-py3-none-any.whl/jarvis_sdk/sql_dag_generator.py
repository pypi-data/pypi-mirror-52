...
def process_gcp_composer_bucket(gcp_composer_bucket, gcp_default_composer_bucket):
    if gcp_composer_bucket is None:
        print("Using default GCP Composer Bucket : %s" % gcp_default_composer_bucket)
        return gcp_default_composer_bucket
    else:
        print("Using provided GCP Composer Bucket : %s" % gcp_composer_bucket)
        return gcp_composer_bucket
...
def execute_bq_create_table(gcp_project_id,
    ...
    if (bq_table_timepartitioning_field is not None) or (bq_table_timepartitioning_expiration_ms is not None) or     (bq_table_timepartitioning_require_partition_filter is not None):
        ...
        table.time_partitioning = bigquery.table.TimePartitioning(field=bq_table_timepartitioning_field, expiration_ms=bq_table_timepartitioning_expiration_ms, require_partition_filter=bq_table_timepartitioning_require_partition_filter)
...
def build_sql_task(payload, env, default_gcp_project_id, default_bq_dataset, default_write_disposition):
...
def build_create_bq_table_task(payload, default_gcp_project_id, default_bq_dataset):
...
def process(args):
    ...
    #
    dag_name = json_payload["dag_name"] + "_" + environment
    ...
    dag_doc = ""
    try:
        dag_documentation = "./" + json_payload["doc_md"]
    ...
    for item in json_payload["workflow"]:
        ...
        if task_type == "copy_gbq_table":
            ...
            generated_code = build_sql_task(
                item, environment, default_gcp_project_id, default_bq_dataset, default_write_disposition)
...
"""
    ...
    # Process GCP default Composer bucket
    #
    gcp_composer_bucket = process_gcp_composer_bucket(args.gcp_composer_bucket, jarvis_configuration["gcp_default_composer_bucket"])
    ...
    try:
        ...
        url = jarvis_configuration["jarvis_api_endpoint"] + "dag-generator"
        payload = {
            "payload": {
                "resource": encoded,
                "dag_file" : {

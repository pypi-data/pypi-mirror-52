...
def get_engine(db_configs: dict,create_if_not_exists=False,drivername='postgres',**connection_kwargs):
	...
	@lu.simple_logger.wrap(log_level=logging.INFO)
	...
	if create_if_not_exists:
		try:
			...
			lu.simple_logger.info(f"Database '{database}' does not exist.")
			__create_database()
			engine=engine_creator(database)
	else:
		engine=engine_creator(database)
...
class Manager(object):
	...
	def create_table(self,table_class: sqla_orms.Base):
		if self.engine.dialect.has_table(self.engine,table_class.__tablename__):
			lu.simple_logger.info(f"Table '{table_class.__tablename__}' already exists.")
		else:
			lu.simple_logger.info(f"Creating table '{table_class.__tablename__}'.")

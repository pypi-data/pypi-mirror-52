...
lu.default_log_level=logging.DEBUG
	...
	if non_blocking:
		...
		if with_popen:
				...
				if ret_triggers:
					for ret_trigger in ret_triggers:
						if ret_trigger in nextline:
							...
							break
				lu.simple_logger.debug(nextline)
			...
			if regular_termination:
				...
				lu.simple_logger.info(r'Manual process termination triggered with line:\n%s',nextline)
				lu.simple_logger.debug('Killing process: %s',process.pid)
...
def try_to_do(func,*args,check_success=None,max_attempts=5,max_wait_seconds=0,**kwargs):
	...
	lu.simple_logger.debug('Trying to accomplish "%s" ...',func.__name__)
	wait_time=iter(waiting_times)
	for i in range(max_attempts):
		time_to_wait=next(wait_time)
		if time_to_wait!=0:
			lu.simple_logger.info('Sleeping for %s sec.',time_to_wait)
			time.sleep(time_to_wait)
		lu.simple_logger.debug('Attempt #%s.',i+1)
		out=func(*args,**kwargs)
		# simple_logger.debug('check_success:')
		if check_success(out):
			lu.simple_logger.debug('Succeeded.')
			return out
		else:
			lu.simple_logger.info('Attempt #%s failed.',i+1)
	return False
...
def run_until_victory(func,retry_timeout,*args,**kwargs):
	...
	while not res_dict:
		count+=1
		lu.simple_logger.info(f'Starting "{func.__name__}". Counter: {count}')
...
def infiterate(iter_obj,max_iter_count=None,next_getter=None,inform_count=8,on_inform=None):
	...
	for i,obj in enumerate(an_iterable):
		if i%(max_iter_count//inform_count)==0:
			on_inform()
			lu.simple_logger.info('%d%%, i=%d',i/(max_iter_count-1)*100,i)
		...
		def wrapper(*args,**kwargs):
			...
			while True:
				try:
					out=func(*args,**kwargs)
					if try_count!=1:
						lu.simple_logger.info('Succeeded at try #%d',try_count)
					break
				except Exception as e:
					if max_tries and max_tries<=try_count:
						lu.simple_logger.info('Reached max. number of tries (%d).',try_count)
					...
					lu.simple_logger.error('error: %s',str(e),exc_info=True)
					lu.simple_logger.info('Sleeping for %.3f seconds.',sleep_time)
					time.sleep(sleep_time)
					lu.simple_logger.info('Try #%d',try_count)

...
class SshMan(object):
	...
	def check_exists(self,program_name):
		...
		if which_result.failed: return False
			...
			lu.simple_logger.info(f'"{program_name}" already exists.')
			return True
	...
	def apt_install(self,*package_names,forced=False):
		...
		if not forced:
			for package_name in names.copy():
				if self.check_exists(package_name):
					...
					lu.simple_logger.info('Skipping installation of "%s".',package_name)
		...
		for package_name in names:
			...
			lu.simple_logger.debug('"%s" path: %s',package_name,package_path)
	...
	def run(self,command,**kwargs):
		lu.simple_logger.info('Running !!!%s!!!',command)
	...
	def put(self,local,remote,preserve_mode=True,absolute_remote=False):
		...
		lu.simple_logger.info('Putting from !!!%s!!! to !!!%s!!!',local,remote)
		out=self.conn.put(local,remote=remote,preserve_mode=preserve_mode)
		return out
	
	def get(self,remote,local=None,preserve_mode=True):
		lu.simple_logger.info('Getting from !!!%s!!! to !!!%s!!!',remote,local)

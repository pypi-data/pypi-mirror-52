	...
	for i,row in control_desk.iterrows():
		if func==row.gen_func:
			if resultant_status==row.ret_stat or (type(row.ret_stat) is types.FunctionType and row.ret_stat(resultant_status)):
				lu.simple_logger.info('resultant_status: %s,data: %s',resultant_status,data)
...
def queue_processor(queue: mp.Queue,control_desk):
	...
	try:
		...
		lu.simple_logger.error(str(e),exc_info=True)
		lu.simple_logger.info(str(e),exc_info=True)
...
class ScheduledTask:
	...
	def run(self,*tasks,sleep_time=1):
		...
		lu.simple_logger.info('Start of the main loop.')
		while True:
			for task in all_tasks:
	...
	def on_process_error(data,exception):
		lu.simple_logger.exception("Exception caught when processing data:\n%s\nException text:\n%s",data,str(exception))

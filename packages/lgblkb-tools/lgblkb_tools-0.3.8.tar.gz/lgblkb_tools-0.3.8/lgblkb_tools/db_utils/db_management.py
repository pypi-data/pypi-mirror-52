...
from .. import TheLogger,global_support as gsup
...
def get_engine(db_configs: dict,create_if_not_exists=False,drivername='postgres',**connection_kwargs):
	...
	@simple_logger.wrap(log_level=logging.INFO)
	...
	if create_if_not_exists:
		try:
			...
			simple_logger.info(f"Database '{database}' does not exist.")
			__create_database()
			engine=engine_creator(database)
	else:
		engine=engine_creator(database)
...
class Manager(object):
	...
	def create_table(self,table_class: sqla_orms.Base):
		if self.engine.dialect.has_table(self.engine,table_class.__tablename__):
			simple_logger.info(f"Table '{table_class.__tablename__}' already exists.")
		else:
			simple_logger.info(f"Creating table '{table_class.__tablename__}'.")

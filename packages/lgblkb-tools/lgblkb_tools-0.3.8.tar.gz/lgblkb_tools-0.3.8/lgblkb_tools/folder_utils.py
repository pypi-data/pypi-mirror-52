...
from lgblkb_tools.locations import get_parent_dir,get_splitted,InfoDict,create_path,_make_zipfile,ZipError,\
	get_existing_path,get_name,CopyError
from .common_utils.logging_utils import TheLogger
...
class Folder(object):
	...
	def clear(self):
		...
		try:
			...
			simple_logger.warning('FileNotFoundError when trying to clear up the results folder. Passing on.')
	...
	@simple_logger.wrap(skimpy=True)
	...
	def copy_to(self,dst_path,create_subdir=True,forced=False):
		...
		simple_logger.debug(f'Copying {self.path} to {destination_folder.path}...')
		...
		for i in range(3):
			...
			if source_hashsum==destination_hashsum:
				...
				simple_logger.warning('source_hashsum',source_hashsum)
				simple_logger.warning('destination_hashsum',destination_hashsum)

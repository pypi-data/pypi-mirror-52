...
class DataBase(object):
    ...
    def select(self, name, limitation=None, keys='*', return_dict=True):  # {{{
        """ select from table
    ...
    def get_global(self, key, default=None):  # {{{
        ...
        data = self.select('global', keys='value', limitation={'key=': key, 'time>':
            time.time()}, return_dict=False)
        if len(data) == 0:
            return default
...
if __name__ == '__main__':
    db = DataBase('/tmp/tmp.db', log_file='/tmp/log')

    data = db.select('global')
    print(data)

    db.set_global('hi', 'asdsadads', 1)
    db.set_global('xi', 'asds')
    time.sleep(0.5)
    x = db.get_global('hi')
    print(x)
    db.update('global', {'value': 'c'}, {'key=': 'c'})
    data = db.select('global')
    print(data)

...
class rimutils:
    ...
    def makeGraph(cls, im: numpy.ndarray = None):
        ...
        for i, coords in enumerate(all_pixels):
            ...
            theneighbours_ind = rimutils.sub2ind(im.shape, theneighbours[:, 0], theneighbours[:, 1])  # Convert all vertices once and use dictionary?
    ...
    def flow(cls, firstslice: numpy.ndarray, secondslice: numpy.ndarray,
        ...
        # desired_step_sz.
        Xval: numpy.ndarray = numpy.nan * numpy.ones([int(numpy.floor(firstslice.shape[0] / calculated_step_sz[0]))+1, int(numpy.floor(
        ...
        Yval: numpy.ndarray = numpy.nan * numpy.ones(
            [int(numpy.floor(firstslice.shape[0] / calculated_step_sz[0]))+1, int(numpy.floor(firstslice.shape[1] / calculated_step_sz[1]))+1])
        ...
        X0, Y0 = numpy.meshgrid(numpy.arange(Xval.shape[1]) * calculated_step_sz[1], numpy.arange(Xval.shape[0]) * calculated_step_sz[0])

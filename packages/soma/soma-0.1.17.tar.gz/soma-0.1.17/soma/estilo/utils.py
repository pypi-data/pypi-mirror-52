...
def delay_colecao(df, delta_colecao, features):
    ...
    if delta_colecao >= 1:
        ...
        replaced_df['col_linha_grupo'] = replaced_df['colecao'] + replaced_df['linha'] + replaced_df['grupo']
        ...
        df = df.merge(replaced_df.drop(columns='col_linha'), how='left', on=['col_linha_grupo'], suffixes=('','_lg'))
        ...
        df.drop(columns=l_cols+lg_cols+['col_linha','col_linha_grupo'], inplace=True)
    return df
...
class Datasets:
    ...
    def load_sales(
        ...
        if self.dataset is None:
            try:
                dataset = pd.read_pickle(os.environ.get("SALES_DATASET_PATH"))
            except:
                dataset = self.create_sales()
                dataset.to_pickle(os.environ.get("SALES_DATASET_PATH"))
        ...
        columns_to_encode = list(set(self.label_columns) & set(cols))
        ...
        dataset_train = dataset[train_mask][cols]
        dataset_test = dataset[test_mask][cols]

        self.train = (dataset_train[cols[:-1]], dataset_train[cols[-1]])
        self.test = (dataset_test[cols[:-1]], dataset_test[cols[-1]])
    ...
    def create_sales(self):
        ...
        animale_df["delta_lancamento"] = (animale_df["lancamento"] - animale_df["data_venda"]).dt.days
        animale_df["delta_fim"] = (animale_df["data_fim"] - animale_df["data_venda"]).dt.days

        animale_df["delta_prog"] = (animale_df["min(entrega_final)"] - animale_df["data_venda"]).dt.days

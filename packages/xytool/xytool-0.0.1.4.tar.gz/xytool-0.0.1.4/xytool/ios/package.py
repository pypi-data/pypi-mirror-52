...
from xytool.ios.CommonScript import IOSProjectTool
import xytool.common.globalvar as gv
from xytool.ios.iosProjectTool import IOSTool
from xytool.config import Config
from xytool.common.file import File
from xytool.ios.gitandjira import GitLab
...
class Package:
    ...
    def merge_code(self,code_branch,package_branch,email):
        """
        合并代码(开发分支名、打包分支名)
        """
        if code_branch == None:
        ...
        try:
            ...
            if email is not None:
                GitLab().get_all_commit_between_push(code_branch=code_branch,package_branch=package_branch,workspace=currentPath,mail=email)

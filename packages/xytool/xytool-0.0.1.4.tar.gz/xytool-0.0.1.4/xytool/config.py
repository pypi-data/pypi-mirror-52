...
from xytool.common.xylog import logitem,warninglog
from xytool.common.interaction import Interaction
...
class Config:
    ...
    def check_version(self):
        """ 检查版本号 """
        shell_str = "pip3 search xytool"
        shell_str_brew = "brew info xytool"
        result = os.popen(shell_str)
        text = result.read()
        result = re.findall(r"xytool \((.+?)\)",text)
        if len(result) == 1:
            local_version = self.get_version()
            remote_version = result[0]
            if local_version != remote_version:
                warninglog("################################################################################################")
                warninglog("# the latest version of xytool is:" + remote_version + ", but yours is still:" + local_version)
                warninglog("# this will cause errors or some features will not work")
                warninglog("# you can use 'pip3 install xytool -U' to update the latest version")
                warninglog("################################################################################################")

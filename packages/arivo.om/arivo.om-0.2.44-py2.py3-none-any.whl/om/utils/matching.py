...
try:
    ...
    try:
        ...
        from yaml import Loader  # fallback for old pyyaml versions
...
class PlateYamlSelector(object):
    ...
    # def get_scheme_versions_decreasing(self, scheme):
    #     if scheme not in self.schemes:
    #         scheme = "DEFAULT"
    #     return sorted(self.schemes[scheme].keys(), key=lambda x: self.schemes[scheme][x].exactitude, reverse=True)
    ...
    def create_db_record(self, plate, country, scheme, version, id=None):
        """
    ...
    def insert_into_db(self, db, plate, country, scheme, version, id=None):
        """
        ...
        :param id: id only used in testcases
        """
        record = self.create_db_record(plate, country, scheme, version, id)

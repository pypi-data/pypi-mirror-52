...
class OpenScaleClient(OpenScaleReset):
    ...
    def use_existing_subscription(self, asset_details_dict):
        ...
        elapsed = time.time() - start
        self._model.expected_payload_row_count = self._reliable_count_payload_rows()
    ...
    def define_bkpi_application(self, params):
        if params:
            params['name'] = self._model.metadata['model_name'] + '_BKPIapp'
    ...
    @retry(tries=5, delay=8, backoff=2)
    def define_bkpi_payload(self, params):
        if params:
            params['name'] = self._model.metadata['model_name'] + '_BKPIpayload'
            params['data_mart_id'] = self._credentials['data_mart_id']
            params['target']['target_id'] = self._model.metadata['bkpiapp_id']
            bkpipayload_url = '{}/{}/v2/data_sets'.format(self._credentials['url'], self._credentials['data_mart_id'])
            token = self._args.iam_token if self._args.iam_token else None
            iam_headers = get_iam_headers(self._credentials, self._args.env_dict, token)
            start = time.time()
            response = requests.post(bkpipayload_url, json=params, headers=iam_headers, verify=self._verify)
            elapsed = time.time() - start
            logger.log_debug('TIMER: define business payload in {:.3f} seconds'.format(elapsed))
            self._model.metadata['bkpipayload_id'] = None
            if 'metadata' in response.json() and 'id' in response.json()['metadata']:
                self._model.metadata['bkpipayload_id'] = response.json()['metadata']['id']
            else:
                logger.log_debug('DEBUG: create bkpi payload response: {}'.format(str(response)))
                logger.log_debug('DEBUG: create bkpi payload response json: {}'.format(str(response.json())))
                error_msg = 'ERROR: business payload id not returned'
                logger.log_error(error_msg)
                raise Exception(error_msg)
    ...
    def configure_bkpi(self, params):
        if params:
            if 'business_application' in params:
                ...
                logger.log_info('PLACEHOLDER: Configuring business application commented out for now')
                ...
                logger.log_error(error_msg)
                raise Exception(error_msg)
            if 'business_payload' in params:
                #logger.log_info('Configuring business payload ...')
                logger.log_info('PLACEHOLDER: Configuring business payload commented out for now')
                #self.define_bkpi_payload(params['business_payload'])
                #logger.log_info('Business payload configured successfully (guid: {})'.format(self._model.metadata['bkpipayload_id']))
            else:
                error_msg = 'ERROR: "business_payload" tag missing in bkpi_configuration'
                logger.log_error(error_msg)
                raise Exception(error_msg)
            self._use_bkpi = True
    ...
    def generate_sample_metrics(self):
        ...
        if self._use_bkpi:
            for day in range(self._args.history_first_day, self._args.history_first_day + self._args.history):
                ...
                logger.log_info(' - Loading day {}'.format(day + 1))
                logger.log_info('PLACEHOLDER: load historical business kpi metrics for day {}'.format(day+1))
                if (day + 1) == self._args.history:
                    logger.log_info('Historical business kpi metrics loaded successfully')

            for day in range(self._args.history_first_day, self._args.history_first_day + self._args.history):
                records = self._model.get_bkpi_payloads_history(day)
                if day == self._args.history_first_day:
                    if len(records) == 0:
                        logger.log_info('No historical business kpi payloads provided - skipping')
                        break
                    else:
                        logger.log_info('Loading historical business kpi payloads to {} ...'.format(self._args.service_name))
                logger.log_info(' - Loading day {}'.format(day + 1))
                logger.log_info('PLACEHOLDER: load historical business kpi payloads for day {}'.format(day+1))
                if (day + 1) == self._args.history:
                    logger.log_info('Historical business kpi payloads loaded successfully')

        self.wait_for_payload_logging(8+int(self._model.historical_payload_row_count/250))
        for day in range(self._args.history_first_day, self._args.history_first_day + self._args.history):
    ...
    def generate_sample_scoring(self, engine_client, numscores, values_per_score, to_init_payload_logging=False):
        ...
        self.wait_for_payload_logging()

    @retry(tries=5, delay=4, backoff=2)
    def _reliable_count_payload_rows(self):
        if self._database is None: # PostgreSQL internal db
            return 0
        start = time.time()
        actual_payload_rows = self._database.count_table_rows(self._datamart_name, 'Payload_{}'.format(self.get_subscription_id()))
        elapsed = time.time() - start
        logger.log_debug('TIMER: subscription count payload rows in {:.3f} seconds, rows={}'.format(elapsed, actual_payload_rows))
        return actual_payload_rows

    def wait_for_payload_logging(self, initial_pause=8):
        if self._database is None: # for PostgreSQL internal db, temporary workaround for issue 10677
            logger.log_info('Wait {} seconds to give payload logging time to complete'.format(initial_pause))
        ...
        for pause in [16, 32, 64, 64]:
            actual_payload_rows = self._reliable_count_payload_rows()
            if self._model.expected_payload_row_count == actual_payload_rows:
                logger.log_info('Confirmed that all payloads have been stored to the datamart database')
                return
                ...
                time.sleep(pause)
        message = 'WARNING: unable to confirm all payloads are stored into the datamart database'
    ...
    def reliable_post_drift_metrics(self, start_time, end_time, windows=8): # eight 3-hour drift windows in 24 hours
        ...
        drift_tasks_url += '?start_time={}&end_time={}&compute_windows={}&execute_mode=synch'.format(start_time_param, end_time_param, windows)

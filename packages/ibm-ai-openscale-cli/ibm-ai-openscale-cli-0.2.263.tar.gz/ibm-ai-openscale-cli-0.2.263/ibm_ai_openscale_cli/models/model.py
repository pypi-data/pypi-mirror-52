...
class Model():
    ...
    BKPI_PAYLOADS_HISTORY_FILENAME = 'history_bkpi_payloads.json'
    ...
    def get_bkpi_metrics_history(self, num_day):
        ...
        if history_file:
            with open(history_file) as f:
                ...
                logger.log_info('PLACEHOLDER: read bkpi metrics history file for day {}'.format(num_day+1))
                fullRecordsList = bkpi_records
        return fullRecordsList

    def get_bkpi_payloads_history(self, num_day):
        fullRecordsList = []
        history_file = self._get_file_path(Model.BKPI_PAYLOADS_HISTORY_FILENAME)
        if history_file:
            with open(history_file) as f:
                bkpi_records = json.load(f)
                logger.log_info('PLACEHOLDER: read bkpi payloads history file for day {}'.format(num_day+1))
                fullRecordsList = bkpi_records
        return fullRecordsList

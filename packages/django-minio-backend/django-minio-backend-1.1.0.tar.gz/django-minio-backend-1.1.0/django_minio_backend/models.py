...
class MinioBackend(Storage):
    def __init__(self, *args, **kwargs):
        ...
        self.MINIO_USE_HTTPS: bool = get_setting("MINIO_USE_HTTPS", None)
    ...
    def is_minio_available(self) -> MinioServerStatus:
        ...
        url = f'{self.protocol}{self.MINIO_ENDPOINT}/minio/index.html'
        try:
            # noinspection PyProtectedMember
            response = self.client._http.urlopen("GET", url)
            return MinioServerStatus(response)
    ...
    @property
    def protocol(self):
        if self.MINIO_USE_HTTPS is None:
            raise ValueError('MINIO_USE_HTTPS must be configured in Django Settings in order to access minIO')
        if self.MINIO_USE_HTTPS:
            return 'https://'
        else:
            return 'http://'

...
def publish_wrapper(wrapped, instance, args, kwargs):
    ...
    with opentracing.tracer.start_active_span(
        ...
        opentracing.tracer.inject(scope.span.context, format=opentracing.Format.HTTP_HEADERS,
                                         carrier=headers)
...
def receive_wrapper(wrapped, instance, args, kwargs):
    ...
    try:
        context = opentracing.tracer.extract(format=opentracing.Format.HTTP_HEADERS, carrier=headers)

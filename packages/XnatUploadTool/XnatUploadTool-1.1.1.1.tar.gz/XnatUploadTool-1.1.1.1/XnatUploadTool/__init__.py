...
class XnatUploadTool:
    ...
    def pull_ingestion_field(self, pattern, filename):
        ...
        if 'split' in pattern:
            ...
            if pattern['source'] == 'filename':
                ...
                if 'tag' in pattern:
                    myoutput = self.pull_dicom_tag(os.path.abspath(self.target + '/' + filename), pattern['tag'])
                else:
                    myoutput = None
            else:
                myoutput = filename
    ...
    def upload_singlefile(self, scan):
        ...
        mydata = {os.path.basename(self.target): ((open(self.target, 'rb')), 'application/dicom')}
    ...
    def upload_ingestedfiles(self, subject=None, session=None, scan=None, scandata=None, resource=None):
        ...
        mydata = dict()
        for myfile in scandata:
            mydata[os.path.basename(myfile['path'])] = open(myfile['path'], 'rb')

        try:
            myurl = ("%s/resources/%s/files?event_reason=batch_upload&event_action=UploadedFile&"
                     "update_stats=false&format=json" % (myurl, resource))

...
def get_account(index):
    ...
    return Account(Web3.toChecksumAddress(address), pswrd, key_file)
...
def process_fulfill_condition(args, condition_contract, condition_id, logger_handle, keeper, num_tries=10):
    ...
    for i in range(num_tries):
        try:
            ...
            logger_handle.debug(f'done trial {i} {contract_name}.fulfill for agreement {agreement_id}, success?: {bool(success)}')
            time.sleep(2)
            ...
            if i == (num_tries-1):
                logger_handle.debug(f'{contract_name}.fulfill for agreement {agreement_id} FAILED with error: {e}')
            else:
                logger_handle.debug(f'Error when doing {contract_name}.fulfill for agreement {agreement_id}: retrying trial # {i}')
                time.sleep(2)

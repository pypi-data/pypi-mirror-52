...
class TestMueller_analysis(object):
    ...
    def test_diattenuator(self):
        ...
        for ind in range(N):
            ...
            M1 = Mueller('M1')
            M1.diattenuator_carac_angles_from_Jones(p1, p2, alpha, delta)
            proposal = M1.analysis.diattenuator(param='carac')
            proposal = np.array(proposal)
            errors[ind] = comparison(proposal, solution, eps)
        assert all(errors), sys._getframe().f_code.co_name + "@ Carac angles"

        for ind in range(N):
            (p1a, p2a, az, el) = np.random.rand(4)
            az = az * 180 * degrees
            el = (1 - el * 2) * 45 * degrees
            p1 = max(p1a, p2a)
            p2 = min(p1a, p2a)
            solution = np.array([p1, p2, az, el])
            M1 = Mueller('M1')
            M1.diattenuator_azimuth_ellipticity_from_vector(p1, p2, az, el)
    ...
    def test_retarder(self):
        ...
        for ind in range(N):
            ...
            M1 = Mueller('M1')
            M1.retarder_carac_angles_from_Jones(delta, alpha, delay)
            proposal = M1.analysis.retarder(param='carac')
            proposal = np.array(proposal)
            errors[ind] = comparison(proposal, solution, eps)
        assert all(errors), sys._getframe().f_code.co_name + "@ Carac angles"

        for ind in range(N):
            (delta, az, el) = np.random.rand(3)
            delta = delta * 180 * degrees
            az = az * 90 * degrees
            el = el * 360 * degrees
            solution = (delta, az, el)
            M1 = Mueller('M1')
            M1.retarder_carac_angles_from_Jones(delta, az, el)
            proposal = M1.analysis.retarder(param='carac')
            proposal = np.array(proposal)
            errors[ind] = comparison(proposal, solution, eps)
        assert all(
    ...
    def test_decompose_pure(self):
        ...
        for ind in range(N):
            ...
            # Create objects
            M1 = Mueller('Mp')
            M1.diattenuator_carac_angles_from_Jones(p1, p2, alphaP, delayP)
            M2 = Mueller('Mr')
            M2.retarder_carac_angles_from_Jones(delta, alphaR, delayR)
            M = M2 * M1
            # Decompose

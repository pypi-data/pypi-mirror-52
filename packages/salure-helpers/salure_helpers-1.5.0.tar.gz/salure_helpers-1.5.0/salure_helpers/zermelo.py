...
class Zermelo():
    ...
    def run_all_extracts(self):
        ...
        self.get_zermelo(endpoint='plannedlessons', fields=['id', 'clockHoursPerLesson', 'plannedGroups', 'lessonDemand', 'branchOfSchool', 'departmentOfBranches', 'subjects', 'sectionOfBranches', 'maxTeachingLevel', 'regularTeachingAssignments'],
                         nested=True, nested_fields=['plannedGroups', 'departmentOfBranches', 'subjects', 'sectionOfBranches', 'regularTeachingAssignments'])
    ...
    def get_zermelo_substituded_lessons(self, endpoint, fields):
        ...
        if self.initial_zermelo_extract:
            years_back_in_time = 3
        else:
            years_back_in_time = 1
        ...
        df = pd.DataFrame()
        for week in range(52 * years_back_in_time, 0, -1):
            start_epoch = int(datetime.datetime.now().timestamp()) - (3600 * 24 * 7 * week)
            end_epoch = start_epoch + (3600 * 24 * 7)
            try:
                print(week, start_epoch, end_epoch)
                ...
                print('{} - Error at week {}: {}'.format(time.strftime('%H:%M:%S'), week, e))

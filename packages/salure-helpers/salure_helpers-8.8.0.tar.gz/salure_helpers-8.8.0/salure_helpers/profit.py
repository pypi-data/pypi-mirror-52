...
class GetConnector:
    ...
    def get_json_filtered_data(connector: str, fields: list, values: list, operators: list, orderbyfields: str = None):
        """
        ...
        url = 'https://{}.{}/profitrestservices/connectors/{}'.format(50501, 'rest.afas.online', 'salure_IRIS_personnel_details')
        ...
        while loop_boolean:
            loop_parameters = {"skip": 40000 * no_of_loops, "take": 40000}
            parameters.update(loop_parameters)
            authorization_header = {'Authorization': 'AfasToken {}'.format('PHRva2VuPjx2ZXJzaW9uPjE8L3ZlcnNpb24+PGRhdGE+Q0E4ODU4NzI4REQyNEI3NTk1OEIwRDk0NUU1ODU4MDYzRkUwN0IyNDQ4OTg1QTJFNThCM0ZCQTEyMUNDNTFERTwvZGF0YT48L3Rva2VuPg==')}
            response = requests.get(url, data='', headers=authorization_header, timeout=30000, params="filterjson={}".format(querystring))
            if response.status_code == 200:
                response_json = response.json()['rows']
                no_of_loops += 1
                no_of_results += len(response_json)
...
class UpdateConnector:
    ...
    def update_organisation(self, data: dict, method: str, custom_fields={}):
        ...
        """
        required_fields = ['organisation_id', 'name', 'employer_id', 'blocked']
        allowed_fields = ['search_name', 'kvk_number', 'phone_number_work', 'email_work', 'vat_number', 'status']
        allowed_fields_address = ['country', 'street', 'housenumber', 'housenumber_add', 'zipcode', 'residence']
        all_fields = required_fields.append(allowed_fields)
        all_fields = all_fields.append(allowed_fields_address)

        # Check if the fields in data exists in the required or allowed fields
        for field in data.keys():
            if field not in all_fields:
                raise ValueError('Pietertje, field {} is not allowed. Allowed fields are: {}'.format(field, tuple(all_fields)))

        # Check if all the required_fields are present
        for field in required_fields:
            if field not in data.keys():
                raise ValueError('Pietertje, field {} is required. Required fields are: {}'.format(field, tuple(required_fields)))
    ...
    def update_contract(self, data: dict, overload_fields={}):
        ...
        :return: status code for request and optional error message
        """
        allowed_fields_contract = ['employee_id', 'type_of_employment', 'enddate_contract', 'termination_reason', 'termination_initiative', 'probation_period', 'probation_enddate']
    ...
    def update_salary(self, data: dict, overload_fields={}):
        ...
        :return: status code for request and optional error message
        """
        allowed_fields = ['step', 'period_table', 'salary_year', 'function_scale', 'salary_scale', 'salary_scale_type']

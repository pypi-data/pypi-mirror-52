...
i = 1
today = datetime.date(year=datetime.datetime.today().year - i, month=8, day=1).timetuple()
print(today)
print(datetime.datetime.fromtimestamp(mktime(today)).strftime('%Y-%m-%d'))
...
class Zermelo():
    ...
    def get_zermelo_substituded_lessons(self, endpoint, fields, startdate, enddate):
        ...
        file = '{}{} - {}.csv'.format(self.storage_location, 'substituded_lessons', datetime.datetime.fromtimestamp(mktime(startdate)).strftime('%Y-%m-%d'))
        df.to_csv(file, '|')
    ...
    def get_zermelo_appointments(self, endpoint, fields, startdate, enddate):
        ...
        #     if self.initial_zermelo_extract:
        #         start_of_data = datetime.date(year=(datetime.datetime.today().year - 6), month=8, day=1).timetuple()
        #     else:
        #         start_of_data = datetime.date(year=(datetime.datetime.today().year - 1), month=8, day=1).timetuple()
        # else:
        #     start_of_data = datetime.date(year=datetime.datetime.today().year, month=8, day=1).timetuple()
        ...
        while start_epoch < last_epoch:
            try:
                if (start_epoch + (3600 * 24 * 2)) > last_epoch:
                    end_epoch = int(last_epoch)
                else:
        ...
        if len(df) > 0:
            ...
            df.index.name = '{0}_id'.format(endpoint)
            file = '{}{} - {}.csv'.format(self.storage_location, 'appointments', datetime.datetime.fromtimestamp(mktime(startdate)).strftime('%Y-%m-%d'))

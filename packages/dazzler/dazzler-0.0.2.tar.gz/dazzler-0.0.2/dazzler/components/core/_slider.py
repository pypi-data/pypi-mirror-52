...
class Slider(Component):
    """
    A slider with a caret contained within a min and max value.  Available CSS
    classes (dazzler-core-slider scoped): - slider-area: The outer div of the
    slider. - slider-handle: The drag handle - slider-spacer: The space that
    is before the handle.
    """
    _aspect_keys = []
    _children = []
    minimum = Aspect(required=True)
    maximum = Aspect(required=True)
    value = Aspect(default=0)
    round = Aspect()
    debounce = Aspect(default=50)
    class_name = Aspect()
    style = Aspect()

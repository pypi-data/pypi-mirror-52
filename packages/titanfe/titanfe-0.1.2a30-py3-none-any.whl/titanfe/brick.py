...
class BrickBase(metaclass=ABCMeta):
    ...
    def __init__(self, adapter: BrickAdapter, parameter: Optional[Dict] = None):
    ...
    @abstractmethod
    def process(self, input: Type[UjoBase] = UJO_VARIANT_NONE):  # pylint: disable=redefined-builtin
...
class LegacyBrick(BrickBase):
    """Wraps "old" modules into a Brick-Class"""
    def __init__(self, adapter, parameter, module):
        super().__init__(adapter, parameter)
        self.module = module

    def process(self, input: Type[UjoBase] = UJO_VARIANT_NONE):  # pylint: disable=redefined-builtin
        return self.module.do_brick_processing(self.adapter, self.parameter, input)

    def stop_processing(self):
        self.adapter.terminated = True

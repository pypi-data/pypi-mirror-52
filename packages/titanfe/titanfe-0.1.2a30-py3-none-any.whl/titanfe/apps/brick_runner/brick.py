...
class Brick:
    ...
    def __init__(self, runner, description):
        ...
        self.flow = description.flow_name
        self.name = description.brick_name
        self.uid = description.brick_uid
    ...
    def create_instance(self):
        ...
        try:
            self.instance = self.module.Brick(self.adapter, self.parameter)
        except AttributeError:
            self.log.debug("Module has no 'Brick' "
                           ", must implement 'do_brick_processing'"
                           "- create a LegacyBrick")

            self.instance = LegacyBrick(self.adapter, self.parameter, self.module)
    ...
    async def execute(self, packet):
        ...
        self.log.info("(%s) execute Brick: %s(%s) for %r", self.flow, self.name, self.uid, packet)
        ...
        if result:
            ...
            await self.results.async_q.put(packet)

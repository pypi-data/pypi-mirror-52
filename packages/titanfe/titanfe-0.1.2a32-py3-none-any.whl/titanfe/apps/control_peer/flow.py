...
from titanfe.utils import create_uid, flatten
...
class Flow:
    ...
    def __init__(self, control_peer, flow_config, bricks_config, path_to_bricks):
        ...
        for connection in flow_config["Connections"]:
            connections[connection["Source"]].append(connection["Target"])

        bricks_by_name = {b.name: b for b in self.bricks}
        targets = set(flatten(connections.values()))
        for brick in self.bricks:
            brick.output = [bricks_by_name[target] for target in connections.get(brick.name, [])]

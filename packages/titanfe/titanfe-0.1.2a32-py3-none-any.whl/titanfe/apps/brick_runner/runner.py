...
class BrickRunner:
    ...
    async def setup(self):
        ...
        if output_targets:
            self.outputs = await asyncio.gather(
                *[Output.create(self, name, address) for name, address in output_targets]
            )
    ...
    async def output_results(self):
        ...
        async for packet in self.brick.get_results():
            self.log.debug("add to output: %r", packet)
            await asyncio.gather(*[output.add(packet) for output in self.outputs])

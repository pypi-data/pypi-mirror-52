...
from titanfe.messages import AssignmentMessage, MessageType
...
class BrickRunner:
    ...
    async def send_assignment(self, input_address):
        ...
        if self.brick.output:
            output_to_runner = await self.brick.output.runner_available()
            output_target = (output_to_runner.brick.name, output_to_runner.input_address)

        assignment = AssignmentMessage((self.brick.to_dict(), output_target))
        log.info("Assign runner: %r", assignment)
        await self.send(assignment)

...
from .mock.mock_worldbank_data import MockResponse


class TestWorldBankClient(unittest.TestCase):
    def setUp(self):
        resp_patcher = patch('datasetmaker.clients.worldbank.requests.get')
        ...
        self.df = self.client.get(indicators=['worldbank_sp.dyn.imrt.in'],
                                  periods=[2017])

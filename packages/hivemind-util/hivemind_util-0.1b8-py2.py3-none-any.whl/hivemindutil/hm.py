...
def help():
    ...
    print('')
    print('- createInstance(taskId, baseUrl, apiKey, name, instructions=\'\', data={}, overrideSchema={}, tags=[])')
    print('Make an API call to create an instance. \'Name\' is a compulsory string. \'Instructions\' is the markdown shown to the contributor for the instance. \'Data\' is a dictionary.')
    print('')
    print('-----')
    print('')
...
def listFilesUploaded(taskId, baseUrl, apiKey):
    ...
    with requests.Session() as s:
        ...
        while paginatedUrl is not None:
            ...
            n = response.links["next"] if "next" in response.links else None
            if n is not None:
                paginatedUrl = HivemindUrl + n["url"]
            else:
                paginatedUrl = None
...
def deleteIteration(taskId, baseUrl, apiKey, instanceId, iterationId):    
    ...
    with requests.Session() as s:
        s.headers.update(authHeader)
        response = checkHttpResponse(s.delete('{0}/api/tasks/{1}/instances/{2}/iterations/{3}'.format(HivemindUrl, taskId, instanceId, iterationId)))     

def createInstance(taskId, baseUrl, apiKey, name, instructions='', data={}, overrideSchema={}, tags=[]):
    authHeader = makeAuthHeader(apiKey)
    with requests.Session() as s:
        s.headers.update(authHeader)
        baseUrl = cleanBaseUrl(baseUrl)               
        ...
        createInstance = checkHttpResponse(s.post('{0}/api/tasks/{1}/instances'.format(baseUrl, taskId), json = instance))

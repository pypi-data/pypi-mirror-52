...
class GraphUtils:
    ...
    #     print(obj_[obj_['appVersion']==last_release].head)
        sns.boxplot(ax=ax1, x="entitytype", y="EPT", data=obj_[obj_['releaseVersion']==self.last_release_version], order=objects_to_colors.keys(), palette=objects_to_colors.values())
        sns.boxplot(ax=ax2, x="entitytype", y="EPT", data=obj_[obj_['releaseVersion']==self.curr_release_version], order=objects_to_colors.keys(), palette=objects_to_colors.values())
        plt.ylim(0,None)
        ax1.set_title(str(self.last_release_version) + ' - Wifi')
        ax2.set_title(str(self.curr_release_version) + ' - Wifi')

...
class GraphUtils:
    ...
    def visualize_using_boxplot(self, obj: object, numcols: int, release_criteria: int, ylimit: int):
      try:
        ...
        if (numcols == 1): 
          ...
          # fig, ax = plt.subplots(nrows=1, ncols=numcols, figsize=(12,8), sharey=True)
          sns.boxplot(ax=ax1, x="entitytype", y="EPT", data=obj[obj['appVersion']==self.last_release], order=self.objects_to_colors.keys(), palette=self.objects_to_colors.values())
          sns.boxplot(ax=ax2, y="EPT", x="entitytype", data=obj[obj['appVersion']==self.this_release], order=self.objects_to_colors.keys(), palette=self.objects_to_colors.values())
    ...
    #     print(obj_[obj_['appVersion']==last_release].head)
        sns.boxplot(ax=ax1, x="entitytype", y="EPT", data=obj_[obj_['releaseVersion']==self.last_server], order=objects_to_colors.keys(), palette=objects_to_colors.values())
        sns.boxplot(ax=ax2, x="entitytype", y="EPT", data=obj_[obj_['releaseVersion']==self.current_server], order=objects_to_colors.keys(), palette=objects_to_colors.values())
        plt.ylim(0,None)
        ax1.set_title(str(self.last_server) + ' - Wifi')
        ax2.set_title(str(self.current_server) + ' - Wifi')
    ...
    def cmp_aura_flexipage_release_version(self, obj, xaxislimit: int):
      try:
        if 'entitytype' in obj.columns:
          fig, ax = plt.subplots(nrows=len(self.sobjects), ncols=1, figsize=(12,14), sharex=True)
          for i, sobj in enumerate(self.sobjects):
            for release_version in [self.prev_release_version, self.curr_release_version]:

...
def timer(func: staticmethod, *args, cycles=256, **kwargs):
    ...
    n = statistics.stdev(results) * 1e+9 / 2
...
def distribution(func: staticmethod, *args, num_cycles=1000000, post_processor: staticmethod = None, **kwargs):
    ...
    try:
        ...
        ave = statistics.mean(stat_samples)
        median_lo = statistics.median_low(stat_samples)
        median_hi = statistics.median_high(stat_samples)
        median = median_lo if median_lo == median_hi else (median_lo, median_hi)
        std_dev = statistics.stdev(stat_samples, ave)
        output = (

...
class SuperLearner(object):
    ...
    _default_pipelines_shallow = ['extratreesclassifier', 'xgbclassifier', 'randomforestclassifier',
                                  'logisticregression', 'kneighborsclassifier']

    _default_groups = Counter({'boost': 2, 'trees': 2, 'nns': 1, 'linear': 1, 'neighbor': 1})
    _default_groups_map = {'boost': ['lgbmclassifier', 'gradientboostingclassifier',
                                     'xgbclassifier', 'adaboostclassifier'],
                           'trees': ['extratreesclassifier', 'randomforestclassifier',
                                     'decisiontreeclassifier'],
                           'nns': ['deepneuralnetworkclassifier'],
                           'linear': ['linearsvc', 'logisticregression'],
                           'neighbor': ['kneighborsclassifier']}

    def __init__(self, options, metadata, method='slsqp', opt_trials=3, trim_eps=1e-6, num_pipelines=5,
                 shallow=False):
        ...
        if self.classification:
            self._loss = 'nloglike'
            keys = ('name', 'params', 'y_predicted_proba')
        else:
            self._loss = 'l2'
            keys = ('name', 'params', 'y_predicted')

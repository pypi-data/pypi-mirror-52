...
class Plugin(SlideRunnerPlugin.SlideRunnerPlugin):
    ...
    configurationList = list((
                            ...
                            SlideRunnerPlugin.ComboboxPluginConfigurationEntry(uid='source', name='Heatmap shows',options=['Results','Database'])
                            ))
    ...
    def queueWorker(self):
        ...
        while not quitSignal:
            ...
            if not(sourceChanged) and (job.configuration['file'] == oldArchive) and (job.configuration['threshold'] == oldThres) and (job.slideFilename == oldSlide) and np.all(job.coordinates == oldCoordinates):
                continue
            
            if not (os.path.exists(job.configuration['file'])) and (job.configuration['source']==0):
                continue
            ...
            if (sourceChanged or dbfilechanged) and (job.configuration['source']==1) and (len(job.configuration['dbfile'])>0):
                    self.slideObj = openslide.open_slide(job.slideFilename)
            ...
            if ((sourceChanged and job.configuration['source']==0) or (slideChanged) or (thresChanged)) and len(job.configuration['file'])>0:
                pname,fname = os.path.split(job.slideFilename)
            ...
            np.savez('mc.npz',mitoticCount=mitoticCount)

            coords_ds = np.int16(np.array(job.coordinates)/self.ds)
            print('Coords ds: ', coords_ds)
            ...
            print('Center img: ',centerImg.shape)

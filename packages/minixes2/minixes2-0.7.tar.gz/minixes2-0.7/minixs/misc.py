...
from minixs.scanfile import ScanFile
import logging
...
def gen_rects(horizontal_bounds=None, vertical_bounds=None):
  ...
  return [
      ((l,t),(r,b))
      for (l,r) in zip(horizontal_bounds[0:-1], horizontal_bounds[1:])
      for (t,b) in zip(vertical_bounds[0:-1], vertical_bounds[1:])
      ]
...
def build_rixs(spectra, energies):
  ...
  i = 0
  for s,E in zip(spectra, energies):

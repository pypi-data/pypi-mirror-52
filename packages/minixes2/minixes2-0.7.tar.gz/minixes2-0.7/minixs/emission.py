...
import minixs.calibrate
from minixs.exposure import Exposure
from minixs.misc import read_scan_info, gen_file_list
from minixs.constants import *
from minixs.filter import  get_filter_by_name
from minixs.parser import Parser, STRING, INT, FLOAT, LIST
from minixs import METHOD_ENTER_STR
try:
    from itertools import izip as zip     # Python 2 way, from Python 3 can use zip directly
except ImportError:
...
def process_spectrum(cal, exposure, energies, I0, direction, xtals, solid_angle=None, skip_columns=[]):
  ...
  """
  logger.debug(METHOD_ENTER_STR % ((cal, exposure, energies, I0, direction, xtals),))
  ...
  logger.debug("cal shape %s,exposure shape %s" %(cal.shape, exposure.pixels.shape))
  for xtal in xtals:
    (x1,y1), (x2,y2) = xtal
...
def binned_emission_spectrum(calib, exposure, low_energy, high_energy, energy_step, I0):
  ...
  for E,counts in zip(calib.flat,exposure.pixels.flat):
...
def process_all(calibfile, scanfile, base_image, image_nums, E_column=0, I0_column=6, low_cutoff=0, high_cutoff=1000, low_energy=None, high_energy=None, energy_step = 0.5, zero_pad=3):
  ...
  spectra = [ emission_spectrum(calib, e, low_energy, high_energy, energy_step, I0) for e, I0 in zip(exposures, I0s) ]
...
class EmissionSpectrum:
  ...
  def process(self, emission_energies=None, skip_columns=[]):
    calibration = minixs.calibrate.Calibration()
  ...
  def process_binned(self, E1, E2, Estep):
    calib = minixs.calibrate.load(self.calibration_file)

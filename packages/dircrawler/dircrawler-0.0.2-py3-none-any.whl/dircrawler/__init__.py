...
def crawl(
    ...
    *,
    ...
    """
    iterates through directores starting with path and returns (files, dirs) from start_depth(inclusive) to end_depth(exclusive)

    default start_depth is 0
    default end_depth is -1 (till all paths exhausted)
    skip_permission_error when true skips through all directores that requires elevated privilages

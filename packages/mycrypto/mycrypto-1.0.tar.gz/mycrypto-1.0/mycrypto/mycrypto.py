...
def viger_decode(ciphertext,length,key):
	flag=0
	plaintext=''
	for i in ciphertext:
		if i==' ':
...
def viger_encode(plaintext,length,key):
	flag=0
	ciphertext=''
	for i in plaintext:
		if i==' ':
...
def viger_nokey(ciphertext,choice):
	...
	if (choice=='1'):
		...
		for i in range(len(keymap)):
			print(str.format("The key is : {0},and the plaintext is {1}!",keymap[i],viger_decode(Ciphertext,len(keymap[i]),key[i])))

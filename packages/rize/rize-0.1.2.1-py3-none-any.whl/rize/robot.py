...
class robot:
    ...
    def __init__(self, robot_name, pattern = "survey"):
        ...
        if self.middleware == "nanomsg":
            ...
            self.sub_action = self.node.new_callback("rize_robot", "json", self.callback)
    ...
    def isValidRequest(self, action_request):
        try:
            if 'robots' in action_request:
                # If list of robots
                if  (type(action_request['robots']) is list):
                    if self.robot_name in action_request['robots']: 
                        return  True
                    else:
                        return  False
                else: # If only one robot in a string
    ...
    def __listen_request(self):
        ...
        if self.middleware == "nanomsg":
            ...
            success, action_request = self.sub_action.listen()
    ...
    def callback(self, action_request):

        valid_request = self.isValidRequest(action_request)
        if valid_request:
            if action_request["node"] == "cancel":
                self.isCanceled = True
            else: 

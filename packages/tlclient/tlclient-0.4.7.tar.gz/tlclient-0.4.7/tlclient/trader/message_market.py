...
from tlclient.trader.constant import (MODULE_ERR_MSG_LENGTH, MODULE_TICKER_LENGTH,
                             MODULE_TRADINGDAY_LENGTH, ORDER_BOOK_SIZE,
                             AssetType, BarType, ExchangeID, MsgType, OrderType)
...
class MktBasic(ctypes.Structure):
    ...
    def to_influx(self):
        return {
            ...
            "fields": {
                "vol_precision": self.vol_precision
            }
        }
...
class MktSnapAgg(MktSnap):
    ...
    def __init__(self, d):
        ...
        self.ticker = bytify(d['ticker'])
        self.asset_type = AssetType.parse(d['asset_type'])
        self.mkt_time = d['mkt_time']
        self.trading_day = bytify(d['trading_day'])
        for i in range(0, ORDER_BOOK_SIZE):
            self.ask_price[i] = d['ask_prices'][i]
            self.bid_price[i] = d['bid_prices'][i]
            self.ask_volume[i] = d['ask_volumes'][i]
            self.bid_volume[i] = d['bid_volumes'][i]
        self.bid_levels = [MktPriceLevel(dd) for dd in d['bid_levels']]
        self.ask_levels = [MktPriceLevel(dd) for dd in d['ask_levels']]

    def __str__(self):
        return '<snap-{} ask1={}@{} bid1={}@{} al1={} bl1={}>'.format(
            MktBasic.__str__(self),
            self.get_real_vol(self.ask_volume[0]), self.ask_price[0],
            self.get_real_vol(self.bid_volume[0]), self.bid_price[0],

...
from tlclient.linker.constant import MsgType, CommType, FistType, ANY_FIST_NAME
...
class Fist:
    ...
    def join(self):
        while True:
            time.sleep(0.01)
            if self.is_stopped():
                self.logger.info('[main_thread] ended (sig){}'.format(self.received_signal))
                break

    def signal_handler(self, signum=None, frame=None):
        if self.received_signal is None:
            self.logger.debug('[signal_handler] received signal, to stop... (sig){}'.format(signum))

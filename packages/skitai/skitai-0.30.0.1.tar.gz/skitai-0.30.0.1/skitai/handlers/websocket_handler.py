...
class Handler (wsgi_handler.Handler):
	...
	def handle_request (self, request):
		...
		assert (design_spec_ & 31) in (1,5,6), "design_spec  should be one of (WS_CHANNEL, WS_GROUPCHAT, WS_THREADSAFE)"
		headers = [
		...
		if design_spec in (skitai.WS_CHANNEL, 6): # 6 is old WS_THREADSAFE
			varnames = varnames [:1]
			...
			if design_spec == 6 or (design_spec_ & skitai.WS_THREADSAFE == skitai.WS_THREADSAFE):

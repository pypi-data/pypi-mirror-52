...
class MySqlDatabase():
    ...
    def do_list(self, query, return_value=[]):
        data = self.do_fetch(query)
        if len(data) > 0:
            return data
        else:
            return return_value

    def do_list_to_array(self, query, key):
        data = self.do_fetch(query)
        if len(data) > 0:
            new_data = []
            for row in data:
    ...
    def do_obj_key(self, query, key, return_value):
        data = self.do_fetch(query)
        if len(data) > 0:
            return data[0][key]
        else:

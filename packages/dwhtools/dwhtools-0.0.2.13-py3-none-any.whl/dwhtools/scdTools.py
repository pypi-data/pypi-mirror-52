...
class scdManager:
  ...
  def buildCreateTableStatement(self) -> str:
    ...
    skCol = self.getColumnsByScdTyp("SK")[0]
  ...
  def buildFillScdTableStatement(self) -> str:
    ...
    scdZeroColumns = self.getColumnsByScdTyp("0")
    ...
    query.addRow("MERGE INTO " + scd.scdTable + " as target")
  ...
  def getBusinessKeyStringList(self):
    businessKeys = self.getColumnsByScdTyp("BK")
    if not businessKeys:

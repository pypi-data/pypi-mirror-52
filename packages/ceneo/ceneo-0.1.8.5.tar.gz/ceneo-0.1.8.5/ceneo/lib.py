...
def items(q, count=10, session=None):
    ...
    if not res and count < 100 and item_links:
        return items(q, count=100)
    elif not res and retry(q).replace(' ', ''):
...
if __name__ == "__main__":
    if len(sys.argv)>=2:
        ...
        prettyprint=lambda x:print(json.dumps(x,sort_keys=True,indent=4))
        prettyprint(parsed)
    else:
        """sample code"""
        ...
        link = links[0]
        print(links)
#  print(json.dumps(parse(link, session=session), sort_keys=True, indent=4))

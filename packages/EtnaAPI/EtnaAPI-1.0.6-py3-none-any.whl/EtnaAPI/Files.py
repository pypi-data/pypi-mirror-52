...
class Controller(EtnaAPI.BasicController):
    ...
    def delete_file(self, path: str):
        ...
        """
        resp = self.req_session.delete(self.module_api + "/" + self._activity_url + path, headers=self.session.header)
    ...
    def download_file(self, path: str):
        ...
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + "/download/" + path,
    ...
    def list_files(self):
        ...
        :raises EtnaAPI.NotFound:       if the module or activity cannot be found
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + "/files", headers=self.session.header)
        if resp.status_code == 401:
            raise EtnaAPI.AuthorizationRequired("Authorization required")
        if resp.status_code == 403:
    ...
    def list_stage_files(self, stage: str):
        ...
        :raises EtnaAPI.NotFound:       if the module or activity cannot be found
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + f"/stage/{stage}/files",
                                    headers=self.session.header)
        if resp.status_code == 401:
            raise EtnaAPI.AuthorizationRequired("Authorization required")
        if resp.status_code == 403:

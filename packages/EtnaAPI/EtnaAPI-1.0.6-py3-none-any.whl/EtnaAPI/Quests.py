...
class Controller(EtnaAPI.BasicController):
    ...
    def get_stages(self):
        ...
        :return:
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + "/stages", headers=self.session.header)
        ...
        if resp.status_code == 404:
            raise EtnaAPI.NotFound(
                "Unable to get stages for activity with ID {}: access denied".format(self.activity_id))
        return resp.json()

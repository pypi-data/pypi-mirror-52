...
class Controller(EtnaAPI.BasicController):
    ...
    def get_groups(self):
        ...
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + "/groups", headers=self.session.header)
    ...
    def get_unregistered_students(self):
        ...
        :raises EtnaAPI.NotFound:       if the module or activity cannot be found
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + "/unregistered",
    ...
    def get_group_by_id(self, group_id: int):
        ...
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + "/groups" + "/" + str(group_id),
                                    headers=self.session.header)
        if resp.status_code == 401:
            raise EtnaAPI.AuthorizationRequired("Authorization required")
        if resp.status_code == 403:

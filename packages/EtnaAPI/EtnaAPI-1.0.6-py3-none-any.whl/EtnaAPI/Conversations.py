...
class Controller(EtnaAPI.BasicController):
    ...
    def get_circles(self):
        ...
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + "/conversations/circles",
    ...
    def add_conversation_for_activity(self, data):
        ...
        """
        resp = self.req_session.post(self.module_api + "/" + self._activity_url + "/conversations", json=data,
                                     headers=self.session.header)
        if resp.status_code == 401:
            raise EtnaAPI.AuthorizationRequired("Authorization required")
        if resp.status_code == 403:

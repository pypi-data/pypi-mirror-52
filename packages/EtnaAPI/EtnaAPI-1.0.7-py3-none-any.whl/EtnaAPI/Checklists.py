...
class Controller(EtnaAPI.BasicController):
    ...
    def get_checklist(self):
        ...
        """
        resp = self.req_session.get(self.module_api + "/" + self._activity_url + "/checklist",
    ...
    def add_checklist(self, data: dict):
        ...
        """
        resp = self.req_session.post(self.module_api + "/" + self._activity_url + "/checklist", json=data,
                                     headers=self.session.header)
        if resp.status_code == 401:
            raise EtnaAPI.AuthorizationRequired("Authorization required")
        if resp.status_code == 403:

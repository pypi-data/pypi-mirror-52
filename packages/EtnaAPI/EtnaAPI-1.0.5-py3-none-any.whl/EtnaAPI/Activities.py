...
class Controller(EtnaAPI.BasicController):
    ...
    def get_module_activities(self):
        ...
        """
        resp = self.session.get(self.module_api + "/" + self.module_id_str + "/activities", headers=self.session.header)
    ...
    def get_activity(self, activity_id: int):
        ...
        resp = self.session.get(self.module_api + "/" + self.module_id_str + "/activities" + "/" + str(activity_id),
    ...
    def get_student_activities(self, student_login: str):
        resp = self.session.get(self.module_api + "/students" + "/" + student_login + "/currentactivities",
                                headers=self.session.header)
        if resp.status_code == 401:
            raise EtnaAPI.AuthorizationRequired("Authorization required")
        if resp.status_code == 403:
    ...
    def add_activity(self, activity_info):
        ...
        """
        resp = self.session.post(self.module_api + "/" + self.module_id_str + "/activities", json=activity_info,
                                 headers=self.session.header)

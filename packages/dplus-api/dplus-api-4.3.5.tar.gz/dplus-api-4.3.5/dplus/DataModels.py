...
class Model:
    ...
    def _init_from_metadata(self):
        ...
        for val in location_vals:
            self.location_params[val] = Parameter(name=val)
        ...
        for index, param in enumerate(e_params):
            ...
            self.extra_params[param["name"]] = Parameter(value=param["defaultValue"], name=param["name"])
    ...
    def load_from_dictionary(self, json):
        ...
        for param_index in range(len(json.get("ExtraParameters", []))):
            param_name=self.extra_param_index_map[param_index]
        ...
        for layer in layerinfo:
            ...
            for param_index, parameter in enumerate(params):
                if layer["index"] == -1:
                    ...
                    self._default_layer = Parameter(value=layer["defaultValues"][param_index], name=parameter)
                else:
    ...
    def load_from_dictionary(self, json):
        ...
        for layer_index in range(len(json["Parameters"])):
            for param_index in range(len(json["Parameters"][layer_index])):
                param_name=self.layer_param_index_map[param_index]
                param = Parameter(value=json["Parameters"][layer_index][param_index],
                                  ...
                                  name=param_name)
                try:
                    self.layer_params[layer_index][param_name] = param
                except IndexError:

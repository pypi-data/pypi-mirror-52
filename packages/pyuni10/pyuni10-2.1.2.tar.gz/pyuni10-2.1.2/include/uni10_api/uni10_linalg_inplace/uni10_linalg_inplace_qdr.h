#ifndef __UNI10_LINALG_INPLACE_QDR_H__
#define __UNI10_LINALG_INPLACE_QDR_H__

#include "uni10_api/Matrix.h"

namespace uni10{

  /// @ingroup linalg_inplace
  /// @brief Compute the QR decomposition of a matrix.
  /// 
  /// Factor the matrix \c kblk as \c q*d*r, where \c q is orthonormal, \c d is diagonal and \c r is upper-triangular.
  /// @param[in] kblk Matrix to be factored.
  /// @param[out] q A matrix with orthonoraml columns.
  /// @param[out] d The diagonal matrix. 
  /// @param[out] r The upper-triangular matrix.\c d*r is equivalent to the upper triangular matrix in the conventional QR decomposition.
  template<typename UniType>
    void Qdr( const Block<UniType>& kblk, Matrix<UniType>& q, Matrix<UniType>& d, Matrix<UniType>& r, UNI10_INPLACE on );

  template<typename UniType>
    void Qdr( const Block<UniType>& kblk, Matrix<UniType>& q, Matrix<UniType>& d, Matrix<UniType>& r, UNI10_INPLACE on ){

      uni10_error_msg(on != 1, "%s", "Setting a wrong flag of uni10_Inplace." );

      uni10_error_msg(kblk.row_ < kblk.col_, "%s", "Cannot perform QDR decomposition when row_ < col_. Nothing to do." );

      q.Assign(kblk.row_, kblk.col_);
      d.Assign(kblk.col_, kblk.col_, true);
      r.Assign(kblk.col_, kblk.col_);

      linalg_unielem_internal::Qdr(&kblk.elem_, &kblk.diag_, &kblk.row_, &kblk.col_, &q.elem_, &d.elem_, &r.elem_);

    }

}

#endif

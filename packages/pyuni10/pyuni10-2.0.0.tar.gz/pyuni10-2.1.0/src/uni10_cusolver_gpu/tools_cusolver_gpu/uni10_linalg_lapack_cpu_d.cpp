#include "uni10_cusolver_gpu/tools_cusolver_gpu/uni10_linalg_lapack_cpu_d.h"
...
namespace uni10{
  ...
  namespace linalg_driver_internal{
    ...
    double Norm_cpu(double* X, uni10_uint64 N, uni10_int inc){
    ...
    void Transpose_cpu(double* A, uni10_uint64 M, uni10_uint64 N, double* AT){
      for(uni10_uint64 i = 0; i < M; i++)
        for(uni10_uint64 j = 0; j < N; j++)
          AT[j * M + i] = A[i * N + j];
    }

    void Transpose_cpu(double* A, uni10_uint64 M, uni10_uint64 N){
    ...
    void Dagger_cpu(double* A, uni10_uint64 M, uni10_uint64 N, double* AT){
      Transpose_cpu(A, M, N, AT);
    }

    void Dagger_cpu(double* A, uni10_uint64 M, uni10_uint64 N){
      Transpose_cpu(A, M, N);
    }

    void Svd_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* U, double* S, double* vT){
    ...
    void Sdd_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* U, double* S, double* vT){
    ...
    // dorgrq -> ql 
    // dorgql -> rq
    void Qr_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* Q, double* R){

      uni10_error_msg(M < N, "%s", "M must be larger than N in Qr()");
      ...
      //getR
      tools_internal::GetUpTri(Q, R, M, N);
      //getQ
    ...
    void Rq_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* R, double* Q){

      uni10_error_msg(N < M, "%s", "N must be larger than M in Rq()");
    ...
    void Lq_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* L, double* Q){

      uni10_error_msg(N < M, "%s","N must be larger than M in Lq()");
      ...
      //getL
      tools_internal::GetDnTri(Q, L, M, N);
      //getQ
      lwork = (uni10_int)worktestdor;
      double* workdor = (double*)malloc(lwork*sizeof(double));
    ...
    void Ql_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* Q, double* L){

      uni10_error_msg(M < N, "%s", "M must be larger than N in Ql()");
    ...
    void Qdr_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* Q, double* D, double* R){

      uni10_error_msg(M < N, "%s", "M must be larger than N in Qdr()");
      ...
      //getR
      tools_internal::GetUpTri(Q, R, M, N);
      tools_internal::GetDiag(R, D, N, N, N);
    ...
    void QdrColPivot_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* Q, double* D, double* R){
      ...
      Transpose_cpu(Mij_ori, M, N, Mij); //column major + square matrix  // TP(M x M)
      ...
      uni10_error_msg(info != 0, "%s" ,"lack of error");     // D(1 x M): pivoted diagonals
      Transpose_cpu(Mij, M, N, Q);	// Q Here!!!
      free(Mij);
      free(work);
      free(jpvt);
      free(tau);
    ...
    void Ldq_cpu(double* Mij_ori, uni10_int M, uni10_int N, double* L, double* D, double* Q){

      uni10_error_msg(N < M, "%s","N must be larger than M in Ldq()");
      ...
      //get D and L
      tools_internal::GetDnTri(Q, L, M, N);
      tools_internal::GetDiag(L, D, M, M, M);
      for(uni10_int i = 0; i < M; i++)
    ...
    void Inverse_cpu(double* A, uni10_int N){
    ...
    double Det_cpu(double* A, uni10_int N){
    ...
    void Identity_cpu(double* elem, uni10_uint64 M, uni10_uint64 N){
      uni10_uint64 min;
      if(M < N)
        min = M;
      else  
    ...
    void SyTriMatEigDecompose_cpu(double* D, double* E, uni10_int N, 
    ...
    void SyEigDecompose_cpu(double* Kij, uni10_int N, double* Eig, double* EigVec){
    ...
    void DiagMatMulDenseMat_cpu(const double* D, const double* a, uni10_uint64 m, uni10_uint64 n, double* v){
    ...
    void DenseMatMulDiagMat_cpu(const double* a, const double* D, uni10_uint64 m, uni10_uint64 n, double* v){

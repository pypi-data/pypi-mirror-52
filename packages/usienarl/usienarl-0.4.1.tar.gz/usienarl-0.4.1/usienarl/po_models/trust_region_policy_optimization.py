...
class TrustRegionPolicyOptimization(PolicyOptimizationModel):
    ...
    def _define_graph(self):
        ...
        with tensorflow.variable_scope(self._scope + "/" + self._name):
            ...
            if self._agent_action_space_type == SpaceType.discrete:
                ...
                self._old_policies_log_likelihoods_unmasked = tensorflow.placeholder(shape=(None, *self._agent_action_space_shape), dtype=tensorflow.float32, name="old_policy_log_likelihood_unmasked")
                ...
                self._old_policies_expected_values = tensorflow.placeholder(shape=(None, *self._agent_action_space_shape), dtype=tensorflow.float32, name="old_policy_expected_value")
                # Define the old policy std as a placeholder of adaptable shape Nx(A) where (A) is the shape of the action space
                self._old_policies_log_stds = tensorflow.placeholder(shape=(None, *self._agent_action_space_shape), dtype=tensorflow.float32, name="old_policy_log_std")
    ...
    def update(self,
        ...
        if self._agent_action_space_type == SpaceType.discrete:
            ...
            targets = numpy.eye(*self._agent_action_space_shape)[targets_array]

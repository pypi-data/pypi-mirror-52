...
class DocID(object):
            ...
            if self.debug:
                ...
                try:
                    if self.duplicate and len(docids) < topk:
                        r = requests.post(self.url_send, json=pay_load, timeout=10, encoding="utf8")
                        print(r.json())
                        docids.append(docid)
                        time.sleep(0.5)
                    else:
    ...
    def _get_info(self, url, method='GET'):
        try:
            r = requests.request(method, url, timeout=10, encoding="utf8")
            if r.json()['success']:
                return r.json()
        except Exception as e:
            print(format_exc().strip())
    ...
    def _get_newsSummary(self, title, text, maxlen=128):
        try:
            ...
            r = requests.post(url, json=json, timeout=10, encoding="utf8").json()
            print(r)

...
def image_handler(bot, update):
    ...
    chat_id = update.message.chat.id
    log.info('Received image. chat-id: {}. file-id: {}', chat_id, file_id)
    file = bot.get_file(file_id)
    image_uri = '/tmp/{}.jpg'.format(update.message.photo.file_id)
    log.info('Download image: "%s"', image_uri)
...
def process(update, context):
    ...
    if engine.index(namespace, reference, embeddings):
        log.info("%s/%s, %s inserted", namespace, reference, embeddings)
    else:
        log.info("%s/%s, %s updated", namespace, reference, embeddings)
...
def serve(bot_token=None):
    if bot_token is None and "BOT_TOKEN" not in os.environ:
        log.error("Environment variable BOT_TOKEN not defined, bot won't start!")
        return None
    ...
    dp.add_handler(MessageHandler(Filters.forwarded & Filters.photo, image_handler))
    dp.add_handler(MessageHandler(Filters.photo & (~ Filters.forwarded), image_handler))

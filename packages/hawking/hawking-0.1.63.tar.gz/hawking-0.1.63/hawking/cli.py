...
from hawking.common import info, hilite, bold


_HAWKING_BASE_IMAGE = 'hawking-base:0.1'
_HAWKING_CLI_VERSION = '0.1.62'
_BASE_PIP_PACKAGES = 'hawking==' + _HAWKING_CLI_VERSION + ' hawking_proto==0.1.15 faiss-cpu'
...
@click.argument('model_dir')
def bert(model_dir):
    os.system('$(which bert-serving-start) -model_dir={}'.format(model_dir))
...
def caption(doc_ids, server_ip, port):
    ...
    with grpc.insecure_channel('{}:{}'.format(server_ip, port)) as channel:
        ...
        for snippet in response.snippets:
            ...
            record['body'] = snippet.body
            record['metadata'] = snippet.metadata
...
@click.option('--iterator-type', help="Type of entity list iterator: ['txt', 'tsv'].", default='txt')
def batch_index(entity_list, batch_size, doc_count, index_name, dimension, random_index, encoder_config,
                output_dir, iterator_type):
    from hawking.index import batch_index
    filename, ext = os.path.splitext(entity_list)
    if not ext:
        iterator_type = 'txt'
    else:
        if ext == '.txt':
    ...
    batch_index(entity_list, batch_size, doc_count, index_name, dimension, random_index, encoder_config,
                output_dir, iterator_type)
...
@click.option('--encoder-config', help='Encoder config file.', default='/tmp/encoder.json')
def encode(phrases, encoder_config):
...
@click.option('--encoder-config', help='Encoder config file.', default='$HAWKING_ROOT/configs/encoder.image.json')
def encode(uris, encoder_config):
    """ Encode phrase(s) into vector embedding.
    """
    from hawking.encoder import create_encoder
    if not isinstance(encoder_config, dict):
        with open(encoder_config, 'r') as f:
            encoder_config = json.load(f)
    enc = create_encoder(encoder_config)
    for uri in uris:
        v = enc.encode(uri)
        print(v)
        print(v.shape)

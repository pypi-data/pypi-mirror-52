...
class OCLint:
    ...
    def setup_lint_for_package_machine(self):
        ...
        path=''
        os.chdir(path)
        if not os.path.exists(path + '/modules.txt'):
            faillog("have no file named ‘modules.txt’ in current workspace")
            sys.exit()

        with os.open(path + '/modules.txt','r') as module_file:
            print(module_file.read())

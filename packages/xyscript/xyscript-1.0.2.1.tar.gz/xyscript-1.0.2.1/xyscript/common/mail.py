...
class Email():
    ...
    def send_merge_result(self,merge_result):
        ...
        for shell in shell_array:
            # 此处jiraurl需要用替换bug号
            bug_list = self.commit_is_a_bug(shell,merge_result['jiraurl'])
        ...
        for sub in submodule_array:
            ...
            for sub_commit in sub['commit_log']:
                sub_commit_link = self.commit_is_a_bug(sub_commit,merge_result['jiraurl'])
...
if __name__ == "__main__":
    ...
    print(Email().commit_is_a_bug('T_ASR-14997_【ios】【司机端&乘客端】【IM】【ATE】发送超长文案时切换到发送语音，格式错乱，切换回文字后，键盘盖住文案 T_ASR-14992_【ios】【司机端&乘客端】【IM】【ATE】IM发送图片手动放大查看，图片比例失调','http://jira.saicmobility.com:8080/browse/?filter=-1'))

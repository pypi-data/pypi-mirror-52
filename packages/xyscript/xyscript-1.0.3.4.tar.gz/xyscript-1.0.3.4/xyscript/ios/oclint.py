...
class OCLint:
    ...
    def run_oclint(self,git_objc=None):
        try:
            ...
            # path = '/Users/v-sunweiwei/Desktop/saic/sample-oc-lint'
    ...
    def setup_lint_for_package_machine(self):
        ...
        with open(path + '/modules.json','r') as module_file:
            ...
            for module_name,module_url in json_data.items():
                printandresult('git clone ' + module_url)
                os.chdir(path + '/' + module_name)
                Package().change_branch('zuche-Develop')
                
                gitlab_ci_path = path + '/' + module_name + '/.gitlab-ci.yml'
...
script: \n \
    - xyscript diagnose -t $branch_url -b $CI_COMMIT_REF_NAME -o $CI_COMMIT_MESSAGE -a $commit_url -d $CI_COMMIT_SHA -c test@test.com' % (path,module_name)
                    with open(gitlab_ci_path,'w+',encoding="utf-8") as ci_file:
                        ci_file.write(ci_str)
                    
                    repo = Repo(path + '/' + module_name)
                    repo.git.add('.')
                    repo.git.commit('-m','add ci file')
...
def main():
    ...
    # print(OCLint().get_result_html_file('/Users/v-sunweiwei/Desktop/saic/ios-shell-passenger/build/reports/report.json'))
...
if __name__ == "__main__":
    main()
    # p = re.compile(r'(.((\d+):(\d+):).)')
    # # line_content = '/Users/v-sunweiwei/Desktop/saic/ios-shell-passenger/Pods/Headers/Public/SCStyle_libs/SCStyle.h:104:1: multiple declarations of method \'green\' found and ignored'
    # line_content = '/Users/v-sunweiwei/Desktop/saic/ios-shell-passenger/Pods/Headers/Public/SCShare/SCSocialPlatformConfig.h:14:17: pointer is missing a nullability type specifier (_Nonnull, _Nullable, or _Null_unspecified)'

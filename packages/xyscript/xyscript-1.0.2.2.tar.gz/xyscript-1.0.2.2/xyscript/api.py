...
def merge(*parameters):
    ...
    合并代码
    """
    global DEV_BRANCH, PACKAGE_BRANCH, NOTIFICATION_EMAILS,JIRA_URL
    Package().merge_code(code_branch=DEV_BRANCH, package_branch=PACKAGE_BRANCH,email=NOTIFICATION_EMAILS,url=JIRA_URL)
...
def main(prog=None,args=None):
    global PROJECT_ADDRESS, PROJECT_BRANCH, TEST_PLATFORM, NET_ENV, PROJECT_VERSION, PROJECT_BUILD,\
           ...
           CERT_PASSWORD, WITHOUT_POD_INSTALL,JIRA_URL
    ...
    longargs = ['address=', 'platform=', 'branch=', 'environment=', 'version=', 'build=', 'email=', 'log', 'package=','code=','tag=','order=','cache=','password=','no-pod','link='] #长选项模式
    try:
        try:

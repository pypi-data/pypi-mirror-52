...
class Package:
    ...
    def merge_code(self,code_branch,package_branch,email,url=None):
        """
        合并代码(开发分支名、打包分支名)
        """
        if code_branch == None:
        ...
        try:
            ...
            if email is not None:
                GitLab().get_all_commit_between_push(code_branch=code_branch,package_branch=package_branch,workspace=currentPath,mail=email,url=url)

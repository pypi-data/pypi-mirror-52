...
# noinspection PyProtectedMember
from azureml.widgets._constants import WEB_WORKBENCH_PARENT_RUN_ENDPOINT
...
class _UserRunDetails(_WidgetRunDetailsBase):
    ...
    _incomplete_child_runs_unmerged_metrics_cache = {}
    _parent_run_unmerged_metrics_cache = []
    ...
    def get_widget_data(self, widget_settings=None):
        ...
        if self.run_properties['properties'].get('pipeline_graph_version') == '1.0.0':
            ...
            self.run_instance.download_file(filename, path)
            with open(filename) as f:
                graph = json.load(f)['data']
            ...
            if not_completed_runs:
                ...
                for run_id, metrics in metrics.items():
                    if run_id in completed_run_ids:
                        _UserRunDetails._incomplete_child_runs_unmerged_metrics_cache.pop(run_id)
                        metrics_to_be_cached[run_id] = metrics
    ...
    def _get_run_metrics(self):
        ...
        _UserRunDetails._parent_run_unmerged_metrics_cache.extend(unmerged_run_metrics)

        # merge metrics
        run_metrics = copy.deepcopy(_UserRunDetails._parent_run_unmerged_metrics_cache)
    ...
    def _get_metrics(self, child_runs):
        ...
        for run_id, metric_dtos in unmerged_metrics.items():
            if run_id not in _UserRunDetails._incomplete_child_runs_unmerged_metrics_cache:
                _UserRunDetails._incomplete_child_runs_unmerged_metrics_cache[run_id] = metric_dtos
            else:
                _UserRunDetails._incomplete_child_runs_unmerged_metrics_cache[run_id].extend(metric_dtos)

        # merge those metrics
        merged_metrics = copy.deepcopy(unmerged_metrics)

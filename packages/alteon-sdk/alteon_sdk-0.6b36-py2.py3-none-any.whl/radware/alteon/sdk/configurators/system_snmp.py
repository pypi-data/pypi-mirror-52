...
SystemSNMPParameters.__annotations__ = {
...
class SystemSNMPConfigurator(AlteonConfigurator):
    ...
    def _read(self, parameters: SystemSNMPParameters) -> SystemSNMPParameters:
        def _read_sys_props():
            for k, v in system_params.items():
                setattr(parameters, v, json.loads(self._rest.read_data_object(k))[k])

        self._read_device_beans(parameters)
        if self._beans:
            _read_sys_props()
            parameters.authentication_failure_traps = EnumSnmpEnableAuthenTraps.enum(
                parameters.authentication_failure_traps)
            return parameters

    def _update(self, parameters: SystemSNMPParameters, dry_run: bool) -> str:
        def _write_sys_props():
            for k, v in system_params.items():
                value = getattr(parameters, v)
                if value is not None:
                    if k == 'snmpEnableAuthenTraps':
                        value = self._enum_to_int(EnumSnmpEnableAuthenTraps, value)
                    self._rest.update_data_object('', json.dumps({k: value}))
    ...
    def dry_run_delete_procedure(self, diff):
        ...
        dry_run_procedure.non_removable_bean_map_attrs_list.append(system_params)
        dry_run_procedure.ignore_prop_by_value = delete_values
        return dry_run_procedure


SystemSNMPConfigurator.__annotations__ = {
    'parameters_class': SystemSNMPParameters
}

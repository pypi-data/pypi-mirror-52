...
class FVLayer(GMMLayer):
	...
	def encode(self, x, eps=1e-6):
		gamma = self.soft_assignment(x)
		_x, *params = self._expand_params(x)

		_gamma = self._gamma = self.xp.expand_dims(gamma.array, axis=2)
		...
		_std = F.sqrt(_sig)
		_x_mu_sig = self._x_mu_sig = (_x - _mu) / _std

		G_mu = F.mean(_gamma * _x_mu_sig, axis=1)
		G_sig = F.mean(_gamma * (_x_mu_sig**2 - 1), axis=1)

...
from auquan_qq14_toolbox.problem2_data_loader import CsvDataSource
from auquan_qq14_toolbox.problem2_execution_system import Problem2ExecutionSystem
from auquan_qq14_toolbox.problem2_time_rule import DayTimeRule
...
class TrainingPredictionFeature(Feature):
    ...
    def computeForInstrument(cls, updateNum, time, featureParams, featureKey, instrumentManager):
        ...
        predictions = tf.getPrediction(time, updateNum, lookbackInstrumentFeatures, lookbackMarketFeatures, predictions, targetVariable)

        if (predictions > 1).any() or (predictions < 0).any():
            logError('Predictions can only take values between 0 and 1' +
                    '\n' + 'Code will exit!!!')
            print(predictions)
...
class AUC_ROC_Calculator(Feature):
    ...
    def computeForInstrument(cls, updateNum, time, featureParams, featureKey, instrumentManager):
        ...
        # temp = instrumentLookbackData.getFeatureDf('Accounts Payable')
        # print('temp: %s'%temp)
...
class LogLossCalculator(Feature):
    ...
    def computeForMarket(cls, updateNum, time, featureParams, featureKey, currentMarketFeatures, instrumentManager):
        lookbackInstrumentFeatures = instrumentManager.getLookbackInstrumentFeatures()
        auc_roc_scores = lookbackInstrumentFeatures.getFeatureDf('log_loss').iloc[-1,:]
        mean_scores = np.nanmean(auc_roc_scores)

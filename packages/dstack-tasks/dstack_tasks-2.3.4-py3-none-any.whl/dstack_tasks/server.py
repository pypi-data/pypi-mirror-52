...
@task
def configure(ctx, name, username='ubuntu', user_id=1000, port=22, compose_version=None):
    do(ctx, f'scp ./server/setup.sh {name}:/root/setup.sh')
    do(ctx, f'ssh {name} -- "./setup.sh {username} {user_id} {port} {compose_version}"')
    # do(ctx, f'')

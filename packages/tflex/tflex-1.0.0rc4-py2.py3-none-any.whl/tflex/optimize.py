...
def find_father(node_name):
    if global_father_dict.get(node_name) is None:
        ...
        for input_name in global_node_dict[node_name].input:
            ...
            global_father_dict[node_name] |= find_father(this_input_name)
...
def refresh_dict(input_graph_def):
    global global_node_dict, global_next_node_dict, global_son_dict, global_father_dict
    global_node_dict, global_next_node_dict, global_son_dict, global_father_dict = {}, {}, {}, {}
    ...
    use_node_names = set()
    for node_name in global_output_node_names:
...
def pre_and_post(func):
    def wrapper(*args, **kwargs):
        ...
        if global_step is None or func_name in global_step:
            ...
            return init_res
        print(func_name, "don't need to run")
        return input_graph
...
def optimize_for_inference(input_graph_def, input_node_names, output_node_names, placeholder_type_enum=None,
    ...
    global_step = set()
    step_names = ['base_optimize', 'BN_optimize', 'EPU_core_optimize', 'EPU_advanced_optimize','EPU_make_op_async_optimize']
...
def replace_depthwise_to_conv2d(input_graph_def):
    ...
    for n in global_node_dict.values():
        ...
        if n.op != 'DepthwiseConv2dNative':
            continue
        #print('DepthwiseConv2dNative is getting converted')
...
def replace_avgpool_to_conv2d(input_graph_def):
    ...
    for avgpool_node in global_node_dict.values():
        ...
        #print('avgpool_node input_channel =', avgpool_node.name, avgpool_node.input, input_channel)
        ...
        # print ("avgpool_node.attr['strides'] =",avgpool_node.attr['strides'])
        # print ("avgpool_node.attr['strides'] =",type(avgpool_node.attr['strides'].list),avgpool_node.attr['strides'].list)
        # print ("avgpool_node.attr['strides'] =",type(avgpool_node.attr['strides'].list.i),avgpool_node.attr['strides'].list.i)
        # print ("avgpool_node.attr['strides'] =",[each for each in avgpool_node.attr['strides'].list.i])
        # print ("avgpool_node.attr['strides'] =",type([each for each in avgpool_node.attr['strides'].list.i][0]))
        # print ("avgpool_node.attr['strides'] =",type(avgpool_node.attr['strides'].list.i[0]))
        # print ("avgpool_node.attr['T'] =",avgpool_node.attr['T'],avgpool_node.attr['T'].type,type(avgpool_node.attr['T'].type))
        # print ("avgpool_node.attr['data_format'] =",avgpool_node.attr['data_format'])
        # print ("avgpool_node.attr['padding'] =",avgpool_node.attr['padding'].s,type(avgpool_node.attr['padding'].s))
        # print ("avgpool_node.attr['ksize'] =",avgpool_node.attr['ksize'])
        # print ('-'*120)
        each_weight_value = 1.0 / (ksize[1] * ksize[2])
        #print('each_weight_value =', each_weight_value)
    ...
    while flag:
        ...
        for concat_node in global_node_dict.values():
            if concat_node.op[:6] != 'Concat':
                continue
            node_extend = []
            print('replace start', '*' * 120)
            print(concat_node.op, concat_node.name, global_next_node_dict[concat_node.name])
            for name in concat_node.input:
                print('input_name =', name)
                node = global_node_dict[name]
                print('input =', node.op, node.name, use_bfs_find_input_channel(name, flag=True))
            ...
            if False in [next_node.op.__contains__('Conv2D') for next_node in next_nodes]:
                continue
            print('input_infos =', sum([info[1] for info in input_infos]), input_infos)
            for next_node in next_nodes:
                print('-' * 120)
                print('init next_node =', next_node.op, next_node.name)
                weight_node = node_from_map(global_node_dict, next_node.input[1])
                print(weight_node.attr['dtype'])
                weight_value = values_from_const(weight_node)
                print('weight_value =', weight_value.dtype, weight_value.shape, sum([info[1] for info in input_infos]))
                start = 0
                ...
                for i in range(len(input_infos)):
                    ...
                    print(i, start, input_infos[i][1], copy_value.shape)
                ...
                for name in global_next_node_dict[next_node.name]:
                    this_node = global_node_dict[name]
                    print('skip before', this_node.op, this_node.name, this_node.input)
                skip_one_node(next_node.name, conv2depu_nodes[-1])
                insert_map_info(conv2depu_nodes[-1],node_extend[-1].op,next_node.name,next_node.op,'replace_concat_v1')
                for name in global_next_node_dict[next_node.name]:
                    this_node = global_node_dict[name]
                    print('skip end', this_node.op, this_node.name, this_node.input)
                print('-' * 120)
                print('zzz node_extend =', concat_node.name, len(node_extend), len(input_infos), len(next_nodes))
            input_graph_def.node.extend(node_extend)
            input_graph_def = refresh_dict(input_graph_def)
            flag = True
            break
...
def add_to_bias(input_graph_def):
    for add_node in global_node_dict.values():
        if add_node.op != 'Add':
            continue
        one_node, two_node = [global_node_dict[add_node.input[i]] for i in range(2)]
...
def clear_map(input_graph_def):
    for name in list(new_2_old_map.keys()):
        if global_node_dict.get(name) is None:
            # print ("new graph doesn't have",name)
    ...
    # res = list(new_2_old_map.keys())
    print(len(new_2_old_map), len(res))
    f = open('map.txt', 'w')
    for key in res:
        f.write(key + ',' + new_2_old_map[key][2] + ',' + new_2_old_map[key][3] + '\n')
        print(key, '\t', new_2_old_map[key])

    return input_graph_def


def use_bfs_find_input_channel(name, flag=False):
    node_names = [name]
...
def add_conv2D_before_BN(input_graph_def):
    ...
    for node in global_node_dict.values():
        ...
        if fore_node.op in ["Conv2D", "DepthwiseConv2dNative"]:
            continue
        # #print ('add_conv2D_before_BN',fore_node.op,node.name)
        ...
        # conv2D_weight_node = create_node_Const(node.name + '_fore_Conv2D_weight', conv2D_weight_value, np.float32,
        #                                        'DT_FLOAT')
...
def standardize_batchNorm(input_graph_def):
    ...
    for node in input_graph_def.node:
        if node.op != 'Add':
            continue
        if global_node_dict[node.input[0]].op != 'Mul':
            continue
        sub_beta_node = global_node_dict[node.input[1]]
        if sub_beta_node.op != 'Sub':
            continue
        mul_mean_node = global_node_dict[sub_beta_node.input[1]]
        if mul_mean_node.op != 'Mul':
            print('standardize_batchNorm', 'mul_mean_node=', mul_mean_node.op, mul_mean_node.name, mul_mean_node.input)
            continue
        mul_gama_node = global_node_dict[mul_mean_node.input[1]]
        ...
        if mul_gama_node.op == 'Rsqrt':
            ...
            rsqrt_node = global_node_dict[mul_mean_node.input[1]]
            mul_gama_node = None
        else:
            rsqrt_node = global_node_dict[mul_gama_node.input[0]]
        if rsqrt_node.op != 'Rsqrt':
            print('standardize_batchNorm', 'rsqrt_node=', rsqrt_node.op, rsqrt_node.name, rsqrt_node.input)
            continue
        add_var_node = global_node_dict[rsqrt_node.input[0]]
...
@pre_and_post
def append_identity_output(input_graph_def, output_node_names, op_type=['BiasAdd', 'Conv2D', 'Relu', 'Add']):
    ...
    nodes_to_extend = []
    for node in input_graph_def.node:
        if node.name in output_node_names and node.op in op_type:
            new_node = node_def_pb2.NodeDef()
...
def fp32_to_fp16(optimized_graph_def):
    ...
    # print('device_node_names =', len(device_node_names))
    first_device_nodes = []
    last_device_nodes = []
    ...
    # print('not_32_to_16_nodes', not_32_to_16_nodes)
    ...
    for node_name in inner_nodes:
        ...
        for input_name in this_node.input:
            if input_name in outer_nodes:
                ...
                # #print('32 to 16', input_node.op, input_node.name)
    ...
    for node_name in outer_nodes:
        ...
        for input_name in this_node.input:
            if input_name in inner_nodes:
                input_node = node_from_map(global_node_dict, input_name)
                # print('16 to 32', input_node.op, input_node.name)
...
def modify_pad_maxpool(input_graph_def, graph_type,strict_padding = False):
    ...
    for node in input_graph_def.node:
        if node.op == 'Pad' and any([node.name in global_node_dict[n].input for n in first_conv2d_name_list]):
            ...
            if strict_padding and padding != b'SAME':
                tf_logging.warning(
                    "EPU MaxPool only supports the padding SAME, but found MaxPool node: %s padding=%s" % (
            ...
            if strides[1] != strides[2] or strides[1] not in [2]:
                tf_logging.warning(
                    "EPU MaxPool only supports the stride 2*2, but found MaxPool node: %s stride=%s" % (
...
def pad_slice_conv2d2(input_graph_def, ck_multiplier=32):
    ...
    for node in input_graph_def.node:
        ...
        if K_mod != 0:
            ...
            print("conv2d_dict['last'] =", conv2d_dict['last'])
...
def fold_conv2d_bias_relu(input_graph_def):
    ...
    for conv2d_node in input_graph_def.node:
        ...
        if strides1 != strides2:
            tf_logging.warning(
                "EPU Conv2D only supports the same stride in H&W, but found Conv2D node: %s stride=%s" % (conv2d_node.name, str([strides1,strides2])))
            continue
        ...
        if len(next_names) != 1:
            ...
            if relu_node.op in ['Relu']:
                ...
                if relu_node.op in ['LeakyRelu','Relu6']:
                    tf_logging.warning("EPU Activation only supports Relu, but found %s node: %s" % (relu_node.op,relu_node.name))
                conv2d_node.attr['relu'].CopyFrom(attr_value_pb2.AttrValue(b=False))

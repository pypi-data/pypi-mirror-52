...
from foxyproxy.csp import BaseCryptoService, TokenRecord
...
class ClientThread(Process):
    ...
    MAX_COUNTER_RUNS = 500  # maximum number of requests to process
    ...
    def __init__(self, signer, queue_in, queue_out, queue_ack, queue_rest, queue_internal, _id, mylock=None):
        ...
        self.sync_back = None  # type: ClientResult,None
    ...
    @classmethod
    def create_new_worker(cls, signer, name, work_q, result_q, ack_q, ack_rest, internal_q, index, mylock=None):
        """
        A simple creation of a new worker process
        :param signer:
        :param name:
        :param work_q:
        :param result_q:
        ...
        new_client = ClientThread(signer, work_q, result_q, ack_q, ack_rest, internal_q, index, mylock)
    ...
    @classmethod
    def restart_worker(cls, signer, name, work_q, result_q, ack_q, ack_rest, index):
        """
        Kills a worker and starts a new instance
        :param signer:
        :param name:
        :param work_q:
        :param result_q:
            ...
            if len(work.req_commands) > 0:
                if work.req_commands[0]['name'] in ['CHAIN', 'SIGN']:
                    ...
                    if (len(_name) == 1) and (_name[0].reader in name_map):  # get  record and extract the reader name
                        ...
                        logging.error("Name transformation failed, %s not found" % work.real_reader_name)
            if not set_new_name:
                work.real_reader_name = None  # make sure it's None
                reader_index = 0
        return reader_index

    def serve_work_queue(self, stop, internal_jobs):
        """
        A simple method / thread that will check work queue and store jobs
        :return:
        """

        while not stop():  # safe stopping of the feeder
    ...
    def run(self):
        ...
        while keep_running:
            # noinspection PyBroadException
            try:
                new_work = self.internal_jobs.get()  # type: ClientWork
                self.run_counter += 1
                if new_work is not None:  # type: ClientWork
                    if new_work.command == ClientWork.CMD_KILL:
                        keep_running = False
                        ...
                        try:
                            ...
                            _data[1] = int(_data[1])
                            new_list = self.signer.prioritize(_data[0], _data[1])
                ...
                if time.time() - self.start_time > ClientThread.MAX_UPTIME:
                    keep_running = False

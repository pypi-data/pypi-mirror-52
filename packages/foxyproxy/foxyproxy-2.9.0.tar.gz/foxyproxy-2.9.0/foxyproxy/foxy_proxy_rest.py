                        ...
                        while not confirmed:
                            ...
                            try:
                                ...
                                if ack_cmd is not None:
                                    ...
                                    try:
                                        if not FoxyProxyREST.WORKERS.workers[reader_index].worker.is_alive():
                    ...
                    try:
                        ...
                        for key, value in self.signer.storage.certificates.items():  # type: str, ReaderRecord
                            if key in result.priority_dict:
                                ...
                                logging.error("No key in certificats storage %s" % key)
                        if found == returned_results:
                            logging.error("REST select - updated error; in: %s, set: %s" %

        ...
        def terminal2():
            ...
            try:
                ...
                if ("name" in request_in) and (len(request_in['name'].strip()) > 0):
                    new_list, priorities, last_priority = self.signer.prioritize(request_in['name'].strip(), _limit)
                    self.signer.storage.last_priority = last_priority
                    self.signer.storage.max_enum_readers = _limit
                    for key, value in self.signer.storage.certificates:  # type: str, ReaderRecord
                        if key in priorities:
                            value.priority = priorities[key]
                            logging.debug("Updating priority for reader %s to %d (rest)" % (key, priorities[key]))

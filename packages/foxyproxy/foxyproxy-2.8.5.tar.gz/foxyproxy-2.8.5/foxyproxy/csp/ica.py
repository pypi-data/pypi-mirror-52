...
class CryptoICA(BaseCryptoService):
        ...
        if response_data is None:
            ...
            while keep_trying:
                ...
                if response_data is None:
                    ...
                    response_data = BaseCryptoService.check_response(response_all, reader.reader,
                                                                     "Signing unsuccessful id=%d" % temp_file_id, error=0)
    ...
    def _read_certificates(self, session):
        ...
        for reader in self.storage.certificates.keys():
            ...
            processThread.start()
            list_of_threads.append(processThread)

        threads = len(list_of_threads)
        while threads > 0:
        ...
        for _item in list_of_threads:
            logging.debug("Joining thread - card processing %s" % _item.name)
            _item.join()
            logging.debug("Joined thread - card processing %s" % _item.name)
    ...
    @staticmethod
    def _process_new_card(reader, storage, server, queue):

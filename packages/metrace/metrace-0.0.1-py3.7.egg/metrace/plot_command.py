...
import os
import glob
from metrace.static import plotly_js_string
import webbrowser
...
def generate_figure(json_data, field, title):
    ...
    color_pallette_traces = [
        ...
        "rgba(170, 110, 40, 130)",
        "rgba(240, 50, 230, 130)",
        "rgba(255, 250, 200, 130)",
        "rgba(170, 255, 195, 130)",
        "rgba(210, 245, 60, 130)",
        "rgba(250, 190, 190, 130)",
        "rgba(0, 128, 128, 130)",
        "rgba(230, 190, 255, 130)",
    ]
    color_pallette_pid = [
        "rgb(158,1,66)",
        "rgb(213,62,79)",
    ...
    for pid, data in consumption_over_time.items():
        ...
        traces.append(
            go.Scatter(
                ...
                hovertemplate="Memory : %{text}",
            )
        )
    max_used = int(max_used * 1.1)
    trace_radius = max_used / 30
        ...
        for name, array in annotations[PID].items():
            ...
            for d in array:
                if d["type"] == "begin":
                    ...
                    x.extend(
                        [
                            tmp["x"] - timedelta(seconds=0.000001),
                            tmp["x"],
                            d["x"],
                            d["x"] + timedelta(seconds=0.000001),
                        ]
                    )
                    y_low.extend(
                        [

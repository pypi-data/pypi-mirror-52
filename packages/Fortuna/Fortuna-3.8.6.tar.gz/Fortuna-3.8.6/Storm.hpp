...
namespace Storm {  // Version 3.2.0
    ...
    namespace GearBox {
        ...
        auto analytic_continuation(Func && func, std::int64_t input, std::int64_t offset) -> std::int64_t {
            if (input < 0) return -func(-std::max(input, -std::numeric_limits<std::int64_t>::max())) + offset;
            if (input > 0) return func(input);
            return offset;
        }
        auto smart_clamp(std::int64_t a, std::int64_t b, std::int64_t c) -> std::int64_t {
            return std::clamp(a, std::min(b, c), std::max(c, b));
        }
        template <typename Func>
        auto approximation_clamp(Func && approximation, std::int64_t target, std::int64_t upper_bound) -> std::int64_t {
    ...
    auto uniform_int_variate(std::int64_t a, std::int64_t b) -> std::int64_t {
        std::uniform_int_distribution<std::int64_t> distribution { std::min(a, b), std::max(b, a) };
        return distribution(Engine::Hurricane);
    }
    ...
    auto binomial_variate(std::int64_t number_of_trials, double probability) -> std::int64_t {
        std::binomial_distribution<std::int64_t> distribution {
            std::max(number_of_trials, std::int64_t(1)),
            std::clamp(probability, 0.0, 1.0)
        };
        return distribution(Engine::Hurricane);
    }

    auto negative_binomial_variate(std::int64_t number_of_trials, double probability) -> std::int64_t {
        std::negative_binomial_distribution<std::int64_t> distribution {
            std::max(number_of_trials, std::int64_t(1)),
            std::clamp(probability, 0.0, 1.0)
        };
        return distribution(Engine::Hurricane);
    ...
    auto geometric_variate(double probability) -> std::int64_t {
        std::geometric_distribution<std::int64_t> distribution { std::clamp(probability, 0.0, 1.0) };
        return distribution(Engine::Hurricane);
    }

    auto poisson_variate(double mean) -> std::int64_t {
        std::poisson_distribution<std::int64_t> distribution { mean };
        return distribution(Engine::Hurricane);
    }
    ...
    auto random_below(std::int64_t number) -> std::int64_t {
        return Storm::uniform_int_variate(0, std::nextafter(number, 0));
    }

    auto random_range(std::int64_t start, std::int64_t stop, std::int64_t step) -> std::int64_t {
    ...
    auto d(std::int64_t sides) -> std::int64_t {
        if (sides > 0) {
            std::uniform_int_distribution<std::int64_t> distribution {1, sides};
            return distribution(Engine::Hurricane);
        }
        return GearBox::analytic_continuation(Storm::d, sides, 0);
    }

    auto dice(std::int64_t rolls, std::int64_t sides) -> std::int64_t {
        if (rolls > 0) {
            auto total {0};
     ...
     auto ability_dice(std::int64_t number) -> std::int64_t {
        const std::int64_t num { std::clamp(number, std::int64_t(3), std::int64_t(9)) };
        if (num == 3) return Storm::dice(3, 6);
        std::vector<std::int64_t> the_rolls(num);
    ...
    auto plus_or_minus(std::int64_t number) -> std::int64_t {
        return Storm::uniform_int_variate(-number, number);
    }

    auto plus_or_minus_linear(std::int64_t number) -> std::int64_t {
        const auto num { std::abs(number) };
        return Storm::dice(std::int64_t(2), num + 1) - (num + 2);
    }

    auto plus_or_minus_gauss(std::int64_t number) -> std::int64_t {
        static const auto PI { 4 * std::atan(1) };
        const auto num { std::abs(number) };
        const auto result { std::int64_t(std::round(Storm::normal_variate(0.0, num / PI))) };
        if (result >= -num and result <= num) return result;
        return Storm::plus_or_minus_linear(num);
    }

    auto random_index(std::int64_t number) -> std::int64_t {
        return GearBox::analytic_continuation(Storm::random_below, number, -1);
    }

    auto back_linear(std::int64_t) -> std::int64_t;
    auto front_linear(std::int64_t number) -> std::int64_t {
        if (number > 0) {
    ...
    auto back_linear(std::int64_t number) -> std::int64_t {
        if (number > 0) {
    ...
    auto middle_linear(std::int64_t number) -> std::int64_t {
        if (number > 0) {
    ...
    auto quantum_linear(std::int64_t number) -> std::int64_t {
    ...
    auto back_gauss(std::int64_t) -> std::int64_t;
    auto front_gauss(std::int64_t number) -> std::int64_t {
        if (number > 0) {
            const auto result { std::int64_t(std::floor(Storm::gamma_variate(1.0, number / 10.0))) };
    ...
    auto middle_gauss(std::int64_t number) -> std::int64_t {
        if (number > 0) {
            const auto result { std::int64_t(std::floor(Storm::normal_variate(number / 2.0, number / 10.0))) };
    ...
    auto back_poisson(std::int64_t) -> std::int64_t;
    auto front_poisson(std::int64_t number) -> std::int64_t {
        if (number > 0) {
    ...
    auto quantum_monty(std::int64_t number) -> std::int64_t {
        const auto rand_num { Storm::d(3) };

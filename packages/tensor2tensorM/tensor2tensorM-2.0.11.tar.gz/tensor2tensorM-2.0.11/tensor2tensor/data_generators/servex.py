...
class servex(speech_recognition.SpeechRecognitionProblem):
...
@registry.register_problem()
class servexClean(servex):
  """Problem to train on full 960h, but evaluate on clean data only."""

  def training_filepaths(self, data_dir, num_shards, shuffled):
    return servex.training_filepaths(self, data_dir, num_shards, shuffled)

  def dev_filepaths(self, data_dir, num_shards, shuffled):
    return servexClean.dev_filepaths(self, data_dir, num_shards, shuffled)

  def test_filepaths(self, data_dir, num_shards, shuffled):
    return servexClean.test_filepaths(self, data_dir, num_shards, shuffled)
...
def add_sevex_hparams(hparams):
  ...
  hparams.learning_rate = 0.07
  hparams.train_steps = 5000000
  hparams.num_hidden_layers = 4
  return hparams


def set_servex_length_hparams(hparams):

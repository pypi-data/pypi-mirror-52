...
class JSONRPCDecoder(Decoder):
    ...
    def decode(self, encoded_value, *args, **kwargs) -> dict:
        try:
            ...
            obj_jsonrpc = json.loads(encoded_value)
            jsonschema.validate(obj_jsonrpc.copy(), self.validation_schema)
            return obj_jsonrpc
        except Exception as e:
            raise ValueError('JSONRPC Decode error. %s' % str(e))
...
def json_serializable(value) -> bool:

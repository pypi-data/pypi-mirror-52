...
class Picture:
    ...
    def MatchThePictureRegion(self,matchedPicture,region,confidence = 0.6):
        ...
        re = img.crop(region)
        re.save(os.path.join(BASE_DIR + "\\resource\\Capture", 'CurrentScreen_region.png'))
        imsrc = ac.imread(os.path.join(BASE_DIR + "\\resource\\Capture", 'CurrentScreen_region.png'))
        imobj = ac.imread(matchedPicture)
        match_result = ac.find_template(imsrc, imobj, confidence)
        if match_result is not None:
            match_result['shape'] = (imsrc.shape[1], imsrc.shape[0])  # 0为高，1为宽
    ...
    def MatchTheTaskPage(self,task):
        ...
        for k, v in task.items():
            pagePic = v.get("pic")
            pagePicUrl = os.path.join(BASE_DIR, "resource", "Page", str(pagePic))

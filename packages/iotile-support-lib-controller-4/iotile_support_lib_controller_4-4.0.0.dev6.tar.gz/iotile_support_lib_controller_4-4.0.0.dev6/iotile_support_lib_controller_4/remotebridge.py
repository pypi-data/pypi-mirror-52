...
class RemoteBridgePlugin(TileBusProxyPlugin):
    ...
    @param("firmware", "path", "readable", desc="Firmware file to reflash with")
    @param("enhanced", "bool", desc="Flag to use enhanced reflash controller record")
    @param("auto_update", "bool", desc="Flag to auto update bootloader and softdevice if needed.")
    def reflash_controller(self, firmware, enhanced=False, auto_update=False):
    ...
    def _check_required_updates(self, firmware, auto_update):
        ...
        if controller_info['major_version'] < RemoteBridgePlugin.SD_UPGRADE_VERSION \
            and firmware_info.get_firmware_version()[0] >= RemoteBridgePlugin.SD_UPGRADE_VERSION:
            if auto_update is False:
                ...
                return True
        elif controller_info['major_version'] >= RemoteBridgePlugin.SD_UPGRADE_VERSION \
            and firmware_info.get_firmware_version()[0] < RemoteBridgePlugin.SD_UPGRADE_VERSION:
            raise ArgumentError("Firmware downgrading not allowed")

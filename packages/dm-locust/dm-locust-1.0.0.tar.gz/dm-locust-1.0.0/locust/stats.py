...
def requests_csv():
    ...
    for s in chain(sort_stats(runners.locust_runner.request_stats), [runners.locust_runner.stats.total]):
        rows.append('"%s","%s",%i,%i,%i,%i,%i,%i,%.2f' % (
            s.method,
            s.name,
            s.num_requests,
            s.num_failures,
...
def report_xls():
    ...
    requests = workbook.add_worksheet('请求统计(%s秒)' % str(web.duration_time))
    rt = workbook.add_worksheet('响应时间分布')
    failures = workbook.add_worksheet('Failures')
    exceptions = workbook.add_worksheet('Exceptions')

    write_requests(runners, requests)
    write_rt(runners, rt)
    write_failures(runners, failures)
    write_exceptions(runners, exceptions)
...
def write_requests(runners, worksheet):
    worksheet.set_column(0, 0, width=8.5)
    worksheet.set_column(1, 1, width=60)
    worksheet.set_column(2, 2, width=10)
    worksheet.set_column(3, 3, width=10)
    worksheet.set_column(4, 4, width=20)
    worksheet.set_column(5, 5, width=17)
    worksheet.set_column(6, 6, width=17)
    worksheet.set_column(7, 7, width=19)
    worksheet.set_column(8, 8, width=10)
    rows = [
            "Method",
            "Name",
            "# requests",
    ...
    for s in chain(sort_stats(runners.locust_runner.request_stats), [runners.locust_runner.stats.total]):
        worksheet.write_row(i, 0, (
            ...
            int(s.min_response_time) if s.min_response_time else 0,
...
def write_rt(runners, worksheet):
    worksheet.set_column(0, 0, width=66)
    worksheet.set_column(1, 1, width=10)
    worksheet.set_column(2, 2, width=10)
    worksheet.set_column(3, 3, width=10)
    worksheet.set_column(4, 4, width=10)
    worksheet.set_column(5, 5, width=10)
    worksheet.set_column(6, 6, width=10)
    worksheet.set_column(7, 7, width=10)
    worksheet.set_column(8, 8, width=10)
    worksheet.set_column(9, 9, width=10)
    worksheet.set_column(10, 10, width=10)
    rows = [
            "Name",
            "# requests",
            "50%",
    ...
    for s in chain(sort_stats(runners.locust_runner.request_stats), [runners.locust_runner.stats.total]):
        ...
        if s.num_requests:
            worksheet.write_row(i, 0, s.percentile(tpl='%s,%i,%i,%i,%i,%i,%i,%i,%i,%i,%i').split(','))
        else:
            worksheet.write_row(i, 0, ('%s,0,"N/A","N/A","N/A","N/A","N/A","N/A","N/A","N/A","N/A"' % s.name).split(','))
        
        i += 1

def write_failures(runners, worksheet):
    worksheet.set_column(0, 0, width=8.5)
    worksheet.set_column(1, 1, width=60)
    worksheet.set_column(2, 2, width=80)
    worksheet.set_column(3, 3, width=10)
    rows = [
            "Method",
            "Name",
...
def write_exceptions(runners, worksheet):

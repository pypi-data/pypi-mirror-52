...
from .stats import distribution_csv, failures_csv, median_from_dict, requests_csv, sort_stats, report_xls
...
@app.route("/stats/report/xlsx")
def report_stats_xls():
    response = make_response(report_xls())
    file_name = "report_{0}.xlsx".format(time())
...
def request_stats():
    ...
    for s in chain(sort_stats(runners.locust_runner.request_stats), [runners.locust_runner.stats.total]):
        stats.append({
            ...
            "min_response_time": int(s.min_response_time) if s.min_response_time else 0,
            "max_response_time": int(s.max_response_time),
    ...
    if stats:
        ...
        report["avg_response_time"] = runners.locust_runner.stats.total.avg_response_time
    ...
    if stopped is False:
        duration_time = int(time() - runners.locust_runner.stats.start_time) if runners.locust_runner.stats.start_time else 0
        if test_time !=0 and duration_time >= test_time:
            stop()
    elif test_time != 0:
        duration_time = test_time
    report["time"] = duration_time
    report["test_time"] = test_time

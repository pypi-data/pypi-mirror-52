...
from vitaoptimum.result import VitaOptimumPlusIcsResult


class Ics(VitaOptimumPlusConstrained):
    """Integer Constrained Global Optimization Method"""

    def __init__(self, fobj, dim, ng, nh, low, high, np=20,
    ...
    def run(self, restarts=1, verbose=False):
        ...
        self._lib_plus.vitaOptimumPlus_Ics.restype = ctypes.c_double
        self._lib_plus.vitaOptimumPlus_Ics.argtypes = [
            callback_type,  # fobj
        ...
        best = self._lib_plus.vitaOptimumPlus_Ics(
        ...
        return VitaOptimumPlusIcsResult(self._dim, self._low, self._high,
                                        self._fobj_orig, best, xopt, constr,
                                        converged, self._qmeasures)

    def info(self):
        self._lib_plus.vitaOptimumPlus_Ics_info()

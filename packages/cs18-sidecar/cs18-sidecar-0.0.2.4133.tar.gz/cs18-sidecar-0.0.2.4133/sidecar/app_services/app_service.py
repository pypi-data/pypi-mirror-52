...
class K8sAppService(AppService):
    ...
    @CallsLogger.wrap
    def update_network_status(self, app_name: str, status: str):
        self._update_app_status(app_name=app_name,
                                status_tag=Const.HEALTH_CHECK_STATUS,
                                status=status)

    @CallsLogger.wrap
    def update_artifacts_status(self, app_name: str, status: str):
        self._update_app_status(app_name=app_name,
                                status_tag=Const.ARTIFACTS_INTO_SIDECAR_STATUS,
                                status=status)

    def _update_app_status(self, app_name: str, status_tag: str, status: str):

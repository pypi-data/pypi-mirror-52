...
from sidecar.services.metadata.sandbox_metadata_provider import SandboxMetadataMembers, SandboxMetadata
...
_sandbox_metadata = None  # type: SandboxMetadata
...
def get_status_updater(config: ISidecarConfiguration,
                       ...
                       own_public_ip: str,
                       sandbox_metadata: SandboxMetadata) -> SandboxDeploymentStateTracker:
    ...
    if isinstance(config, KubernetesSidecarConfiguration):
        ...
        sandbox_metadata.register_fetcher(SandboxMetadataMembers.PUBLIC_ADDRESS, K8sSandboxPublicAddressFetcher())
    ...
    if isinstance(config, AwsSidecarConfiguration):
        ...
        sandbox_metadata.register_fetcher(SandboxMetadataMembers.VIRTUAL_NETWORK_ID,
                                          AwsVirtualNetworkFetcher(aws_config))
        sandbox_metadata.register_fetcher(SandboxMetadataMembers.PUBLIC_ADDRESS, _sandbox_public_address_fetcher)
    ...
    if isinstance(config, AzureSidecarConfiguration):
        ...
        sandbox_metadata.register_fetcher(SandboxMetadataMembers.PUBLIC_ADDRESS, _sandbox_public_address_fetcher)
        sandbox_metadata.register_fetcher(SandboxMetadataMembers.VIRTUAL_NETWORK_ID, azure_virtual_network_fetcher)
...
@flask.route("/sandbox/meta-data/<string:metadata_name>")
def get_sandbox_metadata(metadata_name: SandboxMetadataMembers):
    value = _sandbox_metadata.get(metadata_name)
    if value is not None:
        return value, 200
    _logger.error(f"unable to get metadata '{metadata_name}'")
    return '', 404
...
def update_app_artifacts_status(app_name: str, status: str):
    global _app_service
    _app_service.update_artifacts_status(app_name=app_name,
                                         status=status)
    return '', 200
...
@flask.route("/deployment_output_resolver", methods=['POST'])
def deployment_output_resolver():
    req = request  # type: Request
    output = req.data.decode(req.charset)
    res = _deployment_outputs_service.get_deployment_output(output)
    return res, 200
...
def run_impl(config: ISidecarConfiguration, own_public_ip: str):
    ...
    _sandbox_metadata = SandboxMetadata()
    aws_session = None
    if isinstance(config, AwsSidecarConfiguration):
        aws_config = config  # type: AwsSidecarConfiguration
    ...
    sandbox_deployment_state_tracker = get_status_updater(
        ...
        aws_session=aws_session,
        own_public_ip=own_public_ip,
        sandbox_metadata=_sandbox_metadata)

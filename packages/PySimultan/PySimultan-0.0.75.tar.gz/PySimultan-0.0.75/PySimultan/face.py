...
from PySimultan.geo_functions import unit_normal, convert_to_global, convert_to_local, polygon_area_3d, collinear_check, calc_plane_equation
...
class Face(GeoBaseClass):
    ...
    def find_floor(self):
        ...
        for existing_floor in existing_floors:
            if np.any((self.Coords[:, 2]) >= existing_floor.FloorHeight) and np.any(self.Coords[:, 2] <= (existing_floor.FloorHeight + existing_floor.StoreyHeight)):
                #               i
                #       |       x       ||              |
                #
                floors.append(existing_floor.StoreyHeight)
            elif np.any(self.Coords[:, 2] < existing_floor.FloorHeight) and np.any(self.Coords[:, 2] > existing_floor.FloorHeight + existing_floor.StoreyHeight):
                #                               i
                #       |       x       ||              ||      x       |
                #
                floors.append(existing_floor.StoreyHeight)

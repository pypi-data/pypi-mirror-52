...
class ResifDataTransfer:
    ...
    def __init__(
        ...
        logging.info(
            "%s v%i.%i starting"
            % (
                ResifDataTransfer.APPNAME,
                ResifDataTransfer.VERSION[0],
                ResifDataTransfer.VERSION[1],
            )
        )
    ...
    def send_data(self):
        ...
        logging.info(
            "Calculating size of %s (%s)" % (self.myDirectoryName, mycommandline)
        )
...
def main():
    try:
        ...
        usage = USAGE.format(
            ...
            version=str(ResifDataTransfer.VERSION[0])
            + "."
            + str(ResifDataTransfer.VERSION[1]),
            config=ResifDataTransfer.myConfigurationFile,
            contact=ResifDataTransfer.CONTACT,
            vmin=str(ResifDataTransfer.PYTHON_VERSION_MIN),
            vmax=str(ResifDataTransfer.PYTHON_VERSION_MAX),
            bold="\033[1m",
            clear="\033[0m",
        )

...
def is_blocked(user):
    ...
    cache = flask.current_app.config['USER_MFA_FAILED_AUTH_CACHE']
    key = f'mfa_failed_auth:{user._id}'

    return (cache.get(key) or 0) \
            >= flask.current_app.config['USER_MFA_MAX_FAILED_AUTH_ATTEMPTS']
...
def log_failed_attempts(state):
    ...
    lockout = flask.current_app.config['USER_MFA_FAILED_AUTH_LOCKOUT']
...
def authorize(state):
    ...
    if totp in user.mfa_recovery_codes:
        ...
        flask.current_app.manage.send_email(
            [user.email],
            'Recovery code used',
            'users/emails/recovery_code_used.html',
    ...
    if 'scoped' in flask.request.args:
        ...
        session_token = secure.mfa.create_scoped_session(
            ...
            flask.current_app.config['USER_MFA_SCOPED_SESSION_LIFESPAN']
        )

...
from cortex_common.types import MessageResponse, Profile, ProfileSchema, EntityEvent, ProfileSummary, \
    ProfileAttributeType, ProfileVersionSummary, \
...
class ProfilesRestClient(_Client):
    ...
    def __init__(self, url, version, token):
        super(ProfilesRestClient, self).__init__(url, version, token)
    ...
    def _get_profile(self, profileId:str, schemaId:Optional[str]=None, historic:bool=False, version:Optional[str]=None) -> Optional[List[dict]]:
        ...
        uri = ProfilesRestClient._build_get_profile_uri(profileId, schemaId, historic=historic, version=version)
    ...
    def pushEvents(self, events:List[EntityEvent]) -> List[str]:
        """
        ...
        return [
            r.get("message") if (r.get("code") >= 200) and (r.get("code") < 300) else r.get("error")
            for r in response
        ]

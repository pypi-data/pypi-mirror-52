...
def normalizeInstrument(instrument: Instrument) -> Instrument:
    if isinstance(instrument, Stock):
        ...
        return replace(instrument, underlying=normalizeSymbol(instrument.underlying))
    else:
        return instrument
...
def activityAffectsSymbol(activity: Activity, symbol: str) -> bool:
    ...
    if isinstance(activity, CashPayment):
        return (
            activity.instrument is not None
            and normalizeSymbol(activity.instrument.symbol) == normalized
        )
    elif isinstance(activity, Trade):
        return (
            isinstance(activity.instrument, Option)
            and normalizeSymbol(activity.instrument.underlying) == normalized
        ) or normalizeSymbol(activity.instrument.symbol) == normalized
    else:
        return False
...
def realizedBasisForSymbol(symbol: str, activity: Iterable[Activity]) -> Optional[Cash]:
    ...
    return reduce(f, (t for t in activity if activityAffectsSymbol(t, symbol)), None)
...
# most recent.
def timelineForSymbol(symbol: str, a: Iterable[Activity]) -> Iterable[TimelineEntry]:
    realizedProfit: Optional[Cash] = None
    positions: Dict[Instrument, Decimal] = {}

    for t in sorted(
        (t for t in a if activityAffectsSymbol(t, symbol)), key=lambda t: t.date
    ):
        ...
        yield TimelineEntry(
            date=t.date, positions=positions.copy(), realizedProfit=realizedProfit
        )
...
def deduplicatePositions(positions: Iterable[Position]) -> Iterable[Position]:
    return (
        ...
        for i, ps in groupby(
            sorted(positions, key=lambda p: p.instrument), key=lambda p: p.instrument
        )
    )
...
def currencyConversionRates(
    ...
    return (
        (instrument.baseCurrency, quote.market)
...
def convertCashToCurrency(
    quoteCurrency: Currency, cash: Sequence[Cash], dataProvider: MarketDataProvider
) -> Cash:
    currencyRates = dict(
        currencyConversionRates(
            quoteCurrency=quoteCurrency,
            otherCurrencies=(c.currency for c in cash if c.currency != quoteCurrency),
            dataProvider=dataProvider,
        )
    )
    currencyRates[quoteCurrency] = Cash(currency=quoteCurrency, quantity=Decimal(1))

...
class GetFileRequest(TLRequest):
    ...
    def __init__(self, location: 'TypeInputFileLocation', offset: int, limit: int, precise: Optional[bool]=None):
        """
    ...
    def __bytes__(self):
        return b''.join((
            b'\xfc\x9aZ\xb1',
            struct.pack('<I', (0 if self.precise is None or self.precise is False else 1)),
            bytes(self.location),
            struct.pack('<i', self.offset),
            struct.pack('<i', self.limit),
        ))
    ...
    def from_reader(cls, reader):
        ...
        _offset = reader.read_int()
        _limit = reader.read_int()
        return cls(location=_location, offset=_offset, limit=_limit, precise=_precise)

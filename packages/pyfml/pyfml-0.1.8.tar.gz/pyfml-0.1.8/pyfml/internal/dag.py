...
class Dag:
    _edge = namedtuple('_edge', ['src', 'dest'])
    ...
    def add(self, name: str, node: A) -> bool:
        ...
        if res:
            self.__nodes[name] = Dag._node(node)
        return res
    ...
    def parents(self, node_name: str) -> List['_node']:
        return [
            n for n in i_values(self.__nodes)
            if node_name in n.get_edges()
        ]

    def children(self, node_name: str) -> List['_node']:
        res = []
        if node_name in self.__nodes:
            res = [
                self.__nodes[n] for n in self.__nodes[node_name].get_edges()
            ]
        return res

    def to_list(self) -> List['_node']:

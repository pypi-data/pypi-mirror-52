...
def train(conf, date):
    if _env['distribution']:
        process_pid = _distribute.distributeMulti('train', conf, date, _env['retrain'], _env['ps_num'], _env['ev_num'], emrName=_emr['emrName'],
        ...
        if _env['backend']:
            ...
            retrain = '' if _env['retrain'] else '--retrain'
            ...
            _main.run('train', conf=conf, retrain=_env['retrain'], useSpark=False, date=date, ps=_env['ps_num'],
...
def infer(conf, date):
    if _env['distribution']:
        process_pid = _distribute.distributeMulti('infer', conf, date,_env['ps_num'], _emr['emrName'],
...
def export4online(conf=None, from_pb=True, **kwargs):
    if conf is None:
        conf = _env['conf']
    if conf == '' and kwargs:
...
def wait():
    import psutil
    import time
    time.sleep(15)  # After initialization, the PID of child process would changes in a moment

...
def distributeTrain(confDir, date, retrain, ps_num, ev_num, emrName, keyFile, logDir=emr_home_path, dirs2issue=''):
    if confDir[-1] == '/':
        confDir = confDir[:-1]
    if retrain:
        clear_summary(confDir)
...
def distributeInfer(conf, date, ps, emrName, logpath=emr_home_path, use_TFoS=True):
    ...
    date = '--date {}'.format(date) if date != '' else ''
    command = r'''
    ${{SPARK_HOME}}/bin/spark-submit --master yarn {py_file} {archive} {TFoS} --num-executors {executor} --executor-memory 15G --driver-memory 15G --conf spark.executorEnv.JAVA_HOME="$JAVA_HOME" --conf spark.executorEnv.PYSPARK_PYTHON="/usr/bin/python3" --conf spark.executorEnv.PYSPARK_DRIVER_PYTHON="/usr/bin/python3"  --conf spark.executorEnv.CLASSPATH="$($HADOOP_HOME/bin/hadoop classpath --glob):${{CLASSPATH}}"  --conf spark.executorEnv.LD_LIBRARY_PATH="/usr/lib/hadoop/lib/native:${{JAVA_HOME}}/lib/amd64/server" {exec_file} {date} --mode infer --useSpark --logpath {tf_log_path} --ps {ps} --num_executor {num_executor} {use_TFoS} --conf {conf}  > {logpath}
    '''.format(py_file=py_file, archive= '', TFoS=use_TFoS_param, date=date, executor=hostnum, exec_file=os.path.join(_project_path, _exec_file),

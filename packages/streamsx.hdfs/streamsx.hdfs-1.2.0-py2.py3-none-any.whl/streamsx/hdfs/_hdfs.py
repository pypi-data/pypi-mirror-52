...
def _convert_copy_direction_string_to_enum(value):
    ...
    if value == 'copyFromLocalFile':
        ...
        raise NotImplementedError("CopyDirection parsing not implemented: " + value)
    return direction
...
def scan(topology, credentials, directory, pattern=None, init_delay=None, name=None):
    ...
    Args:
        ...
        credentials(dict|file|str): The credentials of the IBM cloud Analytics Engine service in *JSON* or the path to the *configuration file* (``hdfs-site.xml`` or ``core-site.xml``). If the *configuration file* is specified, then this file will be copied to the 'etc' directory of the application bundle.     
    ...
    Returns:
        Output Stream containing file names. Default output schema is :py:const:`~streamsx.hdfs.DirectoryScanSchema`.
     """
...
def copy(stream, credentials, direction, hdfsFile=None, hdfsFileAttrName=None, localFile=None, name=None):
    ...
    Returns:
        Output Stream containing the result message and teh elapsed time. Default output schema is ``'tuple<rstring message, uint64 elapsedTime>'``.
        Output Stream with schema :py:const:`~streamsx.hdfs.FileCopySchema`.
    """

...
def create_parser(parser, use_gooey=True):
    ...
    optional_group.add_argument(
        ...
        # type=int,
        nargs="+",
        help="Specifiy which gpu(s) should be used. Multiple GPUs are separated by a whitespace. If not defined otherweise by your system, it will use GPU 0 by default.",
    )
    ...
    optional_group.add_argument(
        "--monitor",
        action="store_true",
        help='When this option is activated, crYOLO will monitor your input folder. This especially usefull for automatisation purposes. You can stop the monitor mode by writing an empty file with the name "stop.cryolo" in the input directory.',
    )
    ...
    if filament_mode:
        ...
        if picking_result_with_boxes:
            ...
            for picking_result_subset in picking_result_with_boxes_subsets:
                ...
                #    trace_subset_filements, filament_tracers
                # )
                # pool.close()
                # pool.join()
...
def min_distance_filter(boxes, min_distance):
    ...
    all_comb = itertools.combinations(boxes, 2)

    distsqs = map(utils.box_squared_distance, all_comb)
    low_distance_pairs = list(itertools.compress(all_comb, distsqs < min_distance_sq))
    ...
    if "filter" in config["model"]:
        ...
        if len(filter_options) > 2:
            ...
            if not otf:
                ...
                img_paths_filtered = utils.filter_images_nn_dir(
                    img_paths=img_paths,
                ...
                if image is not None or image.shape[0] == 0 or image.shape[1] == 0:
                    ...
                    if num_written_tiles == batchsize or current_index_image == (
                        ...
                        for box_img_index, boxes in enumerate(boxes_per_image_nms):
                            ...
                            imgh, imgw = imagereader.read_width_height(
                                boxes_image_path
...
def trace_subset_filements(tracer):
    return tracer.trace_filaments()

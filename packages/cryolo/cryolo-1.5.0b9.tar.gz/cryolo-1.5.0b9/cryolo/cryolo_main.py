...
def config_configuration_parser(parser_config):
    ...
    config_required_group.add_argument(
        "boxsize",
        type=int,
        help="You should specifiy the same box size here as you used in your training data.  If you train on several datasets with varying box sizes, use something like the average of the used box sizes. In case of the general model fill in your target box size.",
    )

    config_required_group.add_argument(
        "--train_image_folder",
        help="Path to the image folder containing the images to train on. The field can only be left empt when you fine tune a model.",
        default="",
        widget="DirChooser",
    )
    config_required_group.add_argument(
        "--train_annot_folder",
        help="Path to folder containing the your annotation files like box or star files. The field can only be left empty when you fine tune a model.",
        default="",
        widget="DirChooser",
    )
    ...
    config_denoising_group = parser_config.add_argument_group(
        "Denoising options",
        "By default crYOLO denoises your data before training/picking. Iit will use a low pass filter but you can also use neural network denoising (JANNI). Default options are good and typically don't have to be changed.",
    )

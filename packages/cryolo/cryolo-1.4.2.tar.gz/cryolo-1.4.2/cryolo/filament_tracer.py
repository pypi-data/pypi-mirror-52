...
class FilamentTracer:
    ...
    def contains_box(self, box, list_box):
        contains = False
        for b in list_box:
    ...
    def merge_filaments(self, filaments):
        ...
        while merged:
            ...
            min_dist_filament_distance = None
            ...
            for i in range(len(filaments)):
                for j in range((i + 1), len(filaments)):
                    if i != j:
                        ...
                        if self.distancesq_filament(filaments[i],
                                                    filaments[j]) > search_radius_sq:
    ...
    def distancesq_filament(self, filament_a, filament_b):
        start_end_boxes_a = [filament_a.boxes[0],filament_a.boxes[-1]]
        start_end_boxes_b = [filament_b.boxes[0],filament_b.boxes[-1]]
        min_dist = 999999999
        for start_end_combination in itertools.product(start_end_boxes_a, start_end_boxes_b):
            dist = sq_distance_between_two_boxes(start_end_combination[0],start_end_combination[1])
            if dist < min_dist:
                min_dist = dist
        return min_dist
    ...
    def trace_filaments(self):
        ...
        for box_i, box in enumerate(self.boxes):
            ...
            if not self.contains_box(
                box, assigned_boxes
            ):  # box not in assigned_boxes:
                fil = Filament()
                ...
                while new_box_found:
                    new_box_found = False
                    candidate_boxes = self.get_box_neightboorhood(fil.boxes[-1], self.search_radius)
                    candidate_boxes = [
                        box
                        for box in candidate_boxes
                        if not self.contains_box(box, assigned_boxes)
                    ]
        ...
        # Rescale
        for fil in filaments:
            for boxi, box in enumerate(fil.boxes):
                box.x = box.x / self.rescale_factor
                box.y = box.y / self.rescale_factor
                box.w = box.w / self.rescale_factor
                box.h = box.h / self.rescale_factor
    ...
    def merge_filament(self, filament_a, filament_b):
        ...
        if candidate_box_1 is not None:
            if box_are_equal(candidate_box_1,b_start):
                ...
                reversed_a = list(reversed(filament_a.boxes))
                is_compatible_dir = self.merge_is_compatible_direction(
                    basis_filament=filament_a, to_add_filament=Filament(reversed_b),from_head_a=True
                )
                ...
                is_compatible_dir = self.merge_is_compatible_direction(
                    basis_filament=filament_a, to_add_filament=filament_b_for_dir_est, from_head_a=True
                )
                if is_compatible_dir:
            ...
            if box_are_equal(candidate_box_2,b_start):
                is_compatible_dir = self.merge_is_compatible_direction(
                    basis_filament=filament_a, to_add_filament=filament_b, from_head_b=True
                )
                if is_compatible_dir:
                ...
                is_compatible_dir = self.merge_is_compatible_direction(
                    basis_filament=filament_a, to_add_filament=Filament(reversed_b), from_head_b=True
                )
                if is_compatible_dir:
    ...
    def get_parallel_boxes(self, ref_box, boxes, angle_image, perp_search_radius):
        sqdistance = self.search_radius * self.search_radius
        parallel_boxes = []
        filament_orientation
        help_boxes = [ref_box]
        ref_orientation = filament_orientation(Filament(help_boxes), angle_image)

        for box in boxes:
            if box != ref_box:
                dist_perp = distance_perpendicluar_to_box_direction(
                    ref_box=ref_box, candiate_box=box, ref_angle=ref_orientation
                )
                if (
                    sq_distance_between_two_boxes(ref_box, box) < sqdistance
                    and dist_perp < perp_search_radius
                ):
                    is_box_in_search_region = in_angular_search_region(
                        ref_box=ref_box,
                        candidate_box=box,
                        angle_delta=self.dir_delta,
                        ref_angle=ref_orientation,
                    )
                    if not is_box_in_search_region:
                        parallel_boxes.append(box)
        return parallel_boxes
    ...
    def get_box_neightboorhood(self, ref_box, search_radius):
    ...
    def next_valid_box(self, filament, candidate_boxes, ref_box, debug=False):
        ...
        to_delete_indices = [
            candidate_boxes.index(box) for box in to_ignore
        ]
    ...
    def merge_is_compatible_direction(
        self, basis_filament, to_add_filament, filament_direction_delta=50,
            from_head_a = False, from_head_b = False
    ):
...
def nms_for_filaments(filaments,filament_width):
    ...
    for i in range(len(filaments)):
        for j in range((i + 1), len(filaments)):
            if i != j:
                if not is_in_search_radius(filaments[i],
                                         filaments[j],
                                         filament_width):
                    continue
                long = i
                ...
                split_indices = []
                for box_combination_indices in itertools.product(range(len(filaments[short].boxes)),
                                                         range(len(filaments[long].boxes))):
                    dist = sq_distance_between_two_boxes(filaments[short].boxes[box_combination_indices[0]],
                ...
                if split_indices:
                    ...
                    if len(filaments[short].boxes) > 1:
                        splitted_filament = split_filamant(filaments[short],split_indices)
                        new_filaments.extend(splitted_filament)

    filaments_nms_remove = [filaments[fil_index] for fil_index in filaments_nms_remove_indices]
...
def box_are_equal(box1, box2):
    if int(box1.x) == int(box2.x) and int(box1.y) == int(box2.y) and int(box1.w) == int(box2.w) and int(box1.h) == int(box2.h):
        return True
    return False
...
def filter_filaments_by_num_boxes(filaments, minimum_number_boxes):
    filtered_filaments = [
        f
        for f in filaments
        if len(f.boxes) > minimum_number_boxes
    ]
    return  filtered_filaments


def split_filaments_by_straightness(resamples_filaments, straightness_threshold=0.95):
    filamants_which_were_splitted = []
    new_fiament_segments = []
    for i, fil in enumerate(resamples_filaments):
        segments = split_filament_by_straightness(fil,straightness_threshold)
...
def remove_double_picked_filaments(resampled_filaments,filament_width):
    fw = filament_width
    filament_boxes = [filament.boxes for filament in resampled_filaments]
    filament_boxes.sort(key=len, reverse=True)
    import itertools

    new_filament_boxes = []
    for longer_fil_boxes,shorter_fil_boxes in itertools.permutations(filament_boxes,r=2):

        split_points_short = []
        for box_long_index, box_short_index in itertools.product(range(len(longer_fil_boxes)),range(len(shorter_fil_boxes))):
            pass
...
def split_filament_by_straightness(filament, straightness_threshold=0.95):
    ...
    if _get_straightness(filament.boxes) >= straightness_threshold:
        return [filament]
    ...
    while len(list_splitted_filaments) > last_list_size:
        ...
        for fil in list_splitted_filaments:
            if _get_straightness(fil.boxes) >= straightness_threshold:
                continue

            max_straightness = -1
            max_straightness_index = -1
            for i in range(2, len(fil.boxes) - 2):
                straightness_to_i = _get_straightness(fil.boxes[:i])
                straightness_from_i = _get_straightness(fil.boxes[i:])
                mean_straightness = (straightness_to_i + straightness_from_i) / 2
                if mean_straightness > max_straightness:

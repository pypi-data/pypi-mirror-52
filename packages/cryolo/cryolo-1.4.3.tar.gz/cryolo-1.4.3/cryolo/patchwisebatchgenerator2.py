...
class PatchwiseBatchGenerator(Sequence):
    ...
    def aug_image(self, train_instance, jitter, region=None):
        ...
        if jitter:
            ...
            image = np.array(Image.fromarray(image).resize((int(image.shape[1]*scale), int(image.shape[0]*scale)), resample=Image.BILINEAR))
            ...
            image = np.array(Image.fromarray(image).resize(
                (self.config["IMAGE_H"], self.config["IMAGE_W"]), resample=Image.BILINEAR))
            # flip the image

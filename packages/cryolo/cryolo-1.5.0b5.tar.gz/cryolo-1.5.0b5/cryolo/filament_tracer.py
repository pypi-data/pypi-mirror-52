...
class FilamentTracer:
            ...
            for i in range(len(filaments)):
                for j in range((i + 1), len(filaments)):
                    if i != j:
                        ...
                        if (
                            self.distancesq_filament(filaments[i], filaments[j])
                            > search_radius_sq
                        ):
    ...
    def trace_filaments(self):
        ...
        for box_i, box in enumerate(self.boxes):
            ...
            if not self.contains_box(box, assigned_boxes):  # box not in assigned_boxes:
                fil = Filament()
                ...
                while new_box_found:
                    new_box_found = False
                    candidate_boxes = self.get_box_neightboorhood(
...
def remove_double_picked_filaments(resampled_filaments, filament_width):
    ...
    new_filament_boxes = []
    for longer_fil_boxes, shorter_fil_boxes in itertools.permutations(
        filament_boxes, r=2
    ):

        split_points_short = []
        for box_long_index, box_short_index in itertools.product(
            range(len(longer_fil_boxes)), range(len(shorter_fil_boxes))
        ):
            pass

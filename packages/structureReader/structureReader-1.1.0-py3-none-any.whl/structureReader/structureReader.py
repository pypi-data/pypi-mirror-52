...
class CStructureGenerator(object):
    ...
    def init(self):
        try:del self.structure
        except:pass
        self.structure = OrderedDict({})

    def build(self,field,offset,size,inx=0):
        if(len(field)!=len(offset) and len(offset)!=len(size)):
            return False
        while(inx<len(field)):
            self.structure.update({field[inx]:[offset[inx],size[inx],None]})
            inx+=1   
        return True

    def push(self,field,offset,size,value=None):
        self.structure.update({field:[offset,size,value]})

    def pop(self,field):
        return self.structure.pop(field,None)
...
class StructureReader(object):
    ...
    def goto(self,offset=0,mode=os.SEEK_CUR):
        return os.lseek(self.__fd,offset,mode)
    ...
    def bgoto(self,offset=0,mode=os.SEEK_CUR):
        return self.__sfd.seek(offset,mode)

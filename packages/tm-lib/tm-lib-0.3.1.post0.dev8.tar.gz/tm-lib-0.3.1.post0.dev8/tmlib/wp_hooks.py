...
def get_auth_token():
    ...
    try:
        ...
        print(e)
        token_string = request_auth_token()
        WPJWT.objects(username=Settings.WP_USER).update_one(set__token=token_string, upsert=True)
    decoded_token = jwt.decode(token_string.encode('utf-8'), Settings.WP_JWT_SECRET)
    print(decoded_token)

...
from hy.core.shadow import get
import requests
...
def get(link, legit=thru, spare=[], timeout=10, session=None, html=False):
    ...
    if proposition and is_deprecated(lax):
        lax2 = HTMLParser(session.get(proposition, headers=headers, timeout
            =timeout).text)
        ...
        atts = parse_atts(lax)
    result = {'name': title, 'Aukcja': title, 'link': link, 'deprecated':
        is_deprecated(lax), 'proposition': proposition, 'atts': atts,
        'breadcrumbs': breadcrumbs, 'for_kids': is_for_kids(breadcrumbs),
        'description': description, 'rating_count': rating_count,
        'rating_value': rating_value, 'customers_count': customers_count,
        'sales_count': sales_count, 'img': img}
    if html:
        result['html'] = html
        _hy_anon_var_24 = None
    else:
        _hy_anon_var_24 = None
    return result if legit(result) else first(spare)[HyKeyword('legit')][legit
        ][HyKeyword('spare')][list(rest(spare))] if spare else None

...
def only_numbers(s):
    return (lambda hyx_Xpercent_signX1: ''.join(hyx_Xpercent_signX1))((lambda
        arr: list(filter(lambda hyx_Xpercent_signX1: hyx_Xpercent_signX1 in
        '0123456789', arr)))(list(s)))
...
def get_page(link, legit=thru, spare=[], timeout=10, session=None, html=False):
    ...
    if proposition and is_deprecated(lax):
        ...
        atts = parse_atts_deprecated(lax) if is_deprecated(lax
            ) else parse_atts(lax)
    result = {'name': title, 'Aukcja': title, 'ID_Aukcji': link_to_id(link),
        ...
        'img': img, 'proposition': proposition}
    if html:
        result['html'] = html

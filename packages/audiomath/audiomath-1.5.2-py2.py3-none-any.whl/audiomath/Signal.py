...
def IncreaseDimensionality( a, maxdim ):
	"""
...
def ApplyWindow(s, func=Hann, axis=0, **kwargs):
	"""
	Return a copy of `s` (a `numpy.ndarray` or a `audiomath.Sound` object)
	multiplied by the specified window function, along the specified time `axis`.
	
	`func` should take a single positional argument: length in samples. Additional
	`**kwargs`, if any, are passed through. Suitable examples include
	`numpy.blackman`, `numpy.kaiser`, and friends.
	"""
	if isinstance( s, numpy.ndarray ): y = s
	elif hasattr(s, 'y'): s = s.copy(); y = s.y
...
def ModulateAmplitude(s, freq_hz=1.0,phase_rad=None,phase_deg=None,amplitude=0.5,dc=0.5,samplingfreq_hz=None,duration_msec=None,duration_samples=None,axis=None,waveform=sin,**kwargs):
	"""
	Return a copy of `s` (a `numpy.ndarray` or a `audiomath.Sound` object) in
	which the amplitude is modulated sinusoidally along the specified time `axis`.
	...
	if duration_samples is None and duration_msec is None: duration_samples = IncreaseDimensionality(y,0).shape[0]
	envelope = GenerateWaveform(freq_hz=freq_hz,phase_rad=phase_rad,phase_deg=phase_deg,amplitude=amplitude,dc=dc,samplingfreq_hz=samplingfreq_hz,duration_msec=duration_msec,duration_samples=duration_samples,axis=axis,waveform=waveform,**kwargs)
	envelope = IncreaseDimensionality(envelope, len(y.shape)-1)
	y = y * envelope
	if isinstance( s, numpy.ndarray ): s = y
	else: s.y = y
	return s
	
def GenerateWaveform(freq_hz=1.0,phase_rad=None,phase_deg=None,amplitude=1.0,dc=0.0,samplingfreq_hz=None,duration_msec=None,duration_samples=None,axis=None,waveform=cos,container=None,**kwargs):
	"""
	...
	the first argument in its signature is named `cycles`, as is the case for
	`SineWave()`, `SquareWave()`, `TriangleWave()` and `SawtoothWave()` in this
	module.  In that case, the input is assumed to be expressed in cycles (product
	...
	x = x * (IncreaseDimensionality(freq_hz,maxaxis) * duration_sec) # *= won't work for broadcasting here
	# if you get an error here, try setting axis=1 and transposing the return value ;-)
	x = x + (IncreaseDimensionality(phase_rad,maxaxis))              # += won't work for broadcasting here
	try: spec = getargspec( waveform )
	except: pass
	else:
		if spec.args[ 0 ] == 'cycles': x /= TWO_PI
	x = waveform(x, **kwargs)
	x = x * IncreaseDimensionality(amplitude,maxaxis)                # *= won't work for broadcasting here
	if numpy.any(dc.flatten()):
		x = x + IncreaseDimensionality(dc,maxaxis)                   # += won't work for broadcasting here
	if container is not None:
		across_channels = 1
...
def ap2fft(amplitude,phase_rad=None,phase_deg=None,samplingfreq_hz=2.0,axis=0,freq_hz=None,fullfreq_hz=None,nsamp=None):
	...
	if nsamp is None:
		...
		elif freq_hz is not None:   nsamp = len(freq_hz) * 2 - 2

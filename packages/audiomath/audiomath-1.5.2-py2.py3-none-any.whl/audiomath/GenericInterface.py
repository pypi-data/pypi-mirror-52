...
class GenericPlayer( object ):
	...
	or list of filenames. The current `Sound` instance is
	available in the `.sound` property and the complete `Queue`
	is available in the `.queue` property.
	...
	def synchronizeDynamics( self ):
		...
		resume on a schedule. So this example will play
		repeatedly, with pauses::
	...
	def levels( self ):
		...
		to each channel in turn. It is indepdent of (i.e. its
	...
	def queue( self ):
		...
		The `.repeatQueue` and `.autoAdvance` properties allow you
		to specify how the queue is used when a given `Sound` finishes
		playing.  The `.track` property allows you to jump immediately
		to a given place in the queue.
		...
		that sound has finished played.  If you want to change tracks
...
class GenericRecorder( object ):
	...
	def __init__( self, seconds, fs=44100, numberOfChannels=1, sampleFormat='float32', start=True, **kwargs ):
	...
	def WaitFor( self, condition, finalRecordingStatus=None ):
		...
		argument may be:
			...
			a `Recorder` instance (most likely `self`):
		...
		if isinstance( condition, GenericRecorder ):
			obj = condition
			condition = lambda: None if obj.recording else '%r finished recording' % obj
		if isinstance( condition, GenericPlayer ):
			obj = condition
			condition = lambda: None if obj.playing else '%r finished playing' % obj
	...
	@classmethod
	def MakeRecording( cls, space=60, prompt=None, verbose=None, cut=True, **kwargs ):
		"""
		...
		"""
		r = cls( seconds=space, start=True, verbose=verbose, **kwargs )

...
class ffmpeg( object ):
	...
	def __init__( self, filename, source=None, format=None, nChannels=None, fs=None, kbps=192, transform=None, verbose=False ):
		...
		if not nChannels: nChannels = 2 if stream is None else stream.nInputChannels if stream.nInputChannels else stream.nOutputChannels
	...
	def __call__( self, data, sampleNumber, fs ):
		if not self.process.stdin.closed:
			...
			self.process.stdin.write( data.tostring() )
	def Close( self ):

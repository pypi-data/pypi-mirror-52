...
def send_data():
    ...
    try:
        ...
        # *****clubing****************************************************
        if check_version == 0:
            check = input("Do you want to update this computer details on WOL computer list page (Y/n) : ")
        else:
            check = raw_input("Do you want to update this computer details on WOL computer list page (Y/n)  : ")
...
#        os.chmod(path3, 0o600)
        ...
        password = getpass("WiFi passowrd (Hidden Field) : ")

        while checker(password, length=64):
            password = getpass("WiFi passowrd (Hidden Field) : ")

        if check_version == 0:
            check = input("Should sub network information need to be collected (Y/n) : ")
            
        if check_version == 0:
            daily_scan = input("Do you want to run daily sub-computer scan on this computer ( ! Helps to update the computer list)  (Y/n) : ")

        print("????????????????????????????????????????????????????????????????????????")
        if check_version == 0:
            submit = input("Do you finally wants to submit all above info? Sure!  (Y/n): ")
        else:
            submit = raw_input("Do you finally wants to submit all above info? Sure!  (Y/n): ")

        if submit == 'n' or submit == 'N':
            return 0

        # ********************************************** daily.py to cron *********************************
        if daily_scan == 'y' or daily_scan == 'Y':
            cron = CronTab(user='root')
            cmd_run = dirname + '/daily.py &'
            cmd_run = "python3 " + cmd_run
            job2 = cron.new(command=cmd_run, comment='daily_run')

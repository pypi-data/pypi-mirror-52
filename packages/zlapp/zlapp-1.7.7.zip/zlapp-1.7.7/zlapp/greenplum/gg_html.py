...
def gg_html_cdc(max_html_key,conp_pg=None):
    ...
    sql="select html_key,page.tran_page(page) as page from src.t_gg where html_key>%d  "%max_html_key
    print(sql)
    ...
    pg2pg(sql,'gg_html_cdc',conp_hawq,conp_pg,chunksize=10000,datadict=datadict,if_exists='replace')
...
def gg_html_cdc1(max_html_key,conp_pg=None):
    if conp_pg is None:conp_pg=["postgres","since2015","192.168.4.188","bid","cdc"]
    conp_pg[4]='cdc'
    ...
    datadict={"html_key":BIGINT(),
    "page":TEXT()}
    sql1="truncate table cdc.gg_html_cdc1;"
    db_command(sql1,dbtype="postgresql",conp=conp_pg)
    pg2pg(sql,'gg_html_cdc1',conp_hawq,conp_pg,chunksize=10000,datadict=datadict,if_exists='replace')
    print("gg_html_cdc1写入完毕")

    sql="insert into public.gg_html select * from cdc.gg_html_cdc1 as a  where not exists(select 1 from  public.gg_html  as b where a.html_key=b.html_key)"
    db_command(sql,dbtype="postgresql",conp=conp_pg)

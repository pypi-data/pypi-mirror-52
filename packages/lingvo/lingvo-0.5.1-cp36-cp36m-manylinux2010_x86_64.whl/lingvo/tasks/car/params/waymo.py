...
_WAYMO_BASE = 'tfrecord:' + os.environ.get('WAYMO_DIR', 'FILL-ME-IN')


def WaymoTrainSpec(params):
  """Training set."""
  p = params.Copy()
  p.file_pattern = os.path.join(_WAYMO_BASE, 'train-*-of-01000')
...
def WaymoMiniTrainSpec(params):
  ...
  p = params.Copy()
  p.file_pattern = os.path.join(_WAYMO_BASE, 'train-000[0-2]?-of-01000')
  p.num_samples = 4773
  return p
...
def WaymoValSpec(params):
  ...
  """
  p = params.Copy()
  p.file_pattern = os.path.join(_WAYMO_BASE, 'valid-*-of-01000')
  p.num_samples = 40077
  return p
...
def WaymoMinivalSpec(params):
  ...
  p = params.Copy()
  p.file_pattern = os.path.join(_WAYMO_BASE, 'valid-000??-of-01000')
  p.num_samples = 4109
  return p
...
class StarNetBase(base_model_params.SingleTaskModelParams):
  ...
  @classmethod
  def _apply_basic_data_aug(cls, p):
    """Apply basic data augmentations to an input params."""
    # Parameters adopted from KITTI.
  ...
  def _configure_input(cls, p, split):
    ...
    if job_type.startswith('trainer') and cls.USE_BASIC_DATA_AUG:
      p = cls._apply_basic_data_aug(p)
    ...
    if cls.RUN_LOCALLY:
      ...
      raise ValueError('Running locally not supported.')

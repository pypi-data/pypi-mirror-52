name="TransportPy"
def plot_ufm(ufm="",shp="",OD='O',variable_name='Variable',location=[51.704740, -0.042465],zoom_start=9,tiles = "Stamen Toner",epsg=3857,XEXES="C:\\SATWIN\\XEXES_11.3.12W_MC"):
        """
        ...
        if OD == 'O':
                df_org=df.groupby(['Origin']).agg({Variable_name:'sum'}).reset_index()
                name='Origins'
                legend_name='Origin ' + Variable_name
                df_zones=pd.merge(zones,df_org,left_on='tmp_zone',right_on='Origin')
        if OD == 'D':
                df_org=df.groupby(['Destination']).agg({Variable_name:'sum'}).reset_index()
                name='Destinations'
                legend_name='Destination ' + Variable_name
                df_zones=pd.merge(zones,df_org,left_on='tmp_zone',right_on='Destination')
        ...
        def style_function(feature):
                ...
                'weight': 0,
                'fillColor': '#black'}
        def g(geom,zones,Var):
                a=gpd.GeoSeries(geom)
                epsg='epsg:'+str(epsg)
                a.crs={'init':epsg}
                folium.GeoJson(a,control=False,overlay=False,name='zones',tooltip=folium.Tooltip('Zone: ' + str(zones) + '\n;'+ Variable_name +  ' ' + str(round(Var,2))),style_function =style_function).add_to(somewhere)
        if OD == 'O':
                np.vectorize(g)(df_zones.geometry,df_zones.Origin,df_zones.Variable)
        else:
                np.vectorize(g)(df_zones.geometry,df_zones.Destination,df_zones.Variable)
        folium.LayerControl().add_to(somewhere)
        somewhere.save('UFM_'+ legend_name +'.html')
def read_ufm(file, MX="C:\\SATWIN\\XEXES_11.3.12W_MC\\$MX.exe", remove_txt=True, clean_up=True):
        """
...
class UFS:
    ...
    def actual_flow(self,PATH ="",file="",by_userclass="",Clean_up = True,Coordinates=True,Output=False):
        ...
        f = open(stamp+"temp_coor.key","w+")
        f.write(keyfile)
        f.close()
        text= "SET PATH="+PATH+"\nSATDB " + "'" + UFS1+ "'" +" Key "+ stamp +"temp_coor.key " + "VDU vdu"
    ...
    def p1x_dump(self,PATH ="",file="",Data="",Clean_up = True,Coordinates=True,Output=False):
        ...
        if Clean_up:
            ...
            remove("DUMMY.VDU")
        return df
    def plot_flows(self,PATH="",UFS2="",func='x-y',by_userclass="",location=[51.704740, -0.042465],html='',scale=2000):
        import pandas as pd
    ...
    def plot_Data(self,Data="",PATH="",UFS2="",func='x-y',location="",html='',scale=2000):
        import pandas as pd
        from pyproj import Proj, transform
        import folium
        from folium import plugins
        ...
        df['link']=np.vectorize(ColstoCol, otypes=[list])(df.X1,df.Y1,df.X2,df.Y2)
        ###
        m=folium.Map(crs='EPSG3857',location=[51.704740, -0.042465],zoom_start=9,weight=0.2,tiles='cartodbpositron')
        net = folium.FeatureGroup(name='network')
        m.add_child(net)
        folium.PolyLine(locations=df['link'],color="black",weight=0.25).add_to(net)

...
from swimlane_platform.lib import names, DockerImage, DockerComposeManager
...
from swimlane_platform.questions_groups import turbine_enable_questions
    ...
    def modify_compose_file(self, main=None, turbine=None):
        ...
        for name, service in docker_compose_turbine.get('services').items():
            if 'image' in service and 'swimlane' in service['image']:
                docker_image = DockerImage.parse(service['image'])
                docker_image.version = version
                docker_compose_turbine.set(docker_image.get(is_dev=self.config.args.dev), 'services', name, 'image')

from swimlane_platform.lib import Configuration, names, \
    BaseWithLog, info_function_start_finish, create_cert
from os import path
from OpenSSL import crypto
from shutil import copy2
from swimlane_platform.questions_groups import ssl_questions
...
class EnableWebSsl(BaseWithLog):
    ...
    def generate_certificates(self):
        ...
        c, k = create_cert(country=self.config.args.ssl_country, state=self.config.args.ssl_state,
                           location=self.config.args.ssl_location, company=self.config.args.ssl_company,
                           application=self.config.args.ssl_application)
        with open(path.join(self.secrets, names.SSL_WEB_KEY), "w") as fw:
            fw.write(crypto.dump_privatekey(crypto.FILETYPE_PEM, k))
        with open(path.join(self.secrets, names.SSL_WEB_CERTIFICATE), "w") as fw:

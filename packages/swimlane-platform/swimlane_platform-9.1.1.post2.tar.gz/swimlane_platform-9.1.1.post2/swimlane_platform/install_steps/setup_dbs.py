...
class SetupDbs(BaseWithLog):
    ...
    def start_mongo_install_container(self):
        ...
        mongo_success_message = "Successfully completed MongoDB initialization for Swimlane."
        error_message = "ERROR: SWIMLANE INITIALIZATION ERROR OCCURRED"
        by_line = self.docker_manager.logs_by_line(names.SW_MONGO)
        try:
            start = time()
            for line in by_line:
                if mongo_success_message in line:
                    self.logger.info(mongo_success_message)
                    break
            ...
            if self.config.args.show_db_init_logs:
                print(self.docker_manager.logs(names.SW_MONGO))
            raise
        finally:
            by_line.close()

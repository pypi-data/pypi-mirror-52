...
class StackstormDB(object):
    ...
    def _build_session(self, user, password=None, db_name=None, host=None, port=5432):
        ...
        connection = 'postgresql://{0}:{0}@{0}:{0}/{0}'.format(user, password, host, port, db_name)
    ...
    def get_alerts(self, sensor_ref_name, tenant_id = None):
        """
            Gets all alerts
        """
        try:
            ...
            if tenant_id is None:
                ...
                alerts = self.session.query(Alert).filter(
                        and_(Alert.sensor_ref_name == sensor_ref_name, Alert.tenant_id == tenant_id)).all()
            return alerts
        except SQLAlchemyError as e:
            error = str(e.__dict__['orig'])
            return error

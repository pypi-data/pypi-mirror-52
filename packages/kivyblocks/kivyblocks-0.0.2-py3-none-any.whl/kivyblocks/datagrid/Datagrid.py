...
class DataGridRow(GridLayout):
	...
	def __init__(self, data, **kwargs):
		print('kwargs=',kwargs)
		super(DataGridRow, self).__init__(cols=kwargs['cols'])
		...
		for colIndex in range(0, self.cols):
			...
			# border lable cell
				...
				if data[colIndex].get('borders'):
					x.borders= data[colIndex]['borders']
				else:
					x.borders= (.5, 'solid', (1,1,1,0.3))
			# button cell define
				...
				if data[colIndex].get('callback'):
					...
					if not param_kwargs:
						param_kwargs = {}
					x.bind(on_press=partial(callback, **param_kwargs))
...
class DataGridContent(ScrollView):
	def __init__(self, **kwargs):
		...
		kw.update(kwargs)
		del kw['row_height']
		super(DataGridContent, self).__init__(**kw)
		self.rowStoreWidget = []

...
class VPlayer(FloatLayout):
	...
	def createProgressbar(self,obj,v):
		...
		if self.pb is None:
			...
			self.curposition = Label(text='0',width=60,
				size_hint_x=None)
	...
	def buildMenu(self,obj,touch):
		...
		self.menubar.add_widget(btn_audioswitch)
	...
	def volumeinc(self,btn):
		self.vp.volume += 0.05
		if self.vp.volume > 1.0:
			self.vp.volume = 1.0

	def volumedec(self,btn):
		self.vp.volume -= 0.05
		if self.vp.volume < 0.0:
			self.vp.volume = 0.0

	def moveback(self,btn):
		f = self.vp.position / self.vp.duration
		self.vp.seek(f - 0.04,precise=True)

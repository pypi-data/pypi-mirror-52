...
def get_devices_48_106():
    ...
    return db_session.query(Device).filter((Device.device_last_transmission is not None) and (Device.device_last_transmission <= deltatime_48) and (Device.device_last_transmission > deltatime_106)).all()

def get_devices_w_low_power():
    return db_session.query(Device).filter(int(Device.device_battery_level, 16) <= 2800).all()

...
def load_model_file(eslib, id, inference_type=constants.ESTIMATOR):
        ...
        if (not _.is_empty(res)):
            try:
                filename = retrieve_file_from_minio(config['ELASTIC']['index_models'], res['_source']['model_file_path'], storage_folder, constants.MODEL_EXTENSION)
                pred.load_model(filename)
                model = pred.model
                filename = retrieve_file_from_minio(config['ELASTIC']['index_models'], res['_source']['model_file_path'], storage_folder, constants.SCALER_EXTENSION)
                scaler = joblib.load(filename)
                if inference_type == constants.CLASSIFIER:
                    filename = retrieve_file_from_minio(config['ELASTIC']['index_models'], res['_source']['model_file_path'], storage_folder, constants.LABELENCODER_EXTENSION)

...
def importer(algorithm, inference_type=constants.ESTIMATOR):
    # Keras lstm config
    if(inference_type == constants.ESTIMATOR):
        if(algorithm == constants.LSTM):
            ...
            ft_range = (-1, 1)
            activation = constants.LINEAR
        elif(algorithm == constants.DENSE):
    ...
    # Imported ML class
    pred = Predictor(activation=activation, ft_range=ft_range)
    # # -----------------------------------------------------------------------------------------------------------------
    return pred
...
def load_model_file(eslib, id, inference_type=constants.ESTIMATOR):
        ...
        :return: model, scaler,  predictor -> if inference_type==constants.ESTIMATOR
                 model, scaler,  predictor, labelencoder -> if inferencer_type==constants.CLASSIFIER
        """
        start_time = time_lib.clock()

        ok, res = eslib.get_last_document_with_params(config['ELASTIC']['index_models'], id, compose_model_params_filter(inference_type==constants.CLASSIFIER))
        ...
        if not ok:
            ...
            if inference_type == constants.ESTIMATOR:
                ...
                return None, None, pred, None
    

        if (not _.is_empty(res)):
            try:
                filename = retrieve_file_from_minio(config['ELASTIC']['index_models'], res['_source']['model_file_path'], '../storage/', constants.MODEL_EXTENSION)
                pred.load_model(filename)
                model = pred.model
                filename = retrieve_file_from_minio(config['ELASTIC']['index_models'], res['_source']['model_file_path'], '../storage/', constants.SCALER_EXTENSION)
                scaler = joblib.load(filename)
                if inference_type == constants.CLASSIFIER:
                    if('labelencoder_b64' in res['_source']):
                        filename = retrieve_file_from_minio(config['ELASTIC']['index_models'], res['_source']['model_file_path'], '../storage/', constants.LABELENCODER_EXTENSION)
                        labelencoder = joblib.load(filename)
                    else:
                        labelencoder = None
        ...
        if inference_type == constants.ESTIMATOR:
            ...
            return model, scaler, pred, labelencoder
...
def check_initial_config(eslib, is_classifier):
    ...
    """
    res = eslib.check_model_params(config['ELASTIC']['index_models'], compose_model_params_filter(is_classifier))
    if res:
...
def scale_dataset(scaler, data, i, ft_range=(-1, 1)):
    """
...
def compose_model_params(is_classifier):
    model_params = {
            ...
            'minio_address': config['MINIO']['minio_host'] + ':' + config['MINIO']['minio_port']
    }
    if is_classifier:
...
def save_model(eslib, id, model, scaler, metadata, labelencoder=None):
    try:
        filename = generate_file_name(dir='../storage')
        joblib.dump(scaler, filename + '.' + constants.SCALER_EXTENSION)

        model.save(filename + '.' + constants.MODEL_EXTENSION)

        uploaded = upload_file_to_minio(config['ELASTIC']['index_models'], filename, constants.MODEL_EXTENSION, id, config['ML']['algorithm'])
        if not uploaded:
            _logger.error('Error on uploading file to min.io filesystem(' + filename + '.' + constants.MODEL_EXTENSION + ') - ')
            return ''
        uploaded = upload_file_to_minio(config['ELASTIC']['index_models'], filename, constants.SCALER_EXTENSION, id, config['ML']['algorithm'])
        if not uploaded:
            _logger.error('Error on uploading file to min.io filesystem(' + filename + '.' + constants.SCALER_EXTENSION + ') - ')
            return ''
        if labelencoder:
            joblib.dump(labelencoder, filename + '.' + constants.LABELENCODER_EXTENSION)
            uploaded = upload_file_to_minio(config['ELASTIC']['index_models'], filename, constants.LABELENCODER_EXTENSION, id, config['ML']['algorithm'])
            if not uploaded:
                _logger.error('Error on uploading file to min.io filesystem(' + filename + '.' + constants.LABELENCODER_EXTENSION + ') - ')
                return ''
    ...
    doc = {
        ...
        'model_params': compose_model_params(labelencoder!=None)
    }

    # Save to elastic
    ok = eslib.add_to_index(config['ELASTIC']['index_models'], doc)

    with open(filename + '.' + constants.JSON_EXTENSION, 'w') as f:
        json.dump(doc, f)
        f.close()
        uploaded = upload_file_to_minio(config['ELASTIC']['index_models'], filename, constants.JSON_EXTENSION, id, config['ML']['algorithm'])
        if not uploaded:
...
def generate_file_path(filename, id, algorithm):
    return id + '/' + algorithm + '/' + filename.split('/')[-1]
...
def retrieve_file_from_minio(index, file_path, storage_folder, extension):
    try:
        if not minioClient.bucket_exists(index):
            ...
            return ''
        minioClient.fget_object(index, file_path + '.' + extension, storage_folder + 'tempfile' + '.' + extension)
        return storage_folder + 'tempfile' + '.' + extension
    except Exception as ex:
        _logger.error(str(ex))
        return ''

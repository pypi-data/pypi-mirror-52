...
def parse_conditions(raw):
    ...
    """
    if len(raw) > 5:
        raise ParseException("Only enter up to 5 stop_condition arguments", raw)
    stop_condition_list = []
    for arg in raw:
        stop_conditions = []
        and_conditions = arg.split("&&")
        for stop_condition in and_conditions:
            conditionsPattern = r'(>=|<=|>|<)'
            condition_op = ""
            for m in re.finditer(conditionsPattern, stop_condition):
                if condition_op != "":
                    raise ParseException("Too many conditions", arg)
                condition_op = m.group().strip()
                metric = stop_condition[:m.start(0)].strip()
                if len(metric.split(" ")) > 1:
                    raise ParseException("Only one metric is permitted per condition", metric)
                value, min_indices = parse_condition_requirements(stop_condition[m.end(0):])
            stop_conditions.append(ConditionSpecs(metric, condition_op, value, min_indices).to_payload())

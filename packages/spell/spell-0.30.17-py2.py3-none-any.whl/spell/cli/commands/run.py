...
@click.option("--stop-condition", multiple=True, metavar="CONDITION[:MIN_INDICES]",
              help="Enter metric conditions to stop a run early. A condition with a metric is required."
              "An optional minimum number of indices for the given metric may also be specified. "
              "To specify multiple conditions being met jointly use && between two conditions and "
              "their minimum number of indices if applicable (e.g --stop-condition "
              "'keras/val_acc > .9 && custom_metric > 1:2."
              "If multiple flags are provided the run will stop if any are met.")
@click.pass_context
def run(ctx, command, args, machine_type, pip_packages, requirements_file, apt_packages, stop_condition,
        ...
        conda_file, force, verbose, idempotent, provider, tensorboard_enabled, tensorboard_dir, distributed,
        run_type="user", **kwargs):
    """
    ...
    try:
        ...
        raise ExitException(click.wrap_text(
            "Incorrect formatting of param '{}', you can only provide up to 5 conditions of form".format(e.token) +
            "METRIC CONDITION (<,>,<=,>=,=) VALUE[:MIN]"))
    run_req = create_run_request(ctx, command, args, machine_type, pip_packages, requirements_file, apt_packages,
                                 ...
                                 run_type, tensorboard_enabled=tensorboard_enabled, tensorboard_dir=tensorboard_dir,
                                 distributed=distributed, stop_conditions=stop_conditions, **kwargs)
...
def create_run_request(ctx, command, args, machine_type, pip_packages, requirements_file, apt_packages,
                       ...
                       github_url, github_ref, distributed=None, tensorboard_enabled=False,
                       tensorboard_dir=None, stop_conditions=None, **kwargs):
    ...
    if framework is not None:
        ...
        if tensorboard_enabled and framework != "tensorflow":
            raise ExitException("Framework specified does not support TensorBoard")
    ...
    # TensorBoard checks
    if not tensorboard_enabled and tensorboard_dir is not None:
        raise ExitException("TensorBoard was not enabled!")
    if tensorboard_dir is None:
        tensorboard_dir = "/tensorboard"
    for mount_path in attached_resources.values():
        if (mount_path == tensorboard_dir):

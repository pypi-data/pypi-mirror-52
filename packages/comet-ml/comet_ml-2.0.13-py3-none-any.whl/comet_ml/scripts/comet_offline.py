...
class ArchivedExperiment(object):
    ...
    def __init__(self, filename, args, show_header=True):
        ...
        self.values = defaultdict(lambda: defaultdict(lambda: None))
        self.workspace = None
        self.project = None
    ...
    def process(self):
        ...
        try:
            ...
            print("Error in processing experiment; skipping...", file=sys.stderr)
        try:
            self.process_message()
        except Exception:
            print("Error in processing messages; skipping...", file=sys.stderr)

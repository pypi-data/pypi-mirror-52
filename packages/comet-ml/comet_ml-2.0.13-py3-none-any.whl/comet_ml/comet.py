...
from .messages import CloseMessage, Message, OsPackagesMessage, UploadFileMessage
...
class Streamer(BaseStreamer):
                ...
                if messages is not None:
                    ...
                    for message in messages:
                        if isinstance(message, CloseMessage):
                            ...
                            LOGGER.info(
                                "Invalid message received %r, discarding", message
                            )
    ...
    def _process_os_package_message(self, message):
        ...
        try:
            response = self.rest_api_client.set_experiment_os_packages(
                self.experiment_key, message.os_packages
            )
            if response.status_code != 200:
                LOGGER.debug(
                    OS_PACKAGE_MSG_SENDING_ERROR, response.status_code, response.content
                )
        except Exception:
...
class OfflineStreamer(BaseStreamer):
    ...
    def wait_for_finish(self):
        ...
        if self.isAlive():
            ...
            LOGGER.info("OfflineStreamer finished in time")
            return True

...
class Optimizer(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._api = OptimizerAPI(kwargs.get("api_key", None))
        self.id = None
    ...
    def next(self):
        ...
        if data is not None:
            return self._create_experiment(
                data["parameters"], data["pid"], data["trial"], data["count"]
            )
        else:
    ...
    def end(self, experiment):
        ...
        experiment.log_other(
            ...
            str({key: experiment.get_parameter(key) for key in status["parameters"]}),
        )
    ...
    def _create_experiment(self, parameters, pid, trial, count):
        ...
        if self.experiment_class == "Experiment":
            exp = Experiment(**self.experiment_kwargs)
        elif self.experiment_class == "OfflineExperiment":
            exp = OfflineExperiment(**self.experiment_kwargs)
        elif callable(self.experiment_class):
            exp = self.experiment_class(**self.experiment_kwargs)
        else:
            raise OptimizerException(

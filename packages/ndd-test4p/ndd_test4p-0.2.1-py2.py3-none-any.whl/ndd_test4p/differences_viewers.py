...
class YamlDifferencesViewer(DifferencesViewer):
    ...
    The expected data can be any data or a :class:`pathlib.PosixPath`.
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        if exc_type == AssertionError:
            if isinstance(self._expected_data_or_file_path, PosixPath):
    ...
    def _compare_data_with_file_content(self, actual_data: Any, expected_file_path: PosixPath) -> None:
        """
    ...
    @staticmethod
    def _write_yaml(directory_path: PosixPath, file_name: str, data: Any) -> PosixPath:

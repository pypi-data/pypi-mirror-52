...
def create_or_update(request, params):
    update_configuration = params.get('update_configuration',False)
    config = matialvarezs_node_configuration_utils.create_configuration(key=params['key'], value=params['identity'],update_configuration=update_configuration)
    ret = {
        "error": None,
        "ret": True,
    }

...
class Pump(_ObjectParameterWrapper):
    ...
    def __init__(self, optimization_problem, symbol, energy_price_symbol, semi_continuous=None):
...
class PumpingStation(_ObjectParameterWrapper):
    ...
    def pumps(self):
        ...
        if self._pumps is None:
            ...
            self._pumps = [Pump(self.optimization_problem, x, e, s)
                           for x, e, s in zip(self.pump_symbols, energy_price_symbols, semi_continuous_symbols)]
...
class _MinimizePumpGoal(Goal):
    ...
    def __init__(self, use_dynamic_nominal=True, exclude_continuous=False, *args, **kwargs):
        """
    ...
    def function(self, o, ensemble_member):
        if self.use_dynamic_nominal:
            priorities, nominals = list(zip(*o._pump_minimization_nominal[self._type]))
...
class PumpingStationMixin(OptimizationProblem):
    ...
    def pre(self):
        ...
        self._pump_minimization_nominal = OrderedDict()
        self._pump_minimization_function_values = []
        ...
        for ps in self.pumping_stations():
            for p in ps.pumps():
                ...
                min_power, _ = self._solve_power_subproblem(
                ...
                self._pump_power_bounds[power_sym] = (0.0, max(max_power))
        ...
        for ps in self.pumping_stations():
            ...
            for p in ps.pumps():
                ...
                if need_on_sym:
                    self._pump_status_bounds[sw_on_sym] = (0, 1)
                if need_off_sym:
        ...
        for ps in self.pumping_stations():
            for p in ps.pumps():
                avg_pump_power = max(self._pump_power_range_on[p.symbol + '__power'][1]) / 2
        ...
        for k, v in minimization_nominals.items():
            self._pump_minimization_nominal[k] = [(-np.inf, v * (self.times()[-1] - self.times()[0]))]
    ...
    def constraints(self, ensemble_member):
        ...
        for ps in self.pumping_stations():
            for p in ps.pumps():
                ...
                try:
                    pump_history = self.get_timeseries(p.symbol + '_status_hist')
                    ...
                    constraints.append((self.state_at(status_sym, times[0]),
                ...
                if p.minimum_on > 0.0:
                    if hist_on > 0.0 and p.minimum_on > hist_on:
                        min_on = p.minimum_on - hist_on

                        max_ind = np.searchsorted(times, min_on) + 1
                        constraints.append((ca.sum1(self.state_vector(status_sym)[1:max_ind]), 1.0, 1.0))

                    # Figure out at what times the pump status symbol should
                    # be on when the pump switches on at a particular timestep
                    status_end_inds = np.searchsorted(times, times[:-1] + p.minimum_on).tolist()
                    ...
                    for i in range(1, num_tsteps):
                        ...
                        constraints.append(
                            (sum_ - len(cur_status_inds) * self.state_vector(sw_on_sym)[i], 0, inf))

                if p.minimum_off > 0.0:
                    if hist_off > 0.0 and p.minimum_off > hist_off:
                        min_off = p.minimum_off - hist_off

                        max_ind = np.searchsorted(times, min_off) + 1
                        constraints.append((ca.sum1(self.state_vector(status_sym)[1:max_ind]), 0.0, 0.0))

                    # Figure out at what times the pump status symbol should
                    # be off when the pump switches off at a particular timestep
                    status_end_inds = np.searchsorted(times, times[:-1] + p.minimum_off).tolist()
                    status_range_inds = [np.arange(i + 1, min((e + 1), len(times))).tolist()
                                         for i, e in enumerate(status_end_inds)]
                    assert isinstance(status_range_inds[0][0], int), \
                        "Indexing CasADi symbols only fast with list of _Python_ ints"
    ...
    def _solve_hq_subproblem(self, H, Q, f, constraints=None, bounds=None):
        ...
        self._hq_subproblem_cache[pickle_key] = objective_value, solver_output[:2]
    ...
    def _solve_power_subproblem(self, working_area, working_area_direction, head_range,
        ...
        obj_power, (h, q) = self._solve_hq_subproblem(H, Q, X, constraints)
        ...
        if not np.allclose(power_eq, obj_power,
                           rtol=self._ineq_relative_error,
                           atol=self._ineq_absolute_error * min(abs(power_eq), abs(obj_power))):
    ...
    def path_constraints(self, ensemble_member):
        ...
        for ps in self.pumping_stations():
            for p in ps.pumps():
                ...
                constraints.append((status - continuous, 0, np.inf))
                ...
                m, Ms = self._pump_power_range_on[power_sym]
    ...
    def priority_completed(self, priority):
        ...
        minimal_nominals[_MinimizePumpGoalType.ENERGY] = \
            min(m for m, M in self._pump_power_range_on.values()) * min(tsteps)
        ...
        for ps in self.pumping_stations():
            for p in ps.pumps():
                ...
                min_power, max_power = self._pump_power_range_on[p.symbol + '__power']
        ...
        for k, v in minimization_nominals.items():
            ...
            self._pump_minimization_nominal[k].append((priority, integral_nominal))
        ...
        for g in goals:
            ...
            self._pump_minimization_function_values.append((g, float(obj_val)))
    ...
    def post(self):
        ...
        if any_pump_on:
            for g, obj_val in self._pump_minimization_function_values:
        ...
        for ps in self.pumping_stations():
            for p in ps.pumps():
                ...
                max_power = self._pump_power_bounds[p.symbol + "__power"][1]

                if not np.allclose(power_calculated[inds], power_realised[inds],
                                   rtol=self._ineq_relative_error,
                                   atol=self._ineq_absolute_error * max_power):
            ...
            for r in ps.resistances():
                ...
                if not np.allclose(head_loss_target, head_loss_realised,
                                   rtol=self._ineq_relative_error,
                                   atol=self._ineq_absolute_error):
...
def plot_operating_points(optimization_problem, output_folder, plot_expanded_working_area=True):
    ...
    for ps in optimization_problem.pumping_stations():
        for p in ps.pumps():
            ...
            hr = optimization_problem._head_range[p.head_option][ps.symbol]

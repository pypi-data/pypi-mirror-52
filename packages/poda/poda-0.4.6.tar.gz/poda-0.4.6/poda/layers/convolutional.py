...
def batch_normalization(input_tensor, is_trainable=True, decay = 0.999, epsilon = 1e-3):
    ...
    pop_mean = tf.Variable(tf.zeros(input_tf.get_shape().as_list()[-3:]), trainable=False)
    pop_var = tf.Variable(tf.ones(input_tf.get_shape().as_list()[-3:]), trainable=False)
...
def convolution_1d(input_tensor, number_filters, kernel_sizes=3, stride_sizes=(1,1), paddings='same', activations='relu', dropout_rates=None, names=None):
    """[summary]
...
def convolution_2d(input_tensor, number_filters, kernel_sizes=(3,3), stride_sizes=(1,1), paddings='same', activations='relu', dropout_rates=None, names=None):
...
def convolution_3d(input_tensor, number_filters, kernel_sizes=(3,3,3), stride_sizes=(1,1), paddings='same', activations='relu', dropout_rates=None, names=None):
    """[summary]
...
def depthwise_convolution_2d(input_tensor, number_filters=1, kernel_sizes=(3,3), stride_sizes=(1,1), paddings='same', activations='relu', dropout_rates=None, names=None):

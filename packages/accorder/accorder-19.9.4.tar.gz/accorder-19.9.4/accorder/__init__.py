...
def get_version(ctx, param, value):
    ...
    click.echo(f'accorder {pkg_resources.get_distribution("accorder").version}')
    click.get_current_context().exit()

@click.group(cls=NaturalOrderGroup)
@click.option("--version", "-v", is_flag=True, expose_value=False, callback=get_version)
def cli():
...
@cli.command(short_help="Manage configuration. For more help, type: accorder configuration --help")
@click.argument("configuration_id", required=False)
@click.option("--show", "-s", help="Show summary from the configuration file.", is_flag=True)
@click.option("--replace", "-r", help="Replace current configuration with the new one set by CONFIGURATION_ID. This flag requires CONFIGURATION_ID to be provided. (e.g: `accorder configuration --replace CONFIGURATION_ID``)", is_flag=True)
...
def build(profile, directory, librarian):
...
"""
    run_build(APP_NAME, profile, directory, librarian)
...
@cli.command(short_help="Maintain the shared Calibre's collection at the Memory of the World Library. For more help: accorder submit --help")
@click.argument("profile")
@click.option(
    "--librarian",
    "-l",
...
def submit(profile, librarian):
    ...
    run_submit(APP_NAME, profile, librarian)


@cli.command(short_help="Release local Calibre collection to the Memory of the World Library. For more help, type: accorder release --help")
@click.argument("profile")
@click.option(
    "--directory",
    "-d",
...
def release(profile, directory, librarian, delete_residue):

...
def process_peptides(args):
    ...
    # rt_thresholds = []
    # rt_thresholds.append(scoreatpercentile(rt_all, 25))
    # rt_thresholds.append(scoreatpercentile(rt_all, 50))
    # rt_thresholds.append(scoreatpercentile(rt_all, 75))


    mz_thresholds = []
    mz_thresholds.append(scoreatpercentile(mzraw_all, 25))
    mz_thresholds.append(scoreatpercentile(mzraw_all, 50))
    mz_thresholds.append(scoreatpercentile(mzraw_all, 75))

    # mz_group_all = np.array([sum(mz >= mz_threshold for mz_threshold in mz_thresholds) + 10 * sum(rt >= rt_threshold for rt_threshold in rt_thresholds) for mz, rt in zip(mzraw_all, rt_all)])

    # mz_thresholds = []
    # mz_thresholds.append(scoreatpercentile(mzraw_all, 25))
    # mz_thresholds.append(scoreatpercentile(mzraw_all, 50))
    # mz_thresholds.append(scoreatpercentile(mzraw_all, 75))
    mz_group_all = np.array([sum(mz >= mz_threshold for mz_threshold in mz_thresholds) for mz in mzraw_all])
    # mz_group_all = np.zeros(len(mzraw_all))
    ...
    if len(p1):
        ...
        true_group.extend(mz_group_all[e_ind])
        true_group = np.array(true_group)

        e_ind = true_isotopes >= 4
        true_md = true_md[e_ind]
        true_seqs = true_seqs[e_ind]
        true_group = true_group[e_ind]
        ...
        mass_shift_groups = dict()
        mass_sigma_groups = dict()
        for ii in set(mz_group_all):
            ind = true_group == ii
            print 'group', ii
            print len(true_md[ind])

            mass_shift, mass_sigma, covvalue = calibrate_mass(0.1, mass_left, mass_right, true_md[ind])
            if np.isinf(covvalue):
                mass_shift, mass_sigma, covvalue = calibrate_mass(0.01, mass_left, mass_right, true_md[ind])
            print 'Group:', ii
            print 'Calibrated mass shift: ', mass_shift
            print 'Calibrated mass sigma in ppm: ', mass_sigma
            print covvalue
            mass_shift_groups[ii] = mass_shift
            mass_sigma_groups[ii] = mass_sigma

        print mass_shift_groups
        mass_shifts = [mass_shift_groups[xyz] for xyz in mz_group_all]
        mass_sigmas = [mass_sigma_groups[xyz] for xyz in mz_group_all]
        # print mass_shifts[:50]
        e_all = abs(md_all - mass_shifts) / (mass_sigmas)
        r = 3.0
        e_ind = e_all <= r
    ...
    if len(p1):
        ...
        # mass diffs for PFMs of 1% FDR proteins

        ax = fig.add_subplot(4, 1, 4)
        bns = np.arange(md_all.min() - 0.5, md_all.max() + 0.5, 0.1)
        ax.hist(md_all, bins = bns)
        ax.set_xlabel('mass diff, ppm', size=16)
        ax.set_ylabel('# PFMs', size=16)

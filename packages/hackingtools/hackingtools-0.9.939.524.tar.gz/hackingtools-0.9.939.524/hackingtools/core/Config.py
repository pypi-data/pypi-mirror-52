...
def __createModuleTemplateConfig__(module_name, category):
    ...
    with open(os.path.join(os.path.dirname(__file__) , 'config.json')) as json_data_file:
        ...
        if not module_name in config_tmp['modules']:
            ...
            from . import Logger
            Logger.printMessage(message='__createModuleTemplateConfig__', description='Seem\'s to be a config into your config.json yet for that module. Ignoring this initialization', color=Logger.Fore.YELLOW)

    category_dir = os.path.join(os.path.dirname(__file__), 'config_modules_django', category)
    if not os.path.isdir(category_dir):
        os.mkdir(category_dir)

    module_config_file = os.path.join(category_dir, '{mod}.json'.format(module_name))
    ...
    new_conf['_django_form_main_function_']['__function__'] = "test_{mod}_FUNCTION_NAME".format(mod=module_name)

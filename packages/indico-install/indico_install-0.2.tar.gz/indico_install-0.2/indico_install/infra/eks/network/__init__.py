...
def _val_postgres(db_endpoint, vpc, subnet_ids, session=None):
    ...
    postgres = session.RDSClient.describe_db_instances(
        Filters=[{"Name": "db-instance-id", "Values": [db_endpoint.split(".")[0]]}]
    )["DBInstances"][0]
    ...
    if uncovered_subnet_cidrs:
        secho(f"subnets {list(uncovered_subnet_cidrs.keys())} may not covered in the DB security group rules for your database. Please review", fg="yellow")
...
def _val_fileshares(info, vpc, subnet_ids, session=None):
    ...
    for fs in shares:
        ...
        for cidr in subnet_cidrs:
            assert any(
                [is_covered_by(cidr, client) for client in clients]
            ), f"{fs['FileShareId']} does not allow access to subnet CIDR {cidr}"
...
def _val_subnets(cluster, vpc, session):
    ...
    for subnet in vpc.subnets.filter(SubnetIds=subnet_ids):
        ...
        if private_sub_tag not in subnet.tags:
            secho(
                f"No tag {private_sub_tag} on {subnet.subnet_id} - add this tag if this subnet is private!",
                fg="yellow",
            )
        assert (
            cluster_tag in subnet.tags

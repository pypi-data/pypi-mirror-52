...
VOLUME_TEMPLATE = Path(__file__).parent / ".." / ".." / "templates" / "volumes.yaml"
...
def _val_indicoapifs(fs, session=None):
    ...
    assert fs["LocationARN"] == "arn:aws:s3:::indico-platform-clients"
    assert fs["ObjectACL"] == "bucket-owner-full-control"
    assert fs["ReadOnly"]
    assert fs["GuessMIMETypeEnabled"]
...
def _val_sgs(sg_ids, conf, session=None):
    ...
    ), "Gateway security rules do not expose NFS port 2049 to all cluster subnets"

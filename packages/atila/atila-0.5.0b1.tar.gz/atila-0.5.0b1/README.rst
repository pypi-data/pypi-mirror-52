...
- static: Directory, Place static files like css, js, images.
  This directory should be mounted for using
- services: Directory, Module components, utils or helpers
  for helping app like config.py, model.py etc...
- templates: Directory, Jinaja and Chameleon template files
- resources: Directory, Various files as app need like sqlite
  db file. In you app, you use these files, you can access file in
  resources by app.get_resource ("db", "sqlite3.db") like os.path.join manner.
...
You can create __init__.py at same directory with app.py. And
...
If your app has large scale and namespace is highly required,

.. code:: python

  app.enable_namespace = True

That makes enable namespace for all your resources.
...
WS_THREAD
  ...
  - it treats every single websocket message as single request to resources
    like url requests.
...
WS_NOTHREAD
  ...
  - non-threaded function base websocket message handling
...
*Note:* WS_NOTHREAD and WS_SESSION will block SKitai event loop while you generate sendign message. If sending messasge generation time is reltively long, use WS_THREAD
...
.. code:: python
  ...
  @app.route ("/websocket")
  @app.websocket (skitai.WS_SIMPLE | skitai.WS_NOTHREAD, 60, onopen)
  def websocket (was, message):
    return 'you said: ' + message
...
FYI, you can access Django admin by /django/admin with
...
- 0.4
  ...
  - now, modules within \_\_mouint\_\_ are reloadable

...
class LSDDEM(object):
	...
	def CommonFlowRoutines(self, discharge = False, ingest_precipitation_raster = None, precipitation_raster_multiplier = 1):
		...
		if(discharge):
			...
			self.cppdem.calculate_discharge_from_precipitation(temp_loaded["nrows"], temp_loaded["ncols"],temp_loaded["x_min"], temp_loaded["y_min"], temp_loaded["res"], temp_loaded["nodata"][0],temp_loaded["array"])

		print("Done!") if(self.verbose) else 0

	def ExtractRiverNetwork(self, method = "area_threshold", area_threshold_min = 1000):
		"""
		Description:
		...
		if(method=="area_threshold"):
			self.cppdem.calculate_channel_heads("min_contributing_pixels", area_threshold_min)
		else:
			print("Not Implemented yet! sns.")
		self.cppdem.calculate_juctionnetwork()

	def DefineCatchment(self, method="min_area_pixels", test_edges = False, min_area_pixels = 1000, X_coords = [], Y_coords = [], 
	 ...
	 coordinates_system = "UTM", UTM_zone = -1):
		...
		if(method == "min_area_pixels"):
			self.cppdem.calculate_outlets_locations_from_minimum_size(min_area_pixels, test_edges, False)
		elif(method == "from_XY"):
			...
			self.cppdem.calculate_outlet_location_of_main_basin(test_edges)
		else:
	...
	def save_array_to_raster_extent(self, array, name = "custom_raster"):
		""" 
			...
			Date:
				18/03/2019
		"""
		rl.save_raster(array, self.x_min, self.x_max, self.y_max,self.y_min, self.resolution,self.crs,self.save_dir+name+".tif", fmt = 'GTIFF')

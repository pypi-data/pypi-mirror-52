...
def configure_parser(parser, version):
    ...
    if version >= (2019, 6):
        ...
        env_get.add_argument("hostname", type=str, help="worker hostname")

        env_set = env_sub.add_parser("set", help="set the worker environment")
        env_set.add_argument("hostname", type=str, help="worker hostname")
...
def handle_env(proxy, options, _):
    if options.sub_sub_sub_command == "get":
        env = proxy.scheduler.workers.get_env(options.hostname)
        print(str(env).rstrip("\n"))
    else:
        env = options.env.read()
        ret = proxy.scheduler.workers.set_env(options.hostname, env)
        if not ret:

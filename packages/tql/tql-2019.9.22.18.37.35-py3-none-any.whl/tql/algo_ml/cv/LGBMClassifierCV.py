...
class LGBMClassifierCV(object):
    ...
    def fit(self, X, y, X_test=None, feval=roc_auc_score, fix_valid_index=None, sample_weight=None, init_score=None,
            eval_metric='auc', early_stopping_rounds=100, verbose=100, feature_name='auto', categorical_feature='auto',
            callbacks=None):
        """
        :param X: 数组
        :param y:
        ...
        :return:
        """
        fix_valid_index = set(fix_valid_index)
        if X_test is None:
            X_test = X[:1]
        ...
        for n_fold, (train_index, valid_index) in enumerate(self._kf.split(X, y)):
            ...
            if fix_valid_index is not None:
                valid_index = list(fix_valid_index & set(valid_index))
...
if __name__ == "__main__":
    ...
    clf.fit(X, y, fix_valid_index=range(50))

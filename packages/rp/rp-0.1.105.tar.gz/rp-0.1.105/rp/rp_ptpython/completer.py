...
def ryan_completion_matches(origin:str,candidates:list):
    ...
    # rp.tic()
    import re,regex
    compiled=re.compile('.*'+'.*'.join(regex.escape(x) for x in origin.lower())+'.*')
    c=candidates=[c for c in candidates if compiled.fullmatch(c)]#Speed things up and weed out bad candidates

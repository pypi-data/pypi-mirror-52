...
def ryan_completion_matches(origin:str,candidates:list):
    ...
    import re,regex
    compiled=re.compile('.*'+'.*'.join(regex.escape(x) for x in origin.lower())+'.*')#Using regex to weed out bad candidates sped this function up by an order of magnitude when we had 4000 candidates

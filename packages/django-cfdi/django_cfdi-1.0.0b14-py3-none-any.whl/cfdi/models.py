...
class Cfdi(models.Model):
    ...
    def cancelar_cfdi(self, config, timbrado_prueba, v33=False):
        ...
        cfdi.rfc = cfdi.rfc = config.rfc
        cfdi.uuid = cfdi.cfdi_uuid = self.cfdi_uuid
        cfdi.certificado = config.certificado
        cfdi.key = config.key
        cfdi.csd_pass = config.csd_pass
        cfdi.cfdi_xml = self.cfdi_xml
        cfdi.pem_path = config.pem_path()
        cfdi.cer64_finkok = config.cer64_finkok

        ...
        if "action" in admin_request.keys():
            ...
            if admin_request["action"][0] == "update":
                        ...
                        try:
                            ...
                            # Handle datetime values.
                                ...
                                if isinstance(property_value[0], str):
                                    print("DATETIME PROP = ", property_value)
                                    try:
                            ...
                            # Handle date values.
                                ...
                                if isinstance(property_value[0], str):
                                    ...
                                    # Per default the Neo4j database add timezone (represented by 9 characters) to time object.
                                        ...
                                        property_value[0] = datetime.date.fromisoformat(property_value[0][:-9])
                ...
                if request.user.has_perm("delete_" + node_model_name.lower()) or request.user.has_perm("delete"):
                    ...
                    raise PermissionError("Vous n'avez pas la permission de supprimer cette instance.")
                    ...
                    if response.ready():
                        ...
                        if response.successful():
                            if not response.get():
                                ...
                                print("ERROR FROM ASYNC TASK = ", response.get())
                                print("ERROR LINE = ", response.get()[2].tb_lineno)
                ...
                yield """
                    <script>
                        function redirect_to_home () {
                            window.location.href = window.location.origin + '/admin/gestion/%s'
                        }
...
def handle_creation(request_POST, request_FILES, node_model, node_model_name, available_objects_dict):
...
def node_creation_view(request, node_model_name):
    ...
    if request.user.has_perm("create_" + node_model_name.lower()) or request.user.has_perm("create"):
        ...
        if request.POST or request.FILES:
            # response = handle_creation(request, node_model, node_model_name, available_objects_dict)
            #
            # if response == "preview fields":
            #     add_message(request, SUCCESS, "L'instance a bien été créée.")
            #     return redirect("node_model_home", node_model_name=node_model_name, permanent=True)
            #
            # elif response == "no preview fields":
            #     add_message(request, SUCCESS, "L'instance a bien été créée.")
            #     return redirect("handling_home", permanent=True)
            ...
            def my_generator():
                ...
                response = pool.apply_async(handle_creation,
                                            (request.POST, request.FILES, node_model, node_model_name,
                                             available_objects_dict))
        ...
        return render(request, "handling/pages/node_creation.html", {"admin_fields_dict": admin_fields_dict,

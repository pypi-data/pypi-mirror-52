...
def node_creation_view(request, node_model_name):
    ...
    if request.user.has_perm("create_" + node_model_name.lower()) or request.user.has_perm("create"):
        ...
        if request.POST or request.FILES:
            ...
            def my_generator():
                response = pool.apply_async(handle_creation, (request.POST, request.FILES, node_model, node_model_name, available_objects_dict))
                ...
                while True:
                    ...
                    print("resp not ready from generator")
                    print(response.__dict__)
                    import time
                    time.sleep(3)
                    yield "pong"

            return HttpResponse(my_generator(), content_type='text/html')

            if response.ready():
                print("response ready")
                print("response = ", response)
                if response.successful():
                    print("RESPONSE IN IF : ", response)
                    add_message(request, SUCCESS, "L'instance a bien été créée.")
                    return redirect("node_model_home", node_model_name=node_model_name, permanent=True)

                # elif response == "no preview fields":
                #     print("RESPONSE IN ELIF : ", response)
                #     add_message(request, SUCCESS, "L'instance a bien été créée.")
                #     return redirect("handling_home", permanent=True)

/* This script is not a bulb's script. It was recovered from this opensource project : https://github.com/maykinmedia/dual-listbox */
...
window.addEventListener('load', function () {
    let form = document.getElementById("handling-form");
    if (!form) {
        form = document.getElementById("creation-form");
    }
    const submit_button = document.querySelector("button#submit-button");

    const to_send_dict = {};
    const dual_listbox_rels = form.getElementsByClassName("dual-listbox-rel");

    for (const dual_listbox_rel of dual_listbox_rels) {
        const dual_listbox = new DualListbox(`#${dual_listbox_rel.getAttribute("id")}`, {
            addButtonText: '>',
            removeButtonText: '<',
            addAllButtonText: '>>',
            removeAllButtonText: '<<',
        ...
        const field_name = dual_listbox.select.id;
        to_send_dict[field_name] = {};
        to_send_dict[field_name]["add"] = [];
        to_send_dict[field_name]["remove"] = [];

        dual_listbox.addEventListener("added", function (event) {
            const added_element = event.addedElement;
            to_send_dict[field_name]["add"].push(added_element.getAttribute("data-id"));

            if (to_send_dict[field_name]["remove"].includes(added_element.getAttribute("data-id"))) {
                to_send_dict[field_name]["remove"].pop(added_element.getAttribute("data-id"))
            }

        });

        dual_listbox.addEventListener("removed", function (event) {
            const removed_element = event.removedElement;
            to_send_dict[field_name]["remove"].push(removed_element.getAttribute("data-id"));

            if (to_send_dict[field_name]["add"].includes(removed_element.getAttribute("data-id"))) {
                to_send_dict[field_name]["add"].pop(removed_element.getAttribute("data-id"))
            }
        });
    }
    ...
    submit_button.addEventListener("click", function (event) {
        ...
        const relationship_hidden_input = document.createElement("input");
        relationship_hidden_input.type = "hidden";
        relationship_hidden_input.name = "relationships-helper";
        relationship_hidden_input.value = JSON.stringify(to_send_dict);

        form.appendChild(relationship_hidden_input);
        form.submit()
        // return false
    })
});

...
class FCStats():
    ...
    def _stats_returner(self, fc, out, **kwargs):
        ...
        if out == 'inplace':
            ...
            # TODO: Include filling factor calcualtion routines (given a density ratio and a critical density ratio)
...
class FractalCube():
    ...
    def write_cube(self, fc=None, fname='data.dbl', app=True, pad=False, prec='double'):
        """
...
class LogNormalFractalCube(FractalCube):
    ...
    def yield_cubes(self, history=True, verbose=True):
        ...
        lkmag, lk_iso = mt.zero_log10(kmag), mt.zero_log10(k_iso)
        ...
        sf_lk_iso = np.s_[np.logical_and(
            np.greater_equal(lk_iso, lkmin),
            np.less_equal(lk_iso, lkmax))]
        # sf_lk_iso = np.s_[np.greater_equal(lk_iso, lkmin)] # without kmax

        sf_lk_iso[0] = False

        sf_lkmag = np.s_[np.logical_and(
            np.greater_equal(lkmag, lkmin),
            np.less_equal(lkmag, lkmax))]
        # sf_lkmag = np.s_[np.greater_equal(lkmag, lkmin)] # without kmax

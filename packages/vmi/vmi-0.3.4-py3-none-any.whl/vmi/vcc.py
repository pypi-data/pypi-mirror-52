...
def ccExplore_Recursive(sh: TopoDS_Shape, target_type: int = ccType[-1]):
...
def ccExplore_Once(sh: TopoDS_Shape):
...
def ccOpenFile_IGES(file: str = None):
    if file is None:
...
def ccOpenFile_STEP(file: str = None):
    if file is None:
...
def ccSaveFile_IGES(sh: TopoDS_Shape, file=None):
    if file is None:
...
def ccSaveFile_STEP(sh: TopoDS_Shape, file=None):
    if file is None:
...
def ccSaveFile_STL(sh: TopoDS_Shape, file=None):
    if file is None:
        file = vmi.askSaveFile(tr('导出 (Export) STL'), '*.stl')
        if file is None:
            return
    return stlapi.Write(sh, file, False)


def ccPd_Sh(sh: TopoDS_Shape):
    if sh.IsNull():
...
def ccSh_Pd(pd: vtk.vtkPolyData):
...
def ccSegments(pts, closed=False):
...
def ccBSpline(pts, tgs=None, tgflags=None, closed=False, scale=True, tol=precision.Confusion()):
    if len(pts) < 2:
        return None
...
def ccBSpline_Kochanek(pts, closed=False, tol=precision.Confusion()):
    if len(pts) < 2:
        return None
...
def ccCurveOnPlane(curve, origin, normal, vt=None):
    if vt is None:
        vt = normal
    return geomprojlib.ProjectOnPlane(curve, ccPlane(origin, normal), ccDir(vt), True)


def ccCircle_Ax2(ax2, radius):
    mk = GC_MakeCircle(ax2, radius)
    return mk.Value() if mk.IsDone() else None


def ccCircle_CS(center, radius, cs: vmi.CS4x4):
    ax2 = ccAx2(center, cs.axis(2), cs.axis(0))
    return ccCircle_Ax2(ax2, radius)


def ccPolygonRegular_CS(center, radius, n, cs: vmi.CS4x4):
    pts, center, cs = [], np.array(center[:3]), cs.copy()
    cs.translate(radius * cs.axis(0))

    for i in range(n):
        cs.rotate(360 / n, cs.axis(2))
        pt = center + radius * cs.axis(0)
        pts.append(pt)
...
def ccLoft(sections: List[TopoDS_Wire], ruled=False, redirect=False):
...
def ccBoolean_Union(sh: List[TopoDS_Shape]):
    if len(sh) == 0:
        return None
    elif len(sh) == 1:
        return sh[0]
...
def ccBoolean_Difference(sh: List[TopoDS_Shape]):
    if len(sh) == 0:
        return None
    elif len(sh) == 1:
        return sh[0]
...
def ccBoolean_Intersection(sh: List[TopoDS_Shape]):
    if len(sh) == 0:
        return None
    elif len(sh) == 1:
        return sh[0]
...
def ccSection(sh: List[TopoDS_Shape]):
    if len(sh) == 0:
        return None
    elif len(sh) == 1:
        return sh[0]
...
def ccCylinder(radius, height, origin, z, x=None):
...
def ccUniformPts_Edge(edge, n, u=None):
    if n < 2:
        return

...
class PolyActor(QObject, vmi.Menu, vmi.Mouse):
    def __init__(self, view: vmi.View, name=None, rgb=None, point=None, line=None, pickable=None):
...
class ImageBox(QObject, vmi.Menu, vmi.Mouse):
    def __init__(self, view: vmi.View, name=None, image=None, size=None, pos=None, anchor=None, pickable=None):
        QObject.__init__(self)

        self.name = name if name else tr('绘图框 （ImageBox)')
        vmi.Menu.__init__(self, name=self.name)
...
class TextBox(ImageBox):
    def __init__(self, view: vmi.View, name=None, text: str = None, text_align=None,
                 fore_color=None, back_color=None, bold=None, italic=None, underline=None,
                 size=None, pos=None, anchor=None, pickable=None):
        self.name = name if name else tr('文本框 （TextBox)')
        ImageBox.__init__(self, view, name, None, size, pos, anchor, pickable)

        self._Font = QFont('等线')
        self._Text = str()
        self._TextAlign = Qt.AlignLeft and Qt.AlignVCenter
        self._ForeColor = QColor('black')
        self._BackColor = QColor('whitesmoke')
        self._Bold = False
        self._Italic = False
        self._Underline = False
        self.draw_text(text, text_align, fore_color, back_color, bold, italic, underline, size, pos, anchor)
    ...
    def draw_text(self, text: str = None, text_align=None, fore_color=None,
                  back_color=None, bold=None, italic=None, underline=None, size: List[float] = None,
        ...
        if rect.width() > 0 and rect.height() > 0:
            font_size *= 0.5 * w / rect.width() if w / rect.width() < h / rect.height() else 0.5 * h / rect.height()

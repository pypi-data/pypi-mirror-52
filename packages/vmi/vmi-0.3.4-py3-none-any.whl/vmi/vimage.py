...
def imVTK_Array(ary: np.ndarray, origin: np.ndarray, spacing: np.ndarray) -> vtk.vtkImageData:
    """
    转换图像数据类型
...
def imReslice(image: vtk.vtkImageData,
              ...
              scalar_background: Union[int, float] = 0) -> vtk.vtkImageData:
    """
    图像重采样
        ...
        for j in range(ary.shape[1]):
            for i in range(ary.shape[2]):
                ...
                if len(voxels):
                    ...
                    ary[0][j][i] = scalar_background

        return ary[0]

    image = imReslice(image, cs, size, scalar_background)

...
class SelectEdit(QObject, vmi.Menu, vmi.Mouse):
    def __init__(self, view: vmi.View, name=None):
        ...
        self.curveActor.bind(self.curveShape)
    ...
    def addNode(self, pt):
        if pt not in self.nodes:
            ...
            self.nodeActors[-1].bind(self.nodeShapes[-1])
    ...
    def rebuild(self):
        for i in range(len(self.nodes)):
            self.nodeShapes[i].clone(vmi.ccVertex(vmi.ccPnt(self.nodes[i])))
        if len(self.nodes) > 2:
            if self.curveType == 0:
                self.curveShape.clone(vmi.ccWire(vmi.ccSegments(pts=self.nodes, closed=True)))
            elif self.curveType == 1:
                self.curveShape.clone(vmi.ccWire(vmi.ccBSpline_Kochanek(pts=self.nodes, closed=True)))
        elif len(self.nodes) > 1:
            if self.curveType == 0:
                self.curveShape.clone(vmi.ccWire(vmi.ccSegments(pts=self.nodes, closed=False)))
            elif self.curveType == 1:
                self.curveShape.clone(vmi.ccWire(vmi.ccBSpline_Kochanek(pts=self.nodes, closed=False)))
        else:
    ...
    def apply(self):
        if self._Bind is not None:
            ...
            if spc is not None:
                ...
                self._Bind.clone(vmi.imResample(self._Bind.data(), ori, ext, spc))
                self.view.updateInTime()
            ...
            for k in range(3):
                for j in range(2):
                    for i in range(2):
                        ...
                        self.regionShapes[k][j][i].clone(vmi.ccEdge(vmi.ccSegment(pt[0], pt[1])))
        else:
            for k in range(3):
                for j in range(2):
                    for i in range(2):

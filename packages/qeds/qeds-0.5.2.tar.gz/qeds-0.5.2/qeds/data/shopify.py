...
def simulate_orders(N=700000, start_date="2014-01-01", end_date=None):
    ...
    unique_cust_ids = np.random.randint(
        100000000, 999999999, cust_counts.sum()
    )
    cust_id_index = list(
        chain(
            *[
    ...
    # ------- Day
    real_N = out.shape[0]
    out["Day"] = pd.Series(random_dates(start_date, end_date,
                                        real_N)).sort_values().values
    ...
    out["Ordered quantity"] = np.round(
        np.interp(np.random.rand(real_N), quantiles, quantity_per_order_qs)
    ) * out["orders"]
    ...
    price_per_item_qs = np.array(
        [
            0.0, 4.95, 6.95, 8.95, 11.95, 15.95, 20.95, 23.95, 31.95, 33.95,
            50.95, 62.95, 144.95
        ]
    )
    price_per_item = np.interp(
        np.random.rand(real_N), quantiles, price_per_item_qs
    )
    ...
    out["Discounts"] = np.interp(
        np.random.rand(real_N), quantiles, non_neg_discount_qs
    ) * discounted
    ...
    out["Tax"] = np.interp(np.random.rand(real_N), quantiles,
                           tax_rate_qs) * out["Gross sales"] * taxed / 100
    ...
    out["Shipping"] = np.interp(
        np.random.rand(real_N), quantiles, non_zero_shipping_per_order_qs
    ) * shipping
    ...
    out["Returned quantity"] = np.round(
        np.interp(np.random.rand(real_N), quantiles, quantity_per_order_qs)
    ...
    col_order = [
        'Day', 'customer_type', 'Customer ID', 'orders', 'total_sales',
        'Returns', 'Ordered quantity', 'Gross sales', 'Net sales', 'Shipping',
        'Tax', 'Net quantity', 'Returned quantity', 'Discounts'
    ]

...
def fourier_df(pd_index, freq, k, name='ffcomp'):
    ...
    """
    return pd.DataFrame(fourier(k=k, n=pd_index.shape[0], freq=freq),
                        index=pd_index,
...
def fixed_start_fourier_df(pd_index, freq, k, name='ffcomp'):
    ...
    """
    assert isinstance(pd_index, pd.Index), "Provide pd.Index subclass as the first argument"
    assert pd_index.freq, "freq for input index is not defined"
    assert freq in FREQ_OFFSETS.keys(), f"Only freqs {FREQ_OFFSETS.keys()} are supported"
    ...
    freq_p = pd.date_range(normalized_start, normalized_start + o_len, freq=pd_index.freq).shape[0] - 1

    return pd.DataFrame(fourier(k=k, n=pd_index.shape[0] + offset, freq=freq_p)[offset:],
                        index=pd_index,
                        columns=[f"{name}_{i}" for i in range(0, k * 2)])

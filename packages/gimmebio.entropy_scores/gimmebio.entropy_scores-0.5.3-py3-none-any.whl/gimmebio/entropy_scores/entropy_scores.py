...
def entropy_score_fastq(fastq_filename, delim=None):
    ...
    if delim is None:
        ...
        return [(row['genome'], row['score']) for _, row in scores.groupby('genome').mean().iterrows()]
...
def _clump_score(samfile, binsize=10000, min_bins=3, delim=None):
    ...
    for read in samfile:
        if read.is_unmapped:
            continue
        nreads += 1
        genome = read.reference_id.split(delim)[0] if delim else 'all'
...
def entropy_and_clump_score(bam_filename, binsize=10000, min_bins=3, delim=None):
    ...
    if delim is None:
        ...
        escores = pd.DataFrame([
            {'genome': read.reference_id.split(delim)[0], 'score': seq_entropy_score(read.query_sequence)}
            for read in samfile
        ])
        escore = [(row['genome'], row['score']) for _, row in scores.groupby('genome').mean().iterrows()]

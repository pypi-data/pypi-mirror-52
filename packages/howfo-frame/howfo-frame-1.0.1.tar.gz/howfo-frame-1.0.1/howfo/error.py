from howfo import logger as log
from howfo.helpers import get_uuid
from howfo.config.const import CODE_MSG
...
class CustomFlaskErr(Exception):
    ...
    def to_dict(self):
        ...
        if self.msg_dict is not None:
            s = CODE_MSG[self.return_code].format(**self.msg_dict)
        else:
            s = CODE_MSG[self.return_code]

        rv['message'] = s
        rv.update({'resp_id': get_uuid()})
        s += ' [resp_id:{}]'.format(rv['resp_id'])
        # 日志打印
        log.warning(s)

...
def usage():
    """ print usage info """
    print("usage: mnnconvert")
    print(" [--framework FRAMEWORK] [--modelFile MODELFILE]")
    print(" [--prototxt PROTOTXT] [--MNNModel MNNMODEL]")
    print(" [--benchmarkModel BENCHMARKMODEL] [--bizCode BIZCODE]")
    print(" see more detail in https://www.yuque.com/mnn/en/tool_converter ")
...
def main():
    ...
    parser.add_argument("--framework", type=str, default='TF',\
        help="model type, for example:TF/CAFFE/ONNX/TFLITE/MNN")
    ...
    parser.add_argument("--MNNModel", type=str,\
        help="MNN model, ex: xxx.mnn")
    parser.add_argument("--benchmarkModel", type=bool, default=False,\
        help="if True, can only be used in benchmark mode")

...
def _decode_keyfile_json_v3(keyfile_json, password):
    ...
    # Ygg used aes-128-cbc and append final 12byte (so 48byte)
    if (crypto['cipher'] == 'aes-128-cbc'):
        private_key = decrypt_aes_cbc(ciphertext, encrypt_key, decode_hex(iv))[:32]
    else:
        iv = big_endian_to_int(decode_hex(iv))
...
def encrypt_aes_cbc(ciphertext, key, iv):
    encryptor = AES.new(key, AES.MODE_CBC, IV=iv)

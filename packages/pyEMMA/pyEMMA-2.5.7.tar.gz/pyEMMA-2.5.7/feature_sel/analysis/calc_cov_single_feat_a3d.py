...
from pyemma.coordinates import source, featurizer
...
files = {
    name: {feature: sorted(glob(PRE_CALC_FEATURES_PATH + '/*{sys}*{feat}*.h5'.format(sys=name, feat=feature)))
           for feature in features if feature != 'xyz'
...
grid = ParameterGrid([{'test_system': test_systems,
...
def run(id):
    ...
    try:
        if feature == 'xyz':
            ...
            reader = source(traj_top_pairs['trajs'], features=create_featurizer(), chunk=10000)

        else:
            if feature.startswith('dist_ca_'):
                files_for_features = [files[test_system]['dist_ca']]
            else:
            ...
            if feature.startswith('dist_ca_'):
                name = feature
                match = re.match('dist_ca_c_(.*)', feature)
                if match is not None:
                    arg = {'c': float(match.group(1))}
                else:
                    arg = {'scheme': re.match('dist_ca_(.*)', name).group(1)}
                trans = DistTrans(**arg)
...
if __name__ == '__main__':
    try:
        ...
        raise
        # this does not work for some celery internal things.
        # print('evaluating the grid')
        # tasks = [run.s(id_) for id_ in range(0, len(grid))]
        # jobs = group(tasks)

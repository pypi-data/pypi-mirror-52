...
class ModelExploreWidget(BaseWidget):
    ...
    def _find_child_models(self, kps):
        ...
        for index in kps[~kps.kp_description.isna()].index:
            ...
            child_uuid.extend([knowledgepack_description[child_model]['uuid'] for child_model in list(
    ...
    def _recognize_signal(self, capture_file, recognize=True):
        ...
        if self.results is None:
            ...
            if self._w_file_list.value == 'captures':
                ...
                self.results, s = self.kp.recognize_signal(
                    datafile=capture_file, renderer=self.renderer)
    ...
    def _read_knowledgepack_description(self, kp, widget_tab):
        ...
        for child_model in knowledgepack_description.keys():
            ...
            if widget_tab == 'feature_summary':
                ...
                df_html = temp_kp.confusion_matrix_by_metric(
                    'validation').__html__()

            report = str([v + ": " + temp_kp.class_map[i] for (i, v)
                          in knowledgepack_description[child_model]['results'].items()])

            df_text = "<br>" + \
                '<h1 style="font-size:300%;">' + child_model + ' </h1>' + \
                '<h1 style="font-size:150%;"> Class Map: ' + report + "</h1>"
    ...
    def _get_confusion_matrix_html(self, kp):
        if self.kp.knowledgepack_description:
            df_text = "<br>" + '<h1 style="font-size:300%;"> Confusion Matrix : Feature Recall </h1>'
            html_list = df_text + \
                self.kp.confusion_matrix_by_metric('hierarchical').__html__()
            return html_list + self._read_knowledgepack_description(kp, 'confusion_matrix')
        else:
    ...
    def create_widget(self):
        ...
        self._w_file_list = Dropdown(
            description="Select File Type", options=['captures', 'datafiles'], value='captures',  style={'description_width': '112px'})
        ...
        model_explore_tabs = Tab([
            VBox([HBox([self._html_capture_explanation, self._w_file_list]),

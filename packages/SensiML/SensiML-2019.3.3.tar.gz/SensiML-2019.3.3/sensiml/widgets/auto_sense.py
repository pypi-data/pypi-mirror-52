from ipywidgets import Layout, Button, VBox, HBox, Dropdown, Checkbox, Label, widgets, HTML, Tab
...
autosense_metric_column = [
    'Accuracy',  'Classifer Size(B)', 'Num. Features', 'Sensitivity', 'f1-score']
...
class AutoSenseWidget(BaseWidget):
    ...
    def _run_pipeline_optimizer(self):
        ...
        demean_segments = self._c_strip_mean.value
        ...
        params = {'iterations': iterations,
                  ...
                  'survior_rate': survivor_rate,
                  'allow_unknown': allow_unk,
                  'validation_method': validation_method,
                  'balanced_data': balanced_data,
                  "demean_segments": demean_segments,
                  'outlier_filter': outlier_filter,
                  "allow_hierarchical_model": hierarchical_model,
        ...
        if seed_choice == 'Custom Feature Generatorset':
            params['generatorset'] = self.custom_feature_generator.get_fature_generator()
    ...
    def _set_auto_sense_results(self):
        ...
        if self.summary is not None and isinstance(self.summary, dict) and isinstance(self.summary.get('fitness_summary', None), DataFrame):
            ...
            df_results = self.summary['fitness_summary'][[
                'accuracy', 'classifiers_sram',  'features', 'sensitivity', 'f1_score']].astype(int).head()
            ...
            df_results = df_results.rename(
                index=str, columns={"f1_score": "f1-score"})
            df_results = df_results.rename(
    ...
    def _on_value_change(self, change):
        ...
        if self._dsk.pipeline and change['new']:
            ...
            self._w_validation_metadata.options = [x for x in self._dsk.pipeline.group_columns if x not in [
                self._dsk.pipeline.label_column, 'SegmentID']]
        else:
    ...
    def _on_validation_method_change(self, change):
        ...
        if change['new'] in ['Stratified Shuffle Split']:
            set_states_visible([self._w_validation_size,
                                self._info_validation_method_size])
        else:
            set_states_hidden([self._w_validation_size,
                               self._info_validation_method_size])
        ...
        if change['new'] in ['Stratified K-Fold Cross-Validation', 'Stratified Shuffle Split',
                             'Metadata k-fold', 'Stratified Metadata k-fold']:
            set_states_visible(
    ...
    def _refresh(self, b=None):
        if self._dsk:
            ...
            if self._dsk.pipeline:
                self._w_pipeline_label.value = "Pipeline Name: <b>" + \
                    self._dsk.pipeline.name + "</b>"
    ...
    def create_widget(self):
        ...
        self._w_segment = widgets.Dropdown(description='Segmenter',
                                           options=['', 'Query Segmenter', 'Windowing'], style=description_style)
        ...
        self._info_segmenter = widgets.Button(
            icon="question",
            disabled=True,
            tooltip='Defines type of segmenter to use in your pipeline.',
            layout=info_layout
        )
        ...
        self._info_hierarchical_model = widgets.Button(
            icon="question",
            disabled=True,
            tooltip='Optimize the prediction results by creating Hierarchical Binary Models.',
            layout=info_layout
        )
        ...
        self._info_strip_mean = widgets.Button(
            icon="question",
            disabled=True,
            tooltip='Remove the mean from  each data source before before extracting features. This helps avoid overfitting models to noise and drift.',
            layout=info_layout
        )
        ...
        self._info_outlier_filter = widgets.Button(
            icon="question",
            disabled=True,
            tooltip='Filter oultiers for each class using Isolation forest filtering. Filters 5pct. of oultiers',
            layout=info_layout
        )
        ...
        tooltip_window_size = """
            ...
            range information. A larger model will also need to store more data for 
        ...
        self._w_classifiers_sram = widgets.BoundedIntText(
            description='Classifier Size (B)', value=32000, min=0, max=512000, style=description_style)

        self._w_iterations = widgets.IntSlider(
            description="Iterations", value=4, min=1, max=15, step=1, style=description_style)

        self._w_population_size = widgets.IntSlider(
            description="Initial Population", value=100, min=25, max=200, step=1, style=description_style)
        ...
        self._c_hierarchical_model = Checkbox(
            description='Hierarchical Model Optimization', value=False)

        self._c_balanced_data = Checkbox(
            description='Balance Data', value=False)

        self._c_strip_mean = Checkbox(
            description='Strip Mean', value=False)

        self._c_outlier_filter = Checkbox(
            description='Filter Outliers', value=False)

        self._w_combine_labels = widgets.Text(
            description='Combine Labels', value='')

        self._w_validation_method = widgets.Dropdown(description='Validation', options=[
            'Stratified K-Fold Cross-Validation', 'Stratified Shuffle Split', 'Metadata k-fold', 'Stratified Metadata k-fold', 'Recall'], style=description_style)

        self._w_validation_size = widgets.IntSlider(
            description='Validation(%)', value=20, min=0, max=100, step=10, style=description_style)

        self._w_validation_size = widgets.IntSlider(
            description='Validation(%)', value=20, min=0, max=100, step=10, style=description_style)

        self._w_validation_metadata = widgets.Dropdown(description='metadata', options=[
        ], style=description_style)

        self._w_number_folds = widgets.IntSlider(
            description="# Folds", value=5, min=2, max=10, step=1, style=description_style)

        self._w_mutation_rate = widgets.FloatSlider(
            description="Mutation Rate", value=.1, min=0, max=1, step=.05, style=description_style)

        self._w_recreation_rate = widgets.FloatSlider(
            description="Recreation Rate", value=.2, min=0, max=1, step=.05, style=description_style)

        self._w_survivor_rate = widgets.FloatSlider(
            description="Survivor Rate", value=.5, min=0, max=1, step=.05, style=description_style)

        self._w_optimization_metric = widgets.Dropdown(description='Optimization Metric', options=[
            'Accuracy', 'f1-score', 'Sensitivity'], style=description_style)

        self._w_results = qgrid.show_grid(DataFrame([[0, 0, 0, 0, 0] for _ in range(5)], columns=autosense_metric_column),
        ...
        self._b_run = Button(
            icon='play', description='Optimize', layout=Layout(align_self='center', width='250px'))
        self._b_refresh = Button(icon="refresh", layout=Layout(
            width='35px',  align_self='flex-end'))
        ...
        autosense_tabs = Tab([
            HBox([
                VBox([self._w_pipeline_label,
                      ...
                      HBox([self._w_optimization_metric,
                            self._info_optimization_metric]),
                      HBox([self._w_classifiers_sram,
            ...
            VBox([
                ...
                HBox([self._c_strip_mean, self._info_strip_mean],
                     layout=Layout(align_self='flex-start')),
                HBox([self._c_balanced_data, self._info_balanced_data],
                     layout=Layout(align_self='flex-start')),
                # HBox([self._c_hierarchical_model,
                #      self._info_hierarchical_model],
                #     layout=Layout(align_self='flex-start')),
                HBox([self._c_outlier_filter, self._info_outlier_filter],
                     layout=Layout(align_self='flex-start')),
                HBox([self._c_allow_unk, self._info_allow_unk],
                     layout=Layout(align_self='flex-start')),
                ...
                HBox([self._w_validation_method,
                      self._info_validation_method], layout=Layout(align_self='flex-start')),
                HBox([self._w_number_folds,
                      self._info_validation_method_folds], layout=Layout(align_self='flex-start')),
                HBox([self._w_validation_size,
                      self._info_validation_method_size], layout=Layout(align_self='flex-start')),
                HBox([self._w_validation_metadata,
                      self._info_validation_metadata], layout=Layout(align_self='flex-start')),
            ]),
            VBox([
                ...
                HBox([self._w_iterations, self._info_iterations],
                     layout=Layout(align_self='flex-start')),
                HBox([self._w_recreation_rate, self._info_recreation_rate],
                     layout=Layout(align_self='flex-start')),
                HBox([self._w_mutation_rate, self._info_mutation_rate],
                     layout=Layout(align_self='flex-start')),
                HBox([self._w_survivor_rate, self._info_survivor_rate],
                     layout=Layout(align_self='flex-start')),
            ]),

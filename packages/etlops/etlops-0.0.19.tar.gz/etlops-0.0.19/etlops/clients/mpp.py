...
class SnowflakeClient(SQLClient):
    ...
    def __set_sql_aqlchemy_engine(self):
        self.sql_alchemy_engine = create_engine('snowflake://{user}:{password}@{account}/'.format(
            ...
            connect_args={'insecure_mode': self.configuration_dict[SnowflakeClient.INSECURE_MODE]}
        )
    ...
    def alter_warehouse(self, warehouse, action):
        ...
        if action == SnowflakeClient.SUSPEND_COMMAND and status not in SnowflakeClient.SUSPENDED_WAREHOUSE_STATUS:
            self.__alter_warehouse_query(warehouse, SnowflakeClient.SUSPEND_COMMAND)
        elif action == SnowflakeClient.RESUME_COMMAND and status in SnowflakeClient.SUSPENDED_WAREHOUSE_STATUS:

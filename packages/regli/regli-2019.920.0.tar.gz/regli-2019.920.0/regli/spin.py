...
class SpIn(object):
    ...
    def add_band(self, wave, flux, params):
        ...
        self.reglis.append(Regli.init_from_flats(params, flux))
    ...
    def generate_mock_p_uniform(n=10, ranges=[(0, 1)], seed=0):
        ...
        ndim_ = len(ranges)
        pmock = np.random.rand(n, ndim_) * np.diff(ranges, axis=1) \
    ...
    def generate_mock_flux(self, pmock, mock_snr=50, rv=0, wave_interp=None,
        ...
        mock_z = np.random.randn(nmock) * rv / (const/1e3)
...
def add_gaussian_noise_to_spec(spec, snr=100, fill_ivar=0.):
    ...
    errn = spec/snr
    ivarn = errn**-2
    ivarn = np.where((errn<=0)|(specn<=0), fill_ivar, ivarn)
    specn = np.where(specn <= 0, 0., specn)
    return specn, ivarn

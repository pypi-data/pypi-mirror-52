...
from .utils import JSONResponse, page_it, can_view_expired_live
...
class LiveList(ListView):
    ...
    def get_queryset(self):
        ...
        self.queryset = UserLive.objects.filter(q1 | q2 | q3).distinct()
        user = self.get_user()
        if user:
        ...
        if scope in ["day", 'week', "month"]:
            q = Q(pk=None)
            if scope == 'day':
                q = UserLive.get_start_time_filter(day=offset)
            if scope == 'week':
                q = UserLive.get_start_time_filter(week=offset)
            if scope == 'month':
                q = UserLive.get_start_time_filter(month=offset)
    ...
    def get_context_data(self, **kwargs):
        ...
        if not user:
            if hasattr(settings, "COURSE_LIVE_PROVIDER_LIST"):
                ...
                provider_list = None
            context["search_form"] = UserLiveSearchForm(provider_list, {"user_name": user_name, "provider": provider})
        scope = self.request.GET.get("scope")
        offset = self.request.GET.get("offset")
        title_str = ""
        if scope in ["day", 'week', "month"]:
...
def manage_user_assignments(request):
    ...
    if request.user.has_perm("bee_django_course.view_teach_usercoursessection"):
        users = get_teach_users(request.user)
        q2 = Q(user_course__user__in=users)
    else:
        q2 = Q(pk=None)

    if request.user.has_perm("bee_django_course.view_child_usercoursesection"):
        users = request.user.get_child_users()
        q3 = Q(user_course__user__in=users)
    else:
        q3 = Q(pk=None)
    ...
    data_list = UserCourseSection.objects.filter(status__in=[1, 4]) \
        ...
        .filter(q1 | q2 | q3).distinct().order_by('-updated_at')
...
def notify_mentor(request, ucs_id):
    try:
        ...
        SendRecord.send_message(to_user=mentor, title=title, url=url)
        return JsonResponse(data={
            'rc': 0,
            'msg': '提醒成功'
        })
...
def notify_agent(request, ucs_id):
    try:
        ...
        title = user.get_user_name() + u'学习的课程[' + ucs.section.name + u']已经达标'
        from bee_django_message.models import SendRecord
        SendRecord.send_message(from_user=user, message_identity='course_notify', title=title, url=url)
        return JsonResponse(data={
            'rc': 0,
            'msg': '提醒成功'
        })
...
def live_finish(userlive, timedelta=None):
    ...
    # 更新课件学习时间
    user_course_section = get_user_last_course_section(userlive.user)

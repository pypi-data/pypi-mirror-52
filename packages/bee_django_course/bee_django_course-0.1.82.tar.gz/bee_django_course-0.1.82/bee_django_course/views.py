...
from .utils import JSONResponse, page_it, can_view_expired_live, live_finish
...
def test(request):
    # print('a', form)
    from utils import gensee_vod_sync
    a = gensee_vod_sync()
    print a
...
class CustomUserCourseDetail(UserCourseDetail):
    ...
    def get_paginate_by(self, queryset):
        if not self.user_course.course.template in [0, "0", None]:

...
def count_by(qset, group, count_field='id', distinct=False, sort=None):
    ...
    fs = group + ['c']
    return [[d[f] for f in fs] for d in dl]

def count_by_generic_relation(qset, group='name', count_field='id', distinct=False, sort=None):
    from django.contrib.contenttypes.models import ContentType
    from django.contrib.contenttypes.fields import GenericForeignKey
    ct_id = ContentType.objects.get(app_label=model._meta.app_label, model=model._meta.model_name).id
    m = qset.model._meta
    ctf = None
    ...
    if not ctf:
        return []
    cond = {ctf.ct_field: ct_id}
    qset = qset.filter(**cond)
    rs = count_by(qset, ctf.fk_field, count_field=count_field, distinct=distinct, sort=sort)
    ids = [a[0] for a in rs]
    nm = dict(model.objects.filter(id__in=ids).values_list('id', group))
    for d in rs:
        d[0] = nm[d[0]]
    return rs

...
def soft(pin,gprpm_file,**krg):
    ...
    for conp in pin:
        ...
        c.run("passwd gpadmin",pty=True,watchers=[Responder("password|密码","since2015\n")],encoding='utf8')
        ...
        c.run("chown -R gpadmin: /data/greenplum",encoding='utf8')
    for conp in pin:
        c=Connection(conp[0],connect_kwargs={"password":conp[1]})
        ...
        c.run("chown -R gpadmin:gpadmin /usr/local/greenplum-db*",pty=True,encoding='utf8')
...
def data(pin,segs_pernode=3,init_tag=True):
    ...
    c.sudo("sed -i /MASTER_DATA_DIRECTORY/d /home/gpadmin/.bashrc  ",user="gpadmin" ,encoding='utf8')
    c.sudo("sed -i /greenplum_path.sh/d /home/gpadmin/.bashrc  ",user="gpadmin" ,encoding='utf8')
    c.sudo("""cat >> /home/gpadmin/.bashrc << EOF
export MASTER_DATA_DIRECTORY=/data/greenplum/data/master/seg-1
source /usr/local/greenplum-db/greenplum_path.sh
EOF""",user='gpadmin',encoding='utf8')

    common.ssh(pin,'gpadmin')


    c.run("""su gpadmin -c "echo '%s' > /home/gpadmin/seg_nodes "  """%("\n".join(seg_nodes)) ,pty=True,encoding='utf8')
    standby=pin[-1][2]
    c.sudo("""mkdir -p /data/greenplum/data/master""",user="gpadmin",encoding='utf8')
    c.run("""su gpadmin -c   " source /home/gpadmin/.bashrc && gpssh -h %s -e 'mkdir -p /data/greenplum/data/master' " """%standby,pty=True,encoding='utf8')
...
EOF"""
    ...
    c.sudo(cmd,user='gpadmin',encoding='utf8')

    if init_tag:c.run("""su gpadmin -c "source /home/gpadmin/.bashrc && gpinitsystem -a -c /home/gpadmin/gpinitsystem_config" """,warn=True,encoding='utf8')
...
def plpython(pin):
    ...
    for conp in pin:
        ...
        c.run("ssh %s chown -R gpadmin:gpadmin /usr/local/greenplum-db*"%(conp[2]),pty=True,encoding='utf8')


def install(pin,gpfile_path,**krg):
    para={"segs_pernode":3,"swap_tag":False,"init_tag":True,"plpython3u":"35"}
    para.update(krg)
    pre(pin)
    soft(pin,gpfile_path)
    if  para["swap_tag"]:swap(pin)
    data(pin,para["segs_pernode"],para['init_tag'])

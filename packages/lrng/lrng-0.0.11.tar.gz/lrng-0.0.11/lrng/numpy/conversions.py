...
def _ranges_np_to_nb(np_ranges, np_labels_order):
    ...
    if np_labels_order.size > 0:
        try:
            np_ranges = np_ranges.reshape(-1, 3)
            np_ranges[:, 0] = labels_to_indices(np_ranges[:, 0], np_labels_order).reshape((-1, ))
        except IndexError:
            print('ERROR', np_ranges, np_ranges.shape, np_labels_order, np_labels_order.shape, np_labels_order.size)
            raise IndexError

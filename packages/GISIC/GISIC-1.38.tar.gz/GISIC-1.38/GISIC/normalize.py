...
def normalize(wavelength, flux, sigma=30, k=3, s=5, cahk=False, band_check=True, flux_min=0.7, boost=True, return_points=False):
    spec = Spectrum(wavelength, flux)
    spec.generate_inflection_segments(sigma=sigma, cahk=cahk, band_check = band_check, lower=flux_min)

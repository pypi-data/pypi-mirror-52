...
def normalize(wavelength, flux, sigma=30, k=3, s=5, cahk=False, band_check=True, return_points=False):
    ...
    if return_points:
        return np.array(spec.wavelength), np.array(spec.flux_norm), np.array(spec.continuum), {"wavelength" : spec.wavelength, "flux": spec.fluxpoints}

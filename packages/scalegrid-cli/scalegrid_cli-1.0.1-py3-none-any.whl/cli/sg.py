...
CLUSTER_VALS = ["name", "id", "clusterType", "shared", "status", "size", "diskSizeGB", "usedDiskSizeGB", "versionStr",
               "displayMachinePoolName", ["monitoringServerData", "id", "name", "shardName", "type"], "encryptionEnabled",
               "sslEnabled", "engine", "compressionAlgo"]
...
PROFILE_VALS = ["providerMachinePoolName", "configJSON.regionDesc", "configJSON.region",
                "configJSON.secretKey", "configJSON.deploymentStyle", "configJSON.vpcID", "configJSON.vpcSubnetID",
                "configJSON.vpcCIDR", "configJSON.vpcSubnetCIDR", "configJSON.securityGroupName", "configJSON.vpcSecurityGroupID", "configJSON.securityGroupId",
                "configJSON.subscriptionID", "configJSON.vnetId", "id", "dbType", "type", "created", "owner", "status", "shared"]
ALERT_VALS = ["alertDescription", "clusterID", "created", "dismissComment", "dismissedDate", "id", "machineName", "state", "userAlertRuleId"]
RULE_VALS = ["alertRuleDescription", "averageType", "clusterId", "created", "databaseType", "enabled", "id", "metric", "notifications", "operator", "ruleMetricName", "threshold", "type"]
...
def create_cluster():
    ...
    Usage:
        sg-cli (mongo) create-cluster --cluster-name <unique-cluster-name> --shard-count <number-of-shards> --replica-count <nodes-per-shard> --size <cluster-size> --version <version-number> --machine-list <list-of-machine-names> [options]
    ...
    Options:
        ...
        --replica-count <nodes-per-shard>       Number of nodes in each shard.
        ...
        --version <version-number>              Version of database
                                                Version options: V366, V401, V3416, V3220, V2611
        --machine-list <list-of-machine-names>  List of machine pool names
                                                Number of names should match:
                                                    number of replicas (number of replicas + 1 if even)
                                                    times number of shards
                                                Example: --machine-ids ['CloudProfile1','CloudProfile2','CloudProfile3']
        --engine <storage-engine>               Name of storage engine [default: wiredtiger]
                                                Engine options: wiredtiger | mmapv1
        --enable-ssl                            Include option to enable SSL [default: false]
        --encrypt-disk                          Include option to encrypt disk [default: false]
        --compression-algo <algorithm>          Compression algorithm. Include option to compress data
                                                Compression options: snappy | zlib
        -v, --verbose                           Increase verbosity
    """

    machines = eval(params['--machine-list'])
    machineIDs = []
    for i in machines:
    ...
    if params['--compression-algo'] != False:
...
def delete_cluster():
    ...
    Usage:
        sg-cli (mongo) delete-cluster --cluster-name <unique-cluster-name> [options]
...
def list_clusters():
    ...
    Usage:
        sg-cli (mongo) list-clusters [options]
...
def list_backups():
    ...
    Usage:
        sg-cli (mongo) list-backups --cluster-name <unique-cluster-name> [options]
...
def start_backup():
    ...
    Usage:
        sg-cli (mongo) start-backup --cluster-name <unique-cluster-name> --backup-name <unique-backup-name> [options] [--primary | --secondary]
...
def peek_at_backup():
    ...
    Usage:
        sg-cli (mongo) peek-at-backup --cluster-name <original-cluster-name> --backup-name <unique-backup-name> --new-name <new-cluster-name> [options]
...
def create_follower_cluster():
    ...
    Usage:
        sg-cli (mongo) create-follower-cluster --target-name <target-cluster-name> --source-name <source-cluster-name> --interval <hours-between-sync> [options]
...
def set_backup_schedule():
    ...
    Usage:
        sg-cli (mongo) set-backup-schedule --cluster-name <unique-cluster-name> [options] [--primary | --secondary]
...
def restore_backup():
    ...
    Usage:
        sg-cli (mongo) restore-backup --cluster-name <unique-cluster-name> --backup-name <unique-backup-name> [options]
...
def delete_backup():
    ...
    Usage:
        sg-cli (mongo) delete-backup --cluster-name <unique-cluster-name> --backup-name <unique-backup-name> [options]
...
def scale_up():
    ...
    Usage:
        sg-cli (mongo) scale-up --cluster-name <unique-cluster-name> --size <new-size> [options]
...
def build_index():
    ...
    Usage:
        sg-cli (mongo) build-index --cluster-name <unique-cluster-name> --db-name <database-name> --coll-name <collection-name> --index <keys-and-options> [options]
...
def patch_os():
    ...
    Usage:
        sg-cli (mongo) patch-os --cluster-name <unique-cluster-name> [options]
...
def get_cluster_credentials():
    ...
    Usage:
        sg-cli (mongo) get-cluster-credentials --cluster-name <unique-cluster-name> [options]
...
def pause_cluster():
    ...
    Usage:
        sg-cli (mongo) pause-cluster --cluster-name <unique-cluster-name> [options]
    ...
    convert_mongo()
    call = "/clusters/pauseCluster?id=%s&dbType=%s" % (id, params['db-type'].upper())
    resp = get_resp(call, "POST")
...
def resume_cluster():
    ...
    Usage:
        sg-cli (mongo) resume-cluster --cluster-name <unique-cluster-name> [options]
    ...
    convert_mongo()
    call = "/clusters/resumeCluster?id=%s&dbType=%s" % (id, params['db-type'].upper())
    resp = get_resp(call, "POST")
...
def add_firewall_rules():
    ...
    Usage:
        sg-cli (mongo) add-firewall-rules --cluster-name <unique-cluster-name> --cidr-list <list-of-cidr> [options]

    Options:
        --cluster-name <unique-cluster-name>  Name of a cluster
        --cidr-list <list-of-cidr>            List of IP CIDR
    ...
    convert_mongo()
    body = {'clusterID': id, 'dbType': params['db-type'].upper(), 'cidrList': eval(params['--cidr-list'])}

    conn.request("POST", "/Clusters/setClusterLevelIPWhiteList", body=json.dumps(body), headers=header)
    check_resp(200)
...
def get_active_alerts():
    ...
    Usage:
        sg-cli (mongo) get-active-alerts --cluster-name <unique-cluster-name>
...
def resolve_alerts():
    ...
    Usage:
        sg-cli (mongo) resolve-alerts --cluster-name <unique-cluster-name> --alert-id-list <list-of-alert-ids> [options]
    ...
    convert_mongo()
    body = {'clusterId': id, 'databaseType': params['db-type'].upper(), 'alertsList': eval(params['--alert-id-list'])}
...
def create_alert_rule():
    ...
    Usage:
        sg-cli (mongo) create-alert-rule --cluster-name <unique-cluster-name> --type <alert-rule-type> --operator <operator> --threshold <threshold> --notifications <notification-types> [options]
    ...
    notif = eval(params['--notifications'])
    for i in range(len(notif)):
        notif[i] = notif[i].upper()
...
def list_alert_rules():
    ...
    Usage:
        sg-cli (mongo) list-alert-rules --cluster-name <unique-cluster-name> [options]
...
def create_cloud_profile():
    ...
    Usage:
        sg-cli (mongo) create-cloud-profile --aws --cloud-profile-name <unique-name-of-cloud-profile> --region <region> --access-key <access-key> --secret-key <secret-key> --vpc-id <vpc-id> --subnet-id <subnet-id> --vpc-cidr <vpc-cidr> --subnet-cidr <subnet-cidr> --security-group-name <security-group-name> --security-group-id <security-group-id> [--connectivity-config <config> --enable-ssh] [-v | --verbose]
        sg-cli (mongo) create-cloud-profile --azure --cloud-profile-name <unique-name-of-cloud-profile> --region <region> --subscription-id <subscription-id> --subnet-name <subnet-name> --vnet-name <vnet-name> --vnet-resource-group <vnet-resource-group> --security-group-name <security-group-name> [--is-public] [--use-single-tennant-sp] [-v | --verbose]

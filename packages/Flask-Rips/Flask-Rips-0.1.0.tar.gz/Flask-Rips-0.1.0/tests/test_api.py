...
from flask_rip.utils import http_status_message, unpack
import flask_rip
from flask_rip import OrderedDict
...
class HelloWorld(flask_rip.Resource):
    def get(self):
        return {}
...
class HelloBomb(flask_rip.Resource):
    def get(self):
...
class APITestCase(unittest.TestCase):
    ...
    def test_unauthorized(self):
        app = Flask(__name__)
        api = flask_rip.Api(app, serve_challenge_on_401=True)
        response = Mock()
        response.headers = {}
        with app.test_request_context('/foo'):
            response = api.unauthorized(response)
    ...
    def test_api_prefix(self):
        app = Mock()
        app.configure_mock(**{'record.side_effect': AttributeError})
        api = flask_rip.Api(app, prefix='/foo')
    ...
    def test_add_two_conflicting_resources_on_same_endpoint(self):
        ...
        class Foo1(flask_rip.Resource):
            def get(self):
                return 'foo1'

        class Foo2(flask_rip.Resource):
            def get(self):
                return 'foo2'
    ...
    def test_abort_custom_message(self):
        try:
            flask_rip.abort(404, message="no user")
            assert False  # We should never get here
        except Exception as e:
            assert_equals(e.data['message'], "no user")

    def test_abort_type(self):
        self.assertRaises(HTTPException, lambda: flask_rip.abort(404))
    ...
    def test_use_custom_jsonencoder(self):
        ...
        class Cabbage(flask_rip.Resource):
            def get(self):
                return {'frob': object()}
    ...
    def test_redirect(self):
        ...
        class FooResource(flask_rip.Resource):
            def get(self):
                return redirect('/')
    ...
    def test_json_float_marshalled(self):
        ...
        class FooResource(flask_rip.Resource):
            fields = {'foo': flask_rip.fields.Float}
            def get(self):
                return flask_rip.marshal({"foo": 3.0}, self.fields)
    ...
    def test_custom_error_message(self):
        ...
        app = Flask(__name__)
        api = flask_rip.Api(app, errors=errors)
    ...
    def test_selectively_apply_method_decorators(self):
        ...
        class TestResource(flask_rip.Resource):

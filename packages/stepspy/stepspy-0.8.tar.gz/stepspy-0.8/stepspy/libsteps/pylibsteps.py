...
def get_base_library(enable_parallel=False):
    ...
    library = dirname+libsteps_name+parallel_flag+libsteps_extension
    if enable_parallel:
        if not os.path.exists(library):
            info = "Warning. Parallel library "+library+" does not exist. Try to load serial version.\n"
            print(info)
            library = get_base_library(enable_parallel=False)
    else:
        if not os.path.exists(library):
            info = "Warning. Serial library "+library+" does not exist. No applications with stepspy will be enabled.\n"
...
def load_library(enable_parallel=False):
    library = get_base_library(enable_parallel)
    ...
    libsteps.api_check_database.restype = None
    libsteps.api_check_database.argtypes = (c_uint, )

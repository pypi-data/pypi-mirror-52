...
class Envio(object):
    ...
    def Documento(self, docs):
        _documentos = []
        CDTE._emsior = self.Emisor
        CDTE._firma = self.firma
        for vals in docs:
            if vals.get('TipoDTE'):
    ...
    @Libro.setter
    def Libro(self, vals):
        if vals.get('TipoOperacion', 'VENTA') in ['VENTA', 'BOLETA', 'GUIA']:
            CDTE._dte_emisor = self.Emisor
    ...
    def Recepciones(self, vals):
        ...
        for recep in vals:
            ...
            respuesta._emisor = self._emisor
...
xsi:schemaLocation="http://www.sii.cl/SiiDte RespuestaEnvioDTE_v10.xsd" >
    ...
    def do_receipt_deliver(self):
        ...
        for r in self.Recepciones:
            self.RespuestaDTE(r.Carratula, r.Recepciones)
            self.firmar(
                'Odoo_resp',
                'env_resp',
            )

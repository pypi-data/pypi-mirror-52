...
def all_dict(dictory, exceptions=None, file_types=None, maps=True, files=True, print_data=False):
    ...
    for dire in data:
        yield dire
from tkinter.filedialog import *
Tk().withdraw() # we don't want a full GUI, so keep the WHEEL_root window from appearing
...
class Application:
    def __init__(self):
        ...
        WHEEL_root = Tk(className='wheel build')
        WHEEL_root.geometry('400x300')
        ...
        class my_class(dict):
            def __init__(self):
                dict.__init__(self)
        widgets = my_class()
        btn = Button(WHEEL_root, text='add folder', command=add_folder)
        btn.place(x='0',y='0')
        widgets['add folder'] = btn
        btn = Button(WHEEL_root, text='add file', command=add_file)
        btn.place(x=70,y=0)
        widgets['add file'] = btn
        btn = Button(WHEEL_root, text='target file', command=save_file)
        btn.place(x=140,y=0)
        widgets['target file'] = btn
        btn = Button(WHEEL_root, text='save', command=close)
        btn.place(x=0,y=40)
        widgets['save'] = btn
        btn = Button(WHEEL_root, text='add files', command=add_files)
        btn.place(x=70,y=40)
        widgets['add files'] = btn
        btn = Button(WHEEL_root, text='extract wheel file', command=read)
        btn.place(x=140,y=40)
        widgets['extract wheel file'] = btn
        btn = Button(WHEEL_root, text='convert zip to whl', command=con)
        btn.place(x=0, y=80)
        widgets['convert zip to whl'] = btn
        btn = Button(WHEEL_root, text='convert whl to zip', command=conz)
        btn.place(x=140, y=80)
        widgets['convert whl to zip'] = btn
        label = Message(WHEEL_root, text='first you need to select a file.\n you do that when you press the button target file.\n then you can choise add an folder, files or one file.\n if you have choise everything you want you just press the save button to close the file')
        label.place(x=0,y=120)
        widgets['help'] = label
        label = Label(WHEEL_root, text='info:')
        label.place(x=0, y=250)
        widgets['working on...'] = label
        label = Message(WHEEL_root, text='None')
        ...
        class log():
            def __init__(self, label):
                self.master = label
            def info(self, *args):
                self.master.config(text=''.join(args))

...
class UNet2D(nn.Module):
    ...
    def train_epoch(self, loader, loss_fn, optimizer, epoch, augmenter=None, print_stats=1, writer=None, write_images=False):
        """
        ...
        for i, data in enumerate(loader):
            ...
            if augmenter is not None:
                bs = data[0].size(0)
                xy = augmenter(torch.cat((data[0].cuda().float(), data[1].cuda().float()), dim=0))
                x = xy[:bs, ...].float()
                y = xy[bs:, ...].long()
            else:
                x = data[0].cuda().float()
    ...
    def train_net(self, train_loader, test_loader, loss_fn, optimizer, epochs, scheduler=None, test_freq=1, augmenter=None,
        ...
        for epoch in range(epochs):
            ...
            self.train_epoch(loader=train_loader, loss_fn=loss_fn, optimizer=optimizer, epoch=epoch, augmenter=augmenter,

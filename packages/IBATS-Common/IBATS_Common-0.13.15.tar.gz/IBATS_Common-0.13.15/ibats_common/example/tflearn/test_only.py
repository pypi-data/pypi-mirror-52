...
class AIStg(StgBase):
    ...
    def train(self, factor_df, predict_test_random_state):
        ...
        with sess.as_default():
            ...
            for num in range(1, 6):
                ...
                self.logger.info("validation accuracy: %.2f%%" % (result[0] * 100))
                val_acc = result[0]
                break
                # if result[0] > 0.60:
                #     break
    ...
    def load_model_if_exist(self, trade_date):
        ...
        if self.enable_load_model_if_exist:
            ...
            if folder_path is not None:
                ...
                self.logger.info("load from path: %s", folder_path)
                return True
    ...
    def on_prepare_min1(self, md_df, context):
        if md_df is None:
            return
        self.logger.debug('%s %s', md_df['trade_date'].iloc[-1], md_df.shape)
    ...
    def on_min1_release(self, md_df):
        ...
        # 预测结果
        self.logger.info("检验预测结果(predict_latest)")

...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None,
        ...
        #tt = self._format_input(x)
        print(np.hstack(list(map(lambda x: np.expand_dims(x, axis=1), x))).shape)
    ...
    def _format_input(self,x):
        fixlen_x = list(filter(lambda arr:len(arr.shape)==2,x))
        varlen_x = list(filter(lambda arr:len(arr.shape)==2,x))

        temp = np.hstack(list(map(lambda x: np.expand_dims(x, axis=1), fixlen_x)))
        print(temp.shape)
        print(varlen_x[0].shape)

...
class Datafile(SirenComponent):
    ...
    def register_datafile_metadata(
        ...
        return siren_to_entity(
            self.client.session.post(
                self.urls.datafiles_index, json=payload
            ).to_siren()
        )
    ...
    def edit_datafile_metadata(
        ...
        response = self.client.session.put(
            self.urls.datafiles_instance.format(id=datafile_id),
    ...
    def get_s3_datafile(self, datafile_id):
        ...
        s3_access = Client.from_dataset(self.client, datafile.dataset_id)
    ...
    def download_datafile(self, datafile_id, destination):
        ...
        datafile = self._get_datafile(datafile_id)
        s3_access = Client.from_dataset(self.client, datafile.datasetId)
    ...
    def _process_s3_upload(self, files, s3_location, dataset_id):
        s3_client = Client.from_dataset(self.client, dataset_id)
    ...
    def add_files_to_datafile(self, datafile_id, s3_prefix, files):
        ...
        """
        datafile = self.get_datafile(datafile_id)
        dataset = self.client.get_dataset(id=datafile.dataset_id)

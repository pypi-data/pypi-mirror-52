...
class Dataset(SirenComponent):
    ...
    def get_dataset(self, id=None, name=None, package_id=None, package_name=None):
        ...
        response = self.session.get(
            dataset_urls.v2_instance.format(id=id)
        )

        return DatasetModel(response.json()['data'], client=self)

    def _get_dataset_id_by_name(self, **kwargs):
        response = self.session.get(dataset_urls.index, params=kwargs)
    ...
    def register_dataset(self, builder : DatasetBuilder):
        ...
        result = self.session.post(
            dataset_urls.v2_index, data=mp_encoder,
    ...
    def edit_dataset(
        ...
        **kwargs
        ...
        :keyword file sample_data: Optional file. File like object. Used in conjuntion with data_preview_type.
        ...
        result = self.session.patch(
            dataset_urls.v2_instance.format(id=dataset_id), data=mp_encoder,
            headers={'Content-Type': mp_encoder.content_type}
        )
        return DatasetModel(result.json()['data'], client=self)
    ...
    def delete_dataset(self, dataset_id):
        ...
        """
        self.session.delete(
            dataset_urls.v2_instance.format(id=dataset_id)
        )
...
class Dictionary(SirenComponent):
    ...
    def get_dictionary(self, dataset_id, version=None):
        ...
        if version:
            schema_old_api = self.session.get(
                dataset_urls.schema_instance_version.format(
                    id=dataset_id,
                    version=version
                )
            ).to_siren()
            ...
            schema = self.session.get(
                dataset_urls.dictionary_instance.format(id=schema_id)
            )
        else:
            schema = self.session.get(

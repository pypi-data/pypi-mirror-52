        ...
        if '--test' in sys.argv:
            ...
            if is_url(args.target):
                ...
                if not os.path.exists(local_target):
                    fail('file does not exist: {}'.format(os.path.abspath(local_target)))
...
def more():
    ...
    if not testing:
        print('\n')
        print('Find more safegets at https://denova.com/safeget/custom/')
...
def fail(msg):
    ...
    raise SafegetException('Failed: {}'.format(msg))
...
def run(*command_args, **kwargs):
    ...
    if len(command_args) is 1:
        ...
        if ' ' in program:
            ...
            debug('DEPRECATED: run({}) as shell command'.format(repr(command_args)))
            kwargs['shell'] = True
    ...
    try:
        ...
        debug(cpe)
        if cpe.returncode: debug('    returncode: {}'.format(cpe.returncode))
...
def install_openssl():
    ...
    open_ssl_path = install('openssl',
                            ...
                            is_installer=True)
    debug('openssl path: {}'.format(open_ssl_path))
...
def install_gpg():
    ...
    gpg_path = install('gpg',
                       ...
                       is_installer=True)
    debug('gpg path: {}'.format(gpg_path))
...
def install(program, windows_url=None, osx_url=None, linux_package=None, is_installer=False):
    ...
    def require_root():
        if not os.geteuid() == 0:
            fail('install {}, or rerun this program as root, so it can install dependencies'.format(program))

    def already_installed():
        verbose('{} already installed'.format(program))

    def install_done():
        debug('installed {} to {}'.format(program, program_path))

    def windows_install(program, windows_url):
        url = windows_url
        verify_source(url)
        if not url:
            fail('no install url for {}'.format(program))

        verbose('install {}'.format(program))
        ...
        try:
            ...
            fail('rerun this program as admin to install {}'.format(program))
    ...
    def osx_install(program, osx_url):
        if installed(program):
            ...
            require_root()
            verbose('{} not found. installing...'.format(program))
    ...
    def linux_install(program, linux_package):
        if installed(program):
            ...
            verbose('install linux package {}'.format(linux_package))
    ...
    if system == 'Windows':
        ...
        fail('unable to install on {}'.format(system))
...
def download(url, localpath, wget=True):
    ...
    verbose('download {} to {}'.format(url, os.path.abspath(localpath)))
        ...
        if ok_to_write(localpath):
            ...
            if args.proxy:
                ...
                if args.proxy.startswith('https'):
                    wget_args.append('https_proxy={}'.format(args.proxy))
                else:
                    wget_args.append('http_proxy={}'.format(args.proxy))
...
def parse_args():
    '''
        ...
        without also changing denova/src/safeget/customize.py
    '''
    ...
    debug('args from argsparse: {}'.format(vars(args)))
...
def verify_file(local_target):
    ...
    notice('Verifying... ')
    verbose('verify target file {}'.format(os.path.abspath(local_target)))
    ...
    if args.after or args.run:
        notice('Verified {}... '.format(os.path.basename(local_target)))
    else:
        print('Verified {}'.format(os.path.basename(local_target)))
    ...
    if is_url(source):
        ...
        if not args.onehost:
            if source != args.target and target_host == host:
                ...
                warn('url is same host as target: {}. Use --onehost to skip this warning.'.format(source))
...
def verify_safeget_itself():
    ...
    def hashes_match(original, local, algo):
        ok = original == local
        if not ok:
            debug('The {} hash does not match the original: {}'.format(algo, original))
        return ok
    ...
    def safeget_ok(result):
        ...
        original_safeget_bytes = result['quick-query']['safeget-bytes']
        ...
        if ok:
            ...
            original_safeget_sha512 = result['quick-query']['safeget-sha512']
            local_safeget_sha512 = hashlib.sha512(lines).hexdigest()
            ok = hashes_match(original_safeget_sha512, local_safeget_sha512, 'SHA512')
            if ok:
                original_safeget_sha256 = result['quick-query']['safeget-sha256']
                local_safeget_sha256 = hashlib.sha256(lines).hexdigest()
                ok = hashes_match(original_safeget_sha512, local_safeget_sha512, 'SHA256')

            # if neither hash is ok, then warn the user
            if not ok:
                error_message = 'The hash of {} does not match the original.\n'.format(filename)
        else:
            error_message = 'The file size of {} does not match the original.\n'.format(filename)

        if error_message is not None:
            error_message += ' You should download it again.'

        return ok, error_message

    FULL_URL = 'https://denova.com/safeget/api/'
    PARAMS = {'api_version': '1.0', 'actions': 'quick-query', 'target': args.target}
    ...
    if 'quick-query' in result:
        if result['quick-query']['ok']:
            ...
            if not ok:
                debug('unable to verify safeget: {}'.format(error_message))
                fail(error_message)
        ...
        debug('unable to verify safeget: {}'.format(result))
        fail("Unable to verify safeget")
    ...
    if args.size:
        ...
        try:
            ...
            if os.path.getsize(localpath) == args_size:
                ...
                fail('file size is not {size}'.format(size=args_size))
    ...
    if args.signedhash:
        ...
        for signed_hash_arg in args.signedhash:
            ...
            if algo not in algos_available:
                fail('{} not in available hash algorithms: {}'.format(algo, ' '.join(algos_available)))

            # verify hashes are in a pgp signed message

            signed_data_files = verify_signed_messages(source)
            debug('{} signed data files'.format(len(signed_data_files)))
        ...
        if not matched:
            fail('no matching signed hash found in {}'.format(args.signedhash))
...
def hash_algorithms():
    ...
    debug('hashlib.algorithms_available: {}'.format(algos))
...
def hash_failed(algo, expected_hash, actual_hash):
    ...
    debug('    expected {} hash: {}'.format(algo, expected_hash))
    debug('    actual {} hash: {}'.format(algo, actual_hash))

def search_for_hash(localpath, signed_hash_file, algo, url_content):
    debug('search_for_hash {}:{}'.format(algo, signed_hash_file))

    actual_hash = hash_data(algo, localpath)
    ok = actual_hash in url_content
    if ok:
        debug('verfied {} hash from url'.format(algo))

    else:
        hash_failed(algo, 'not found in {}'.format(signed_hash_file), actual_hash)

    return ok

def compare_hashes(algo, expected_hash, actual_hash):
    debug('compare_hashes {}:{}'.format(algo, expected_hash))

    ok = (expected_hash == actual_hash)
    if ok:
        debug('verfied explicit {} hash'.format(algo))
    else:
...
def get_pubkeys():
    ...
    if args.pubkey:
        ...
        for keypath in pubkey_paths:
            debug('pubkey path: {}'.format(keypath))
            if clean_gpg_data(keypath):
                run(gpg_path, '--import', keypath)
            debug('imported pgp public key from: {}'.format(keypath))
...
def verify_signed_messages(source):
    ...
    verbose('verify signed message at {}'.format(source))
        ...
        if clean_gpg_data(signedmsg_path):
            ...
            with open(signedmsg_path, 'r') as infile:
                try:
                    ...
                    debug('could not verify signed message saved in {}'.format(signedmsg_path))
                    debug(ex)
                else:
                    verbose('verified pgp signed message: {}'.format(signedmsg_path))
        ...
        for sigpath in sig_paths:
            if clean_gpg_data(sigpath):
                try:
                    ...
                    debug('could not verify pgp detached signature saved in {}'.format(sigpath))
                    debug(ex)
                else:
                    verbose('verified pgp detached signature: {}'.format(args.sig))
...
def parse_hash(text):
    ...
    if (not algo) or (not hash_or_url) or (hash_or_url.startswith('//')):
        fail('in hash expected "ALGORITHM:..." e.g. "SHA512:D6E8..." or "SHA256:https://...", got {}'.format(text))
...
def extract_patterns(pattern, localpath):
    ...
    debug('extracting {} from {}'.format(pattern, localpath))
...
def save_patterns(pattern, sources):
    ...
    for source in sources:
        if is_url(source):
            ...
            path = temppath()
            download(url, path)
            debug('url {} saved in {}'.format(url, path))
        ...
        if not pattern_paths:
            fail('no "{}" patterns found: {}'.format(pattern, path))
...
def clean_gpg_data(path):
    ...
    if ok:
        ...
        debug('gpg data file too short: {}'.format(path))
...
def download_tempfile(url):
    ...
    debug('download {} to temp file'.format(url))
    data = download_data(url)
    path = temppath()
    debug('save url data to {}'.format(path))
...
def ok_to_write(path):
    ...
    if os.path.exists(path) and os.path.getsize(path) and not testing:
        verbose('{} already exists'.format(os.path.abspath(path)))
        prompt = '\nOk to replace {}? '.format(os.path.abspath(path))
        answer = input(prompt)
        answer = answer.lower()
        debug('answered: {}'.format(answer))
        ok = answer in ['y', 'yes']
        if not ok:
            fail('did not replace {}'.format(os.path.abspath(path)))
    else:
        ok = True
...
def persist(func, *args, **kwargs):
    ...
    while not done:
        try:
            ...
            if retries:
                debug('Succeeded after {} retries'.format(retries))
            done = True
...
def test():
    ...
    # file to verify
    # url created below
    FILENAME = 'bitcoin-{version}-x86_64-linux-gnu.tar.gz'.format(version=BITCOIN_VERSION)
    ...
    if debug_test or verbose_test:
        print('test dir is {}'.format(TMP_DIR))

    # if local copies of test files are already in TMP_DIR, use them
    if (os.path.exists(LOCAL_TARGET) and
        os.path.exists(LOCAL_PUBKEY) and
        os.path.exists(LOCAL_SIGNED_HASHES_SOURCE)):

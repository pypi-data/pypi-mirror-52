...
class Dataflow:
    ...
    def _with_partition_size(self, partition_size: int) -> 'Dataflow':
        ...
        if partition_step is None:
            raise ValueError('The current Dataflow does not support custom partition size.')
    ...
    def map_partition(self, fn: Callable[['pandas.DataFrame', int], 'pandas.DataFrame']) -> 'Dataflow':
        """
        Applies a transformation to each partition in the Dataflow.

        .. remarks::
            The function passed in must take in two parameters: a `Link pandas.DataFrame <https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.html>`_
                containing the data for the partition and an index. The return value must be a `Link pandas.DataFrame <https://pandas.pydata.org/pandas-docs/stable/reference/api/pandas.DataFrame.html>`_
        ...
        return self.add_step(step_type='Microsoft.DPrep.MapPartitionsAsDataFrameBlock', arguments={
            "codeBlockType": CodeBlockType.PICKLEDFILE,
            "pickledFile": path
        })

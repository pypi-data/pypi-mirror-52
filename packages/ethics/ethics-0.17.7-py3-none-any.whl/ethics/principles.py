...
class DoNoHarmPrinciple(Principle):
    ...
    def _check(self):
        ...
        f = Formula.makeConjunction([Impl(Bad(c), Not(Causes(a, c))) for a in actions_combination for c in self.cons])
        self.formulae = [f]
        self.result = [self.model.models(f)]
        return self.result
...
class DoNoInstrumentalHarmPrinciple(Principle):
    ...
    def _check(self):
        f = Formula.makeConjunction([Impl(And(Causes(a, c1), And(Causes(c1, c2), I(c2))), Not(Bad(c1))) \
                                        for a in self.model.get_performed_actions() for c1 in self.cons for c2 in self.cons])
        self.formulae = [f]
        self.result = [self.model.models(f)]
        return self.result
...
class IntentionFocusedDeontologicalPrinciple(DeontologicalPrinciple):
    ...
    def _check(self):
        f = Formula.makeConjunction([Impl(I(c), Not(Bad(c))) for c in self.model.get_actual_consequences()+self.model.get_performed_actions()])
        self.formulae = [f]
        self.result = [self.model.models(f)]
        return self.result
...
class KantianHumanityPrincipleReading1(Principle):
    ...
    def _check(self):
        f1 = Impl(Means("Reading-1", self.model.patients[0]), End(self.model.patients[0]))
        for p in self.model.patients[1:]:
            f1 = And(f1, Impl(Means("Reading-1", p), End(p)))
        self.formulae = [f1]
...
class KantianHumanityPrincipleReading2(Principle):
    ...
    def _check(self):
        f1 = Impl(Means("Reading-2", self.model.patients[0]), End(self.model.patients[0]))
        for p in self.model.patients[1:]:
            f1 = And(f1, Impl(Means("Reading-2", p), End(p)))
        self.formulae = [f1]
        self.result = [self.model.models(f1)]
        return self.result

...
class Situation:
    ...
    def makeMoralSuggestion(self, principle, *args):
        ...
        if principle(self, args).permissible():
            return self
    ...
    def models(self, formula):
        ...
        if isinstance(formula, Caused):
            return self.agentivelyCaused(self.__literal_to_dict(formula.f1))
        if isinstance(formula, Finally):
            return self.isSatisfied(self.__literal_to_dict(formula.f1), self.getAllConsequences())
        if isinstance(formula, Means):
            return self.treatsAsMeans(formula.f1, 1)
        if isinstance(formula, Means2):
            return self.treatsAsMeans(formula.f1, 2)
        if isinstance(formula, End):
            return self.treatsAsEnd(formula.f1)
...
class DoNoInstrumentalHarm(Principle):
    ...
    def permissible(self):
        ...
        situation -- The situation
        """
        for h in self.model.getHarmfulConsequences():
            if self.model.isInstrumental(h):
                return False
        return True
...
class KantianHumanity(Principle):
    ...
    def permissible(self):
        ...
        """
        """
        for p in self.model.affects.keys():
            if self.model.treatsAsMeans(p, self.reading) and not self.model.treatsAsEnd(p):
                return False
        return True
        """
        self.__build_formula()

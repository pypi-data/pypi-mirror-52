...
def to_pyeda(f):
    if isinstance(f, And):
        return pyeda.inter.And(to_pyeda(f.f1), to_pyeda(f.f2))
    if isinstance(f, Or):
        return pyeda.inter.Or(to_pyeda(f.f1), to_pyeda(f.f2))
    if isinstance(f, Not):
        return pyeda.inter.Not(to_pyeda(f.f1))
    if isinstance(f, Impl):
        return pyeda.inter.Implies(to_pyeda(f.f1), to_pyeda(f.f2))
    if isinstance(f, BiImpl):
        return pyeda.inter.Equal(to_pyeda(f.f1), to_pyeda(f.f2))

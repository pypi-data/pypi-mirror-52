...
from .node_generator import KT_NodeSequence
...
def graph_node_classifier(name, train_data, layer_sizes=[32,32], verbose=1):
    ...
    Args:
        ...
        train_data (KT_NodeSequence): a ktrain.graph.node_generator.KT_NodeSequence object
    ...
    # check argument
    if not isinstance(train_data, KT_NodeSequence):
        err ="""
            train_data must be a ktrain.graph.node_generator.KT_NodeSequence object
            """
        raise Exception(err)
    ...
    prediction = layers.Dense(units=num_classes, activation=activation)(x_out)

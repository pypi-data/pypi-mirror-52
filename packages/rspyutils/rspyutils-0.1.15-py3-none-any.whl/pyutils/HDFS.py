...
class HDFSCli(object):
    def __init__(self, hosts, user_name="HDFS"):
        self.fs = pyhdfs.HdfsClient(hosts=hosts,
                                    user_name=user_name)
...
if __name__ == '__main__':
    fs = HDFSCli(hosts="../config/dev/train_config.yaml")
    fs.mkdirs("/recommend/samh/rank/data")
    print(fs.list('/recommend/samh/model/rank/lr/'))
    # fs.rm_file_by_date("/recommend/samh/model/deepwalk", date_len=5)
    # print(fs.list('/recommend/samh/model/deepwalk'))
    # fs.delete_hdfs_file(os.path.join("/recommend/samh/model/deepwalk", "comic_graph.mat"))

...
class BuildImage(object):
    ...
    def _generate_image_name(self):
        ...
        main_repo = [r for r in ins_cfg.get('repositories', []) if r.get('main_repo')]
        ...
        if self._config.container_config.get('full_stack'):
            ...
            name = '%s%s' % (main_repo[0].get('name'), main_repo[0].get('branch'))
    ...
    def _apply_patches(self):
        ...
        patches = utils.find_file(self._config.temp_working_folder, 'patches.txt')
        if not patches:
            return res

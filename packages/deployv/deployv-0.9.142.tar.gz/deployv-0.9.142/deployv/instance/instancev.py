...
class InstanceV(DockerV):
    ...
    def generate_backup(self, params):
        ...
        if cformat == 'folder':
            res = self._generate_decompressed_backup(
                files, bkp_name, params.get('dest_folder')
            )
        else:
            res = self.compress_files(files, bkp_name, params.get('dest_folder'), cformat)
        return res
    ...
    def deactivate_database(self, db_name, extra_config=False):
        ...
        try:
            sql_params = sqls.pop("deactivate_parameters")
            self.execute_query(db_config, sql_params, allow_errors=True)
    ...
    def save_repositories(self, filename=False):
        ...
        user = self.__full_config.container_config.get('env_vars').get('odoo_user')
        filename = filename or 'branches.json'
        command = ('bash -c \'su %(user)s -c "branchesv save -p %(home)s/instance'
                   ' -f /tmp/%(name)s"\'' % {'user': user, 'home': home, 'name': filename})

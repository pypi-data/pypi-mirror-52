...
class FeatureUtil():
    ...
    def feature_extraction(self, data, padding=False, sparse=True):
        ...
        # print(len(cross_features_id),len(cross_features_val))
        if padding == True:
            hist_id = sequence.pad_sequences(hist_id, maxlen=self.maxlen, padding='post')
            hist_week_id = sequence.pad_sequences(hist_week_id, maxlen=self.maxlen, padding='post')
            hist_time = sequence.pad_sequences(hist_time, maxlen=self.maxlen, padding='post')
            hist_time_gaps = sequence.pad_sequences(hist_time_gaps, maxlen=self.maxlen, padding='post')
            label_week_id = sequence.pad_sequences(label_week_id, maxlen=self.maxlen, padding='post')
            cross_features_id = np.array(cross_features_id)
            cross_features_val = np.array(cross_features_val)
            L_cross = []
            L_user = []
            for (x, y) in zip(cross_features_id, cross_features_val):
                tmp = scipy.sparse.coo_matrix((y, ([0, ] * len(x), x)), shape=(1, self.cross_size))
                tmp = tmp.toarray() if sparse == False else tmp
                L_cross.append(tmp)
            for (x, y) in zip(user_features_id, user_features_val):
                tmp = scipy.sparse.coo_matrix((y, ([0, ] * len(x), x)), shape=(1, self.user_feature_size)).toarray()
                tmp = tmp.toarray() if sparse == False else tmp
                L_user.append(tmp)
            return [hist_id, hist_week_id, np.concatenate(L_cross, axis=0), np.concatenate(L_user, axis=0), hist_time, hist_time_gaps, label_week_id], np.array(label)
    ...
    def to_tfrecord(self, csvfile, filename):
        ...
        data = open(csvfile, 'r').read().split('\n')[1:-1]
    ...
    def read_tfrecord(self, filename):
        ...
        def preprocess_fn(a, b, c, u, t, g, h, z):
            ...
            return (a, b, c, u, t, g, h), tf.one_hot(z, self.class_num)

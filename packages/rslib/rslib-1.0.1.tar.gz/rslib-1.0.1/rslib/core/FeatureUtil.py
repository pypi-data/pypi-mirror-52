...
class FeatureUtil():
    ...
    def feature_extraction(self, data, padding=False, sparse=True):
        ...
        label_week_id = [int(sample.split('@')[-2].split(':')[1]) for sample in data]
        ...
        if sparse == False:
            ...
            for (x, y) in zip(cross_features_id, cross_features_val):
                tmp = scipy.sparse.coo_matrix((y, ([0, ] * len(x), x)), shape=(1, self.cross_size)).toarray()
                L_cross.append(tmp)
    ...
    def to_tfrecord(self, csvfile, filename):
        ...
        for i in range(len(data)):
            ...
            label_week_id = [label_week_ids[i]] * len(hist_id)
            label = labels[i]

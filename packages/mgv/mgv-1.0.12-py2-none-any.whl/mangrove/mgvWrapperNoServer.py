...
def getWidget(project_name, name=None, uuid=None):
    ...
    if uuid:
        gt = getByUuid(root, uuid)
    if name:
        for t in root[project_name]['widgets']:
            if t['name'] == name:
                gt = t
                break

    node_type = mgvApi.MgvWidget(uuid=gt['uuid'], name=gt['name'], event=gt['event'],
                                  script=gt['script'])
...
def getProject(projectName):
    ...
    batchScripts = root[projectName]['batchScripts']
    if batchScripts:
    ...
    widgets = []
    for gn in root[projectName]['widgets']:
        widid = gn['uuid']
        widname = gn['name']
        widscript = gn['script']
        widevent = gn['event']
...
def getGraphInfo(pattern, keys):
    ...
    return {'uuid': root['uuid'], 'template_name': root['template_name']}
...
def createNode(father, code, **kwargs):
    ...
    if code in ['Project', 'Pattern', 'GraphTemplate', 'Template', 'Context', 'Action', 'TypeParameter', 'Type', 'TypeVersion',
                'TypeFile', 'Widget']:
        lock_file(file_path)
        with open(file_path) as fid:
            root = json.load(fid)
        ...
        if code == 'Project':
            ...
            father_element['widgets'].append(kwargs)
        elif code == 'Type':
            kwargs['versions'] = []
            kwargs['typeFiles'] = []

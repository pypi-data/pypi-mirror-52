...
def getType(project_name, name=None, uuid=None):
    ...
    for gv in gver:
        ...
        v = mgvApi.MgvTypeVersion(uuid=gv['x']['uuid'], version_id=gv['x']['id'], actions=actions,
                                  parameters=params, script=gv['x']['script'])
        versions.append(v)
    ...
    node_type = mgvApi.MgvType(uuid=gt['x']['uuid'], name=gt['x']['name'], category=gt['x']['category'],
                               ...
                               versions=versions, versionActive=versionActive)
    return node_type
...
def getWidget(project_name, name=None, uuid=None):
    ...
    if uuid:
        gtyp = neograph.run(
            "MATCH (:Project {name:'%s'})-->(x:Widget {uuid:'%s'}) RETURN x" % (project_name, uuid)).data()
    if name:
        gtyp = neograph.run(
            "MATCH (:Project {name:'%s'})-->(x:Widget {name:'%s'}) RETURN x" % (project_name, name)).data()
    gt = gtyp[0]

    node_type = mgvApi.MgvWidget(uuid=gt['x']['uuid'], name=gt['x']['name'], event=gt['x']['event'],
                                 script=gt['x']['script'])
    return node_type
...
def getTypes(projectName):
    ...
    for gt in gtyp:
        ...
        if gt['x']['versionActive'] is None:
            gt['x']['versionActive'] = -1

        node_type = mgvApi.MgvType(uuid=gt['x']['uuid'], name=gt['x']['name'], category=gt['x']['category'],
                                   color=gt['x']['color'], shape=gt['x']['shape'], typeFiles=typeFiles,
                                   image=gt['x']['image'], context=gt['x']['context'], help=gt['x']['help'],
                                   width=gt['x']['width'], shapeVector=shapeVector,
                                   linkWith=gt['x']['linkWith'].split(';'), versions=versions,
                                   versionActive=gt['x']['versionActive'])
...
def getProject(projectName):
    ...
    if len(gproj):
        ...
        batchScripts = gproj[0]['x']['batchScripts']
        if batchScripts:
        ...
        widgets = []
        gwid = neograph.run("MATCH (:Project {name:'%s'})-->(y:Widget) RETURN y" % name).data()
        for gn in gwid:
            widid = gn['y']['uuid']
            widname = gn['y']['name']
        ...
        for gn in gpat:
            ...
            templates = []
            gtem = neograph.run("MATCH (:Pattern {uuid:'%s'})-->(y:Template) RETURN y" % patid).data()
            for gm in gtem:
                temid = gm['y']['uuid']
                temname = gm['y']['name']
                temicon = gm['y']['icon']
...
def getGraphInfo(pattern, keys):
    ...
    g = neograph.run(cmd)
    for gn in g:
        return {'uuid': gn['x.uuid'], 'template_name': gn['x.template_name']}
    return {}
...
def getObjects(graph):
    ...
    if graph.path[-1] == '*template*':
        g = neograph.run(
            "MATCH p=(:Pattern {uuid:'%s'})-->(:Template {name:'%s'})-->(x:Graph {name:'%s'}) RETURN x.uuid,x.templatePath,x.template_name" % (
    ...
    for pattern in project.patterns:
        ...
        for x in g:
            ...
            if str(x['x.uuid']) not in [str(t.uuid) for t in pattern.templates]:
                neograph.run("MATCH (:Template {uuid:'%s'})-[*]->(a) DETACH DELETE a" % x['x.uuid'])
                neograph.run("MATCH (a:Template {uuid:'%s'}) DETACH DELETE a" % x['x.uuid'])

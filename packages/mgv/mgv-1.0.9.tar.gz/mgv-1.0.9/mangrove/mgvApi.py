...
Functions:
    ...
    mgvDicoReplace: Replace variables by values in a dictionary.
    mgvcopy: Copy a file or directory.
...
Classes:
    ...
    MgvWidget: Script to launch at events.
    MgvPattern: Class of Mangrove graphs.
    MgvTemplate: Default graph to preset new ones.
...
def mgvDicoReplace(d):
                ...
                for key2 in dico:
                    if key2 != key1:
                        if len(key2):
                            ...
                            d = dico[key2]
                            if isinstance(d, str):
                                d = d.encode('string-escape')
                            elif isinstance(d, unicode):
                                d = d.encode('unicode-escape')
                            s = re.sub('\$%s(?!\w)' % key2, d, s)
                if s != dico[key1]:
                    dico[key1] = s
                    b = True
                    c += 1
    for key in dico.keys():
        if len(key):
            dico[key] = os.path.expandvars(dico[key])
        else:
            del dico[key]
    return dico
...
def ExeScript(script, textEdit=None, env=None, line_offset=0):
    ...
    try:
        ...
        for i, output_text in execute(cmd):
            if isinstance(output_text, basestring):
                if not textEdit:
                    if i == 0:
                    ...
                    if i == 0:
                        ...
                        output_text = re.sub('(?!line) \d+(?=,)', lambda x: `int(x.group()) - line_offset`, output_text)
...
def getCurrentProject():
    ...
    return MgvProject.getProject(name=os.getenv("MGVPROJECTNAME"))


class MgvTemplate(object):
    """Mangrove Template object.
    ...
    def __repr__(self):
        return "MgvTemplate %s" % self.name
    ...
    def _dup(self):
        ...
        new = MgvTemplate(pattern=self.pattern, uuid=self.uuid, name=self.name, icon=self.icon)
        return new

    def getJson(self):
        """Return a dictionary representing the template."""
        return {'code': 'Template', 'uuid': self.uuid, 'name': self.name, 'icon': self.icon}
...
class MgvPattern(object):
    ...
    Attributes:
        ...
        templates (list of MgvTemplate): list of available templates
            for this pattern.
    ...
    def setTemplate(self, keys, template_name):
...
class MgvWidget(object):
    """Mangrove Widget object.

    A widget is a script that is executed in a parallel thread at a
    ...
    Attributes:
        ...
        script (str): the script of the widget.
        ...
        event (str): when to execute the script. It can be,
            ...
            toggle: when the widget box is toggled.
    """
    code = "Widget"
    ...
    def setScript(self, script):
        """Set the script of the widget."""
        self.script = script
        mgvWrapper.setNodeAttr(self, script=script)

    def setEvent(self, event):
        """Set the event of the widget."""
        self.event = event
    ...
    def getScript(self):
        """Return the script of the widget."""
        return self.script

    def getEvent(self):
        """Return the event of the widget."""
        return self.event

    def _dup(self):
        """Return a new widget with the same attributes."""
        return MgvWidget(uuid=self.uuid, name=self.name, script=self.script, event=self.event, project=self.project)
    ...
    def getJson(self):
        """Return a dictionary representing the widget."""
        return {'code': 'Widget', 'uuid': self.uuid, 'name': self.getName(), 'script': self.script, 'event': self.event}
...
class MgvProject(object):
    ...
    Attributes:
        ...
        widgets (list of MgvWidget): widgets of this project.
        script (str): the first part of every node's execution
            python script.
        batchScripts (dict): a 'name'='script' dictionary.
        versions_padding (int): padding of the versions of a node.
        versions_start (int): number of the first version of a node.
    """
    code = "Project"

    def __init__(self, uuid=None, name='', patterns=None, types=None, contexts=None, widgets=None, script='',
        ...
        self.script = script
        self.batchScripts = batchScripts if batchScripts else {'custom': ''}
        for x in self.patterns + self.contexts + self.widgets + self.types:
            x.project = self
    ...
    def create(self):
        """Create the equivalent object in the database."""
        self.uuid = mgvWrapper.createNode(None, self.code, name=self.name, script=self.script,
                                          batchScripts=json.dumps(self.batchScripts), lock='',
    ...
    def getWidgets(self):
        """Return a list of the widgets of the project."""
        return list(self.widgets)

    def setWidgets(self, widgets):
        """Delete the project widgets and add new ones."""
        for widget in self.widgets:
            mgvWrapper.deleteNode(widget)
        self.widgets = widgets
        for widget in widgets:
            widget.create()

    def getBatchScripts(self):
        """Return a dictionary of the batchScripts of the project."""
        return dict(self.batchScripts)

    def getBatchScript(self, name):
        """Return a specific batchScript of the project."""
        if name in self.batchScripts:
            return self.batchScripts[name]
        return None

    def setBatchScripts(self, batchScripts):
        """Set the batchScript dictionary of the project."""
        self.batchScripts = dict(batchScripts)
        mgvWrapper.setNodeAttr(self, batchScripts=json.dumps(batchScripts))

    def setBatchScript(self, name, script):
        """Set a specific batchScript of the project."""
        self.batchScripts = mgvWrapper.getNodeAttr(self, 'batchScripts')
        self.batchScripts = json.loads(self.batchScripts)
        self.batchScripts[name] = script
        mgvWrapper.setNodeAttr(self, batchScripts=json.dumps(self.batchScripts))

    def delBatchScript(self, name):
        """Delete a specific batchScript of the project."""
        self.batchScripts = mgvWrapper.getNodeAttr(self, 'batchScripts')
        self.batchScripts = json.loads(self.batchScripts)
        del self.batchScripts[name]
        mgvWrapper.setNodeAttr(self, batchScripts=json.dumps(self.batchScripts))
    ...
    def _dup(self):
        ...
        return MgvProject(name=self.name, uuid=self.uuid, script=self.script, types=[x._dup() for x in self.types],
                          ...
                          widgets=[x._dup() for x in self.widgets], batchScripts=dict(self.batchScripts),
                          versions_padding=self.versions_padding, versions_start=self.versions_start)
    ...
    def getJson(self):
        ...
        return {'code': 'Project', 'name': self.getName(), 'script': self.script,
                'batchScripts': json.dumps(self.batchScripts),
                'patterns': [x.getJson() for x in self.patterns], 'widgets': [x.getJson() for x in self.widgets],
...
class MgvGraph(object):
    ...
    def getVars(self):
        ...
        dico = {"MGVPROJECTNAME": self.pattern.project.getName(),
                ...
                not isinstance(self.path, basestring) else os.path.splitext(self.path)[0],
    ...
    def _fillWithTemplate(self, template):
    ...
    def _linksToString(self, graphview=None):
        ...
        links = []
        for node in self.nodes:
            links.extend([x for x in node.inputLinks if not isinstance(x, basestring)])
            links.extend([x for x in node.outputLinks if not isinstance(x, basestring)])
            node.inputLinks = [x.linkfrom.uuid if not isinstance(x, basestring) else x for x in node.inputLinks]
            node.outputLinks = [x.linkto.uuid if not isinstance(x, basestring) else x for x in node.outputLinks]
        for link in links:
            if link.item:
    ...
    def _linksFromString(self, graphview=None):
        """Replace links between nodes string name references with
        link objects.
        """
        for node in self.nodes:
            reset = False
            for link in list(node.inputLinks):
                if isinstance(link, basestring):
                    ...
                    for source in self.nodes:
                        if source.name == link or source.uuid == link:
                            if source.name == link:
                                reset = True
                            MgvLink(source, node)
                            found = True
            ...
            for link in list(node.outputLinks):
                if isinstance(link, basestring):
                    ...
                    for dest in self.nodes:
                        if dest.name == link or dest.uuid == link:
                            MgvLink(node, dest)
                            found = True
                            break
                    if not found:
        ...
        if graphview:
            for editor in graphview.editors:
                if isinstance(editor.node, basestring):
    ...
    def getJson(self):
        ...
        return {'code': 'Graph', 'name': self.getName(), 'path': ':'.join(self.path),
...
class MgvNode(object):
    ...
    def __init__(self, graph=None, uuid=None, type='None', name='Noname', inputLinks=None, outputLinks=None,
        ...
        if versionActive is None:
            ...
            if isinstance(versionActive, basestring):
                versionActive = int(versionActive)
            if isinstance(versionActive, int):
    ...
    def create(self):
        ...
        """
        inputlinks = ';'.join([x if isinstance(x, basestring) else x.linkfrom.uuid for x in self.inputLinks])
        self.uuid = mgvWrapper.createNode(self.graph, self.code, name=self.name, typeName=self.type.getName(),
                                          typeUuid=self.type.uuid, posx=self.posx, posy=self.posy, user=self.user,
                                          port=self.port, inputLinks=inputlinks,
    ...
    def getParameter(self, key):
        ...
        self.versionActive.getParameter(key)
    ...
    def getInputNodes(self):
        ...
        return [x.linkfrom for x in self.inputLinks if not isinstance(x, basestring)]

    def getOutputNodes(self):
        """Return the output nodes."""
        return [x.linkto for x in self.outputLinks if not isinstance(x, basestring)]
    ...
    def getFromJson(cls, element, graph):
        ...
        inputLinks = element['inputLinks'].split(';')
        versions = []
    ...
    def getInputs(self):
        ...
        mgvInputs = [pickle.loads(x) if x else None for x in mgvInputs]
        return mgvInputs
...
class MgvNodeVersion(object):
    ...
    def getParameter(self, name):
        ...
        if name in [p.name for p in self.node.type.getParameters(self.node.versionActive.typeForceVersion)]:
            return self.node.type.getParameter(name, version_id=self.node.versionActive.typeForceVersion).getDefault()
        return None

    def setParameter(self, name, value):
        """Set a parameter value of the version."""
        self.node._ungotvar()
        if value != self.node.type.getParameter(name, version_id=self.node.versionActive.typeForceVersion).default:
    ...
    def getVars(self):
        ...
        self.dico["MGVNODETYPE"] = self.node.type.getName()
        ...
        for var in self.variables:
            if var.active:
                self.dico[var.name] = str(var.value)

        # PARAMETERS
        for param in self.node.type.getParameters(version_id=self.typeForceVersion):
            v = str(self.getParameter(param.getName()))
            if param.type == 'python':
                v = "import sys\nsys.path.append('%s')\n%s" % (os.path.dirname(os.path.abspath(__file__)), v)
            self.dico[param.getName()] = str(v)
    ...
    def compileScripts(self, action=None):
        ...
        inputs = [x.versionActive.getOutput() for x in self.node.getInputNodes()]
        inputs = '"""%s"""' % pickle.dumps(inputs).replace('\\n', '\\\\n')
        ...
        if action:
            script = '\n'.join([' ' * 4 + x for x in action.command.split('\n')])
        else:
            script = ' ' * 4 + 'pass'
...
""" % (projHead, typeHead, script)
        ...
        scriptfull = """from __future__ import print_function
...
mgvOutput = {}
mgvInputs = %s
mgvInputs = pickle.loads(mgvInputs)
try:
%s
except Exception as e:
    print(traceback.format_exc(), file=sys.__stderr__)
    ...
    mgvWrapper.setNodeAttr({'project_name': os.getenv('MGVPROJECTNAME'), 'path': os.getenv('MGVGRAPHFILEPATH'), 'code': 'NodeVersion', 'uuid': '%s'},
                           output=out)
""" % (os.path.dirname(os.path.abspath(__file__)), choosen_wrapper, inputs, script, self.node.getName(),
...
class MgvTypeFile(object):
    ...
    def __init__(self, uuid=None, type=None, name='OUT1',
                 path='${MGVNODEPATH}/VERSIONS/${MGVGRAPHFULLNAME}_${MGVNODENAME}_v${MGVNODEVERSION}.ext', copy=True):
        self.uuid = uuid
        self.type = type
        self.name = name
        self.path = path
...
class MgvType(object):
    ...
    def create(self, uuid=None):
        ...
        self.uuid = mgvWrapper.createNode(self.project, self.code, uuid=uuid, context=self.context, category=self.category,
                                          name=self.name, color=self.color, shape=self.shape, image=self.image,
                                          help=self.help, width=self.width, shapeVector=shapeVector, lock='',
                                          linkWith=';'.join(self.linkWith), versionActive=self.versionActive.getId())
    ...
    def getParameters(self, version_id=-1):
        """Return the parameters of the active version
        or a specific version of the type.
        """
        if version_id > -1:
            for v in self.versions:
    ...
    def getParameter(self, name, version_id=-1):
        """Return a parameter of the active version
        or a specific version of the type.
        """
        if version_id > -1:
            for v in self.versions:
    ...
    def getFromJson(cls, each_type):
        ...
        new = cls(uuid=None, category=each_type['category'], name=each_type['name'],
                  ...
                  shapeVector=shapeVector, typeFiles=typeFiles, versions=versions)
        return new
...
class MgvTypeVersion(object):
    ...
    def getParameters(self):
        """Return the parameters of the version."""

        return self.parameters

    def getParameter(self, name):
        """Return a parameter of the version."""
        for x in self.parameters:
            if x.getName() == name:
                return x
        return None

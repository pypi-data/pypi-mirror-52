...
class TestUtil(unittest_toolbox.Modified_TestCase):
  ...
  def test_C4_ensure_all_targets_allowed(self):
    ...
    self.assertTrue(securesystemslib.formats.ROLENAME_SCHEMA.matches(rolename))
    list_of_targets = ['/file1.txt', '/README.txt', '/warehouse/file2.txt']
    self.assertTrue(securesystemslib.formats.RELPATHS_SCHEMA.matches(list_of_targets))

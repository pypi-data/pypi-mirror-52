...
def close_session(session=None):
    ...
    Args:
        session (Session): The sesssion to close
    """
    session.commit()
    session.close()
...
def isolate(func):
    ...
    def _isolate(*args, **kwargs):
        # remove all reference to engines and initialize a new instance
        url = get_engine().url

        cleanup()

        init_engine(url, **ENGINE_CONFIG if ENGINE_CONFIG is not None else {})

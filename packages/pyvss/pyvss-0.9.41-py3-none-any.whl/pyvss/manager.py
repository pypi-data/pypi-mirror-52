...
class VssManager(object):
    ...
    def get_os(self, name=None, show_all=True, **kwargs):
        ...
        if name:
            kwargs.update(
                {'filter': 'full_name,like,{name}%'.format(name=name)}
            )
        data = self._get_objects(
    ...
    def get_supported_nic_types(self, only_type=True):
        """
        Gets supported Virtual Machine network adapter types
        supported by the API instance.
    ...
    def update_vm_nic_type(self, uuid, nic, type, **kwargs):
        ...
        if type not in supported_nic_types:
            raise VssError(
    ...
    def is_powered_on_vm(self, uuid):
        ...
        """
        json = self.request('/vm/{uuid}/state'.format(uuid=uuid))
        power_state = json.get('data').get('power_state')
        if power_state:
    ...
    def validate_vm_nic_type(self, n_type):
        supported_nic_types = self.get_supported_nic_types(only_type=True)
        if n_type not in supported_nic_types:
            raise VssError(
                '%s: unsupported NIC type. Choose between: %s'
                % (n_type, ', '.join(supported_nic_types))
            )
        return n_type

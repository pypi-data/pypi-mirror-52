...
class AutoMLConfig:
    ...
    :param preprocess:
            Flag whether Automated Machine Learning should preprocess and featurize your data
            such as handling missing data, text data, categorical data etc.
            Note: If the input data is sparse, preprocess cannot be set to True.

            Column type is automatically detected. Based on the detected column type
            preprocessing/featurization is done as follows:

            Categorical: Target encoding, one hot encoding, drop high cardinality categories,
    ...
    def __init__(self,
        ...
        if not AzureAutoMLClient._is_xgboost_module_present():
            xgb_algos = [constants.SupportedModels.Classification.XGBoostClassifier,
        ...
        if 'use_incremental_learning' in self.user_settings \
                and 'training_data' not in self.user_settings \
                and 'label_column_name' not in self.user_settings \
                and 'validation_data' not in self.user_settings:
    ...
    def _validate_config_settings(self, workspace: Optional[Workspace] = None) -> None:
        ...
        if has_training_input_dprep_obj and has_training_input_pandas_obj:
            raise ConfigException("A mix of dprep and pandas objects provided. "
        ...
        if compute_target != constants.ComputeTargets.LOCAL:
            ...
            if not data_script_provided and not has_training_input_dprep_obj:
                raise ConfigException("You must provide a 'data_script'"

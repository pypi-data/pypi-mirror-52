...
def _get_feature_map(fitted_model: Pipeline, raw_feature_names_list: Optional[List[str]]) -> DataInputType:
    ...
    if transformer is None:
        feature_map = np.eye(len(raw_feature_names_list), len(raw_feature_names_list))
        return feature_map

    engineered_feature_json_str_list = transformer.get_json_strs_for_engineered_feature_names()
    feature_map = np.zeros(shape=(len(raw_feature_names_list), len(engineered_feature_json_str_list)))
    for engineered_feature_index, engineered_feature_json_str in enumerate(engineered_feature_json_str_list):
        ...
        for raw_feature_name in raw_feature_names:
            ...
            if raw_feature_name in raw_feature_names_list:
                for raw_feature_index in range(0, len(raw_feature_names_list)):
                    if raw_feature_name == raw_feature_names_list[raw_feature_index]:

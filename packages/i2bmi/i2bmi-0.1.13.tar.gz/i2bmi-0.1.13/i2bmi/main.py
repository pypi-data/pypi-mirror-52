...
def loadphecodes():
    icd9 = pd.read_csv(pkg_resources.resource_filename(__name__,'phecode_icd9_rolled.csv'),dtype=str)
    ...
    icd9 = icd9.loc[:,['ICD9','PheCode','Phenotype','Excl. Phenotypes']].copy()
    icd9['ICDX']='9'
    icd10 = icd10.loc[:,['ICD10CM', 'PHECODE', 'Phenotype', 'Excl. Phenotypes']].copy()
    icd10['ICDX']='10'
    icd9.columns = ['ICD Code','PheCode','Phenotype','subcat','ICDX']
    icd10.columns = ['ICD Code','PheCode','Phenotype','subcat','ICDX']
    phecode = pd.concat([icd9,icd10],axis=0)
    phecode = phecode.merge(phemap.loc[:,['phecode','category']],how='left',left_on='PheCode',right_on='phecode').drop('phecode',axis=1)
    phecode = phecode.loc[:,['ICD Code', 'ICDX', 'PheCode', 'Phenotype', 'subcat', 'category']].copy()
	phecode['ICD Code'] = phecode['ICD Code'].str.replace(' ','').str.replace('.','')
    return phecode

def icdtophecode(df,col_icd):
    phecode = loadphecodes()
    df = df.copy()
    df[col_icd] = df[col_icd].str.replace(' ','').str.replace('.','')
    df = df.merge(phecode,how='left',left_on=col_icd,right_on='ICD Code')
    return df

...
def loadphecodes():
    icd9_roll = pd.read_csv(pkg_resources.resource_filename(__name__,'phecode_icd9_rolled.csv'),dtype=str)
    icd9 = pd.read_csv(pkg_resources.resource_filename(__name__,'phecode_icd9_map_unrolled.csv'),dtype=str)
    icd10 = pd.read_csv(pkg_resources.resource_filename(__name__,'Phecode_map_v1_2_icd10cm_beta.csv'),dtype=str)
    phemap = pd.read_csv(pkg_resources.resource_filename(__name__,'phecode_definitions1.2.csv'),dtype=str)
    icd9 = icd9.merge(icd9_roll.loc[:,['ICD9','Excl. Phenotypes']],how='left',left_on=['icd9'],right_on=['ICD9']).drop('ICD9',axis=1)
    icd9 = icd9.loc[:,['icd9','phecode','Excl. Phenotypes']].copy()
    icd10 = icd10.loc[:,['ICD10CM','PHECODE','Excl. Phenotypes']].copy()
    icd9['ICDX']='9'
    icd10['ICDX']='10'
    icd9.columns = ['ICD Code','PheCode','subcat','ICDX']
    icd10.columns = ['ICD Code','PheCode','subcat','ICDX']
    phecode = pd.concat([icd9,icd10],axis=0)
    phecode = phecode.merge(phemap.loc[:,['phecode','phenotype','category']],how='left',left_on='PheCode',right_on='phecode').drop('phecode',axis=1)
    phecode['ICD Code'] = phecode['ICD Code'].str.replace(' ','').str.replace('.','')
    return phecode
def loadphecodemap(granularity=0,prefix='Feature|PheCode|'):
    assert granularity in [0,1,2,3]
    granularity = ['category','subcat','phenotype','PheCode'][granularity]
    phecode = loadphecodes()
    phecode_onehot = pd.concat([phecode.loc[:,['ICD Code']],pd.get_dummies(phecode[granularity])],axis=1).groupby(['ICD Code']).max()
    phecode_onehot.columns = [prefix+i for i in phecode_onehot.columns]
    return phecode_onehot
def icdtophecode(df,col_icd,granularity=0,prefix='Feature|PheCode|'):
    phecodemap = loadphecodemap(granularity=granularity,prefix=prefix)
    df = df.copy()
    df['stripdx'] = df[col_icd].str.replace(' ','').str.replace('.','')
    df = df.merge(phecodemap,how='left',left_on='stripdx',right_on='ICD Code')
    df.loc[:,phecodemap.columns] = df.loc[:,phecodemap.columns].fillna(0.)
    return df
